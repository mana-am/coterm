...
public struct StackAuthClient: AuthClient {
    ...
    public func listTeams() async throws -> [CotermAuthTeam] {
        ...
        var summaries: [CotermAuthTeam] = []
        summaries.reserveCapacity(teams.count)
        for team in teams {
            summaries.append(CotermAuthTeam(id: team.id, displayName: await team.displayName))
        }
        return summaries
    }
    ...
    private static func mapped(_ user: CurrentUser) async -> CotermAuthUser {
        ...
        return CotermAuthUser(id: id, primaryEmail: email, displayName: name, imageURL: imageURL)
    }
}
