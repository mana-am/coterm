...
public struct AuthDebugLog: Sendable {
    ...
    static func debugLogPaths(environment: [String: String]) -> [String] {
        ...
        if let taggedPath = environment["COTERM_DEBUG_LOG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !taggedPath.isEmpty,
    ...
    public static func redacted(_ message: String) -> String {
        ...
        let replacements: [(pattern: String, replacement: String)] = [
            (#"(?i)\b(coterm_access|coterm_refresh|stack_access|stack_refresh|access_token|refresh_token|id_token|token|login_code|polling_code|code|state|coterm_auth_state)=([^\s&#,)]+)"#, "$1=<redacted>"),
            (#"(?i)(coterm_access|coterm_refresh|stack_access|stack_refresh|access_token|refresh_token|id_token|token|login_code|polling_code|code|state|coterm_auth_state)%253[dD]([^\s&#,)]+)"#, "$1%253D<redacted>"),
            (#"(?i)(coterm_access|coterm_refresh|stack_access|stack_refresh|access_token|refresh_token|id_token|token|login_code|polling_code|code|state|coterm_auth_state)%3[dD]([^\s&#,)]+)"#, "$1%3D<redacted>"),
            (#"(?i)\b(access|refresh)=([^\s,;)]+)"#, "$1=<redacted>"),
            (#"(?i)(access|refresh)%253[dD]([^\s,;)]+)"#, "$1%253D<redacted>"),
            (#"(?i)(access|refresh)%3[dD]([^\s,;)]+)"#, "$1%3D<redacted>"),
            (#"(?i)\b(authorization|x-coterm-refresh-token|x-coterm-refresh-token|x-stack-access-token|x-stack-refresh-token)\s*[:=]\s*(?:Bearer\s+)?([^\s,;)]+)"#, "$1=<redacted>"),
