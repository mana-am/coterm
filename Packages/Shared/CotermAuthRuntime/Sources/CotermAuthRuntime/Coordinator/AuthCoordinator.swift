...
private let authLog = Logger(subsystem: "ai.emergent.inc.coterm", category: "auth")

/// The shared, injected auth orchestrator for coterm.
///
...
///     sessionCache: CotermAuthSessionCache(keyValueStore: defaults, key: "auth_has_tokens"),
///     userCache: CotermAuthIdentityStore(keyValueStore: defaults, key: "auth_cached_user"),
///     teamSelection: CotermAuthTeamSelectionStore(keyValueStore: defaults, key: "auth_selected_team"),
...
public final class AuthCoordinator {
    ...
    public internal(set) var currentUser: CotermAuthUser?
    ...
    public private(set) var availableTeams: [CotermAuthTeam] = []
    /// The user's selected team id. Writes persist through the injected
    /// ``CotermAuthCore/CotermAuthTeamSelectionStore``.
    ...
    private let userCache: CotermAuthIdentityStore
    private let teamSelection: CotermAuthTeamSelectionStore
    ...
    var debugCredentials: CotermAuthAutoLoginCredentials?
    ...
    public init(
        ...
        sessionCache: CotermAuthSessionCache,
        userCache: CotermAuthIdentityStore,
    ...
    public func sendCode(to email: String) async throws {
        ...
        if launch.includesDevAuth,
           email.trimmingCharacters(in: .whitespacesAndNewlines) == "42" {
            let creds = CotermAuthAutoLoginCredentials(email: "l@l.com", password: "abc123")
    ...
    public func verifyCode(_ code: String) async throws {
        ...
        let fullCode = CotermAuthMagicLinkCode(code: code, nonce: nonce).composed
        do {
            let client = self.client
    ...
    func applySignedInUser(_ user: CotermAuthUser) async {
    ...
    func apply(_ state: CotermAuthState) {
        currentUser = state.currentUser
        isAuthenticated = state.isAuthenticated
        isRestoringSession = state.isRestoringSession
    }

    func loadCachedUser() -> CotermAuthUser? {
        do {
    ...
    func saveCachedUser(_ user: CotermAuthUser) {
        do {
    ...
    var autoLoginCredentials: CotermAuthAutoLoginCredentials? {
    ...
    var fixtureUser: CotermAuthUser? {
        CotermAuthUser(
    ...
    static let uiTestMockUser = CotermAuthUser(
        id: "uitest_user",
        primaryEmail: "uitest@coterm.local",
        displayName: "UI Test"
    )
}
