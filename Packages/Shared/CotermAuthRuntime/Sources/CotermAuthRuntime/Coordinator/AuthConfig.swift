...
///
/// Consolidates the Stack project credentials (from ``CotermAuthConfig``) with the
/// magic-link callback URL and the coterm web API base URL, so both apps build
...
public struct AuthConfig: Equatable, Sendable {
    ...
    public let stack: CotermAuthConfig
    /// The auth callback URL the magic-link email should target.
    public let magicLinkCallbackURL: String
    /// The base URL of the coterm web API (device-token registration, push).
    public let apiBaseURL: String

    /// Creates an auth configuration from its resolved parts.
    public init(stack: CotermAuthConfig, magicLinkCallbackURL: String, apiBaseURL: String) {
        self.stack = stack
    ...
    public init(
        environment: CotermAuthEnvironment,
        overrides: [String: String] = [:]
    ) {
        let stack = CotermAuthConfig(
