...
/// The shared, localized error vocabulary for coterm authentication flows.
///
/// Lifted out of the iOS-only `CotermMobileAuth` package so both the macOS and iOS
...
/// previously surfaced through `CotermMobileCore.L10n`.
public enum AuthError: Error, LocalizedError, Equatable, Sendable {
    ...
    /// The OS/browser auth session closed before a valid callback reached coterm.
