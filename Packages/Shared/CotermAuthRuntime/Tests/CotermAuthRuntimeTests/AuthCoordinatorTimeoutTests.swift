...
@Suite struct AuthCoordinatorTimeoutTests {
    ...
    private func makeCoordinator(
        ...
        cachedUser: CotermAuthUser? = nil,
        hasCachedTokens: Bool = false,
        onSignedIn: @escaping @Sendable () async -> Void = {}
    ) -> AuthCoordinator {
        let store = FakeKeyValueStore()
        let sessionCache = CotermAuthSessionCache(keyValueStore: store, key: "has_tokens")
        let userCache = CotermAuthIdentityStore(keyValueStore: store, key: "cached_user")
