...
@Suite struct HostBrowserSignInFlowFailureTests {
    @Test func invalidCallbackPayloadIsRejected() async {
        let harness = HostBrowserSignInFlowHarness(user: CotermAuthUser(id: "u1", primaryEmail: nil, displayName: nil))

        let attempt = Task { await harness.flow.signIn(timeout: 60) }
        await harness.waitForSession()
        harness.factory.sessions[0].deliver(URL(string: "coterm-dev://auth-callback?other=1&coterm_auth_state=\(harness.callbackState(harness.factory.sessions[0]))")!)
    ...
    @Test func callbackValidationDisplayUnsafeErrorRecordsGenericServerFailure() async {
        let harness = HostBrowserSignInFlowHarness(
            user: CotermAuthUser(id: "u1", primaryEmail: nil, displayName: nil)
        )
