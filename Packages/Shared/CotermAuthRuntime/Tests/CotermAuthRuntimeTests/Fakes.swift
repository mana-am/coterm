...
final class FakeKeyValueStore: CotermAuthKeyValueStore, @unchecked Sendable {
...
actor FakeAuthClient: AuthClient {
    ...
    var user: CotermAuthUser?
    var teams: [CotermAuthTeam] = []
    ...
    init(access: String? = nil, refresh: String? = nil, user: CotermAuthUser? = nil) {
        self.access = access
        self.refresh = refresh
        self.user = user
    }

    func setUser(_ user: CotermAuthUser?) { self.user = user }
    ...
    func setTeams(_ teams: [CotermAuthTeam]) { self.teams = teams }
    ...
    func currentUser(throwOnMissing: Bool) async throws -> CotermAuthUser? {
        currentUserCallCount += 1
        if let throwOnCurrentUser { throw throwOnCurrentUser }
        return user
    }

    func listTeams() async throws -> [CotermAuthTeam] {
...
extension AuthConfig {
    static let test = AuthConfig(
        stack: CotermAuthConfig(projectId: "test", publishableClientKey: "test"),
