...
///
/// Logs to the unified log (`mosaic.com.emergent.app` / `auth`) in all builds. macOS
...
public struct AuthDebugLog: Sendable {
    ...
    private static let logger = Logger(subsystem: "mosaic.com.emergent.app", category: "auth")
    ...
    public static func redacted(_ message: String) -> String {
        ...
        let replacements: [(pattern: String, replacement: String)] = [
            (#"(?i)\b(mosaic_access|mosaic_refresh|stack_access|stack_refresh|access_token|refresh_token|id_token|token|login_code|polling_code|code|state|mosaic_auth_state)=([^\s&#,)]+)"#, "$1=<redacted>"),
            (#"(?i)(mosaic_access|mosaic_refresh|stack_access|stack_refresh|access_token|refresh_token|id_token|token|login_code|polling_code|code|state|mosaic_auth_state)%253[dD]([^\s&#,)]+)"#, "$1%253D<redacted>"),
            (#"(?i)(mosaic_access|mosaic_refresh|stack_access|stack_refresh|access_token|refresh_token|id_token|token|login_code|polling_code|code|state|mosaic_auth_state)%3[dD]([^\s&#,)]+)"#, "$1%3D<redacted>"),
            (#"(?i)\b(access|refresh)=([^\s,;)]+)"#, "$1=<redacted>"),
            (#"(?i)(access|refresh)%253[dD]([^\s,;)]+)"#, "$1%253D<redacted>"),
            (#"(?i)(access|refresh)%3[dD]([^\s,;)]+)"#, "$1%3D<redacted>"),
            (#"(?i)\b(authorization|x-mosaic-refresh-token|x-cmux-refresh-token|x-stack-access-token|x-stack-refresh-token)\s*[:=]\s*(?:Bearer\s+)?([^\s,;)]+)"#, "$1=<redacted>"),
