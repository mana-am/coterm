...
/// It lives in `CotermMobileCore` (the universally-imported, dependency-free base
/// package) so the lowest fire-site — `MobileClientIDRepository` in
/// `CotermMobileShellModel` — can depend on the seam without any upward edge. The
/// concrete `AnalyticsEmitter` actor lives in `CotermMobileAnalytics`, is built
