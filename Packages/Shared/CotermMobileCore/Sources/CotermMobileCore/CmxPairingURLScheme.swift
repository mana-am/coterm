...
/// The channel-specific URL scheme carried by coterm pairing/attach deep links.
///
/// All builds used to register and emit one scheme (`coterm-ios`), so scanning a
...
///
/// The iOS app's registered scheme comes from `COTERM_IOS_URL_SCHEME` in
...
public struct CmxPairingURLScheme {
    ...
    public static let release = "coterm-ios"

    /// The scheme development (DEBUG/tagged) builds register and emit.
    public static let development = "coterm-ios-dev"

    /// Every scheme any coterm build may emit; parsers accept all of them.
    ...
    /// Whether `scheme` is a pairing scheme from any coterm channel.
