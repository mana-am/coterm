...
public enum CmxRoutePingResult: Sendable, Equatable {
    ...
    /// listening on the port (coterm not running, or mobile pairing off).
    case refused
...
extension CmxRoutePingResult {
    ...
    /// listening on the port. Use ``isListening`` for "the coterm port answered".
    ...
    /// Whether a service actually accepted the connection on the coterm port (only
