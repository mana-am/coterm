...
public struct MobileTerminalRenderGridFrame: Codable, Equatable, Sendable {
    public static let currentFormat = "coterm.render-grid.v1"

    /// Format tags accepted on decode. GhosttyKit now emits ``currentFormat``;
    /// the legacy pre-rename tag `cmux.render-grid.v1` remains accepted so
    /// cached frames and older builds keep decoding during rollout. Rejecting
    /// the legacy tag broke collaboration seeding (viewers saw a black mirror
    /// pane until live output arrived). Legacy tags are normalized to
    /// ``currentFormat`` so re-encoded frames carry one canonical name downstream.
