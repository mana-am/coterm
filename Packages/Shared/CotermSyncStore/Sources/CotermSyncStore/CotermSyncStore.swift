...
public actor CotermSyncStore: CotermSyncStoring {
    ...
    /// The default on-disk location, `coterm-sync.sqlite3` next to the paired-Mac
    /// db under Application Support/coterm.
    public static func defaultDatabaseURL(fileManager: FileManager = .default) throws -> URL {
        ...
        let dir = appSupport.appendingPathComponent("coterm", isDirectory: true)
        if !fileManager.fileExists(atPath: dir.path) {
            try fileManager.createDirectory(at: dir, withIntermediateDirectories: true)
        }
        return dir.appendingPathComponent("coterm-sync.sqlite3")
    }
    ...
    private func runMigrations() throws {
        ...
        case 1:
            break
        default:
            throw CotermSyncStoreError.unknownSchemaVersion(Int(version))
        }
    }
