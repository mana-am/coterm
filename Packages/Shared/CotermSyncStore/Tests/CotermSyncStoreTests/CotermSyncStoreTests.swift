...
func makeStore() throws -> (CotermSyncStore, URL) {
    let dir = FileManager.default.temporaryDirectory
        .appendingPathComponent(UUID().uuidString, isDirectory: true)
    try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    let store = try CotermSyncStore(databaseURL: dir.appendingPathComponent("coterm-sync.sqlite3"))
    return (store, dir)
}
...
@Suite struct CotermSyncStoreApplyTests {
