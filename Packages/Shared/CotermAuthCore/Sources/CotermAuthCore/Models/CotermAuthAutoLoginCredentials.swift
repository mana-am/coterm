...
/// sign-in.
public struct CotermAuthAutoLoginCredentials: Equatable, Sendable {
    /// The account email.
    ...
    /// (`COTERM_UITEST_STACK_EMAIL` / `COTERM_UITEST_STACK_PASSWORD`), or `nil`
    ...
    public init?(
        ...
        guard let email = environment["COTERM_UITEST_STACK_EMAIL"], !email.isEmpty else {
            return nil
        }
        guard let password = environment["COTERM_UITEST_STACK_PASSWORD"], !password.isEmpty else {
            return nil
        }
