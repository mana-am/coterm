...
/// One tagged coterm app instance running on a registered device, as returned by
...
public struct RegistryAppInstance: Equatable, Sendable, Identifiable {
    /// The coterm build tag this instance runs (`"stable"`, a dev tag, or
...
/// registry, with its running coterm app instances.
///
...
/// `deviceId` here is the coterm-generated device UUID (the wire `deviceId`), not
...
public struct RegistryDevice: Equatable, Sendable, Identifiable {
    /// Stable, cross-platform coterm device UUID (matches `MobileHostIdentity` /
    ...
    public var lastSeenAt: Date
    /// The device's running coterm app instances (tags), newest-first.
