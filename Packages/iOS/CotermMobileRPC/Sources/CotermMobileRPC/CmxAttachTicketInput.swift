...
/// Decodes a scanned or pasted `coterm-ios://` pairing/attach URL into a
/// validated ``CmxAttachTicket``.
public struct CmxAttachTicketInput {
    private init() {}

    /// Decode and validate a `coterm-ios://pair` or `coterm-ios://attach` URL.
    ///
    ...
    /// by Stack account, not ticket age). Only the ancient `coterm-ios://pair`
    ...
    public static func decode(_ rawValue: String) throws -> CmxAttachTicket {
        ...
        // Accept any channel's pairing scheme (coterm-ios for release builds,
        // coterm-ios-dev for development); cross-channel pairing still works when
        ...
        // A QR minted by a newer coterm whose grammar version this build does not
