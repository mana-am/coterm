...
/// *same* coterm account as the Mac. Each gate that is not yet cleared is its own
...
public enum MobileSetupGuidanceState: Hashable, Sendable, CaseIterable {
    ...
    /// in with the same coterm account the Mac uses.
    case notSignedIn

    /// Signed in, but this device has never paired a Mac and none is reachable.
    /// The next step is to get the coterm Mac app installed and running, then pair.
    ...
    /// different coterm account than the Mac. The next step is to sign this device
