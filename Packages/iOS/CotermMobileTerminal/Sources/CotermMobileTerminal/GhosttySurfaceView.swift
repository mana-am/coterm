...
import CotermMobileDiagnostics
...
private let log = Logger(subsystem: "ai.emergent.inc.coterm.ios", category: "ghostty.surface")
...
enum TerminalInputDebugLog {
    private static let isEnabled = ProcessInfo.processInfo.environment["COTERM_INPUT_DEBUG"] == "1"
    private static let logger = Logger(subsystem: "ai.emergent.inc.coterm.ios", category: "ghostty.input")
...
public final class GhosttySurfaceView: UIView, TerminalSurfaceHosting {
    ...
    #if DEBUG
    /// Repro hook for the `COTERM_ZOOM_STRESS` harness: drive one font-zoom
    ...
    private func ensureCursorOverlayLayer() -> CALayer {
        ...
        let layer = CALayer()
        layer.name = "coterm.cursorOverlay"
