...
private let log = Logger(subsystem: "ai.emergent.inc.coterm.ios", category: "ghostty.runtime")
...
private func cotermIOSRuntimeReadClipboardCallback(
...
public final class GhosttyRuntime {
    ...
    init() throws {
        ...
        runtimeConfig.read_clipboard_cb = cotermIOSRuntimeReadClipboardCallback
    ...
    nonisolated static func iOSConfigURLs(
        ...
        #if os(iOS)
        var urls: [URL] = []
        if let overridePath = processInfo.environment["COTERM_GHOSTTY_CONFIG_PATH"], !overridePath.isEmpty {
