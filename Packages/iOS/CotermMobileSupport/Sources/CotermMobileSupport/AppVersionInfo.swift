...
/// (the `CotermDevTag` / `CotermGitSHA` Info.plist keys), e.g.
...
public struct AppVersionInfo: Sendable, Equatable {
    ...
    /// The dev `--tag` (`CotermDevTag`); empty for release/TestFlight builds.
    public let devTag: String

    /// The short git SHA (`CotermGitSHA`); empty for release/TestFlight builds.
    ///
    ...
    public init(infoDictionary: [String: Any]?, isDevBuild: Bool) {
        func trimmedString(_ key: String) -> String {
            ...
            // Unexpanded build-setting placeholders (e.g. "$(COTERM_GIT_SHA)" when
        ...
        self.devTag = trimmedString("CotermDevTag")
        self.gitSHA = trimmedString("CotermGitSHA")
