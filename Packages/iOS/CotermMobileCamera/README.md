...
iOS camera service for coterm mobile QR-code pairing.
...
- ``QRCodeScanStream`` — an `AsyncStream<String>` of decoded QR payloads, fed by
  ...
  injected predicate (coterm pairing only accepts `coterm-ios://` links).
...
stream.yield("coterm-ios://example")
stream.finish()
var seen: [String] = []
for await code in stream.codes { seen.append(code) }
#expect(seen == ["coterm-ios://example"])
```
