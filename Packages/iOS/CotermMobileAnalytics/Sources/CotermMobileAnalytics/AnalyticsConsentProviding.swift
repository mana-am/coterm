...
///
/// The analytics package must not depend on the settings domain (`CotermSettings`),
...
/// `CotermSettings.catalog.app.sendAnonymousTelemetry`; tests provide a fixed
...
///
/// The iOS app cannot import the macOS-only `CotermSettings` package, so this reads
/// the same backing key that `CotermSettings.catalog.app.sendAnonymousTelemetry`
