...
///
/// Construct one at the composition root and hand it to views (coterm injects it
/// through the `\.tailscaleStatusMonitor` SwiftUI environment key in
/// `CotermMobileShellUI`). Inject a custom provider to drive tests.
@MainActor
@Observable
public final class TailscaleStatusMonitor {
    ...
    public init(
        ...
        monitor.start(queue: DispatchQueue(label: "dev.coterm.tailscale-status", qos: .utility))
    }
