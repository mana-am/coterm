...
struct PresenceServiceURLResolutionTests {
    ...
    @Test func infoPlistUsedWhenNoEnvOrDefaults() {
        let url = PresenceClient.resolvedServiceBaseURL(
            environment: [:],
            defaults: emptyDefaults(),
            infoPlistValue: "https://coterm-presence-dev-alice.acct.workers.dev",
            isDebugBuild: true
        )
        #expect(url == "https://coterm-presence-dev-alice.acct.workers.dev")
    }

    @Test func fallsBackToBuildDefault() {
        // Debug -> dev worker; Release -> nil until the user configures a
        // self-hosted presence worker.
