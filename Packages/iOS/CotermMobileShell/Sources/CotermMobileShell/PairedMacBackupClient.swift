...
private let pairedMacBackupLog = Logger(subsystem: "coterm.com.emergent.app", category: "PairedMacBackup")
...
/// `X-Coterm-Team-Id`, with tokens supplied through ``PresenceTokenSource``.
public actor PairedMacBackupClient: PairedMacBackingUp {
    ...
    private func makeRequest(
        ...
        if let scope = await clientScope() {
            request.setValue(scope, forHTTPHeaderField: "X-Coterm-Client-Scope")
        }
        if let body {
