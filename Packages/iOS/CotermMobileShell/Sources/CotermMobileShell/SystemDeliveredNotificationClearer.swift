...
/// authoritative `coterm.notificationId` payload key (with the request
...
public struct SystemDeliveredNotificationClearer: DeliveredNotificationClearing {
    ...
    /// `coterm.notificationId` payload key when present (authoritative — the Mac
    ...
    static func macNotificationID(for request: UNNotificationRequest) -> String {
        if let coterm = request.content.userInfo["coterm"] as? [String: Any],
