...
/// optional `X-Coterm-Team-Id` so the server scopes to the chosen team (defaults to
...
public actor DeviceRegistryService: DeviceRegistryRefreshing {
    ...
    /// - Parameters:
    ///   - apiBaseURL: The coterm web API base URL (no trailing slash).
    ...
    private static let deviceIDKey = "coterm.deviceRegistry.iosDeviceID"

    /// This iOS device's stable coterm identity for the device registry.
    ///
    /// A coterm-GENERATED persisted UUID (NOT `identifierForVendor`, which resets
    /// when the last coterm app is removed, and NOT a hardware fingerprint).
    ...
    private func makeRequest(method: String, path: String, body: [String: Any]?) async -> URLRequest? {
        ...
        if let teamID = await teamIDProvider(), !teamID.isEmpty {
            request.setValue(teamID, forHTTPHeaderField: "X-Coterm-Team-Id")
        }
        if let body {
