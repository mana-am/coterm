public import CotermMobileCore
public import CotermAgentChat
internal import CotermMobileDiagnostics
public import CotermMobilePairedMac
public import CotermMobileRPC
public import CotermMobileShellModel
internal import CotermMobileSupport
public import CotermMobileTransport
public import Foundation
import Observation
internal import OSLog

private let mobileShellLog = Logger(
    subsystem: Bundle.main.bundleIdentifier ?? "dev.coterm.ios",
    category: "mobile-shell"
)

/// Transitional alias for the decomposed shell facade.
///
/// The iOS views and push coordinator still bind to `CotermMobileShellStore`;
...
public typealias CotermMobileShellStore = MobileShellComposite
...
public final class MobileShellComposite: MobileTerminalOutputSinking {
    ...
    private static let hasKnownPairedMacDefaultsKey = "coterm.mobile.hasKnownPairedMac"
    ...
    public static func preview(runtime: (any MobileSyncRuntime)? = nil) -> CotermMobileShellStore {
        CotermMobileShellStore(
            runtime: runtime,
    ...
    /// The team's registered devices and their coterm app instances (tags), for the
    ...
    /// The coterm device id of the Mac the live connection currently targets, or
    ...
    private func connectPairingURLResult(
        ...
        do {
            ...
            if case MobileSyncPairingPayloadError.loopbackRouteRejected = error {
                ...
                // A real coterm QR whose grammar version this build predates: the
    ...
    private var multiMacAggregationEnabled: Bool {
        if let raw = ProcessInfo.processInfo.environment["COTERM_MULTI_MAC_AGGREGATION"]?
    ...
    static let foregroundAnonymousKey = "__coterm_foreground__"
    ...
    private func versionWarning(for ticket: CmxAttachTicket) -> String? {
        ...
        let format = L10n.string(
            ...
            defaultValue: "This iPhone is running coterm %@, but the Mac is running coterm %@. Pairing across different compatibility levels can break terminal input, workspace sync, or notifications. Continue only if you trust this Mac and accept that some features may fail."
        )
        return String(
            format: format,
    ...
    func prepareTerminalReplayFailureRetry(
        ...
        guard retryCount < Self.maxTerminalReplayFailureRetries else {
            MobileDebugLog.anchormux(
                "COTERM_REPLAY retry_exhausted surface=\(surfaceID) attempts=\(retryCount)"
            )
            return nil
        }
        terminalReplayFailureRetryCountsBySurfaceID[surfaceID] = retryCount + 1
        MobileDebugLog.anchormux(
            "COTERM_REPLAY retry_after_failure surface=\(surfaceID) attempt=\(retryCount + 1)"
        )
    ...
    private func requestTerminalReplayForCurrentBarrier(
        ...
        MobileDebugLog.anchormux("COTERM_REPLAY retry_\(reason) surface=\(surfaceID)")
    ...
    private func registerTerminalOutput(
        ...
        pendingTerminalByteEndSeqBySurfaceID.removeValue(forKey: surfaceID)
        #if DEBUG
        mobileShellLog.info("COTERM_REPLAY register sink surface=\(surfaceID, privacy: .public) connected=\(self.connectionState == .connected, privacy: .public) hasClient=\(self.remoteClient != nil, privacy: .public) workspaceCount=\(self.workspaces.count, privacy: .public)")
        #endif
        requestTerminalReplay(surfaceID: surfaceID)
    }
    ...
    func requestTerminalReplay(
        ...
        guard let client = remoteClient else {
            ...
            #if DEBUG
            mobileShellLog.error("COTERM_REPLAY skip surface=\(surfaceID, privacy: .public) reason=no_remote_client")
            #endif
            return
        }
        guard let workspaceID = workspaceID(forTerminalID: surfaceID) else {
            ...
            #if DEBUG
            mobileShellLog.error("COTERM_REPLAY skip surface=\(surfaceID, privacy: .public) reason=workspace_not_found")
            #endif
            return
        }
        let remoteWorkspaceID = remoteWorkspaceID(for: workspaceID)
        if let replayBarrierTokenForRequest {
            guard terminalReplayBarrierTokensInFlightBySurfaceID[surfaceID] != replayBarrierTokenForRequest else {
                #if DEBUG
                mobileShellLog.info("COTERM_REPLAY skip surface=\(surfaceID, privacy: .public) reason=barrier_in_flight")
                #endif
                return
            }
        } else {
            guard !terminalReplaySurfaceIDsInFlight.contains(surfaceID) else {
                #if DEBUG
                mobileShellLog.info("COTERM_REPLAY skip surface=\(surfaceID, privacy: .public) reason=in_flight")
                #endif
                return
            }
        }
        ...
        let replayTask = Task { @MainActor [weak self] in
            ...
            case .success(let data):
                guard self.terminalReplayRequestIDsInFlightBySurfaceID[surfaceID] == replayRequestID else {
                    MobileDebugLog.anchormux("COTERM_REPLAY stale_request surface=\(surfaceID)")
                    return
                }
                ...
                if let replayBarrierTokenForRequest {
                    guard self.terminalReplayBarrierTokensBySurfaceID[surfaceID] == replayBarrierTokenForRequest else {
                        MobileDebugLog.anchormux("COTERM_REPLAY barrier_stale surface=\(surfaceID)")
                        return
                    }
                }
                #if DEBUG
                ...
                if let renderGrid {
                    deliverBytes = nil
                    MobileDebugLog.anchormux("COTERM_REPLAY render_grid surface=\(surfaceID) spans=\(renderGrid.rowSpans.count) seq=\(renderGrid.stateSeq)")
                } else if let snapshotBytes, !snapshotBytes.isEmpty {
                    deliverBytes = Self.terminalSnapshotReplacementBytes(snapshotBytes)
                    MobileDebugLog.anchormux("COTERM_REPLAY snapshot surface=\(surfaceID) bytes=\(snapshotBytes.count) seq=\(replaySeq ?? 0)")
                } else {
                    deliverBytes = bytes
                    MobileDebugLog.anchormux("COTERM_REPLAY raw_tail surface=\(surfaceID) bytes=\(bytes?.count ?? -1) seq=\(replaySeq ?? 0)")
                }
                if let renderGrid {
            ...
            case .failure(let error):
                guard self.terminalReplayRequestIDsInFlightBySurfaceID[surfaceID] == replayRequestID else {
                    MobileDebugLog.anchormux("COTERM_REPLAY stale_request_failed surface=\(surfaceID)")
                    return
                }
                mobileShellLog.error("COTERM_REPLAY failed surface=\(surfaceID, privacy: .public) error=\(String(describing: error), privacy: .private)")
                guard self.remoteClient === client else {
                    self.clearTerminalReplayInFlightIfCurrent(
                        surfaceID: surfaceID,
                        requestID: replayRequestID
    ...
    private func handleTerminalRenderGridEvent(_ event: MobileEventEnvelope) {
        ...
        #if DEBUG
        mobileShellLog.info("COTERM_REPLAY live render_grid surface=\(renderGrid.surfaceID, privacy: .public) full=\(renderGrid.full, privacy: .public) spans=\(renderGrid.rowSpans.count, privacy: .public) cleared=\(renderGrid.clearedRows.count, privacy: .public) seq=\(renderGrid.stateSeq, privacy: .public) hasSink=true")
        #endif
    ...
    private func handleTerminalBytesEvent(_ event: MobileEventEnvelope) {
        ...
        mobileShellLog.info("COTERM_REPLAY live bytes surface=\(surfaceID, privacy: .public) byteCount=\(bytes.count, privacy: .public) seq=\(debugSeq, privacy: .public) hasSink=\(self.hasTerminalOutputSink(surfaceID: surfaceID), privacy: .public)")
        #endif
