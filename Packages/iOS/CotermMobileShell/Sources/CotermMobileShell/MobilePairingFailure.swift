...
internal import CotermMobileRPC
internal import CotermMobileSupport
internal import CotermMobileTransport
import Foundation
...
public enum MobilePairingFailureCategory: Equatable, Sendable {
    ...
    /// The address was reachable but nothing accepted the connection: Coterm is not
    ...
    /// The Mac is signed in to a different coterm account than this device.
    ...
    case ticketExpired
    /// The scanned/typed input was not a pairing QR coterm recognizes (malformed,
    /// not a coterm code, or an unreadable URL).
    case invalidCode
    /// The QR is a coterm pairing code, but it uses a grammar version newer than
    /// this build understands (the Mac is on a newer coterm). The fix is updating
    ...
    public var message: String {
        ...
        case let .listenerNotRunning(host, port):
            ...
            return L10n.string(
                ...
                defaultValue: "Your Mac is reachable, but coterm isn't running there (or mobile pairing is off). Open coterm on the Mac, then try again."
            )
        case .localNetworkBlocked:
            return L10n.string(
                "mobile.pairing.localNetworkPermission",
                defaultValue: "iOS blocked the connection. Allow coterm to use the Local Network in iOS Settings, then try again."
            )
        ...
        case .accountMismatch:
            return L10n.string(
                ...
                defaultValue: "This Mac is signed in to a different coterm account. Sign out and sign back in with the account that owns this Mac."
            )
        ...
        case .invalidCode:
            return L10n.string(
                ...
                defaultValue: "This isn't a coterm pairing QR. Scan the code shown in the Pair iPhone window on your Mac."
            )
        case .unrecognizedVersion:
            return L10n.string(
                "mobile.pairing.unrecognizedVersion",
                defaultValue: "This QR needs a newer version of coterm. Update the app and try again."
            )
        case .loopbackRejected:
            return L10n.string(
    ...
    public var guidance: String? {
        ...
        case .hostUnreachable, .dnsFailed, .handshakeTimedOut:
            return L10n.string(
                ...
                defaultValue: "Check that this phone and your Mac are on the same Wi-Fi or both running Tailscale, that the Mac is awake, and that coterm is open on it."
            )
        case .listenerNotRunning, .connectionDropped:
            return L10n.string(
                "mobile.pairing.guidance.openMacApp",
                defaultValue: "Open coterm on your Mac, then scan the QR or link from it again."
            )
        case .localNetworkBlocked:
            return L10n.string(
                "mobile.pairing.guidance.localNetwork",
                defaultValue: "Settings > coterm > Local Network, then try again."
            )
        case .accountMismatch, .emailMismatch, .authFailed:
            return L10n.string(
                "mobile.pairing.guidance.sameAccount",
                defaultValue: "Both devices must be signed in to the same coterm account."
            )
        ...
        case .unrecognizedVersion:
            return L10n.string(
                ...
                defaultValue: "Update coterm from the App Store (or TestFlight), then scan again."
            )
