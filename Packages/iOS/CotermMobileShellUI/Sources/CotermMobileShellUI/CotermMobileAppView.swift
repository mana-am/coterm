...
public struct CotermMobileAppView: View {
    @State private var store: CotermMobileShellStore
    ...
    public init(
        store: CotermMobileShellStore = .preview(),
    ...
    public var body: some View {
        #if os(iOS)
        CotermMobileRootView(store: store, onboardingStore: onboardingStore)
            .environment(browserStore)
        #else
        CotermMobileRootView(store: store)
            .environment(browserStore)
        #endif
    }
}
