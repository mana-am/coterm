...
struct CotermMobileRootView: View {
    ...
    #if os(iOS)
    init(store: CotermMobileShellStore, onboardingStore: MobileOnboardingStore) {
        self.store = store
        self.onboardingStore = onboardingStore
        _hasSeenOnboarding = State(initialValue: onboardingStore.hasSeenOnboarding)
    }
    #else
    init(store: CotermMobileShellStore) {
        self.store = store
    }
    #endif
    ...
    /// authorization grounds (a different coterm account, or a token it could not
    ...
    private func connectUITestAttachURLIfNeeded() -> Bool {
        ...
        //   - COTERM_DOGFOOD_ATTACH_URL (UITestConfig.dogfoodAttachURL): NOT gated on
        ...
        //     for real (COTERM_UITEST_STACK_* with COTERM_UITEST_MOCK_DATA=0) and wants
        ...
        //   - COTERM_UITEST_ATTACH_URL (UITestConfig.attachURL): gated on mock data,
