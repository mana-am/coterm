...
/// ``CotermAuthRuntime/PushRegistrationService``, and routes foreground
/// presentation + taps to the active ``CotermMobileShellStore`` for "mirror macOS"
...
public final class MobilePushCoordinator {
    ...
    private static let enabledKey = "coterm.notifications.pushEnabled"

    /// APNs `aps.category` the web sets on every coterm terminal push (see
    /// `COTERM_APNS_CATEGORY` in `web/services/apns/payload.ts`). The matching
    ...
    public static let dismissSyncCategoryIdentifier = "coterm.terminal"

    @ObservationIgnored private weak var store: CotermMobileShellStore?
    ...
    public func bind(store: CotermMobileShellStore) {
        self.store = store
    ...
    public func configure(delegate: any UNUserNotificationCenterDelegate) {
        ...
        // coterm banner delivers `UNNotificationDismissActionIdentifier` to the
    ...
    ///     with `coterm.notificationId` as a fallback.
    ///   - macDeviceId: The Mac that owns the notification, from the `coterm`
