...
import CotermAgentChatUI
import SwiftUI

/// Debug-only host for the plain-terminal chat (#5). Drives the real
/// ``CotermAgentChatUI/ChatScreen`` with a terminal-kind descriptor and a
/// ``CotermAgentChat/FixtureChatEventSource`` serving command blocks, so the
