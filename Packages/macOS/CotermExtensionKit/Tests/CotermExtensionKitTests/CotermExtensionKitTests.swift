import Foundation
import Testing
@_spi(CotermHostTransport) @testable import CotermExtensionKit

@Suite
struct CotermExtensionKitTests {
    @Test
    func testSidebarSnapshotRoundTripsStableContract() throws {
        let workspaceID = UUID(uuidString: "11111111-1111-1111-1111-111111111111")!
        let snapshot = CotermSidebarSnapshot(
            sequence: 42,
            ...
            workspaces: [
                CotermSidebarWorkspace(
                    ...
                    listeningPorts: [3000],
                    pullRequestURLs: ["https://github.com/emergent-inc/coterm/pull/1"]
                ),
            ]
        )

        let encoded = try JSONEncoder().encode(snapshot)
        let decoded = try JSONDecoder().decode(CotermSidebarSnapshot.self, from: encoded)

        #expect(decoded == snapshot)
        #expect(decoded.apiVersion == CotermExtensionAPIVersion.sidebarV2)
    ...
    func testManifestDecodingDefaultsMissingActionScopesToNone() throws {
        ...
        let manifest = try JSONDecoder().decode(CotermExtensionManifest.self, from: payload)
        ...
        let snapshot = CotermSidebarSnapshot(
            ...
            workspaces: [
                CotermSidebarWorkspace(
                    ...
                    pullRequestURLs: ["https://github.com/emergent-inc/coterm/pull/4994"]
                ),
            ]
        )

        let filtered = snapshot.filtered(for: [CotermExtensionScope.workspaceMetadata])
    ...
    func testSidebarSnapshotFilteringWithNoScopesRemovesWorkspaceMetadata() {
        ...
        let filtered = snapshot.filtered(for: [CotermExtensionScope]())
    ...
    func testSidebarSnapshotDecodingDefaultsMissingGrantedScopes() throws {
        ...
        let snapshot = try JSONDecoder().decode(CotermSidebarSnapshot.self, from: payload)
    ...
    func testSidebarXPCCodecRoundTripsSnapshotActionAndResult() throws {
        ...
        let decodedSnapshot = try CotermSidebarXPCCodec.decodeSnapshot(
            try CotermSidebarXPCCodec.encodeSnapshot(snapshot)
        )
        #expect(decodedSnapshot == snapshot)

        let actionScopedSnapshot = snapshot.filtered(
            for: [CotermExtensionScope.workspaceMetadata],
            actionScopes: [CotermExtensionActionScope.selectWorkspace]
        )
        ...
        let decodedManifest = try CotermSidebarXPCCodec.decodeManifest(
            try CotermSidebarXPCCodec.encodeManifest(manifest)
        )
        #expect(decodedManifest == manifest)

        let action = CotermSidebarAction.selectWorkspace(workspaceID)
        #expect(action.requiredScopes == [.selectWorkspace])
        let decodedAction = try CotermSidebarXPCCodec.decodeAction(
            try CotermSidebarXPCCodec.encodeAction(action)
        )
        #expect(decodedAction == action)

        let result = CotermSidebarActionResult.rejected("Not found")
        let decodedResult = try CotermSidebarXPCCodec.decodeActionResult(
            try CotermSidebarXPCCodec.encodeActionResult(result)
        )
        #expect(decodedResult == result)

        let cancelledResult = CotermSidebarActionResult.cancelled
        let decodedCancelledResult = try CotermSidebarXPCCodec.decodeActionResult(
            try CotermSidebarXPCCodec.encodeActionResult(cancelledResult)
        )
    ...
    func testSidebarHostTypedHelpersSendExpectedActions() async throws {
        var actions = [CotermSidebarAction]()
        var refreshCount = 0
        ...
        let host = CotermSidebarHost(
            performAction: { action, reply in
                ...
                reply(CotermSidebarActionResult(accepted: true))
            },
            refreshSnapshot: {
                refreshCount += 1
            }
        ...
        let snapshot = CotermSidebarSnapshot(
            ...
            workspaces: [
                CotermSidebarWorkspace(
                    ...
                    pullRequestURLs: ["https://github.com/emergent-inc/coterm/pull/4994"],
                    surfaces: [
                        CotermSidebarSurface(id: UUID(), title: "Private Surface", kind: .terminal),
                    ]
                ),
            ]
        )

        let filtered = snapshot.filtered(for: [CotermExtensionScope.workspaceList])
        let workspace = try #require(filtered.workspaces.first)
    ...
    func testURLBearingBrowserActionsRequireOpenURLScope() {
        ...
        #expect(CotermSidebarAction.createBrowserSurface(workspaceID: workspaceID, url: nil).requiredScopes == [.createSurface])
        #expect(CotermSidebarAction.createBrowserSurface(workspaceID: workspaceID, url: "https://example.com").requiredScopes == [.createSurface, .openURL])
        #expect(CotermSidebarAction.splitBrowser(workspaceID: workspaceID, surfaceID: surfaceID, direction: .right, url: nil).requiredScopes == [.splitSurface])
        #expect(CotermSidebarAction.splitBrowser(workspaceID: workspaceID, surfaceID: surfaceID, direction: .right, url: "https://example.com").requiredScopes == [.splitSurface, .openURL])
    }

    @Test
    func testWorkspaceCreationWithPathRequiresWorkspacePathScope() {
        #expect(CotermSidebarAction.createWorkspace(title: nil, workingDirectory: nil, select: true).requiredScopes == [.createWorkspace])
        #expect(CotermSidebarAction.createWorkspace(title: nil, workingDirectory: "/tmp/project", select: true).requiredScopes == [.createWorkspace, .createWorkspaceWithPath])
    }
    ...
    func testSidebarHostCancelsPendingAsyncAction() async {
        ...
        let host = CotermSidebarHost(
            performCancellableAction: { _, _ in
                ...
                return CotermSidebarActionCancellation {
        ...
        do {
            ...
            #expect(error as? CotermSidebarActionError == .cancelled)
        }
    ...
    func testSidebarXPCCodecRejectsOversizedManifestPayload() {
        let payload = Data(repeating: 0x20, count: CotermSidebarXPCCodec.maximumManifestPayloadBytes + 1) as NSData

        do {
            _ = try CotermSidebarXPCCodec.decodeManifest(payload)
            Issue.record("Expected oversized manifest payload to be rejected")
        } catch {
            #expect(
                error as? CotermExtensionValidationError == .payloadTooLarge(
                    kind: "manifest",
                    actualBytes: payload.length,
                    maximumBytes: CotermSidebarXPCCodec.maximumManifestPayloadBytes
                )
            )
        }
    }
    ...
    func testSidebarXPCCodecRejectsOversizedActionPayload() {
        let payload = Data(repeating: 0x20, count: CotermSidebarXPCCodec.maximumActionPayloadBytes + 1) as NSData

        do {
            _ = try CotermSidebarXPCCodec.decodeAction(payload)
            ...
            #expect(
                error as? CotermExtensionValidationError == .payloadTooLarge(
                    kind: "action",
                    actualBytes: payload.length,
                    maximumBytes: CotermSidebarXPCCodec.maximumActionPayloadBytes
                )
            )
        }
    }
    ...
    func testSidebarXPCCodecRejectsOversizedSnapshotOnEncodeAndDecode() {
        let oversizedTitle = String(repeating: "x", count: CotermSidebarXPCCodec.maximumSnapshotPayloadBytes)
        let snapshot = CotermSidebarSnapshot(
            sequence: 46,
            selectedWorkspaceID: nil,
            workspaces: [
                CotermSidebarWorkspace(id: UUID(), title: oversizedTitle),
            ]
        )

        do {
            _ = try CotermSidebarXPCCodec.encodeSnapshot(snapshot)
            Issue.record("Expected oversized snapshot payload to be rejected on encode")
        } catch {
            if case let CotermExtensionValidationError.payloadTooLarge(kind, actualBytes, maximumBytes) = error {
                #expect(kind == "snapshot")
                #expect(actualBytes > maximumBytes)
                #expect(maximumBytes == CotermSidebarXPCCodec.maximumSnapshotPayloadBytes)
            } else {
                Issue.record("Unexpected error: \(error)")
            }
        }

        let payload = Data(repeating: 0x20, count: CotermSidebarXPCCodec.maximumSnapshotPayloadBytes + 1) as NSData
        do {
            _ = try CotermSidebarXPCCodec.decodeSnapshot(payload)
            ...
            #expect(
                error as? CotermExtensionValidationError == .payloadTooLarge(
                    kind: "snapshot",
                    actualBytes: payload.length,
                    maximumBytes: CotermSidebarXPCCodec.maximumSnapshotPayloadBytes
                )
            )
        }
    }
    ...
    func testSidebarXPCCodecRejectsOversizedActionResultOnEncodeAndDecode() {
        let result = CotermSidebarActionResult(
            accepted: false,
            message: String(repeating: "x", count: CotermSidebarXPCCodec.maximumActionResultPayloadBytes)
        )

        do {
            _ = try CotermSidebarXPCCodec.encodeActionResult(result)
            Issue.record("Expected oversized action result payload to be rejected on encode")
        } catch {
            if case let CotermExtensionValidationError.payloadTooLarge(kind, actualBytes, maximumBytes) = error {
                #expect(kind == "actionResult")
                #expect(actualBytes > maximumBytes)
                #expect(maximumBytes == CotermSidebarXPCCodec.maximumActionResultPayloadBytes)
            } else {
                Issue.record("Unexpected error: \(error)")
            }
        }

        let payload = Data(repeating: 0x20, count: CotermSidebarXPCCodec.maximumActionResultPayloadBytes + 1) as NSData
        do {
            _ = try CotermSidebarXPCCodec.decodeActionResult(payload)
            ...
            #expect(
                error as? CotermExtensionValidationError == .payloadTooLarge(
                    kind: "actionResult",
                    actualBytes: payload.length,
                    maximumBytes: CotermSidebarXPCCodec.maximumActionResultPayloadBytes
                )
            )
        }
    }
    ...
    func testManifestValidationRejectsUnsupportedAPIVersion() throws {
        ...
        do {
            ...
            #expect(
                error as? CotermExtensionValidationError == .unsupportedAPIVersion(
                    requested: CotermExtensionAPIVersion(major: 2, minor: 1),
                    supported: .sidebarV2
                )
            )
        }
    ...
    func testManifestValidationRejectsOldMajorVersion() throws {
        ...
        do {
            ...
            #expect(
                error as? CotermExtensionValidationError == .unsupportedAPIVersion(
                    requested: CotermExtensionAPIVersion(major: 1, minor: 0),
                    supported: .sidebarV2
                )
            )
        }
