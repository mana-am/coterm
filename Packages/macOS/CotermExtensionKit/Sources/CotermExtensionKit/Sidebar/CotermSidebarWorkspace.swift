...
public struct CotermSidebarWorkspace: Codable, Equatable, Identifiable, Sendable {
    public var id: UUID
    ...
    public var surfaces: [CotermSidebarSurface]
    ...
    public init(from decoder: Decoder) throws {
        ...
        surfaces = try container.decodeIfPresent([CotermSidebarSurface].self, forKey: .surfaces) ?? []
    }

    @_spi(CotermHostTransport)
    public func filtered(for scopes: some Sequence<CotermExtensionScope>) -> CotermSidebarWorkspace {
        let scopeSet = Set(scopes)
        return CotermSidebarWorkspace(
            id: id,
            title: title,
            detail: detail,
            isPinned: isPinned,
