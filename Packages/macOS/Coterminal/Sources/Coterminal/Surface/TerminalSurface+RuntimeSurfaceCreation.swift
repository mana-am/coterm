...
internal import CoterminalCore
internal import CotermAgentLaunch
internal import Darwin
#if DEBUG
internal import CotermDebugLog
#endif
...
extension TerminalSurface {
    ...
    func createNativeRuntimeSurface(
        ...
        var baseConfig = configTemplate ?? CotermSurfaceConfigTemplate()
        var surfaceConfig = ghostty_surface_config_new()
        let magnificationPercent = globalFontMagnificationPercent()
        surfaceConfig.font_size = CotermSurfaceConfigTemplate.runtimeFontSize(
        ...
        Self.applyManagedCotermContextEnvironment(
            Self.cotermContextEnvironment(
                workspaceId: tabId,
                surfaceId: id,
        ...
        setManagedEnvironmentValue("COTERM_SOCKET", "")
        ...
        if let bundledCLIURL = Bundle.main.resourceURL?.appendingPathComponent("bin/coterm"),
           runtimeFilesystem.isExecutableFile(bundledCLIURL.path) {
            setManagedEnvironmentValue("COTERM_BUNDLED_CLI_PATH", bundledCLIURL.path)
        }
        if let bundleId = Bundle.main.bundleIdentifier, !bundleId.isEmpty {
            setManagedEnvironmentValue("COTERM_BUNDLE_ID", bundleId)
        }
        ...
        do {
            ...
            setManagedEnvironmentValue("COTERM_PORT", String(startPort))
            setManagedEnvironmentValue("COTERM_PORT_END", String(startPort + sessionPortRangeSize - 1))
            setManagedEnvironmentValue("COTERM_PORT_RANGE", String(sessionPortRangeSize))
        }

        let spawnPolicy = spawnPolicyProvider.currentSpawnPolicy()
        let claudeHooksEnabled = spawnPolicy.claudeHooksEnabled
        if !claudeHooksEnabled {
            setManagedEnvironmentValue("COTERM_CLAUDE_HOOKS_DISABLED", "1")
        }
        ...
        if !spawnPolicy.codexHooksEnabled {
            setManagedEnvironmentValue("COTERM_CODEX_HOOKS_DISABLED", "1")
        }
        if let customClaudePath = spawnPolicy.customClaudePath {
            setManagedEnvironmentValue("COTERM_CUSTOM_CLAUDE_PATH", customClaudePath)
        }
        ...
        if !spawnPolicy.cursorHooksEnabled {
            setManagedEnvironmentValue("COTERM_CURSOR_HOOKS_DISABLED", "1")
        }
        if !spawnPolicy.geminiHooksEnabled {
            setManagedEnvironmentValue("COTERM_GEMINI_HOOKS_DISABLED", "1")
        }
        if !spawnPolicy.kiroHooksEnabled {
            setManagedEnvironmentValue("COTERM_KIRO_HOOKS_DISABLED", "1")
        }
        setManagedEnvironmentValue("COTERM_KIRO_NOTIFICATION_LEVEL", spawnPolicy.kiroNotificationLevel)
        if !spawnPolicy.ampHooksEnabled {
            setManagedEnvironmentValue("COTERM_AMP_HOOKS_DISABLED", "1")
        }
        ...
        if let claudeShim {
            setManagedEnvironmentValue("COTERM_CLAUDE_WRAPPER_SHIM", claudeShim.executablePath)
            setManagedEnvironmentValue("COTERM_CLAUDE_WRAPPER_SHIM_ROOT", claudeShim.directoryPath)
            ...
            // session resolves the codex executable through COTERM_CODEX_WRAPPER_SHIM
            // (see AgentResumeArgv.codexWrapperShellExecutableToken), so without
            // this the resumed codex bypasses coterm-codex-wrapper and loses its
            ...
            if let codexShim = claudeShim.codexCommandShim {
                setManagedEnvironmentValue("COTERM_CODEX_WRAPPER_SHIM", codexShim.executablePath)
                setManagedEnvironmentValue("COTERM_CODEX_WRAPPER_SHIM_ROOT", codexShim.directoryPath)
            }
            let currentPath = env["PATH"]
                ?? getenv("PATH").map { String(cString: $0) }
                ?? ProcessInfo.processInfo.environment["PATH"]
        ...
        if spawnPolicy.shellIntegrationEnabled,
           ...
           Self.shellIntegrationDirectoryExists(integrationDir) {
            setManagedEnvironmentValue("COTERM_SHELL_INTEGRATION", "1")
            setManagedEnvironmentValue("COTERM_SHELL_INTEGRATION_DIR", integrationDir)
