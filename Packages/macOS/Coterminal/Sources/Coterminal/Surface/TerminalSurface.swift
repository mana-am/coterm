...
public final class TerminalSurface: Identifiable, ObservableObject {
    ...
    // The surface value DTOs live in CoterminalCore; these aliases keep the
    // nested TerminalSurface.NamedKeySendResult/.InputSendResult names that
    // other files use.
    public typealias NamedKeySendResult = CoterminalCore.NamedKeySendResult
    public typealias InputSendResult = CoterminalCore.InputSendResult
    ...
    public typealias CotermContextEnvironment = TerminalSurfaceCotermContextEnvironment
    ...
    /// Port ordinal base/range for COTERM_PORT assignment, snapshotted by the app composition root.
    ...
    /// coterm renderer reclamation: whether the current runtime surface's GPU
    ...
    /// Port ordinal for COTERM_PORT range assignment. Captured at construction so
    /// every runtime startup path uses the same immutable workspace port range.
    let portOrdinal: Int
    let surfaceContext: ghostty_surface_context_e
    let configTemplate: CotermSurfaceConfigTemplate?
    ...
    /// The retained byte-tee lease for the libghostty PTY tee callback (coterm
    ...
    static func cotermContextEnvironment(
        workspaceId: UUID,
        surfaceId: UUID,
        socketPath: String
    ) -> CotermContextEnvironment {
        CotermContextEnvironment(
