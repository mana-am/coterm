...
internal import CotermAgentLaunch
...
extension TerminalSurface {
    ...
    /// Applies the managed coterm context identity keys and protects them.
    public static func applyManagedCotermContextEnvironment(
        _ context: CotermContextEnvironment,
        to environment: inout [String: String],
        protectedKeys: inout Set<String>
    ) {
        let values = [
            "COTERM_SURFACE_ID": context.surfaceId.uuidString,
            "COTERM_WORKSPACE_ID": context.workspaceId.uuidString,
            "COTERM_PANEL_ID": context.surfaceId.uuidString,
            "COTERM_TAB_ID": context.workspaceId.uuidString,
    ...
    public static func applyManagedGitWatchEnvironment(
        ...
        environment["COTERM_NO_GIT_WATCH"] = watchGitStatusEnabled ? "" : "1"
        protectedKeys.insert("COTERM_NO_GIT_WATCH")
        environment["COTERM_NO_PR_WATCH"] = (watchGitStatusEnabled && showPullRequestsEnabled) ? "" : "1"
        protectedKeys.insert("COTERM_NO_PR_WATCH")
    }
    ...
    public static func installClaudeCommandShimIfPossible(
        ...
        do {
            ...
            let script = """
            #!/usr/bin/env bash
            coterm_wrapper=\(shellSingleQuoted(wrapperURL.path))
            ...
            export COTERM_CLAUDE_WRAPPER_SHIM=\(shellSingleQuoted(shimURL.path))
            export COTERM_CLAUDE_WRAPPER_SHIM_ROOT=\(shellSingleQuoted(shimDirectory.path))
            if [[ -x "$coterm_wrapper" ]]; then
                exec "$coterm_wrapper" "$@"
            fi
            ...
            // dir so typed `codex` resolves to coterm-codex-wrapper through the
            ...
            // so runtime surface creation can export COTERM_CODEX_WRAPPER_SHIM into
            // the managed env, which a resumed `codex` session needs to route its
            // resume through the wrapper and keep coterm hooks.
    ...
    /// bundled `coterm-codex-wrapper` exists alongside `coterm-claude-wrapper`. The
    /// shim resolves and execs the codex wrapper; if the wrapper is gone it
    /// strips every coterm shim dir from `PATH` and execs the real `codex`, so the
    ...
    public static func installCodexCommandShimIfPossible(
        ...
        let codexWrapperURL = claudeWrapperURL
            ...
            .appendingPathComponent("coterm-codex-wrapper", isDirectory: false)
            .standardizedFileURL
        ...
        do {
            let script = """
            #!/usr/bin/env bash
            coterm_wrapper=\(shellSingleQuoted(codexWrapperURL.path))
            if [[ ! -x "$coterm_wrapper" && -n "${COTERM_BUNDLED_CLI_PATH:-}" ]]; then
                coterm_candidate="$(dirname "$COTERM_BUNDLED_CLI_PATH")/coterm-codex-wrapper"
                if [[ -x "$coterm_candidate" ]]; then
                    coterm_wrapper="$coterm_candidate"
                fi
            fi
            if [[ ! -x "$coterm_wrapper" ]]; then
                coterm_cli="$(command -v coterm 2>/dev/null || true)"
                if [[ -n "$coterm_cli" ]]; then
                    coterm_candidate="$(dirname "$coterm_cli")/coterm-codex-wrapper"
                    if [[ -x "$coterm_candidate" ]]; then
                        coterm_wrapper="$coterm_candidate"
                    fi
                fi
            fi
            export COTERM_CODEX_WRAPPER_SHIM=\(shellSingleQuoted(shimURL.path))
            export COTERM_CODEX_WRAPPER_SHIM_ROOT=\(shellSingleQuoted(shimDirectory.path))
            if [[ -x "$coterm_wrapper" ]]; then
                exec "$coterm_wrapper" "$@"
            fi
            coterm_path_without_shim=""
            ...
            IFS=:
            for coterm_entry in ${PATH:-}; do
                if [[ "$coterm_entry" == "$COTERM_CODEX_WRAPPER_SHIM_ROOT" || "$coterm_entry" == */coterm-cli-shims/* || "$coterm_entry" == */coterm-cli-shims ]]; then
                    continue
                fi
                if [[ -z "$coterm_path_without_shim" ]]; then
                    coterm_path_without_shim="$coterm_entry"
    ...
    public static func applyManagedFishStartupEnvironment(
        ...
        environment["COTERM_FISH_INTEGRATION_FILE"] = integrationFile
        environment["COTERM_FISH_USER_CONFIG_ALREADY_LOADED"] = "1"
        protectedKeys.insert("COTERM_FISH_INTEGRATION_FILE")
        protectedKeys.insert("COTERM_FISH_USER_CONFIG_ALREADY_LOADED")
    }
    ...
    /// callers must not advertise it (COTERM_SHELL_INTEGRATION_DIR) or redirect
    /// shell startup at it.
    ...
    public static func applyManagedShellSpecificStartupEnvironment(
        ...
        case "zsh":
            ...
            if userGhosttyShellIntegrationMode != "none" { setManagedEnvironmentValue("COTERM_LOAD_GHOSTTY_ZSH_INTEGRATION", "1") }
            ...
            if let candidateZdotdir, !candidateZdotdir.isEmpty {
                ...
                if candidateZdotdir != ghosttyZdotdir { setManagedEnvironmentValue("COTERM_ZSH_ZDOTDIR", candidateZdotdir) }
            }
            setManagedEnvironmentValue("ZDOTDIR", integrationDir)
        case "bash":
            if userGhosttyShellIntegrationMode != "none" { setManagedEnvironmentValue("COTERM_LOAD_GHOSTTY_BASH_INTEGRATION", "1") }
            let bashBootstrapPath = (integrationDir as NSString).appendingPathComponent("coterm-bash-bootstrap.bash")
            do {
    ...
    /// The managed fish launch command sourcing the coterm integration file.
    public static func managedFishShellCommand(shell: String) -> String {
        let initCommand = #"source "$COTERM_FISH_INTEGRATION_FILE""#
