...
///
/// Implemented by `TerminalPasteboardService` in `CoterminalServices` and
...
public protocol TerminalClipboardReading: AnyObject, Sendable {
    ...
    /// locations coterm does not support.
    ...
    /// applying coterm's flavor-priority rules (file URLs, image-only guard,
