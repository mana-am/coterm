...
public import CotermFoundation
public import Foundation

/// The resolved coterm terminal configuration: fonts, colors, theme, scrollback,
/// and sidebar appearance parsed from the user's ghostty config files plus
/// coterm's managed defaults.
///
...
/// resolves themes by light/dark color scheme, and folds in coterm's managed
...
public struct GhosttyConfig {
    ...
    /// The terminal theme coterm enforces for every user-facing terminal surface.
    public static let cotermEnforcedThemeName = "Anysphere Dark"
    /// Native fallback light theme name used when resolving coterm-managed colors.
    public static let cotermDefaultLightThemeName = cotermEnforcedThemeName
    /// Native fallback dark theme name used when resolving coterm-managed colors.
    public static let cotermDefaultDarkThemeName = cotermEnforcedThemeName
    ...
    public static let defaultSidebarFontSize = CGFloat(CotermGhosttyConfigSettingEditor.defaultSidebarFontSize)
    /// The minimum sidebar font size the parser will clamp to.
    public static let minSidebarFontSize = CGFloat(CotermGhosttyConfigSettingEditor.minSidebarFontSize)
    /// The maximum sidebar font size the parser will clamp to.
    public static let maxSidebarFontSize = CGFloat(CotermGhosttyConfigSettingEditor.maxSidebarFontSize)
    /// The default surface tab-bar font size, in points.
    public static let defaultSurfaceTabBarFontSize = CGFloat(CotermGhosttyConfigSettingEditor.defaultSurfaceTabBarFontSize)
    /// The minimum surface tab-bar font size the parser will clamp to.
    public static let minSurfaceTabBarFontSize = CGFloat(CotermGhosttyConfigSettingEditor.minSurfaceTabBarFontSize)
    /// The maximum surface tab-bar font size the parser will clamp to.
    public static let maxSurfaceTabBarFontSize = CGFloat(CotermGhosttyConfigSettingEditor.maxSurfaceTabBarFontSize)
    ...
    /// Creates a config with coterm's built-in default appearance, before any
    ...
    private static func cotermConfigPaths(
        ...
        return CotermGhosttyConfigPathResolver().loadConfigURLs(
    ...
    static func loadFromDisk(preferredColorScheme: ColorSchemePreference) -> GhosttyConfig {
        ...
        configPaths.append(contentsOf: cotermConfigPaths())
        ...
        if startupPreviewOverride?.loadsRealUserConfig ?? true {
            ...
            if config.theme == nil,
               Self.shouldApplyManagedDefaultAppearance(configPaths: configPaths) {
                config.applyCotermDefaultAppearance(
                    environment: ProcessInfo.processInfo.environment,
                    bundleResourceURL: Bundle.main.resourceURL,
                    preferredColorScheme: preferredColorScheme
                )
            }
            config.applyCotermEnforcedAppearance(
                environment: ProcessInfo.processInfo.environment,
                bundleResourceURL: Bundle.main.resourceURL,
                preferredColorScheme: preferredColorScheme
            )
    ...
    /// Applies coterm's managed default theme for `preferredColorScheme` by parsing
    /// the bundled (or fallback) theme config contents into this config.
    public mutating func applyCotermDefaultAppearance(
        environment: [String: String],
        bundleResourceURL: URL?,
        preferredColorScheme: ColorSchemePreference
    ) {
        parse(
            Self.cotermDefaultThemeConfigContents(
                preferredColorScheme: preferredColorScheme,
    ...
    /// Applies coterm's enforced terminal theme after user config has loaded.
    public mutating func applyCotermEnforcedAppearance(
        environment: [String: String],
        bundleResourceURL: URL?,
        preferredColorScheme: ColorSchemePreference
    ) {
        theme = Self.cotermEnforcedThemeName
        parse(
            Self.cotermDefaultThemeConfigContents(
                preferredColorScheme: preferredColorScheme,
                environment: environment,
    ...
    /// The coterm managed default theme name for `preferredColorScheme`.
    public static func cotermDefaultThemeName(preferredColorScheme: ColorSchemePreference) -> String {
        switch preferredColorScheme {
        case .light:
            return cotermDefaultLightThemeName
        case .dark:
            return cotermDefaultDarkThemeName
        }
    }

    /// The coterm managed default theme's config contents for `preferredColorScheme`,
    /// read from the resolved theme file when available, otherwise a built-in
    /// fallback palette.
    public static func cotermDefaultThemeConfigContents(
        preferredColorScheme: ColorSchemePreference,
        environment: [String: String] = ProcessInfo.processInfo.environment,
        bundleResourceURL: URL? = Bundle.main.resourceURL
    ) -> String {
        if let url = cotermDefaultThemeConfigURL(
            preferredColorScheme: preferredColorScheme,
            environment: environment,
            bundleResourceURL: bundleResourceURL
        ...
        return cotermDefaultFallbackConfigContents(preferredColorScheme: preferredColorScheme)
    }

    /// The on-disk URL of the coterm managed default theme for `preferredColorScheme`,
    /// or `nil` when no candidate theme file exists.
    public static func cotermDefaultThemeConfigURL(
        preferredColorScheme: ColorSchemePreference,
        environment: [String: String] = ProcessInfo.processInfo.environment,
        bundleResourceURL: URL? = Bundle.main.resourceURL
    ) -> URL? {
        let themeName = cotermDefaultThemeName(preferredColorScheme: preferredColorScheme)
    ...
    private static func cotermDefaultFallbackConfigContents(
    ...
    public mutating func parse(
        ...
        for line in lines {
            ...
            // ignored, matching `CotermGhosttyConfigSettingEditor.parsedSetting`.
    ...
    /// directives that determine whether coterm should apply its managed default
    ...
    public struct UserAppearanceConfigSummary {
        ...
        /// Whether coterm should apply its managed default appearance: true only
    ...
    /// Whether coterm should inject its managed default appearance: true only when
    ...
    public static func clampedSidebarFontSize(_ value: CGFloat) -> CGFloat {
        CGFloat(CotermGhosttyConfigSettingEditor().clampedSidebarFontSize(Double(value)))
    }
    ...
    public static func clampedSurfaceTabBarFontSize(_ value: CGFloat) -> CGFloat {
        CGFloat(CotermGhosttyConfigSettingEditor().clampedSurfaceTabBarFontSize(Double(value)))
    }
    ...
    /// The terminal color-scheme preference coterm renders with. Pinned to
    ...
    /// `.light` and `nil` for `.dark`. coterm injects a resolved plain `theme = X`
    ...
    /// https://github.com/emergent-inc/coterm/issues/3459). Injecting for an unset
    ...
    /// The ordered list of filesystem paths coterm searches for the given theme
    ...
    public static func themeSearchPaths(
        ...
        for appSupportDirectory in CotermApplicationSupportDirectories(environment: environment).userDirectories {
            appendUniquePath(
                appSupportDirectory
                    .appendingPathComponent(CotermGhosttyConfigPathResolver.releaseBundleIdentifier, isDirectory: true)
