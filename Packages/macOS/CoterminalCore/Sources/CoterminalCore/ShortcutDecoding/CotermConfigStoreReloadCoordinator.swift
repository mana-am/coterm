/// Orchestrates reloading every per-window coterm configuration store and refreshing
/// window titles, as the app delegate's `reloadCotermConfigStores(source:)` did.
///
/// Owned by the app delegate and given a weak reference to it through
/// `CotermConfigStoreReloadEnvironment`, so the delegate retaining the coordinator
...
@MainActor
public final class CotermConfigStoreReloadCoordinator {
    private weak var environment: (any CotermConfigStoreReloadEnvironment)?
    ...
    public init(
        environment: any CotermConfigStoreReloadEnvironment,
