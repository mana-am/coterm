...
/// resolves to the coterm wrapper; both paths are exported to the shell as
/// `COTERM_CLAUDE_WRAPPER_SHIM` / `COTERM_CLAUDE_WRAPPER_SHIM_ROOT`.
public struct TerminalSurfaceClaudeCommandShim: Equatable, Sendable {
    ...
    /// directory, if the bundled `coterm-codex-wrapper` was available. Carried
    /// alongside the claude shim so the runtime surface creation can export
    /// `COTERM_CODEX_WRAPPER_SHIM` into the managed terminal environment (the
    /// same way it exports `COTERM_CLAUDE_WRAPPER_SHIM`), which a resumed codex
    /// session needs to route its `codex resume` through the wrapper and keep
    /// coterm hooks. `nil` when the codex wrapper was absent.
