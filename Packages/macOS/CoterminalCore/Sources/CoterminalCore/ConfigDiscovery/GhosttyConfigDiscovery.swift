...
/// Resolves where coterm's Ghostty configuration lives and what it should inject,
...
/// includes) for the font and appearance directives that drive coterm's
...
public struct GhosttyConfigDiscovery {
    ...
    /// own identifier.
    public static let releaseBundleIdentifier = CotermGhosttyConfigPathResolver.releaseBundleIdentifier

    private let fileReader: any GhosttyConfigFileReading
    private let fontProbe: any GhosttyFontProbing
    private let pathResolver: CotermGhosttyConfigPathResolver
    ...
    public init(
        ...
        pathResolver: CotermGhosttyConfigPathResolver = CotermGhosttyConfigPathResolver()
    ) {
    ...
    /// already covers the ranges coterm would otherwise auto-map.
    ...
    /// Returns only the CJK mappings coterm should auto-inject after respecting
    ...
    /// application-support config paths coterm may load at runtime.
    ...
    /// Whether coterm should inject its managed CJK `font-codepoint-map` fallback.
    ...
    /// Whether coterm should apply its managed default appearance across the
    ...
    /// Computes the resolved plain-theme override coterm must inject when the
    ...
    public func conditionalThemeOverrideConfigContents(
        ...
        // syntax, even when both sides resolve to the same theme. `coterm themes
        ...
        // https://github.com/emergent-inc/coterm/issues/3459. Only override sides the
    ...
    /// summarizes the font directives relevant to coterm's injected CJK fallback.
    ...
    /// `config-file` processing, including native Ghostty, legacy, and coterm
    ...
    public func loadedGhosttyConfigScanPaths(
        ...
        let appSupportConfigURLs = cotermAppSupportConfigURLs(
    ...
    /// Whether coterm should load the legacy `config` file because the new
    ...
    /// Resolves the coterm Application Support Ghostty config file URLs for the
    /// running build, via the shared ``CotermGhosttyConfigPathResolver``.
    public func cotermAppSupportConfigURLs(
