import Testing
@testable import CoterminalCore

@MainActor
private final class FakeConfigStore: CotermConfigStoreReloading {
    private(set) var loadAllCount = 0
    func loadAll() { loadAllCount += 1 }
}

@MainActor
private final class FakeEnvironment: CotermConfigStoreReloadEnvironment {
    var stores: [FakeConfigStore]
    private(set) var titleRefreshCount = 0

    init(stores: [FakeConfigStore]) { self.stores = stores }

    var reloadableConfigStores: [any CotermConfigStoreReloading] { stores }
    func refreshWindowTitlesAfterConfigReload() { titleRefreshCount += 1 }
}

@MainActor
@Suite struct CotermConfigStoreReloadCoordinatorTests {
    @Test func reloadsEachDistinctStoreOnceAndRefreshesTitles() {
        ...
        let coordinator = CotermConfigStoreReloadCoordinator(environment: env) { source, count in
    ...
    @Test func reloadWithNoStoresStillRefreshesTitles() {
        ...
        var reportedCount: Int?
        let coordinator = CotermConfigStoreReloadCoordinator(environment: env) { _, count in
            reportedCount = count
        }
