...
@Suite struct TerminalQuicklookPathResolutionTests {
    @Test func fallsBackToStrippedPathWhenLiteralPathIsMissing() {
        let strippedPath = "/tmp/coterm-cmdclick-path.md"
        #expect(
    ...
    @Test func prefersLiteralPathThatReallyEndsWithDot() {
        let literalPath = "/tmp/coterm-cmdclick-literal-dot.md."
        let strippedPath = "/tmp/coterm-cmdclick-literal-dot.md"
        #expect(
    ...
    @Test func prefersLiteralPathThatReallyEndsWithParen() {
        let literalPath = "/tmp/coterm-cmdclick-literal-paren)"
        let strippedPath = "/tmp/coterm-cmdclick-literal-paren"
        #expect(
            TerminalPathResolver(fileExists: existsIn([literalPath, strippedPath])).resolveQuicklookPath(
                literalPath,
                cwd: "/tmp"
    ...
    @Test func unquotesShellQuotedToken() {
        let existingFile = "/tmp/coterm quicklook spaced.md"
        #expect(
            TerminalPathResolver(fileExists: existsIn([existingFile])).resolveQuicklookPath(
                "\"\(existingFile)\"",
                cwd: "/tmp"
    ...
    @Test func unescapesBackslashEscapedSpaces() {
        let existingFile = "/tmp/coterm quicklook escaped.md"
        #expect(
            TerminalPathResolver(fileExists: existsIn([existingFile])).resolveQuicklookPath(
                "/tmp/coterm\\ quicklook\\ escaped.md",
                cwd: "/tmp"
            ) == existingFile
        )
    }
...
@Suite struct TerminalVisibleLineResolutionTests {
    ...
    @Test func resolvesRawSegmentUnderColumn() throws {
        let existingFile = "/tmp/coterm-visible-line.md"
        let line = "open /tmp/coterm-visible-line.md now"
        ...
        #expect(resolution.rawToken == "/tmp/coterm-visible-line.md")
    }

    @Test func resolvesShellEscapedTokenSpanningSpaces() throws {
        let existingFile = "/tmp/coterm visible escaped.md"
        let line = "cat /tmp/coterm\\ visible\\ escaped.md"
        let resolution = try #require(
            TerminalPathResolver(fileExists: existsIn([existingFile])).resolveVisibleLinePath(
                line,
                column: 6,
