...
struct WorkspaceRemoteConfigurationSSHBatchCommandsTests {
    ...
    func daemonTransportArgumentsWithoutSlot() {
        let arguments = configuration().daemonTransportArguments(remotePath: "/remote/cotermd-remote")
        let expectedCommand = #"sh -c 'exec '"'"'/remote/cotermd-remote'"'"' '"'"'serve'"'"' '"'"'--stdio'"'"''"#
        #expect(
            arguments == ["-T"]
                + expectedBatchArguments
                + ["-o", "RequestTTY=no", "coterm-macmini", expectedCommand]
        )
    }
    ...
    func daemonTransportArgumentsWithSlot() {
        ...
        ).daemonTransportArguments(remotePath: "/remote/cotermd-remote")
        let expectedCommand = #"sh -c 'exec '"'"'/remote/cotermd-remote'"'"' '"'"'serve'"'"' '"'"'--stdio'"'"' '"'"'--persistent'"'"' '"'"'--slot'"'"' '"'"'ws-1'"'"''"#
        #expect(
            arguments == ["-T"]
                + expectedBatchArguments
                + ["-o", "RequestTTY=no", "coterm-macmini", expectedCommand]
        ...
        #expect(
            arguments == ["-N", "-T", "-S", "none"]
                ...
                + [
                    ...
                    "-o", "RequestTTY=no",
                    "-L", "127.0.0.1:64123:/run/cotermd-remote.sock",
                    "coterm-macmini",
                ]
        )
    }
    ...
    func reverseRelayControlMasterArguments() throws {
        ...
        #expect(
            arguments == expectedBatchArguments
                + ["-O", "forward", "-R", "127.0.0.1:64007:127.0.0.1:54321", "coterm-macmini"]
        )
    }
    ...
    func reverseRelayControlMasterCancelArguments() throws {
        ...
        #expect(
            arguments == expectedBatchArguments
                + ["-O", "cancel", "-R", "127.0.0.1:64007", "coterm-macmini"]
        )
    }
