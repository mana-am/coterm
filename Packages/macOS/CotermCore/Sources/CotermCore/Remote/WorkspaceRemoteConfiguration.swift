...
public struct WorkspaceRemoteConfiguration: Equatable, Sendable {
    ...
    /// CLI relay port for remote `coterm` command forwarding.
    ...
    /// True for cloud-VM remotes (Freestyle snapshots) where cotermd-remote is pre-baked in
    ...
    /// the baked daemon through an SSH local forward to `/run/cotermd-remote.sock`.
