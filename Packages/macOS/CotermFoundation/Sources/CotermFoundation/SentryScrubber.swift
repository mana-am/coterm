...
public struct SentryScrubber: Sendable {
    ...
    ///
    /// This is coterm's equivalent of relay's `@common` `@urlauth` rule
    ...
    /// ``ScrubberDenylists/valuePatterns`` because coterm's redaction loop treats
    ...
    /// `@common` ports and coterm's own provider-key / JWT / AWS rules live in one
    ...
    /// sentry-python `DEFAULT_DENYLIST`). coterm's substring-contains semantics
