...
/// from Sentry's relay `@common` scrubbers, plus coterm's own provider-key / JWT /
...
public struct ScrubberDenylists: Sendable {
    ...
    /// sentry-python itself credits as "stolen from relay". coterm's substring
    ...
    static let sensitiveKeyMarkers: [String] = [
        ...
        // coterm-original markers retained (not in sentry-python's list) so no
    ...
    /// (PII keys added because coterm runs with `sendDefaultPii = false`), with the
    ...
    static let sensitiveKeyExactMarkers: Set<String> = [
        ...
        // sentry-python DEFAULT_PII_DENYLIST (coterm sends sendDefaultPii = false).
    ...
    ///
    /// The first seven entries are coterm-original and partly exceed relay (relay
    ...
    /// value rules coterm was missing, ported with no capture group so coterm's
    ...
    /// members are intentionally NOT duplicated here: coterm already covers email /
    ...
    static let valuePatterns: [SentryRegexPattern] = [
        // --- coterm-original rules (kept verbatim; partly exceed relay) ---
        ...
        // (relay-pii/src/regexes.rs:341-345), hand-tuned for coterm's free text.
        //
        ...
        // prefixes. No relay equivalent — Coterm-specific add for dev secrets.
        ...
        // --- relay @common value rules coterm was missing (ported, no group) ---
        // @pemkey — a PEM private/public key block. relay captures group 1 (the
        // body) and replaces only the body; coterm redacts the WHOLE block (header,
        ...
        // dotall flag (coterm's SentryRegexPattern sets only .caseInsensitive).
