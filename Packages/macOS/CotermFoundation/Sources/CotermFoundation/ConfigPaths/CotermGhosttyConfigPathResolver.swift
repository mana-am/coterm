...
/// Resolves which coterm-managed Ghostty config file under Application Support is
...
public struct CotermGhosttyConfigPathResolver {
    ...
    /// The path coterm writes edits to: always `config.ghostty` under the active
    ...
    /// The ordered list of coterm-managed config files to load, applying the
