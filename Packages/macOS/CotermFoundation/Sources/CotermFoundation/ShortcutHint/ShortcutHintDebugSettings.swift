...
public struct ShortcutHintDebugSettings {
    ...
    /// `shortcuts.showModifierHoldHints` toggle. `CotermFoundation` is a leaf
    /// module and cannot import `CotermSettings`, so the key is duplicated here;
    ...
    public var alwaysShowHints: Bool {
        Self.defaultAlwaysShowHints || environment["COTERM_UI_TEST_SHORTCUT_HINTS_ALWAYS_SHOW"] == "1"
    }
