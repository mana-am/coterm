...
@Suite struct CommandRunnerTests {
    ...
    @Test func resolvesCommandViaFallbackDirectoryOutsidePath() throws {
        ...
        let dir = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-command-path-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: dir) }

        let name = "coterm-gh-test-\(UUID().uuidString)"
        ...
        #expect(runner.resolvedCommandPath(executable: "coterm-missing-\(UUID().uuidString)") == nil)
    }
    ...
    @Test func unresolvableCommandRunsViaEnvAndExitsNonZero() async {
        ...
        let result = await runner.run(
            directory: tempDir,
            executable: "coterm-definitely-not-a-real-command",
            arguments: [],
            timeout: 10
        )
        #expect(result.executionError == nil)
