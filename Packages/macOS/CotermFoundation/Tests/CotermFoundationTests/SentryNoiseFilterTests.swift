...
final class SentryNoiseFilterTests: XCTestCase {
    ...
    func testDropsExpectedCLISocketDisconnectsInSocketStages() {
        ...
        XCTAssertTrue(filter.isExpectedCLISocketTransportFailure(
            ...
            message: "Failed to connect to socket at /tmp/coterm.sock (Connection refused, errno 61)"
        ))
        XCTAssertTrue(filter.isExpectedCLISocketTransportFailure(
            stage: "socket_connect",
            message: "Socket not found at /tmp/coterm.sock"
        ))
    }
    ...
    func testKeepsActionableSocketFailures() {
        ...
        XCTAssertFalse(filter.isExpectedCLISocketTransportFailure(
            stage: "socket_connect",
            message: "Failed to connect to socket at /tmp/coterm.sock (Permission denied, errno 13)"
        ))
    }
    ...
    func testErrnoMatchingRequiresExactCode() {
        XCTAssertFalse(filter.isExpectedCLISocketTransportFailure(
            stage: "socket_connect",
            message: "Failed to connect to socket at /tmp/coterm.sock (Invalid argument, errno 22)"
        ))
        XCTAssertFalse(filter.isExpectedCLISocketTransportFailure(
            stage: "socket_command",
        ...
        XCTAssertTrue(filter.isExpectedCLISocketTransportFailure(
            stage: "socket_connect",
            message: "Failed to connect to socket at /tmp/coterm.sock (errno=2)"
        ))
    }
