...
/// transitions for coterm's custom (non-Sparkle-UI) update surface.
///
...
final class UpdateDriver: NSObject, @preconcurrency SPUUserDriver {
    ...
    /// Whether the running build is a coterm DEV/staging build that is not on the public release
    ...
    func show(_ request: SPUUpdatePermissionRequest,
        ...
        if env["COTERM_UI_TEST_TRIGGER_UPDATE_CHECK"] == "1" || env["COTERM_UI_TEST_AUTO_ALLOW_PERMISSION"] == "1" {
...
#endif
        // Never show Sparkle's permission UI. coterm always enables scheduled checks and keeps
    ...
    func showUpdateReleaseNotes(with downloadData: SPUDownloadData) {
        // coterm uses Sparkle's UI for release notes links instead.
    }
    ...
    func showUpdateInFocus() {
        // No-op; coterm never shows Sparkle dialogs.
    }
