...
/// Registers coterm's Sparkle preference defaults and performs the one-time migration that
...
/// configuration, so this is a value type constructed with them (defaulting to coterm's
...
public struct UpdateSettings: Sendable {
    ...
    /// coterm's marker that the v2 automatic-checks migration already ran.
    public static let migrationKey = "coterm.sparkle.automaticChecksMigration.v2"
    ...
    /// The scheduled-check interval coterm registers (1h by default).
    public let scheduledCheckInterval: TimeInterval

    /// Creates the settings with coterm's defaults.
    ///
