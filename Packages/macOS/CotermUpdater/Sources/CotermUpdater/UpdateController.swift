...
/// Coordinates coterm's custom Sparkle update flow: owns the `SPUUpdater` and its
...
public final class UpdateController {
    ...
    /// Whether the running build is a coterm DEV/staging build that must never be compared against
    ...
    ///   - settings: The Sparkle defaults/migration configuration. Defaults to coterm's hourly check.
    ...
    public init(log: any UpdateLogging,
        ...
        if isDevLikeBundle {
            ...
            // coterm's launch/background probe is short-circuited by the `automaticallyChecksForUpdates`
    ...
    public func startUpdaterIfNeeded() {
        ...
        if ProcessInfo.processInfo.environment["COTERM_UI_TEST_RESET_SPARKLE_PERMISSION"] == "1" {
    ...
    private func recordUITestTimestamp(key: String) {
        ...
        guard let path = env["COTERM_UI_TEST_TIMING_PATH"] else { return }
...
extension UpdateController {
    ...
    /// `isStagingBundleIdentifier` (in the CotermSettings package). The classification is
    /// duplicated here deliberately to avoid introducing a `CotermUpdater → CotermSettings` package
