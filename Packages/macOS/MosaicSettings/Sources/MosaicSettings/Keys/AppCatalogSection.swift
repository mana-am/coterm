...
public struct AppCatalogSection: SettingCatalogSection {
    ...
    public let openSupportedFilesInMosaic = DefaultsKey<Bool>(
        id: "app.openSupportedFilesInMosaic",
        defaultValue: true,
        userDefaultsKey: "openSupportedFilesInMosaic"
    )
    ...
    public let openMarkdownInMosaicViewer = DefaultsKey<Bool>(
        id: "app.openMarkdownInMosaicViewer",
        defaultValue: true,
        userDefaultsKey: "openMarkdownInMosaicViewer"
    )

    /// App-wide magnification percent for mosaic-owned fonts and chrome.
    ...
    /// Shared, cross-tag default display that DEBUG mosaic builds open new
    /// windows on, identified by the display's `localizedName` (e.g.
    /// `"LG HDR 4K"`). Empty means the system default placement.
    ///
    /// JSON-backed (`JSONKey`) on purpose: `mosaic.json` lives at a fixed path
