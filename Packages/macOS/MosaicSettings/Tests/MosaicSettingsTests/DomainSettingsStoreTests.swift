...
/// the mosaicApp static settings-namespace enums. Each suite pins the legacy
/// read/write semantics (defaults, legacy-key fallback chains, trim rules)
/// against a scratch `UserDefaults` suite.
private func makeScratchDefaults() -> UserDefaults {
    UserDefaults(suiteName: "mosaic.tests.\(UUID().uuidString)")!
}
...
struct FileRouteSettingsStoreTests {
    ...
    private func makeTemporaryFile() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("mosaic-route-\(UUID().uuidString).md")
        try Data("# hi\n".utf8).write(to: url)
        return url
    }
    ...
    @Test func routesOnlyReadableRegularFiles() throws {
        ...
        #expect(!store.shouldRouteMarkdown(path: "/nonexistent/mosaic-test.md"))
    }
    ...
    @Test func disabledTogglesShortCircuitRouting() throws {
        ...
        defaults.set(false, forKey: "openMarkdownInMosaicViewer")
        #expect(!store.shouldRouteMarkdown(path: file.path))

        defaults.set(false, forKey: "openSupportedFilesInMosaic")
        #expect(!store.shouldRouteSupportedFile(path: file.path))
    }
    ...
    @Test func setsPostDidChangeNotifications() {
        ...
        #expect(defaults.object(forKey: "openMarkdownInMosaicViewer") as? Bool == false)
        #expect(defaults.object(forKey: "openSupportedFilesInMosaic") as? Bool == false)
