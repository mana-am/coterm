...
/// Minimal one-shot unix socket server standing in for the local coterm
...
private final class FakeUnixSocketServer: @unchecked Sendable {
    ...
    init(response: Data) throws {
        ...
        path = NSTemporaryDirectory() + "coterm-relay-test-\(UUID().uuidString.prefix(8)).sock"
...
struct RemoteCLIRelayServerTests {
    ...
    func authAndForwardRoundTrip() throws {
        ...
        #expect(challenge["protocol"] as? String == "coterm-relay-auth")
