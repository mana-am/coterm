public import CotermCore
public import Foundation
internal import CotermSettings
internal import CryptoKit

/// Mediates the cotermd-remote release manifest and the local cache of
...
public struct RemoteDaemonManifestRepository: Sendable {
    ...
    /// Creates a repository rooted at `homeDirectory`'s coterm state cache.
    ///
    /// - Parameters:
    ...
    /// (`<state>/remote-daemons/<version>/<goOS>-<goArch>/cotermd-remote`),
    /// creating the cache root directory if needed.
    ///
    /// Cached under the non-TCC coterm state directory (matching the CLI's
    /// `remoteDaemonCacheURL`) rather than Application Support, so the
    /// separately-signed CLI can read it on `coterm ssh` without tripping the
    /// macOS Sequoia "access data from other apps" prompt
    /// (https://github.com/emergent-inc/coterm/issues/5146).
    ...
    public func fetchManifest(releaseURL: String, version: String) -> WorkspaceRemoteDaemonManifest? {
        guard let manifestURL = URL(string: "\(releaseURL)/cotermd-remote-manifest.json") else { return nil }
        let request = NSMutableURLRequest(url: manifestURL)
        request.timeoutInterval = 15
        request.setValue("coterm/\(version)", forHTTPHeaderField: "User-Agent")
    ...
    public func downloadBinary(
        ...
        guard let url = URL(string: entry.downloadURL) else {
            throw NSError(domain: "coterm.remote.daemon", code: 25, userInfo: [
        ...
        session.downloadTask(with: request as URLRequest) { localURL, response, error in
            ...
            if let httpResponse = response as? HTTPURLResponse,
               !(200...299).contains(httpResponse.statusCode) {
                downloadError = NSError(domain: "coterm.remote.daemon", code: 26, userInfo: [
        ...
        guard let downloadedURL else {
            throw NSError(domain: "coterm.remote.daemon", code: 27, userInfo: [
        ...
        if downloadedSHA != entry.sha256.lowercased() {
            ...
            if let releaseURL,
               ...
               downloadedSHA == liveEntry.sha256.lowercased() {
                ...
                throw NSError(domain: "coterm.remote.daemon", code: 28, userInfo: [
    ...
    private func cacheRoot() throws -> URL {
        let cacheRoot = CotermStateDirectory.url(homeDirectory: homeDirectory)
