...
/// coterm socket, then materialized into a `WorkstreamItem` by the store.
///
...
/// `_opencode_request_id`. `context` is Coterm-specific and optional.
public struct WorkstreamEvent: Codable, Sendable, Equatable {
    ...
    /// coterm hook wrappers already emit on stdin so no translation layer is
    /// needed.
