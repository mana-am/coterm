...
/// (`AgentResumeCommandBuilder` in the app target) and the standalone `coterm-cli` surface-restore
...
///
/// Resolution order mirrors the historical app builder: a coterm wrapper launcher
...
public struct AgentResumeArgv: Sendable, Equatable {
    ...
    /// The shell token that resolves coterm's `claude` wrapper at exec time.
    ///
    /// The claude resume/fork argv emits a bare `claude` executable and relies on
    /// coterm's `claude` wrapper (which re-injects the hook `--settings` on `--resume`)
    /// being reachable. Inside coterm terminals the wrapper is reached two ways: a
    ...
    ///
    /// `COTERM_CLAUDE_WRAPPER_SHIM` is a *managed* terminal environment variable (set by
    ...
    /// `-lic` launcher. https://github.com/emergent-inc/coterm/issues/5639
    ///
    ...
    /// fallback used when the variable is unset outside coterm.
    ///
    ...
    public static let claudeWrapperShellExecutableToken =
        "\"$([ -x \"${COTERM_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$COTERM_CLAUDE_WRAPPER_SHIM\" || printf claude)\""

    /// The shell token that resolves coterm's `codex` wrapper at exec time.
    ///
    ...
    /// `coterm-codex-wrapper` and dropping every coterm hook (no `SessionStart`, the
    ...
    /// environment variable `COTERM_CODEX_WRAPPER_SHIM` (set by
    ...
    /// the hooks fire. https://github.com/emergent-inc/coterm/issues/5639
    ///
    ...
    /// variable is unset outside coterm.
    ///
    ...
    public static let codexWrapperShellExecutableToken =
        "\"$([ -x \"${COTERM_CODEX_WRAPPER_SHIM:-}\" ] && printf '%s' \"$COTERM_CODEX_WRAPPER_SHIM\" || printf codex)\""
    ...
    /// `"$_coterm_resume_shell" -c <command>` for its `csh|tcsh` and `*` branches), and the
    ...
    /// apply), `sh` inherits `COTERM_CLAUDE_WRAPPER_SHIM` from the managed terminal
    /// environment and resolves the token, and outside coterm the unset variable still
    ...
    /// composing on the outside. https://github.com/emergent-inc/coterm/issues/5639
    ...
    ///
    /// Claude-launcher resumes that resolve to coterm's own CLI (`coterm claude-teams --resume …`)
    /// emit no bare `claude` executable: they were already portable quoted words, the coterm
    ...
    /// layer lets the app resume builder and the coterm-cli surface-restore publisher share one
    /// wrapper-routing seam. https://github.com/emergent-inc/coterm/issues/5639
    ...
    /// identically while `sh` still inherits `COTERM_CODEX_WRAPPER_SHIM` from the
    ...
    /// Call only for the codex kind. https://github.com/emergent-inc/coterm/issues/5639
    ...
    /// The result of resolving a coterm wrapper launcher (the `claude-teams` / `codex-teams` / `omo`
    /// style launchers coterm injects), checked before the per-kind verb.
    public enum LauncherResolution: Sendable, Equatable {
        /// The launcher is a coterm wrapper; the associated value is its resume argv, or `nil` when the
    ...
    /// Resolves a resume argv from a coterm wrapper launcher, or ``LauncherResolution/passthrough`` when
    ...
    public func launcherResolution(
        ...
        switch launcher {
        case "claudeTeams":
            let parts = commandParts(executablePath: executablePath, arguments: arguments, fallbackExecutable: "coterm")
            var tail = parts.tail
    ...
    /// Builds the claude resume argv, routing it through coterm's `claude` wrapper
    /// so coterm hooks fire on the resumed session.
    ///
    /// coterm injects Claude Code's hook `--settings` from the `coterm-claude-wrapper`,
    ...
    /// *real* claude binary (`COTERM_AGENT_LAUNCH_EXECUTABLE`), so resuming with it
    ...
    /// the `COTERM_CLAUDE_WRAPPER_SHIM` managed env var), because relying on the wrapper
    /// being first on `PATH` does not hold inside the `$SHELL -lic` restore launcher
    /// (https://github.com/emergent-inc/coterm/issues/5639). The captured executable is
    /// intentionally ignored for claude; the wrapper resolves the real binary
    /// (honouring `COTERM_CUSTOM_CLAUDE_PATH`).
