import Foundation
import Testing
@testable import CotermAgentLaunch
...
struct WorkstreamPersistenceTests {
    ...
    func appendAndLoad() async throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-workstream-test-\(UUID().uuidString).jsonl")
    ...
    func loadPageBeforeCursor() async throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-workstream-page-\(UUID().uuidString).jsonl")
        defer { try? FileManager.default.removeItem(at: tmp) }
        let persistence = WorkstreamPersistence(fileURL: tmp)
        for i in 0..<5 {
            try await persistence.append(WorkstreamItem(
    ...
    func appendRedactsSensitiveToolInput() async throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-workstream-redact-\(UUID().uuidString).jsonl")
        defer { try? FileManager.default.removeItem(at: tmp) }
        let persistence = WorkstreamPersistence(fileURL: tmp)
    ...
    func missingFileEmpty() async throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-workstream-missing-\(UUID().uuidString).jsonl")
        let persistence = WorkstreamPersistence(fileURL: tmp)
        let loaded = try await persistence.loadRecent(limit: 10)
    ...
    func nonPositiveLimitEmpty() async throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-workstream-limit-\(UUID().uuidString).jsonl")
        defer { try? FileManager.default.removeItem(at: tmp) }
        let persistence = WorkstreamPersistence(fileURL: tmp)
        try await persistence.append(WorkstreamItem(
    ...
    func clearRemovesFile() async throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-workstream-clear-\(UUID().uuidString).jsonl")
        let persistence = WorkstreamPersistence(fileURL: tmp)
        try await persistence.append(WorkstreamItem(
            workstreamId: "s", source: .claude, kind: .sessionStart, payload: .sessionStart
        ))
