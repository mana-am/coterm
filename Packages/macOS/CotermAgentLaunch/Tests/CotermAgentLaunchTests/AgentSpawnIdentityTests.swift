...
struct AgentSpawnIdentityTests {
    ...
    // focused surface A (which would desync from COTERM_PANEL_ID and restore into the wrong surface).
    ...
    func orphanOwnSurfaceIsNotPairedWithFocusedWorkspace() {
        // Only COTERM_SURFACE_ID was inherited (no COTERM_WORKSPACE_ID), e.g. partial env scrubbing, while
