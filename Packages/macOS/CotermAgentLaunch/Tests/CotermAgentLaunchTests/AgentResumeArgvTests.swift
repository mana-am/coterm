...
struct AgentResumeArgvTests {
    ...
    func opencodeResumeDropsInternalTUISettingsSelector() {
        ...
        #expect(
            ...
            ) == .resolved(["coterm", "omo", "--session", "SID", "--model", "anthropic/claude-sonnet-4-6"])
        )
        #expect(
            AgentResumeArgv().builtInKind(
                kind: "opencode",
    ...
    @Test("coterm wrapper launchers resolve before per-kind verbs")
    func launcherWrappers() {
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "claudeTeams", sessionId: "SID", executablePath: nil, arguments: ["coterm", "claude-teams"]
            ) == .resolved(["coterm", "claude-teams", "--resume", "SID"])
        )
        #expect(
            AgentResumeArgv().launcherResolution(
        ...
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "codexTeams", sessionId: "SID", executablePath: nil, arguments: ["coterm", "codex-teams"]
            ) == .resolved(["coterm", "codex-teams", "resume", "SID"])
        )
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omo", sessionId: "SID", executablePath: nil, arguments: ["coterm", "omo"]
            ) == .resolved(["coterm", "omo", "--session", "SID"])
        )
        // One-shot wrappers have no resumable form (omx and omc share an arm; exercise each).
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omx", sessionId: "SID", executablePath: nil, arguments: ["coterm", "omx"]
            ) == .resolved(nil)
        )
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omc", sessionId: "SID", executablePath: nil, arguments: ["coterm", "omc"]
            ) == .resolved(nil)
        )
    ...
    func renderedPortableClaudeResumeShellCommand() {
        ...
        // Launcher resumes that resolve to coterm's own CLI emit no bare `claude`:
        // already-portable quoted words stay unwrapped.
        #expect(
            AgentResumeArgv.renderedPortableClaudeResumeShellCommand(
                parts: ["/Applications/coterm.app/Contents/Resources/bin/coterm", "claude-teams", "--resume", "SID"],
                quote: quote
            ) == "'/Applications/coterm.app/Contents/Resources/bin/coterm' 'claude-teams' '--resume' 'SID'"
        )
    }

    @Test("Codex wrapper token resolves COTERM_CODEX_WRAPPER_SHIM, degrading to bare codex")
    ...
    func renderedPortableCodexResumeShellCommand() {
        ...
        #expect(
            AgentResumeArgv.renderedPortableCodexResumeShellCommand(
                parts: ["/Applications/coterm.app/Contents/Resources/bin/coterm", "codex-teams", "resume", "SID"],
                quote: quote
            ) == "'/Applications/coterm.app/Contents/Resources/bin/coterm' 'codex-teams' 'resume' 'SID'"
        )
    }
}
