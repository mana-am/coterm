...
/// Ring-buffer event log used by coterm debug builds.
///
...
public final class DebugEventLog: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "coterm.debug-event-log")
    ...
    private static func resolveLogPath() -> String {
        ...
        if let explicit = env["COTERM_DEBUG_LOG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !explicit.isEmpty {
            return explicit
        }

        if let tag = env["COTERM_TAG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !tag.isEmpty {
            return "/tmp/coterm-debug-\(sanitizePathToken(tag)).log"
        }

        if let socketPath = env["COTERM_SOCKET_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !socketPath.isEmpty {
            let socketBase = URL(fileURLWithPath: socketPath).deletingPathExtension().lastPathComponent
            if socketBase.hasPrefix("coterm-debug-") {
