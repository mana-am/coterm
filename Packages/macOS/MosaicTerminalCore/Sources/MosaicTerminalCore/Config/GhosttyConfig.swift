...
public import MosaicFoundation
public import Foundation

/// The resolved mosaic terminal configuration: fonts, colors, theme, scrollback,
/// and sidebar appearance parsed from the user's ghostty config files plus
/// mosaic's managed defaults.
///
...
/// resolves themes by light/dark color scheme, and folds in mosaic's managed
...
public struct GhosttyConfig {
    ...
    /// The terminal theme mosaic enforces for every user-facing terminal surface.
    public static let mosaicEnforcedThemeName = "Anysphere Dark"
    /// Native fallback light theme name used when resolving mosaic-managed colors.
    public static let mosaicDefaultLightThemeName = mosaicEnforcedThemeName
    /// Native fallback dark theme name used when resolving mosaic-managed colors.
    public static let mosaicDefaultDarkThemeName = mosaicEnforcedThemeName
    ...
    public static let defaultSidebarFontSize = CGFloat(MosaicGhosttyConfigSettingEditor.defaultSidebarFontSize)
    /// The minimum sidebar font size the parser will clamp to.
    public static let minSidebarFontSize = CGFloat(MosaicGhosttyConfigSettingEditor.minSidebarFontSize)
    /// The maximum sidebar font size the parser will clamp to.
    public static let maxSidebarFontSize = CGFloat(MosaicGhosttyConfigSettingEditor.maxSidebarFontSize)
    /// The default surface tab-bar font size, in points.
    public static let defaultSurfaceTabBarFontSize = CGFloat(MosaicGhosttyConfigSettingEditor.defaultSurfaceTabBarFontSize)
    /// The minimum surface tab-bar font size the parser will clamp to.
    public static let minSurfaceTabBarFontSize = CGFloat(MosaicGhosttyConfigSettingEditor.minSurfaceTabBarFontSize)
    /// The maximum surface tab-bar font size the parser will clamp to.
    public static let maxSurfaceTabBarFontSize = CGFloat(MosaicGhosttyConfigSettingEditor.maxSurfaceTabBarFontSize)
    ...
    /// Creates a config with mosaic's built-in default appearance, before any
    ...
    private static func mosaicConfigPaths(
        ...
        return MosaicGhosttyConfigPathResolver().loadConfigURLs(
    ...
    static func loadFromDisk(preferredColorScheme: ColorSchemePreference) -> GhosttyConfig {
        ...
        configPaths.append(contentsOf: mosaicConfigPaths())
        ...
        if startupPreviewOverride?.loadsRealUserConfig ?? true {
            ...
            if config.theme == nil,
               Self.shouldApplyManagedDefaultAppearance(configPaths: configPaths) {
                config.applyMosaicDefaultAppearance(
                    environment: ProcessInfo.processInfo.environment,
                    bundleResourceURL: Bundle.main.resourceURL,
                    preferredColorScheme: preferredColorScheme
                )
            }
            config.applyMosaicEnforcedAppearance(
                environment: ProcessInfo.processInfo.environment,
                bundleResourceURL: Bundle.main.resourceURL,
                preferredColorScheme: preferredColorScheme
            )
    ...
    /// Applies mosaic's managed default theme for `preferredColorScheme` by parsing
    /// the bundled (or fallback) theme config contents into this config.
    public mutating func applyMosaicDefaultAppearance(
        environment: [String: String],
        bundleResourceURL: URL?,
        preferredColorScheme: ColorSchemePreference
    ) {
        parse(
            Self.mosaicDefaultThemeConfigContents(
                preferredColorScheme: preferredColorScheme,
    ...
    /// Applies mosaic's enforced terminal theme after user config has loaded.
    public mutating func applyMosaicEnforcedAppearance(
        environment: [String: String],
        bundleResourceURL: URL?,
        preferredColorScheme: ColorSchemePreference
    ) {
        theme = Self.mosaicEnforcedThemeName
        parse(
            Self.mosaicDefaultThemeConfigContents(
                preferredColorScheme: preferredColorScheme,
                environment: environment,
    ...
    /// The mosaic managed default theme name for `preferredColorScheme`.
    public static func mosaicDefaultThemeName(preferredColorScheme: ColorSchemePreference) -> String {
        switch preferredColorScheme {
        case .light:
            return mosaicDefaultLightThemeName
        case .dark:
            return mosaicDefaultDarkThemeName
        }
    }

    /// The mosaic managed default theme's config contents for `preferredColorScheme`,
    /// read from the resolved theme file when available, otherwise a built-in
    /// fallback palette.
    public static func mosaicDefaultThemeConfigContents(
        preferredColorScheme: ColorSchemePreference,
        environment: [String: String] = ProcessInfo.processInfo.environment,
        bundleResourceURL: URL? = Bundle.main.resourceURL
    ) -> String {
        if let url = mosaicDefaultThemeConfigURL(
            preferredColorScheme: preferredColorScheme,
            environment: environment,
            bundleResourceURL: bundleResourceURL
        ...
        return mosaicDefaultFallbackConfigContents(preferredColorScheme: preferredColorScheme)
    }

    /// The on-disk URL of the mosaic managed default theme for `preferredColorScheme`,
    /// or `nil` when no candidate theme file exists.
    public static func mosaicDefaultThemeConfigURL(
        preferredColorScheme: ColorSchemePreference,
        environment: [String: String] = ProcessInfo.processInfo.environment,
        bundleResourceURL: URL? = Bundle.main.resourceURL
    ) -> URL? {
        let themeName = mosaicDefaultThemeName(preferredColorScheme: preferredColorScheme)
    ...
    private static func mosaicDefaultFallbackConfigContents(
    ...
    public mutating func parse(
        ...
        for line in lines {
            ...
            // ignored, matching `MosaicGhosttyConfigSettingEditor.parsedSetting`.
    ...
    /// directives that determine whether mosaic should apply its managed default
    ...
    public struct UserAppearanceConfigSummary {
        ...
        /// Whether mosaic should apply its managed default appearance: true only
    ...
    /// Whether mosaic should inject its managed default appearance: true only when
    ...
    public static func clampedSidebarFontSize(_ value: CGFloat) -> CGFloat {
        CGFloat(MosaicGhosttyConfigSettingEditor().clampedSidebarFontSize(Double(value)))
    }
    ...
    public static func clampedSurfaceTabBarFontSize(_ value: CGFloat) -> CGFloat {
        CGFloat(MosaicGhosttyConfigSettingEditor().clampedSurfaceTabBarFontSize(Double(value)))
    }
    ...
    /// `.light` and `nil` for `.dark`. mosaic injects a resolved plain `theme = X`
    ...
    /// https://github.com/emergent-inc/mosaic/issues/3459). Injecting for an unset
    ...
    /// The ordered list of filesystem paths mosaic searches for the given theme
    ...
    public static func themeSearchPaths(
        ...
        for appSupportDirectory in MosaicApplicationSupportDirectories(environment: environment).userDirectories {
            appendUniquePath(
                appSupportDirectory
                    .appendingPathComponent(MosaicGhosttyConfigPathResolver.releaseBundleIdentifier, isDirectory: true)
