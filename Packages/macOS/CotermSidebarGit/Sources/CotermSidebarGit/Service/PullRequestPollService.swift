...
/// stateless `CotermGit` ``CotermGit/PullRequestProbeService`` pipeline.
///
...
public final class PullRequestPollService: PullRequestProbing {
    ...
    // Resolves slugs for candidate seeds (stateless CotermGit reader).
    let gitMetadataService: GitMetadataService
    // Fetches and matches GitHub PRs (stateless CotermGit pipeline).
