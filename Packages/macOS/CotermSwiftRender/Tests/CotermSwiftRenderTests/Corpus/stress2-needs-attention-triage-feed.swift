    ...
    ForEach(needsAttention) { ws in
      ...
      HStack(alignment: .top, spacing: 9) {
        ...
        VStack(alignment: .leading, spacing: 7) {
          ...
          HStack(spacing: 6) {
            Button { coterm("workspace.select", workspace_id: ws.id) } label: {
              HStack(spacing: 4) {
            ...
            Button { coterm("workspace.markRead", workspace_id: ws.id) } label: {
            ...
            if ws.pr.number > 0 {
              Menu("PR") {
                Button("Open #\(ws.pr.number)") { coterm("pr.open", url: ws.pr.url) }
                Button("Copy branch") { coterm("workspace.copyBranch", workspace_id: ws.id) }
              }
              .font(.system(size: 10))
            }
          }
      ...
      .onTapGesture { coterm("workspace.select", workspace_id: ws.id) }
      .contextMenu {
        Button("Triage") { coterm("workspace.select", workspace_id: ws.id) }
        Button("Mark read") { coterm("workspace.markRead", workspace_id: ws.id) }
        Button(ws.pinned ? "Unpin" : "Pin") { coterm("workspace.togglePin", workspace_id: ws.id) }
      }
    }
