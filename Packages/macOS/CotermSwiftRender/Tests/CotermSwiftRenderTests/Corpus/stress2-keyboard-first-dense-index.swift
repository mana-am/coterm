...
func row(_ w, _ i) -> some View {
  ...
  .opacity(g == "R" ? 0.6 : 1.0)
  .onTapGesture { coterm("workspace.select", workspace_id: w.id) }
  .keyboardShortcut(.return)
  .contextMenu {
    Button("Select") { coterm("workspace.select", workspace_id: w.id) }
    Button("Stop agent") { coterm("workspace.stop", workspace_id: w.id) }
    Button(w.pinned == true ? "Unpin" : "Pin") { coterm("workspace.pin", workspace_id: w.id) }
    if w.pr != nil {
      Button("Open PR") { coterm("workspace.openPR", workspace_id: w.id) }
    }
  }
