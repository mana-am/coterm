...
func prPill(_ ws: Any) -> some View {
    return Group {
        if ws.pr != nil {
            ...
            .help("PR \(ws.pr.label)")
            .onTapGesture { coterm("openURL", param: ws.pr.url) }
        } else {
...
func railRow(_ ws: Any) -> some View {
    ...
    .contextMenu {
        Button("Focus") { coterm("workspace.focus", param: ws.id) }
        Button(ws.pinned ? "Unpin" : "Pin") { coterm("workspace.togglePin", param: ws.id) }
        Button("Mark read") { coterm("workspace.markRead", param: ws.id) }
    }
    .help(ws.directory)
    .onTapGesture { coterm("workspace.select", param: ws.id) }
}
...
func tabCard(_ tab: Any) -> some View {
    ...
    .contextMenu {
        Button("Focus tab") { coterm("tab.focus", param: tab.id) }
        Button(tab.pinned ? "Unpin tab" : "Pin tab") { coterm("tab.togglePin", param: tab.id) }
        Button("Close tab") { coterm("tab.close", param: tab.id) }
    }
    .onTapGesture { coterm("tab.focus", param: tab.id) }
}
...
func cockpitHeader() -> some View {
    return HStack(spacing: 8) {
        ...
        Spacer()
        Menu("•••") {
            Button("Collapse all reads") { coterm("workspace.markAllRead") }
            Button("New workspace") { coterm("workspace.create") }
            Section("Sort") {
                Button("By unread") { coterm("workspace.sort", param: "unread") }
                Button("By recent") { coterm("workspace.sort", param: "recent") }
            }
        }
        .foregroundColor(.secondary)
        Text(clockLabel())
    ...
    HSplitView {
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 6) {
                ...
                Button("+") { coterm("tab.create", param: selectedId) }
                    .foregroundColor(.secondary)
