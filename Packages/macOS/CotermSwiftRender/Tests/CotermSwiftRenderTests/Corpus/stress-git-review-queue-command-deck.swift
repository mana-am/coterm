    ...
    VStack(alignment: .leading, spacing: 14) {
                ...
                ForEach(Array(reviewable.enumerated()), id: \.offset) { idx, ws in
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        HStack(alignment: .top, spacing: 8) {
                            ...
                            // actions menu
                            Menu("") {
                                Button("Open PR #\(pr.number)") { coterm("openURL", param: pr.url) }
                                Button("Focus workspace") { coterm("selectWorkspace", param: ws.id) }
                                Button("Approve") { coterm("prApprove", param: pr.number) }
                                Button("Request changes") { coterm("prRequestChanges", param: pr.number) }
                                Button("Refresh PR") { coterm("prRefresh", param: pr.number) }
                                if pr.status == "approved" {
                                    Button("Merge") { coterm("prMerge", param: pr.number) }
                                }
                            }
                        }
                    ...
                    .contextMenu {
                        Button("Open PR #\(pr.number)") { coterm("openURL", param: pr.url) }
                        Button("Focus workspace") { coterm("selectWorkspace", param: ws.id) }
                        Button("Refresh") { coterm("prRefresh", param: pr.number) }
                    }
                    .onTapGesture { coterm("selectWorkspace", param: ws.id) }
        ...
        ViewThatFits {
            HStack(spacing: 10) {
                ...
                Spacer()
                Button {
                    coterm("prRefreshAll", param: total)
                } label: {
                    HStack(spacing: 4) {
