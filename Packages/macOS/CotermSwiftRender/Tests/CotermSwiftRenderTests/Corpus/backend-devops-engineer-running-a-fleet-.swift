HSplitView {
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        ForEach(services) { svc in
            HStack(spacing: 8) {
                ...
                // restart
                Button(action: { coterm("service.restart", service_id: svc.id) }) {
                    Image(systemName: "arrow.clockwise")
                }
                // tail logs (focuses/opens the service's log surface)
                Button(action: { coterm("surface.focus", surface_id: svc.logSurfaceId) }) {
                    Image(systemName: "text.alignleft")
                }
            }
            .padding(4)
            .onTapGesture { coterm("service.open", service_id: svc.id) }
        }
        ...
        ForEach(ports) { p in
            HStack {
                ...
                Spacer()
                Button(action: { coterm("port.kill", port: p.port) }) {
        ...
        HStack(spacing: 8) {
            Button("Restart all") { coterm("service.restartAll") }
                .foregroundColor("#FF9F0A")
            Button("Stop all") { coterm("service.stopAll") }
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        HStack(spacing: 8) {
            ...
            Spacer()
            Button(action: { coterm("ci.rerun", run_id: build.runId) }) {
                Image(systemName: "arrow.clockwise")
            }
            Button(action: { coterm("ci.open", run_id: build.runId) }) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack {
                    ...
                    Button(d.env == "prod" ? "Ship prod" : "Deploy") {
                        coterm("deploy.start", env: d.env, sha: build.sha)
                    }
