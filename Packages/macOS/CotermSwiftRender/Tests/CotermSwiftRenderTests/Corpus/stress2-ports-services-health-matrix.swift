...
func portChip(_ p: Int, _ wsId) -> some View {
    ...
    .onTapGesture { coterm("workspace.select", workspace_id: wsId) }
}
...
func cell(_ ws) -> some View {
    ...
    .contextMenu {
        Button(action: { coterm("workspace.select", workspace_id: ws.id) }) {
            Label("Focus workspace", systemName: "scope")
        }
        if let pr = ws.pr {
            Button(action: { coterm("open.url", url: pr.url) }) {
