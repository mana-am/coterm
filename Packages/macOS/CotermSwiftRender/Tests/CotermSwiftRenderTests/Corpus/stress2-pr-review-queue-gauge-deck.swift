    ...
    VStack(alignment: .leading, spacing: 14) {
                    ...
                    if let pr = w.pr {
                        ...
                        VStack(alignment: .leading, spacing: 9) {
                            ...
                            HStack(alignment: .top, spacing: 8) {
                                ...
                                Menu {
                                    Button("Open #\(pr.number)") { coterm("pr.open", url: pr.url) }
                                    Button("Focus workspace") { coterm("workspace.select", workspace_id: w.id) }
                                    Divider()
                                    switch pr.status {
                                    case "review_required":
                                        Button("Approve") { coterm("pr.approve", number: pr.number) }
                                        Button("Request changes") { coterm("pr.requestChanges", number: pr.number) }
                                    case "changes_requested":
                                        Button("Re-review") { coterm("pr.review", number: pr.number) }
                                    case "approved":
                                        Button("Merge") { coterm("pr.merge", number: pr.number) }
                                    default:
                                        Button("View checks") { coterm("pr.checks", number: pr.number) }
                                    }
                                    if pr.stale {
                                        Button("Refresh (stale)") { coterm("pr.refresh", number: pr.number) }
                                    }
                                } label: {
                        ...
                        .contextMenu {
                            Button("Open #\(pr.number)") { coterm("pr.open", url: pr.url) }
                            Button("Focus workspace") { coterm("workspace.select", workspace_id: w.id) }
                            Button("Refresh") { coterm("pr.refresh", number: pr.number) }
                        }
                        .onTapGesture {
                            coterm(pr.stale ? "pr.refresh" : "workspace.select", workspace_id: w.id)
                        }
        ...
        ViewThatFits {
            HStack(spacing: 10) {
                ...
                Spacer()
                Button {
                    coterm("pr.refreshAll", count: total)
                } label: {
                    HStack(spacing: 4) {
            ...
            HStack {
                Text("\(total) in queue")
                    ...
                    .foregroundColor(.secondary)
                Spacer()
                Button("Refresh") { coterm("pr.refreshAll", count: total) }
                    .font(.caption)
                    .fontWeight(.semibold)
            }
        }
