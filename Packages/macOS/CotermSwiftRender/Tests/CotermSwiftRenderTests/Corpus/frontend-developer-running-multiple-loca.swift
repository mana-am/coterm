HSplitView {
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        ForEach(workspaces) { ws in
            VStack(alignment: .leading, spacing: 4) {
                Button(action: {
                    coterm("workspace.select", workspace_id: ws.id)
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        ForEach(devServers) { srv in
            Button(action: { coterm("surface.focus", surface_id: srv.surfaceId) }) {
                HStack(spacing: 8) {
        ...
        ForEach(checks) { c in
            ...
            .padding(3)
            .onTapGesture { coterm("surface.focus", surface_id: c.surfaceId) }
        }
        Divider()
