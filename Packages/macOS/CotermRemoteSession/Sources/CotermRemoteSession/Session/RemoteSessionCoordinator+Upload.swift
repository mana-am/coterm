...
// composition, the `/tmp/coterm-drop-<uuid>` path shape, cleanup script text,
...
extension RemoteSessionCoordinator {
    /// Uploads dropped local files to `/tmp/coterm-drop-*` paths on the remote
    ...
    /// The `/tmp/coterm-drop-<uuid>[.ext]` remote path a dropped local file
    ...
    public static func remoteDropPath(for fileURL: URL, uuid: UUID = UUID()) -> String {
        ...
        return "/tmp/coterm-drop-\(uuid.uuidString.lowercased())\(lowercasedSuffix)"
    }
