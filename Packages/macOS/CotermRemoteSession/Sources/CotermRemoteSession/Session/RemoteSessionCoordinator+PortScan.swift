...
extension RemoteSessionCoordinator {
    ...
    private func scanRemotePortsByPanelLocked(ttyNamesByPanel: [UUID: String]) throws -> [UUID: [Int]] {
        ...
        guard result.status == 0 else {
            ...
            throw NSError(domain: "coterm.remote.ports", code: 90, userInfo: [
    ...
    private func shouldUseFallbackRemotePortPollingLocked() -> Bool {
        // `coterm ssh` owns the remote shell bootstrap and can report the remote
    ...
    private func shouldUseTTYFallbackRemotePortPollingLocked() -> Bool {
        // `coterm ssh` can still land in shells without our command hooks, such as
    ...
    static func remotePortScanScript(ttyNames: [String], excluding ports: Set<Int>) -> String {
        ...
        return """
        set -eu
        coterm_tracked_ttys=" \(ttySet) "
        coterm_tty_csv='\(ttyCSV)'
        coterm_excluded_ports=" \(excludedPorts) "

        coterm_emit_port() {
          coterm_tty="$1"
          coterm_port="$2"
          case "$coterm_tracked_ttys" in
            *" $coterm_tty "*) ;;
            *) return 0 ;;
          esac
          case "$coterm_excluded_ports" in
            *" $coterm_port "*) return 0 ;;
          esac
          [ "$coterm_port" -ge 1024 ] && [ "$coterm_port" -le 65535 ] || return 0
          printf '%s\\t%s\\n' "$coterm_tty" "$coterm_port"
        }

        coterm_used_ss=0
        if [ -d /proc ] && command -v ss >/dev/null 2>&1; then
          coterm_ss_output="$(ss -ltnpH 2>/dev/null || true)"
          case "$coterm_ss_output" in
            *pid=*)
              coterm_used_ss=1
              printf '%s\\n' "$coterm_ss_output" | while IFS= read -r coterm_line; do
                [ -n "$coterm_line" ] || continue
                coterm_port="$(printf '%s\\n' "$coterm_line" | awk '{print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ { print $1; exit }')"
                [ -n "$coterm_port" ] || continue
                printf '%s\\n' "$coterm_line" | awk '
                  {
                    line = $0
                ...
                ' | while IFS= read -r coterm_pid; do
                  [ -n "$coterm_pid" ] || continue
                  coterm_tty_path="$(readlink "/proc/$coterm_pid/fd/0" 2>/dev/null || true)"
                  [ -n "$coterm_tty_path" ] || continue
                  coterm_tty="${coterm_tty_path##*/}"
                  [ -n "$coterm_tty" ] || continue
                  coterm_emit_port "$coterm_tty" "$coterm_port"
                done
              done
              ;;
          esac
        ...
        if [ "$coterm_used_ss" -eq 0 ] && command -v lsof >/dev/null 2>&1 && [ -n "$coterm_tty_csv" ]; then
          coterm_tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t coterm-ports)"
          trap 'rm -rf "$coterm_tmpdir"' EXIT INT TERM
          coterm_pid_tty_map="$coterm_tmpdir/pid_tty"
          ps -t "$coterm_tty_csv" -o pid=,tty= 2>/dev/null | awk '
            NF >= 2 {
              tty = $2
              sub(/^.*\\//, "", tty)
              print $1 "\\t" tty
          ...
          ' > "$coterm_pid_tty_map"
          [ -s "$coterm_pid_tty_map" ] || exit 0
          coterm_pid_csv="$(awk '{print $1}' "$coterm_pid_tty_map" | paste -sd, -)"
          [ -n "$coterm_pid_csv" ] || exit 0
          lsof -nP -a -p "$coterm_pid_csv" -iTCP -sTCP:LISTEN -Fpn 2>/dev/null | awk -v map="$coterm_pid_tty_map" '
            BEGIN {
              while ((getline < map) > 0) {
                pid_to_tty[$1] = $2
              }
          ...
          ' | while IFS=$'\\t' read -r coterm_tty coterm_port; do
            [ -n "$coterm_tty" ] || continue
            [ -n "$coterm_port" ] || continue
            coterm_emit_port "$coterm_tty" "$coterm_port"
          done
    ...
    static func remoteAllPortsScanScript(excluding ports: Set<Int>) -> String {
        ...
        if command -v ss >/dev/null 2>&1; then
          ss -ltnH 2>/dev/null | awk '{print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r coterm_port; do
            [ -n "$coterm_port" ] || continue
            coterm_emit_port "$coterm_port"
          done
        elif command -v netstat >/dev/null 2>&1; then
          netstat -lnt 2>/dev/null | awk 'NR > 2 {print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r coterm_port; do
            [ -n "$coterm_port" ] || continue
            coterm_emit_port "$coterm_port"
          done
        elif command -v lsof >/dev/null 2>&1; then
          lsof -nP -iTCP -sTCP:LISTEN 2>/dev/null | awk 'NR > 1 {print $9}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r coterm_port; do
            [ -n "$coterm_port" ] || continue
            coterm_emit_port "$coterm_port"
          done
        fi
