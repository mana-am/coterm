...
public import CotermRemoteDaemon
public import CotermRemoteWorkspace
...
/// orchestration, cotermd-remote bootstrap and install, the reverse CLI relay,
...
public final class RemoteSessionCoordinator: @unchecked Sendable {
    ...
    let queue = DispatchQueue(label: "com.coterm.remote-ssh.\(UUID().uuidString)", qos: .utility)
    ...
    ///   - manifestRepository: cotermd-remote manifest/binary-cache repository.
    ...
    /// The capabilities advertised by the cotermd-remote baked into the Freestyle snapshot
    ...
    static func bakedVMDaemonHello() -> DaemonHello {
        DaemonHello(
            ...
            remotePath: "/usr/local/bin/cotermd-remote"
        )
    }
    ...
    func beginConnectionAttemptLocked() {
        ...
        do {
            ...
            if configuration.skipDaemonBootstrap {
                // Cloud-VM path: cotermd-remote is pre-baked in the image and exposed via
                // systemd socket activation at /run/cotermd-remote.sock. We skip the probe,
            ...
            guard missingCapabilities.isEmpty else {
                throw NSError(domain: "coterm.remote.daemon", code: 43, userInfo: [
