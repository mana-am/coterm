...
// cotermd-remote serve-stdio children reparented to launchd after a crash or
...
extension RemoteSessionCoordinator {
    ...
    /// orphaned (PPID 1) coterm-owned ssh transports for `destination`,
    ...
    public static func orphanedCotermRemoteSSHPIDs(
        psOutput: String,
        destination: String,
        relayPort: Int? = nil,
        ...
        return psOutput
            ...
            .compactMap { line -> Int? in
                ...
                guard isOrphanedCotermRemoteSSHCommand(
                    parsed.command,
    ...
    static func killOrphanedRemoteSSHProcesses(
        ...
        for pid in orphanedCotermRemoteSSHPIDs(
            psOutput: output,
    ...
    private static func isOrphanedCotermRemoteSSHCommand(
        _ command: String,
        destination: String,
        relayPort: Int?,
        ...
        if let relayPort {
            ...
            return isCotermRemotePersistentDaemonServeStdioCommand(
                trimmed,
        ...
        if let trimmedPersistentDaemonSlot {
            if isCotermRemotePersistentDaemonServeStdioCommand(
                trimmed,
                slot: trimmedPersistentDaemonSlot
            ) {
                return true
            }
            return isCotermRemoteNonPersistentDaemonServeStdioCommand(trimmed)
        }
        if isCotermRemoteDaemonServeStdioCommand(trimmed) {
            return true
        }
        return false
    }

    private static func isCotermRemoteDaemonServeStdioCommand(_ command: String) -> Bool {
        guard command.contains("cotermd-remote") else { return false }
    ...
    private static func isCotermRemoteNonPersistentDaemonServeStdioCommand(_ command: String) -> Bool {
        guard isCotermRemoteDaemonServeStdioCommand(command) else { return false }
        let normalized = command
            .replacingOccurrences(of: "'", with: " ")
            .replacingOccurrences(of: "\"", with: " ")
    ...
    private static func isCotermRemotePersistentDaemonServeStdioCommand(
        _ command: String,
        slot: String
    ) -> Bool {
        guard isCotermRemoteDaemonServeStdioCommand(command) else { return false }
