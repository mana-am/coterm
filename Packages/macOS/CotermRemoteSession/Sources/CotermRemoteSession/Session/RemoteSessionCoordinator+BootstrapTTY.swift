...
// relay shell hook to `~/.coterm/relay/<port>.tty`), with a bounded retry.
...
extension RemoteSessionCoordinator {
    ...
    func requestBootstrapRemoteTTYIfNeededLocked() {
        ...
        let command = "sh -c \("tty_path=\"$HOME/.coterm/relay/\(relayPort).tty\"; if [ -r \"$tty_path\" ]; then cat \"$tty_path\"; fi".shellSingleQuoted)"
        do {
