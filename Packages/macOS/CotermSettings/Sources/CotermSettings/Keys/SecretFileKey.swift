...
///
/// `SecretFileKey` is the third key flavor in ``CotermSettings``, alongside
/// ``DefaultsKey`` (UserDefaults) and ``JSONKey`` (the coterm JSON config). It
/// exists for values that must not live in the shared, user-editable
/// `coterm.json` (for example, the socket-control password): each secret is
...
/// store (the app uses `~/.config/coterm`, the same directory as `coterm.json`).
///
/// ```swift
