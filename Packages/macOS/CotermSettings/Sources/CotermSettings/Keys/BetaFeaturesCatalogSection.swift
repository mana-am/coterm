...
/// pattern `<feature-domain>.beta.<flag-name>` so the coterm.json view
...
public struct BetaFeaturesCatalogSection: SettingCatalogSection {
    ...
    /// JSON) discovered from `~/.config/coterm/sidebars/` and selectable in the
    ...
    /// Remote tmux: mirror a remote host's tmux sessions in the coterm sidebar
    ...
    /// create/close propagate to the remote, while quitting coterm leaves the
