...
public struct AppCatalogSection: SettingCatalogSection {
    ...
    public let openSupportedFilesInCoterm = DefaultsKey<Bool>(
        id: "app.openSupportedFilesInCoterm",
        defaultValue: true,
        userDefaultsKey: "openSupportedFilesInCoterm"
    )
    ...
    public let openMarkdownInCotermViewer = DefaultsKey<Bool>(
        id: "app.openMarkdownInCotermViewer",
        defaultValue: true,
        userDefaultsKey: "openMarkdownInCotermViewer"
    )

    /// App-wide magnification percent for coterm-owned fonts and chrome.
    ...
    /// Shared, cross-tag default display that DEBUG coterm builds open new
    /// windows on, identified by the display's `localizedName` (e.g.
    /// `"LG HDR 4K"`). Empty means the system default placement.
    ///
    /// JSON-backed (`JSONKey`) on purpose: `coterm.json` lives at a fixed path
