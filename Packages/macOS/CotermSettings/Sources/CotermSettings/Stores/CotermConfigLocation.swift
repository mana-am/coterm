...
/// Conventional on-disk locations for the coterm JSON config.
///
...
///
/// ```swift
/// let locations = CotermConfigLocation()
/// let store = JSONConfigStore(fileURL: locations.userConfigFile)
/// ```
public struct CotermConfigLocation: Sendable, Hashable {
    /// The primary coterm config file: `<home>/.config/coterm/coterm.json`.
    public let userConfigFile: URL

    /// The legacy fallback: `<home>/.config/coterm/settings.json`. The app's
    ...
    public init(home: URL = FileManager.default.homeDirectoryForCurrentUser) {
        ...
        self.userConfigFile = home.appending(path: ".config/coterm/coterm.json")
        self.legacyFallbackFile = home.appending(path: ".config/coterm/settings.json")
    }
}
