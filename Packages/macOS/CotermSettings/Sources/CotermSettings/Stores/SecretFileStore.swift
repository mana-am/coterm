...
/// `coterm.json`: each secret is written to its own file with owner-only
...
/// store with `~/.config/coterm` (the same directory as `coterm.json`); tests pass
...
public actor SecretFileStore {
    ...
    public static let didChangeNotification = Notification.Name("coterm.secretFileStoreDidChange")
    ...
    ///   (created on first write if absent). The app uses `~/.config/coterm`.
