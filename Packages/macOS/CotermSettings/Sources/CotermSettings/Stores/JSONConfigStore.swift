...
/// Typed read/write/observe access to settings persisted in the coterm JSON config file.
///
...
///
/// Observation uses a single ``CotermFileWatch/FileWatcher`` owned by the store
...
/// let store = JSONConfigStore(fileURL: CotermConfigLocation().userConfigFile)
...
public actor JSONConfigStore {
    ...
    ///     ``CotermConfigLocation/userConfigFile`` for the standard coterm path.
    ...
    /// drains the ``CotermFileWatch/FileWatcher`` events and fans out to every
