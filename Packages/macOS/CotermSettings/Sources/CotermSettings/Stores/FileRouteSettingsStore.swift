...
/// `UserDefaults` under the catalog's `app.openMarkdownInCotermViewer` and
/// `app.openSupportedFilesInCoterm` keys.
///
...
public struct FileRouteSettingsStore: FileRouteSettingsReading {
    ...
    public static let markdownRouteDidChange = Notification.Name("coterm.cmdClickMarkdownRouteDidChange")

    /// Posted after the supported-file route toggle changes.
    public static let supportedFileRouteDidChange = Notification.Name("coterm.cmdClickSupportedFileRouteDidChange")
    ...
    public func setSupportedFileRouteEnabled(_ enabled: Bool) {
        keys.openSupportedFilesInCoterm.set(enabled, in: defaults)
