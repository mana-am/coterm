...
/// `coterm.json` config file and into a secure secret sink, then removes the
/// plaintext from `coterm.json`.
///
/// The socket-control password used to be storable as a plaintext string in
/// `coterm.json` (an editable, copyable, versionable file). Convergence makes the
...
/// very start of launch, before any managed-config layer reads `coterm.json`, so
...
public enum PlaintextSecretMigration {
    ...
    public enum Outcome: Equatable, Sendable {
        /// `coterm.json` does not exist or could not be read; nothing to do.
        case noConfigFile
        ...
        /// Copying the plaintext into the secret sink failed, so `coterm.json` was
        ...
        /// removed from `coterm.json`. Also covers an empty/whitespace plaintext.
        case scrubbedWithoutCopy
    }

    /// Lifts a plaintext secret at `plaintextKeyPath` out of `coterm.json` into the
    /// secret sink (only when the sink is empty), then removes the key from
    /// `coterm.json` after backing the file up.
    ///
    ...
    ///   - configURL: The `coterm.json` location.
    ...
    ///     plaintext is left in `coterm.json` (``Outcome/saveFailedLeftIntact``) so
        ...
        if let plaintext, !plaintext.isEmpty {
            ...
            if existing == nil || existing?.isEmpty == true {
                do {
                    ...
                    // The secure write failed; leave the plaintext in coterm.json so a
    ...
    /// that appear outside of string literals, so a JSONC `coterm.json` can be
