/// A flavor of the coterm app, used to derive per-flavor socket and marker file paths.
///
...
/// nightly/staging/dev builds that carry a tag in their bundle identifier or `COTERM_TAG`.
public enum SocketPathVariant: Equatable, Sendable {
    ...
    /// The marker file name (within ``CotermStateDirectory``) that records this
    ...
    public var tmpPath: String {
        ...
        case .nightly(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/coterm-nightly-\(slug)-last-socket-path"
            }
            return "/tmp/coterm-nightly-last-socket-path"
        case .staging(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/coterm-staging-\(slug)-last-socket-path"
            }
            return "/tmp/coterm-staging-last-socket-path"
        case .dev(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/coterm-dev-\(slug)-last-socket-path"
            }
            return "/tmp/coterm-dev-last-socket-path"
        }
    }
