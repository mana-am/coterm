...
/// Pure resolution policy for the coterm control socket: where its path lives, which
...
public struct SocketControlSettings {
    ...
    /// Environment key that, when truthy, permits honoring `COTERM_SOCKET_PATH` overrides.
    public static let allowSocketPathOverrideKey = "COTERM_ALLOW_SOCKET_OVERRIDE"
    /// Environment key carrying the socket password (highest-priority password source).
    public static let socketPasswordEnvKey = "COTERM_SOCKET_PASSWORD"
    ...
    /// The stable build's default socket path (within ``CotermStateDirectory``, falling back to `/tmp`).
    ...
    public static func shouldBlockUntaggedDebugLaunch(
        ...
        // so isRunningUnderXCTest() misses it. Check for any COTERM_UI_TEST_ env var.
    ...
    /// The control-socket path to use, honoring `COTERM_SOCKET_PATH` overrides only when safe.
    public static func socketPath(
        ...
        guard let override = environment["COTERM_SOCKET_PATH"], !override.isEmpty else {
            return fallback
        }
    ...
    /// The per-user stable socket path (`coterm-<uid>.sock` in ``CotermStateDirectory``, `/tmp` fallback).
    public static func userScopedStableSocketPath(currentUserID: uid_t = getuid()) -> String {
        stableSocketDirectoryURL()?
            ...
            .path ?? "/tmp/coterm-\(currentUserID).sock"
    }
    ...
    /// Whether a `COTERM_SOCKET_PATH` override should be honored for this build.
    ...
    private static func inheritedBundleIdentifierConflicts(
        ...
        guard let inheritedBundleIdentifier = normalizedBundleIdentifier(environment["COTERM_BUNDLE_ID"]),
    ...
    ///
    /// Resolves to ``CotermStateDirectory`` (`~/.local/state/coterm`) rather than
    /// Application Support: the separately-signed `coterm` CLI connects to this
    ...
    /// from other apps" prompt (https://github.com/emergent-inc/coterm/issues/5146).
    public static func stableSocketDirectoryURL(fileManager: FileManager = .default) -> URL? {
        CotermStateDirectory.url(homeDirectory: fileManager.homeDirectoryForCurrentUser)
    }

    /// The stable control socket file URL (within ``CotermStateDirectory``), if it can be resolved.
    ...
    /// The `COTERM_SOCKET_ENABLE` override as a tri-state (`true`/`false`/`nil` when unset/invalid).
    public static func envOverrideEnabled(
        environment: [String: String] = ProcessInfo.processInfo.environment
    ) -> Bool? {
        guard let raw = environment["COTERM_SOCKET_ENABLE"], !raw.isEmpty else {
            return nil
        }
    ...
    /// The `COTERM_SOCKET_MODE` override as a ``SocketControlMode``, or `nil` when unset/invalid.
    public static func envOverrideMode(
        environment: [String: String] = ProcessInfo.processInfo.environment
    ) -> SocketControlMode? {
        guard let raw = environment["COTERM_SOCKET_MODE"], !raw.isEmpty else {
            return nil
        }
        return parseMode(raw)
    }

    /// The effective mode after applying `COTERM_SOCKET_ENABLE`/`COTERM_SOCKET_MODE` overrides.
    /// - Parameters:
    ...
    public static func effectiveMode(
        ...
        if let overrideEnabled = envOverrideEnabled(environment: environment) {
            ...
            return userMode == .off ? .cotermOnly : userMode
        }
