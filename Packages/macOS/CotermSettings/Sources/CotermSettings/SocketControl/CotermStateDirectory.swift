...
/// The per-user directory that holds coterm's control-plane runtime state: the
...
///
/// This directory therefore resolves to `~/.local/state/coterm`, a plain dotfolder
/// macOS does **not** treat as protected app data. It is the sibling of the
/// existing `~/.local/state/coterm/crash` breadcrumb directory.
///
/// ```swift
...
/// let socket = CotermStateDirectory.url(homeDirectory: home).appendingPathComponent("coterm.sock")
/// ```
public enum CotermStateDirectory {
    /// The directory name segment under `~/.local/state` (and the legacy name
    /// under `~/Library/Application Support`).
    public static let directoryName = "coterm"

    /// The coterm state directory: `<home>/.local/state/coterm`.
    ///
