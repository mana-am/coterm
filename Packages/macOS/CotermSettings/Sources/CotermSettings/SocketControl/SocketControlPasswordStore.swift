...
///
/// The password sources, highest priority first, are the `COTERM_SOCKET_PASSWORD`
...
public struct SocketControlPasswordStore: Sendable {
    ...
    public static let didChangeNotification = Notification.Name("coterm.socketControlPasswordDidChange")
    ...
    private static let legacyKeychainService = "coterm.com.emergent.app.socket-control"
    ...
    /// - Parameters:
    ///   - environment: The process environment used to read `COTERM_SOCKET_PASSWORD`.
    ...
    /// The default password file URL within the coterm state directory.
    ///
    /// Resolves to `<directory>/socket-control-password`, where `directory`
    /// defaults to ``CotermStateDirectory`` (`~/.local/state/coterm`). The file lives
    /// outside Application Support so the separately-signed `coterm` CLI can read it
    ...
    ///     ``CotermStateDirectory`` location is resolved via `fileManager`.
    ...
    /// (`~/Library/Application Support/coterm/socket-control-password`) into the
    /// current ``CotermStateDirectory`` location, once.
    ///
    ...
    public static func migrateLegacyApplicationSupportPasswordFileIfNeeded(
        ...
        guard let destination = defaultPasswordFileURL(fileManager: fileManager),
              let legacyDirectory = CotermStateDirectory.legacyApplicationSupportURL(fileManager: fileManager) else {
            return
        }
        migratePasswordFile(
    ...
    private static func loadLegacyPasswordFromKeychain() -> String? {
        ...
        let query: [CFString: Any] = [
            ...
            kSecAttrService: "coterm.com.emergent.app.socket-control",
