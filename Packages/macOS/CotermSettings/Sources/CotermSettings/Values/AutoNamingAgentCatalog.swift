...
/// power user can name a custom agent in `~/.config/coterm/coterm.json` even if it
...
public struct AutoNamingAgentOption: Sendable, Hashable {
    ...
    /// `<agent>` segment of `coterm hooks <agent> auto-name`.
    public let slug: String
    /// Brand display name shown in the picker (proper noun — not localized).
    public let displayName: String
    /// Whether coterm currently knows how to drive this agent as a summarizer.
...
///
/// Lives in `CotermSettings` (imported by both the app's Settings UI and the
/// bundled `coterm` CLI) so the picker and the summarizer dispatch share one
...
public enum AutoNamingAgentCatalog {
    ...
    /// Agents whose binary coterm knows how to invoke for a one-shot
    ...
    /// agent until coterm learns to drive them).
    ...
    /// True only when coterm can drive `slug` as a summarizer. Custom/unknown
