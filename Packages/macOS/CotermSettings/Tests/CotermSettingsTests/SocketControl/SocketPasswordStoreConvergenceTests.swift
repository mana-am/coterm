...
/// without touching the real Application Support, keychain, or `coterm.json`.
@Suite struct SocketPasswordStoreConvergenceTests {
    private let secretKey = SettingCatalog().automation.socketPassword

    /// A throwaway control directory standing in for ``CotermStateDirectory``.
    private func tempBaseDirectory() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-convergence-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    /// the password file's parent directory. This mirrors `cotermApp.init`, which
    ...
    @Test func bothStoresResolveIdenticalFileURL() async throws {
        ...
        // Exactly the derivation cotermApp.init performs.
