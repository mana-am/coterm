...
@Suite struct CotermStateDirectoryTests {
    ...
    @Test func socketPathStaysWellWithinUnixPathLimit() {
        ...
        let socket = CotermStateDirectory.url(homeDirectory: home)
    ...
    @Test func legacyApplicationSupportURLPointsAtOldLocation() {
        let legacy = CotermStateDirectory.legacyApplicationSupportURL(fileManager: .default)
        // Only used for one-time migration; it must still address the old folder.
        #expect(legacy?.lastPathComponent == "coterm")
        #expect(legacy?.path.contains("/Application Support/coterm") == true)
    }
}
...
@Suite(.serialized) struct SocketControlPasswordMigrationTests {
    private func tempDir() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-pw-migrate-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    /// TCC-protected directory (https://github.com/emergent-inc/coterm/issues/5146).
