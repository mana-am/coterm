...
/// Regression coverage for https://github.com/emergent-inc/coterm/issues/5146.
///
/// The control socket directory and the socket password file are read by the
/// separately code-signed `coterm` CLI on every agent session-start/stop hook. On
...
@Suite struct SocketControlTCCLocationRegressionTests {
    ...
    @Test func controlSocketDirectoryIsOutsideTCCProtectedAppData() throws {
        ...
        #expect(directory.hasSuffix("/.local/state/coterm"))
    }
    ...
    @Test func socketPasswordFileIsOutsideTCCProtectedAppData() throws {
        ...
        #expect(path.hasSuffix("/.local/state/coterm/socket-control-password"))
    }
}
