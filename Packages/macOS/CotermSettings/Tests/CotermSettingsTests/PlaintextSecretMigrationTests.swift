...
struct PlaintextSecretMigrationTests {
    ...
    /// A throwaway directory holding a `coterm.json` for one test.
    private func makeConfig(_ contents: String?) -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-secret-migration-\(UUID().uuidString)", isDirectory: true)
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let url = dir.appendingPathComponent("coterm.json", isDirectory: false)
