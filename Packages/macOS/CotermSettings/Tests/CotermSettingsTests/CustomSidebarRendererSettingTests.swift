...
/// store: the on-disk strings users put in `~/.config/coterm/coterm.json` must
...
struct CustomSidebarRendererSettingTests {
    private func makeStore() -> (JSONConfigStore, URL) {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-renderer-tests-\(UUID().uuidString)", isDirectory: true)
        try? FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        let fileURL = tempDir.appendingPathComponent("coterm.json", isDirectory: false)
