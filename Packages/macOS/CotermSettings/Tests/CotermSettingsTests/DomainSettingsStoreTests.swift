...
/// the cotermApp static settings-namespace enums. Each suite pins the legacy
/// read/write semantics (defaults, legacy-key fallback chains, trim rules)
/// against a scratch `UserDefaults` suite.
private func makeScratchDefaults() -> UserDefaults {
    UserDefaults(suiteName: "coterm.tests.\(UUID().uuidString)")!
}
...
struct DefaultsKeyDirectAccessTests {
    ...
    @Test func matchesStoreDecodePath() async {
        ...
        let suiteName = "coterm.tests.\(UUID().uuidString)"
...
struct FileRouteSettingsStoreTests {
    ...
    private func makeTemporaryFile() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-route-\(UUID().uuidString).md")
        try Data("# hi\n".utf8).write(to: url)
        return url
    }
    ...
    @Test func routesOnlyReadableRegularFiles() throws {
        ...
        #expect(!store.shouldRouteMarkdown(path: "/nonexistent/coterm-test.md"))
    }
    ...
    @Test func disabledTogglesShortCircuitRouting() throws {
        ...
        defaults.set(false, forKey: "openMarkdownInCotermViewer")
        #expect(!store.shouldRouteMarkdown(path: file.path))

        defaults.set(false, forKey: "openSupportedFilesInCoterm")
        #expect(!store.shouldRouteSupportedFile(path: file.path))
    }
    ...
    @Test func setsPostDidChangeNotifications() {
        ...
        #expect(defaults.object(forKey: "openMarkdownInCotermViewer") as? Bool == false)
        #expect(defaults.object(forKey: "openSupportedFilesInCoterm") as? Bool == false)
