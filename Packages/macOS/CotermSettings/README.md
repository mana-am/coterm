...
Strongly-typed, migratable settings storage for coterm. Depends only on
Foundation and `CotermFileWatch` (for config-file reload watching). Modern
...
- **`JSONConfigStore`** — wraps `~/.config/coterm/coterm.json`. Structured config
...
- **JSON config** lives at `~/.config/coterm/coterm.json` by default
  (`CotermConfigLocation().userConfigFile`). The store creates the file (and
  parent directory) on first write. File is missing on first launch, in
  which case every read returns the key's default value.

A populated coterm.json looks like this — pretty-printed, sorted keys, JSONC
...
let jsonConfigStore = JSONConfigStore(
    fileURL: CotermConfigLocation().userConfigFile
)
...
let tempDir = FileManager.default.temporaryDirectory
    .appending(path: "coterm-readme-\(UUID().uuidString)")
try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
let fileURL = tempDir.appending(path: "coterm.json")
...
1. Pick the catalog section under `Sources/CotermSettings/Keys/` (e.g.
...
1. Create `Sources/CotermSettings/Keys/FooCatalogSection.swift`:
   ```swift
...
- **`JSONConfigStore`** — `actor`. Async reads/writes/observe. Owns one
  `CotermFileWatch.FileWatcher` and fans out file-change events to per-subscriber
  bounded signal streams (no `N × parse` work under burst changes). File
  watching itself lives in the `CotermFileWatch` package.
...
is the production registry. See `Tests/CotermSettingsTests/` for patterns.
...
comparisons. See `Tests/CotermSettingsTests/ShortcutWhenClauseTests.swift`.

## Concurrency

The coterm repo enforces a strict modern-concurrency policy
(see the coterm `CLAUDE.md` "Modern Swift concurrency" section): no locks,
