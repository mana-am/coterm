...
public enum CommandPaletteRequestKind: String, Sendable, CaseIterable {
    ...
    ///
    /// These strings are byte-identical to the `coterm.*` `Notification.Name`
    /// literals the app target previously posted inline, so observers keyed on
    /// the existing names are unaffected.
    public var notificationName: String {
        switch self {
        case .commands:
            return "coterm.commandPaletteRequested"
        case .switcher:
            return "coterm.commandPaletteSwitcherRequested"
        case .renameTab:
            return "coterm.commandPaletteRenameTabRequested"
        case .renameWorkspace:
            return "coterm.commandPaletteRenameWorkspaceRequested"
        case .editWorkspaceDescription:
            return "coterm.commandPaletteEditWorkspaceDescriptionRequested"
        }
    }
