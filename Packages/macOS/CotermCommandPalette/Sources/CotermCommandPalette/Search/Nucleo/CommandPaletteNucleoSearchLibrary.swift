...
// search state.
/// Loaded `coterm-nucleo-ffi` dylib handle with resolved C entry points.
///
...
final class CommandPaletteNucleoSearchLibrary: @unchecked Sendable {
    ...
    private static func load(path: String) -> CommandPaletteNucleoSearchLibrary? {
        ...
        guard let createIndex = symbol("coterm_nucleo_index_create", from: handle, as: CreateIndex.self),
              let destroyIndex = symbol("coterm_nucleo_index_destroy", from: handle, as: DestroyIndex.self),
              let searchIndexWithBoosts = symbol(
                "coterm_nucleo_index_search_with_boosts",
                from: handle,
                as: SearchIndexWithBoosts.self
              ),
              let version = symbol("coterm_nucleo_ffi_version", from: handle, as: Version.self) else {
            dlclose(handle)
            return nil
        }
    ...
    private static func defaultLibraryPaths() -> [String] {
        var paths: [String] = []
        let environmentPath = ProcessInfo.processInfo.environment["COTERM_NUCLEO_FFI_LIB"]
        ...
        // Packages/macOS/CotermCommandPalette/Sources/CotermCommandPalette/Search/Nucleo/.
        ...
        paths.append(
            sourceRoot
                .appendingPathComponent("Native/CommandPaletteNucleoFFI/target/coterm-nucleo-ffi")
                .appendingPathComponent(Self.libraryFileName)
                .path
        )
        paths.append(
