...
extension BrowserOmnibarPageFocusRepository {
    ...
    /// `window.__cotermAddressBarFocusState` (mirrored to the top frame) and tags
    /// the element with a stable `data-coterm-addressbar-focus-id`. Returns a
    /// `captured:<id>` / `cleared:*` / `error` status string.
    static let captureScript = """
    (() => {
      try {
        const syncState = (state) => {
          window.__cotermAddressBarFocusState = state;
          try {
            if (window.top && window.top !== window) {
              window.top.postMessage({ cotermAddressBarFocusState: state }, "*");
            } else if (window.top) {
              window.top.__cotermAddressBarFocusState = state;
            }
          } catch (_) {}
        };
        ...
        let id = active.getAttribute("data-coterm-addressbar-focus-id");
        if (!id) {
          id = "coterm-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 8);
          active.setAttribute("data-coterm-addressbar-focus-id", id);
        }
    ...
    (() => {
      try {
        if (window.__cotermAddressBarFocusTrackerInstalled) return true;
        ...
        if (window.top === window && !window.__cotermAddressBarFocusMessageBridgeInstalled) {
          window.__cotermAddressBarFocusMessageBridgeInstalled = true;
          window.addEventListener("message", (ev) => {
            try {
              const data = ev ? ev.data : null;
              if (!data || !Object.prototype.hasOwnProperty.call(data, "cotermAddressBarFocusState")) return;
              window.__cotermAddressBarFocusState = data.cotermAddressBarFocusState || null;
            } catch (_) {}
          }, true);
        }
        ...
        const ensureFocusId = (el) => {
          let id = el.getAttribute("data-coterm-addressbar-focus-id");
          if (!id) {
            id = "coterm-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 8);
            el.setAttribute("data-coterm-addressbar-focus-id", id);
          }
          return id;
        };
    ...
    (() => {
      try {
        const readState = () => {
          let state = window.__cotermAddressBarFocusState;
          try {
            if ((!state || typeof state.id !== "string" || !state.id) &&
                window.top && window.top.__cotermAddressBarFocusState) {
              state = window.top.__cotermAddressBarFocusState;
            }
          } catch (_) {}
          return state;
        };
        ...
        const clearState = () => {
          window.__cotermAddressBarFocusState = null;
          try {
            if (window.top && window.top !== window) {
              window.top.postMessage({ cotermAddressBarFocusState: null }, "*");
            } else if (window.top) {
              window.top.__cotermAddressBarFocusState = null;
            }
          } catch (_) {}
        };
        ...
        const selector = '[data-coterm-addressbar-focus-id="' + state.id + '"]';
