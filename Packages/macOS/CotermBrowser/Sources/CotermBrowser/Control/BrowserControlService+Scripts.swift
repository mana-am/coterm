...
extension BrowserControlService {
    ...
    public func findScript(finderBody: String) -> String {
        ...
        \(finderBody)
          ...
          if (!__cotermFound) return { ok: false, error: 'not_found' };
          const selector = __cotermCssPath(__cotermFound);
          if (!selector) return { ok: false, error: 'not_found' };
          return {
            ok: true,
            selector,
            tag: String(__cotermFound.tagName || '').toLowerCase(),
            text: String(__cotermFound.textContent || '').trim()
          };
        })()
        """
    }
