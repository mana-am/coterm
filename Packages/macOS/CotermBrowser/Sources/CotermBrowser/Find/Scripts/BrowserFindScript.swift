...
public struct BrowserFindScript: Sendable, Equatable {
    ...
    public static func search(query: String) -> BrowserFindScript {
        ...
        (() => {
          const MARK_CLASS = '__coterm-find';
          const CURRENT_CLASS = '__coterm-find-current';
          ...
          window.__cotermFindMatches = matches;
          window.__cotermFindIndex = 0;
          ...
          if (!document.getElementById('__coterm-find-style')) {
            const style = document.createElement('style');
            style.id = '__coterm-find-style';
            style.textContent = `
              mark.__coterm-find { background: #facc15; color: #000; border-radius: 2px; }
              mark.__coterm-find.__coterm-find-current { background: #f97316; color: #fff; }
            `;
    ...
    public static func next() -> BrowserFindScript {
        ...
        (() => {
          const matches = window.__cotermFindMatches || [];
          if (matches.length === 0) return JSON.stringify({ total: 0, current: 0 });
          let idx = window.__cotermFindIndex || 0;
          if (!matches[idx] || !matches[idx].isConnected) {
            window.__cotermFindMatches = [];
            window.__cotermFindIndex = 0;
            return JSON.stringify({ total: 0, current: 0 });
          }
          matches[idx].classList.remove('__coterm-find-current');
          ...
          matches[idx].classList.add('__coterm-find-current');
          matches[idx].scrollIntoView({ block: 'center', behavior: 'smooth' });
          window.__cotermFindIndex = idx;
    ...
    public static func clear() -> BrowserFindScript {
        ...
        (() => {
          ...
          window.__cotermFindMatches = [];
          window.__cotermFindIndex = 0;
          const style = document.getElementById('__coterm-find-style');
    ...
    document.querySelectorAll('mark.__coterm-find').forEach(mark => {
