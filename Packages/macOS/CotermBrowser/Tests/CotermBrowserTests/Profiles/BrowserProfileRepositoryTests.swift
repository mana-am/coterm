...
private final class FakeHistoryProvider: BrowserProfileHistoryProviding {
    ...
    var defaultURL: URL? = URL(fileURLWithPath: "/tmp/coterm-test-default/browser_history.json")
...
private func makeRepository(
    suiteName: String = "coterm.browserprofiles.test.\(UUID().uuidString)",
    ...
    bundleIdentifier: String = "ai.emergent.inc.coterm.test"
...
struct BrowserProfileRepositoryTests {
    ...
    @Test func historyFileURLDefaultAndPerProfileShape() {
        let history = FakeHistoryProvider()
        let (repo, _) = makeRepository(history: history, bundleIdentifier: "ai.emergent.inc.coterm.test")
        ...
        #expect(url.path.contains("ns-ai.emergent.inc.coterm.test"))
    ...
    @Test func loadDedupesPersistedBuiltInDefault() {
        let suite = "coterm.browserprofiles.test.dedup.\(UUID().uuidString)"
