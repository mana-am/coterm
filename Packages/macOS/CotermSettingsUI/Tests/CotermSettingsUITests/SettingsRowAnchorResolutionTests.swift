...
///
/// - **Forward** — every coterm.json path a `SettingsCardRow` declares via
...
/// a coterm.json path (pickers and action buttons that don't write a
...
struct SettingsRowAnchorResolutionTests {
    /// Every singular coterm.json path declared by an *unconditionally
    ...
    /// (no single coterm.json path): pickers and action buttons. Each must
