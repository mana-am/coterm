...
/// underlying coterm.json path next to the row when the host enables
/// "show config paths".
///
...
public struct SettingsCardRow<Trailing: View>: View {
    ...
    // map the coterm.json path(s) it declares via `configurationReview`
    ...
    /// custom-control rows that don't write a single coterm.json key);
