...
/// opening the user's editor on coterm.json, sending feedback, posting
...
public protocol SettingsHostActions: AnyObject {
    ...
    /// Opens the coterm JSON config file in the user's preferred
    ...
    /// Opens System Settings to the coterm notifications pane so the
    /// user can grant / revoke OS-level notification permission.
    func openSystemNotificationSettings()

    /// Restarts the coterm app. Used after the user changes the
    ...
    /// Opens the coterm terminal config preview window (the legacy
