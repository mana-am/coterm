...
public struct KeyboardShortcutsSection: View {
    ...
    private var chordsRow: some View {
        SettingsCardRow(
            ...
            subtitle: String(localized: "settings.shortcuts.chords.subtitle", defaultValue: "Add tmux-style multi-step shortcuts in Coterm.json, for example [\"ctrl+b\", \"c\"].")
        ) {
            HStack(spacing: 8) {
                Link(
                    String(localized: "settings.shortcuts.chords.docsButton", defaultValue: "Chord docs"),
                    destination: URL(string: "https://coterm.cc/docs/keyboard-shortcuts#shortcut-chords")!
                )
                .cotermFont(.caption)
                .accessibilityIdentifier("SettingsKeyboardShortcutsChordDocsLink")

                Button(String(localized: "settings.app.settingsFile.openButton", defaultValue: "Open coterm.json")) {
    ...
    private func streamBindings() async {
        ...
        let task = Task {
            for await dictionary in jsonStore.values(for: catalog.shortcuts.bindings) {
                ...
                // non-unbound (e.g. the user edited coterm.json directly to
                ...
                // shortcut changes. Externally-edited coterm.json should
    ...
    /// external `coterm.json` edit or another settings surface restoring a usable
    ...
    private func pruneConflictRejections() {
        ...
        // (coterm.json reload) dismisses the validation banner too.
