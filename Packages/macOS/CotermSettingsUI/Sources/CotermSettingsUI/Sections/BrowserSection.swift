...
/// row-for-row inside a single `SettingsCard`: Enable coterm Browser,
...
public struct BrowserSection: View {
    ...
    public init(
        ...
        _openTermLinks = State(initialValue: DefaultsValueModel(store: defaultsStore, key: catalog.browser.openTerminalLinksInCotermBrowser))
        _interceptOpen = State(initialValue: DefaultsValueModel(store: defaultsStore, key: catalog.browser.interceptTerminalOpenCommandInCotermBrowser))
    ...
    private var mainCard: some View {
        SettingsCard {
            // Enable coterm Browser
            SettingsCardRow(
                configurationReview: .settingsOnly,
                searchAnchorID: "setting:browser:enable-browser",
                String(localized: "settings.browser.enabled", defaultValue: "Enable coterm Browser"),
            ...
            if engine.current == .custom {
                ...
                SettingsCardRow(
                    ...
                    subtitle: String(localized: "settings.browser.customSearchEngineURLTemplate.subtitle", defaultValue: "Use {query} or %s for the search terms. Without a placeholder, coterm appends q=."),
                    controlWidth: 330
                ) {
            ...
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.browser.hiddenWebViewDiscardDelay.subtitle", defaultValue: "How long a browser tab must stay hidden before coterm frees its page memory. Active downloads, popups, developer tools, fullscreen, and loading pages are skipped."),
                controlWidth: Self.columnWidth
            ) {
                HStack(spacing: 8) {
            ...
            // Open Terminal Links
            SettingsCardRow(
                configurationReview: .json("browser.openTerminalLinksInCotermBrowser"),
                String(localized: "settings.browser.openTerminalLinks", defaultValue: "Open Terminal Links in Coterm Browser"),
            ...
            // Intercept open
            SettingsCardRow(
                configurationReview: .json("browser.interceptTerminalOpenCommandInCotermBrowser"),
            ...
            if openTermLinks.current || interceptOpen.current {
                ...
                hostnameEditor(
                    ...
                    subtitle: String(localized: "settings.browser.hostWhitelist.subtitle", defaultValue: "Applies to terminal link clicks and intercepted `open https://...` calls. Only these hosts open in Coterm. Others open in your default browser. One host or wildcard per line (for example: example.com, *.internal.example). Leave empty to open all hosts in Coterm."),
    ...
    private func httpAllowlistRow(model: DefaultsValueModel<String>) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(String(localized: "settings.browser.httpAllowlist.description", defaultValue: "Controls which HTTP (non-HTTPS) hosts can open in Coterm without a warning prompt. Defaults include localhost, *.localhost, 127.0.0.1, ::1, 0.0.0.0, and *.localtest.me."))
                .cotermFont(.caption)
