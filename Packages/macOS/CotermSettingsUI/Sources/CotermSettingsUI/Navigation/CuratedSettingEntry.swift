...
/// localized row text, the coterm.json paths owned by that row, and a
...
/// `[CuratedSettingEntry].cotermDefault` constant.
public struct CuratedSettingEntry: Sendable, Hashable {
    ...
    /// Dotted coterm.json paths that should scroll to this search entry's
    ...
    /// Dotted coterm.json path used as the scroll/highlight anchor.
    ///
