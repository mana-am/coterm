import Foundation
import Testing
@testable import CotermNotifications
...
struct NotificationClickPerformerTests {
    ...
    func selectsExistingFile() {
        ...
        finder.existingPaths = ["/tmp/coterm/report.txt"]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "/tmp/coterm/report.txt")))
        #expect(finder.selected == ["/tmp/coterm/report.txt"])
    ...
    func opensContainingDirectory() {
        ...
        finder.existingPaths = ["/tmp/coterm"]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "/tmp/coterm/missing.txt")))
        #expect(finder.selected.isEmpty)
        #expect(finder.opened == ["/tmp/coterm"])
    }
    ...
    func failsWhenNothingExists() {
        ...
        #expect(performer.perform(.revealInFinder(path: "/tmp/coterm/missing.txt")) == false)
        #expect(finder.selected.isEmpty)
        #expect(finder.opened.isEmpty)
    }
    ...
    func expandsTilde() {
        ...
        let expectedPath = expandedHome + "/coterm-file.txt"
        finder.existingPaths = [expectedPath]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "~/coterm-file.txt")))
