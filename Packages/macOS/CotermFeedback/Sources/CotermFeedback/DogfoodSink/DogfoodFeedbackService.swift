...
/// `~/.cache/coterm-dogfood-feedback/<ISO8601>_<shortid>/` (a `bundle.json`
...
public struct DogfoodFeedbackService: Sendable {
    ...
    public init(
        ...
        self.cacheRoot = cacheRoot ?? FileManager.default.homeDirectoryForCurrentUser
            ...
            .appendingPathComponent("coterm-dogfood-feedback", isDirectory: true)
        self.now = now
    }

    /// The privileged feedback domain. Mirrors `isEmergentIncEmail` in
    /// `CotermMobileShellModel` (the phone's routing source of truth) but is
    ...
    private static func writeBundle(
        ...
        do {
            ...
            let manifest: [String: Any] = [
                "schema": "coterm.dogfood.feedback.v1",
