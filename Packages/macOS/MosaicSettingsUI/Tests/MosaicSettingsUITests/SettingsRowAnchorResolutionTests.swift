...
import Testing
@testable import MosaicSettingsUI

/// Guards the search-result highlight bridge in both directions:
///
/// - **Forward** — every mosaic.json path a `SettingsCardRow` declares via
...
/// a mosaic.json path (pickers and action buttons that don't write a
...
struct SettingsRowAnchorResolutionTests {
    /// Every singular mosaic.json path declared by an *unconditionally
