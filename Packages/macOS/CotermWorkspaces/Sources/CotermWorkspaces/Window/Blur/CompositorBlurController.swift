...
private func cotermCGSDefaultConnectionForThread() -> UnsafeMutableRawPointer?
...
@discardableResult
private func cotermCGSSetWindowBackgroundBlurRadius(
...
///
/// Lifted faithfully from the `cotermResetCompositorBackgroundBlur` free function
...
/// the caller's isolation domain (the legacy `cotermResetCompositorBackgroundBlur`
...
public struct CompositorBlurController: Sendable {
    ...
    /// `cotermResetCompositorBackgroundBlur(on:)`. The caller resolves
    /// `window.windowNumber` in its own (main-actor) isolation domain.
    public func resetBackgroundBlur(windowNumber: Int) {
        _ = cotermCGSSetWindowBackgroundBlurRadius(
            cotermCGSDefaultConnectionForThread(),
            UInt(windowNumber),
            0
        )
    }
