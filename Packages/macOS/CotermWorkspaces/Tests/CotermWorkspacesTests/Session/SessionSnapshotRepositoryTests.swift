import Foundation
import Testing
@testable import CotermWorkspaces
...
struct SessionSnapshotRepositoryTests {
    ...
    @Test("snapshot file paths derive from the sanitized bundle identifier under coterm/")
    func snapshotFileURLShape() throws {
        let dir = try makeTempDirectory()
        defer { try? FileManager.default.removeItem(at: dir) }
        let repository = makeRepository(appSupport: dir, bundleIdentifier: "com.coterm odd/id")

        let primary = try #require(repository.defaultSnapshotFileURL())
        let backup = try #require(repository.manualRestoreSnapshotFileURL())
        #expect(primary.path == dir.appendingPathComponent("coterm/session-com.coterm_odd_id.json").path)
