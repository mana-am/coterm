...
extension SocketTransport {
    ...
    /// reusable lock marker, based on the well-known coterm socket filenames
    ...
    func canReplaceUnmarkedRefusedSocket(at path: String) -> Bool {
        let filename = URL(fileURLWithPath: (path as NSString).standardizingPath)
            .lastPathComponent
            .lowercased()
        let recoverablePrefixes = ["coterm-debug-", "coterm-nightly-", "coterm-staging-"]
        return filename == "coterm-debug.sock" ||
            filename == "coterm-nightly.sock" ||
            filename == "coterm-staging.sock" ||
