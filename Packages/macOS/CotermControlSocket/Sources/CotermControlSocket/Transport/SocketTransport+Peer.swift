...
public extension SocketTransport {
    ...
    ///
    /// Used with ``isProcessDescendant(_:of:)`` to enforce the `cotermOnly`
    ...
    ///     // refuse: client was not started inside coterm
    /// }
    /// ```
    ...
    ///
    /// Pairs with ``peerProcessID(of:)`` for the `cotermOnly` ancestry check;
    /// see that symbol for a usage example.
    /// - Parameters:
    ///   - pid: The process to test.
    ///   - ancestorPid: The candidate ancestor (the coterm process).
