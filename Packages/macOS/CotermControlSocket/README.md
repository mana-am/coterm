...
The coterm control-socket domain: the listener and the transport/policy layer under the Unix-domain socket that external programs (the coterm CLI, agents, tests) use to drive the app.
...
#expect(server.start(socketPath: path, accessMode: .cotermOnly))
...
Run with `swift test --package-path Packages/macOS/CotermControlSocket`.
