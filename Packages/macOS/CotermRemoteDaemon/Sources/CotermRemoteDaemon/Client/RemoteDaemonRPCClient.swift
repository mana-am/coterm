...
/// Synchronous JSON-RPC client for a cotermd-remote daemon over one of three
...
public final class RemoteDaemonRPCClient: @unchecked Sendable {
    ...
    static let bakedVMDaemonSocketPath = "/run/cotermd-remote.sock"
    ...
    let writeQueue = DispatchQueue(label: "com.coterm.remote-ssh.daemon-rpc.write.\(UUID().uuidString)")
    let stateQueue = DispatchQueue(label: "com.coterm.remote-ssh.daemon-rpc.state.\(UUID().uuidString)")
    ...
    ///   - remotePath: Remote path of the cotermd-remote binary (stdio
    ///     transport only).
    ...
    public func start() throws {
        ...
        do {
            ...
            guard missingCapabilities.isEmpty else {
                throw NSError(domain: "coterm.remote.daemon.rpc", code: 2, userInfo: [
