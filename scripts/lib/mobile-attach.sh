...
# reload.sh / coterm-debug-cli.sh exactly).
#
...
# class is deliberate (matches reload.sh + coterm-debug-cli.sh socket/DerivedData
# naming); a locale-sensitive class would keep non-ASCII letters the slug drops.
coterm_attach__slug_raw() {
...
coterm_attach__slug() {
  local cleaned
  cleaned="$(coterm_attach__slug_raw "$1")"
...
coterm_attach_tag_has_alnum() {
  [[ -n "$(coterm_attach__slug_raw "$1")" ]]
}

# bundle id segment: lowercase, non-alnum -> '.', trimmed/collapsed.
coterm_attach__bundle_seg() {
  local cleaned
...
coterm_attach_socket_path() {
  printf '/tmp/coterm-debug-%s.sock' "$(coterm_attach__slug "$1")"
}
...
# sanitized slug, matching reload.sh (`APP_NAME="Coterm DEV ${TAG_SLUG}"`); the raw
# tag would miss for any tag whose slug differs (e.g. "Fix Foo" -> "fix-foo").
coterm_attach_mac_app_path() {
  local slug
  slug="$(coterm_attach__slug "$1")"
  printf '%s/Library/Developer/Xcode/DerivedData/coterm-%s/Build/Products/Debug/Coterm DEV %s.app' \
    "$HOME" "$slug" "$slug"
}
...
coterm_attach_enable_pairing_host() {
  local tag="$1" bundle_id
  bundle_id="$(coterm_attach_mac_bundle_id "$tag")"
...
coterm_attach_mac_socket_ready() {
  local sock
  sock="$(coterm_attach_socket_path "$1")"
  [[ -S "$sock" ]]
}
...
#                     COTERM_ATTACH_ALLOW_RELAUNCH=1 to opt into auto-relaunching
...
coterm_attach_ensure_mac() {
  local tag="$1" repo_root="${2:-}" sock app slug _i
  sock="$(coterm_attach_socket_path "$tag")"
  app="$(coterm_attach_mac_app_path "$tag")"
  slug="$(coterm_attach__slug "$tag")"
  coterm_attach_enable_pairing_host "$tag" || true

  if [[ -S "$sock" ]]; then
    # Quick probe (2 attempts ~1s): if pairing already mints, done.
    if [[ -n "$repo_root" ]] && [[ -n "$(coterm_attach_mint_url "$tag" 60 "$repo_root" 2)" ]]; then
      return 0
    fi
    ...
    if [[ "${COTERM_ATTACH_ALLOW_RELAUNCH:-0}" != "1" ]]; then
      echo "warning: tagged Mac app for '$tag' is running but its iOS pairing listener is not bound (it was likely launched before pairing was enabled, or the macOS Local Network prompt is pending). Relaunch it to enable auto-pair, or re-run with COTERM_ATTACH_ALLOW_RELAUNCH=1; signing in only for now." >&2
      return 1
    fi
    if [[ ! -d "$app" ]]; then
    ...
    echo "==> relaunching tagged Mac app to bind the pairing listener ($tag) [COTERM_ATTACH_ALLOW_RELAUNCH=1]" >&2
    # Scoped to this tag's executable only (never the stable app or other tags).
    pkill -f "Coterm DEV ${slug}.app/Contents/MacOS/Coterm DEV" 2>/dev/null || true
  ...
  # The tagged app derives its socket from its baked CotermDevTag, so a plain launch
  # binds /tmp/coterm-debug-<slug>.sock without extra env.
...
coterm_attach_mint_url() {
  local tag="$1" ttl="$2" repo_root="$3" max="${4:-20}" sock slug payload url _i
  sock="$(coterm_attach_socket_path "$tag")"
  # coterm-debug-cli.sh rejects COTERM_TAG outside [A-Za-z0-9._-] and re-sanitizes it
  ...
  for _i in $(seq 1 "$max"); do
    ...
    payload="$(COTERM_TAG="$slug" "$repo_root/scripts/coterm-debug-cli.sh" rpc mobile.attach_ticket.create \
