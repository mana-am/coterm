...
if [ -n "$console_user" ] && [ "$console_user" != "root" ] \
  ...
  forward=(PATH DEVELOPER_DIR GITHUB_WORKSPACE RUNNER_TEMP \
    COTERM_DERIVED_DATA_PATH COTERM_TAG COTERM_SKIP_ZIG_BUILD \
    COTERM_UNIT_TEST_TIMEOUT_SECONDS \
    COTERM_XCODEBUILD_NONINTERACTIVE_IDLE_TIMEOUT_SECONDS \
    COTERM_XCODEBUILD_NONINTERACTIVE_POST_TEST_TIMEOUT_SECONDS \
    COTERM_XCODEBUILD_NONINTERACTIVE_TIMEOUT_SECONDS \
    COTERM_APP_HOST_XCODEBUILD_ATTEMPTS)
  env_pairs=()
