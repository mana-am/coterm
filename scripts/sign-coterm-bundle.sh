#!/usr/bin/env bash
# Inside-out codesign a coterm .app bundle for Developer ID + notarization.
#
# Usage:
#   scripts/sign-coterm-bundle.sh <app-path> <app-entitlements> <signing-identity>
#
# Example:
#   scripts/sign-coterm-bundle.sh \
#     "build-universal/Build/Products/Release/Coterm NIGHTLY.app" \
#     coterm.nightly.entitlements \
#     "Developer ID Application: emergent.inc (7WLXT3NR37)"
#
# Optional env:
#   COTERM_HELPER_ENTITLEMENTS  (default: coterm-helper.entitlements)
#   COTERM_TIMESTAMP             set to "none" for un-timestamped local sigs
#
...
APP_ENTITLEMENTS="$2"
IDENTITY="$3"
HELPER_ENTITLEMENTS="${COTERM_HELPER_ENTITLEMENTS:-coterm-helper.entitlements}"
...
if /usr/libexec/PlistBuddy -c "Print :com.apple.developer.web-browser.public-key-credential" \
  "$APP_ENTITLEMENTS" >/dev/null 2>&1; then
