...
agent lives in another coterm instance), so the phone never sees it.

Data sources (read live over the tagged debug socket, no app rebuild):
  coterm rpc chat.sessions.dump   -> the registry
  coterm debug-terminals          -> current surfaces and their workspaces

Usage:  COTERM_TAG=<tag> scripts/coterm-chat-debug.py [--live] [--all]
"""
import json
import os
import re
...
CLI = os.path.join(os.path.dirname(__file__), "coterm-debug-cli.sh")


def cli(*args):
    if not os.environ.get("COTERM_TAG"):
        sys.exit("COTERM_TAG must be set, e.g. COTERM_TAG=asot scripts/coterm-chat-debug.py")
    out = subprocess.run([CLI, *args], capture_output=True, text=True, env=dict(os.environ))
    if out.returncode != 0:
        sys.exit("coterm-debug-cli.sh %s failed (rc=%d):\n%s"
...
def render(show_all):
    ...
    print("\033[1magent-chat host view\033[0m  tag=%s" % os.environ.get("COTERM_TAG", "?"))
