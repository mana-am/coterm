#!/usr/bin/env bash
# Lint: every Swift file under cotermTests/ must be wired into
...
# `xcodebuild test -only-testing:cotermTests/<TestClass>` pass with
...
#
# Originally surfaced during the https://github.com/emergent-inc/coterm/issues/4529
# investigation, where SessionIndexJSONLStreamTests.swift on
# https://github.com/emergent-inc/coterm/pull/4536 looked like a clean two-commit
...
TESTS_DIR="$REPO_ROOT/cotermTests"

if [ ! -f "$PBXPROJ" ]; then
  echo "lint-pbxproj-test-wiring: not found: $PBXPROJ" >&2
  echo "  (run from the coterm repo root or pass --repo-root)" >&2
  exit 2
fi
if [ ! -d "$TESTS_DIR" ]; then
...
# Locate the cotermTests PBXNativeTarget and resolve its Sources build phase
# UUID. We then slice out just that build phase block and look for files inside
# it — which is exactly the set of files Xcode compiles into cotermTests.
#
# Targeting the cotermTests Sources phase specifically (instead of the whole
...
#      target).
#   3. File is a member of the wrong target (e.g. cotermUITests or coterm). Its
#      `<file>.swift in Sources` lines exist in the pbxproj, so a global grep
#      would pass, but they are not inside the cotermTests Sources block.
# `/* cotermTests */ = {` appears twice in a typical pbxproj: once for the
...
# `/* cotermTests */ = { ... };` block and keep only the one whose `isa =
# PBXNativeTarget;` line is present.
tests_target_block="$(awk '
  /\/\* cotermTests \*\/ = \{/ { capture = 1; buf = "" }
  capture { buf = buf $0 "\n" }
  capture && /^[[:space:]]*\};[[:space:]]*$/ {
...
if [ -z "$tests_target_block" ]; then
  echo "lint-pbxproj-test-wiring: could not locate cotermTests PBXNativeTarget in $PBXPROJ" >&2
  exit 2
fi
...
if [ -z "$tests_sources_uuid" ]; then
  echo "lint-pbxproj-test-wiring: cotermTests target has no Sources build phase reference" >&2
  exit 2
fi

# Slice the PBXSourcesBuildPhase block whose UUID matches the cotermTests
...
if [ -z "$tests_sources_block" ]; then
  echo "lint-pbxproj-test-wiring: could not slice cotermTests Sources build phase (uuid=$tests_sources_uuid)" >&2
  exit 2
fi
...
while IFS= read -r -d '' file; do
  base="$(basename "$file")"
  checked=$((checked + 1))
  # Look for the file's entry inside the cotermTests Sources phase only.
  #
...
echo "lint-pbxproj-test-wiring: ${#missing[@]} test file(s) not a member of the cotermTests target's Sources build phase (uuid=$tests_sources_uuid) in coterm.xcodeproj/project.pbxproj"
for entry in "${missing[@]}"; do
  echo "  - $entry"
done
echo ""
echo "Each cotermTests/<file>.swift must be wired into coterm.xcodeproj/project.pbxproj"
echo "as a full target member of cotermTests:"
echo "  1. a PBXBuildFile entry (line ends with '<file>.swift in Sources */ = { ... };')"
echo "  2. a PBXFileReference entry"
echo "  3. an entry in the cotermTests group children list"
echo "  4. an entry in the cotermTests target's PBXSourcesBuildPhase files"
echo "     (line ends with '<file>.swift in Sources */,')"
echo ""
echo "This lint slices the cotermTests Sources phase and looks for entry 4 there."
echo "Files wired only into cotermUITests, coterm, or the project tree (without"
echo "cotermTests target membership) are silently skipped by Xcode and will be"
echo "flagged here."
echo ""
echo "Add via Xcode (drag the file into the cotermTests target) or hand-edit"
