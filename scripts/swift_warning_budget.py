#!/usr/bin/env python3
"""Check coterm-owned Swift warnings against a checked-in budget."""
...
OWNED_ROOTS = ("Sources", "CLI", "Packages", "cotermTests", "cotermUITests")
IGNORED_PATH_PARTS = (
    "/vendor/",
    "/ghostty/",
    "/homebrew-coterm/",
...
def write_budget(path: pathlib.Path, budget: WarningBudget) -> None:
    ...
    with path.open("w", encoding="utf-8") as handle:
        handle.write("# coterm-owned Swift warning budget.\n")
...
def main(argv: list[str]) -> int:
    ...
    print_budget_summary("Actual coterm-owned Swift warnings", actual)
