...
final class DisplayResolutionRegressionUITests: XCTestCase {
    private let defaultDisplayHarnessManifestPath = "/tmp/coterm-ui-test-display-harness.json"
    ...
    override func setUp() {
        ...
        let tempPrefix = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ui-test-display-\(token)")
            .path
        launchTag = "ui-tests-display-resolution-\(token.prefix(8))"
        diagnosticsPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ui-test-display-churn-\(token).json")
            .path
    ...
    private let prelaunchManifestPath = "/tmp/coterm-ui-test-prelaunch.json"
    ...
    private func loadPrebuiltHelperBinaryPath(_ env: [String: String]) -> String? {
        guard let helperBinaryPath = env["COTERM_UI_TEST_DISPLAY_HELPER_BINARY_PATH"],
    ...
    private func loadExternalHarnessFromEnvironment(_ env: [String: String]) -> ExternalDisplayHarness? {
        guard let readyPath = env["COTERM_UI_TEST_DISPLAY_READY_PATH"], !readyPath.isEmpty,
              let displayIDPath = env["COTERM_UI_TEST_DISPLAY_ID_PATH"], !displayIDPath.isEmpty,
              let donePath = env["COTERM_UI_TEST_DISPLAY_DONE_PATH"], !donePath.isEmpty else {
            return nil
        }
        ...
        return ExternalDisplayHarness(
            ...
            startPath: env["COTERM_UI_TEST_DISPLAY_START_PATH"],
            donePath: donePath,
            logPath: env["COTERM_UI_TEST_DISPLAY_LOG_PATH"],
            helperBinaryPath: nil
        )
    }

    private func loadExternalHarnessFromManifest(_ env: [String: String]) -> ExternalDisplayHarness? {
        let manifestPath = env["COTERM_UI_TEST_DISPLAY_HARNESS_MANIFEST_PATH"] ?? defaultDisplayHarnessManifestPath
    ...
    private func launchAppProcess(targetDisplayID: String) throws {
        ...
        let logPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ui-test-app-\(launchTag).log").path
    ...
    private func resolveAppBinaryPath() throws -> String {
        ...
        //   .../Build/Products/Debug/cotermUITests-Runner.app/Contents/PlugIns/cotermUITests.xctest
        // The app binary is at:
        //   .../Build/Products/Debug/Coterm DEV.app/Contents/MacOS/Coterm DEV
        let testBundle = Bundle(for: Self.self)
        let productsDir = testBundle.bundleURL
            .deletingLastPathComponent()  // -> .../Contents/PlugIns
            .deletingLastPathComponent()  // -> .../Contents
            .deletingLastPathComponent()  // -> .../cotermUITests-Runner.app
            .deletingLastPathComponent()  // -> .../Debug
        let binaryPath = productsDir
            .appendingPathComponent("Coterm DEV.app")
            .appendingPathComponent("Contents/MacOS/Coterm DEV")
            .path
    ...
    private func launchEnvironment(targetDisplayID: String) -> [String: String] {
        [
            ...
            "COTERM_UI_TEST_DIAGNOSTICS_PATH": diagnosticsPath,
            "COTERM_UI_TEST_DISPLAY_RENDER_STATS": "1",
            "COTERM_UI_TEST_TARGET_DISPLAY_ID": targetDisplayID,
            "COTERM_TAG": launchTag,
        ]
    }
