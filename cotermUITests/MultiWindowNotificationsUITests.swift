...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/coterm-ui-test-multi-window-notifs-\(UUID().uuidString).json"
        socketPath = "/tmp/coterm-ui-test-socket-\(UUID().uuidString).sock"
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        app.launchEnvironment["COTERM_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_MULTI_WINDOW_NOTIF_PATH"] = dataPath
        app.launchEnvironment["COTERM_TAG"] = launchTag
        ...
        XCTAssertTrue(
            ...
            "Expected coterm to be foreground before sending notification shortcut. state=\(app.state.rawValue)"
        )
    ...
    func testNotificationsPopoverCanCloseViaShortcutAndEscape() {
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected coterm to be foreground before opening notifications popover. state=\(app.state.rawValue)"
        )
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        app.launchEnvironment["COTERM_SOCKET_PATH"] = socketPath
        app.launchEnvironment["COTERM_SOCKET_MODE"] = "allowAll"
        app.launchEnvironment["COTERM_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["COTERM_TAG"] = launchTag
        launchAllowingHeadlessBackgroundActivation(app)
        XCTAssertTrue(
            ensureAppRunningAfterLaunch(app, timeout: 12.0),
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected coterm to be foreground before opening empty notifications popover. state=\(app.state.rawValue)"
        )
        app.typeKey("i", modifierFlags: [.command])
        ...
        let marker = "coterm_notif_block_\(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(8))"
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        app.launchEnvironment["COTERM_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] = "1"
        app.launchEnvironment["COTERM_TAG"] = launchTag
        launchAllowingHeadlessBackgroundActivation(app)
        XCTAssertTrue(
            ensureAppRunningAfterLaunch(app, timeout: 12.0),
        ...
        let commandStatusPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ui-test-notify-\(commandResultStem).status")
            .path
        let commandStdoutPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ui-test-notify-\(commandResultStem).stdout")
            .path
        let commandStderrPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ui-test-notify-\(commandResultStem).stderr")
            .path
        let commandScriptPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ui-test-notify-\(commandResultStem).sh")
            .path
        defer {
        ...
        guard let bundledCLIPath = resolveCotermCLIPaths(strategy: .bundledOnly).first else {
            XCTFail("Failed to locate bundled coterm CLI for notify regression test")
            return
        }
        ...
        do {
            ...
            XCTFail(
                "Failed to write delayed bundled `coterm notify` script. " +
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected coterm to be foreground before typing delayed notify command. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected coterm to move to background before delayed notify command runs. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected delayed bundled `coterm notify` command to finish without foregrounding coterm. state=\(app.state.rawValue)"
        )
        ...
        // bundled `coterm notify` command has finished while the app stayed
        ...
        XCTAssertFalse(
            ...
            "Expected coterm to remain in background after bundled `coterm notify`. state=\(app.state.rawValue) stderr=\(notifyStderr)"
        )
        guard notifyExitStatus == "0" else {
            XCTFail(
                "Expected bundled `coterm notify` launched from the in-app shell to succeed. " +
    ...
    func testOpenNotificationCLISelectsWorkspaceSurfaceAndMarksRowRead() throws {
        ...
        let notifyResult = runCotermCommand(
        ...
        let beforeToken = loadData()?["focusToken"]
        let openResult = runCotermCommand(
            socketPath: socketPath,
            arguments: [
                "open-notification",
                "--id",
    ...
    func testNewWorkspaceCLIWindowFlagTargetsSpecificWindow() throws {
        ...
        app.launchEnvironment["COTERM_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_WINDOW_ROUTE_CLI"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_WINDOW_ROUTE_CLI_TITLE"] = title
        app.launchEnvironment["COTERM_TAG"] = launchTag
        launchAllowingHeadlessBackgroundActivation(app)
        XCTAssertTrue(
            ensureAppRunningAfterLaunch(app, timeout: 12.0),
        ...
        XCTAssertEqual(
            ...
            "1",
            "Expected bundled `coterm --window` route test hook to finish. failure=\(setup["windowRouteFailure"] ?? "")"
        )
    ...
    private func waitForCotermPing(timeout: TimeInterval) -> (stdout: String?, stderr: String?) {
        var lastStdout: String?
        var lastStderr: String?
        let didSucceed = waitForCondition(timeout: timeout) {
            let result = self.runCotermCommand(
        ...
        let result = runCotermCommand(socketPath: socketPath, arguments: ["ping"], responseTimeoutSeconds: 2.0)
        let stdout = result.stdout.isEmpty ? nil : result.stdout
        let stderr = result.stderr.isEmpty ? nil : result.stderr
    ...
    private func runCotermCommand(
        socketPath: String,
        arguments: [String],
        responseTimeoutSeconds: Double = 3.0,
        cliStrategy: CotermCLIStrategy = .any
    ) -> (terminationStatus: Int32, stdout: String, stderr: String) {
        var args = ["--socket", socketPath]
        args.append(contentsOf: arguments)
        var environment = ProcessInfo.processInfo.environment
        environment["COTERM_CLI_RESPONSE_TIMEOUT_SEC"] = String(responseTimeoutSeconds)

        let cliPaths = resolveCotermCLIPaths(strategy: cliStrategy)
        if cliPaths.isEmpty, cliStrategy == .bundledOnly {
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to locate bundled coterm CLI"
            )
        }

        var lastPermissionFailure: (terminationStatus: Int32, stdout: String, stderr: String)?
        for cliPath in cliPaths {
            let result = executeCotermCommand(
        ...
        if cliStrategy == .bundledOnly {
            return lastPermissionFailure ?? (
                terminationStatus: -1,
                stdout: "",
                stderr: "Bundled coterm CLI command failed without an executable path"
            )
        }

        let fallbackArgs = ["coterm"] + args
        let fallbackResult = executeCotermCommand(
    ...
    private enum CotermCLIStrategy: Equatable {
        case any
        case bundledOnly
    }
    ...
    private func resolveCotermCLIPaths(strategy: CotermCLIStrategy) -> [String] {
        ...
        if strategy == .any {
            for key in ["COTERM_UI_TEST_CLI_PATH", "COTERM_CLI"] {
        ...
        candidates.append("/tmp/coterm-\(launchTag)/Build/Products/Debug/Coterm DEV.app/Contents/Resources/bin/coterm")
        candidates.append("/tmp/coterm-\(launchTag)/Build/Products/Debug/Coterm.app/Contents/Resources/bin/coterm")
        if strategy == .any {
            candidates.append("/tmp/coterm-\(launchTag)/Build/Products/Debug/coterm")
        }
    ...
    private func appendCLIPathCandidates(
        ...
        strategy: CotermCLIStrategy,
        to candidates: inout [String]
    ) {
        candidates.append("\(productsDir)/Coterm DEV.app/Contents/Resources/bin/coterm")
        candidates.append("\(productsDir)/Coterm.app/Contents/Resources/bin/coterm")
        if strategy == .any {
            candidates.append("\(productsDir)/coterm")
        }
        ...
        for entry in entries.sorted() where entry.hasSuffix(".app") {
            let cliPath = URL(fileURLWithPath: productsDir)
                ...
                .appendingPathComponent("Contents/Resources/bin/coterm")
                .path
            candidates.append(cliPath)
        }
        if strategy == .any {
            for entry in entries.sorted() where entry == "coterm" {
                let cliPath = URL(fileURLWithPath: productsDir)
                    .appendingPathComponent(entry)
                    .path
                candidates.append(cliPath)
    ...
    private func executeCotermCommand(
        ...
        do {
            ...
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to run coterm command: \(error.localizedDescription) (cliPath=\(executablePath))"
            )
        }
    ...
    private func resolveSocketPath(timeout: TimeInterval, requiredWorkspaceId: String? = nil) -> String? {
        ...
        _ = waitForCondition(timeout: timeout) {
            for candidate in primaryCandidates {
                ...
                // Primary candidate is the explicitly requested COTERM_SOCKET_PATH. If it responds,
    ...
    private func expectedSocketCandidates(includeGlobalFallback: Bool) -> [String] {
        ...
        let taggedDebugSocket = "/tmp/coterm-debug-\(launchTag).sock"
        ...
        if includeGlobalFallback {
            ...
            candidates.append("/tmp/coterm-debug.sock")
            candidates.append(stableSocketPath())
            candidates.append("/tmp/coterm.sock")
        }
    ...
    private func stableSocketPath() -> String {
        // Mirrors CotermStateDirectory.url() + coterm.sock (non-TCC ~/.local/state/coterm;
        // see https://github.com/emergent-inc/coterm/issues/5146).
    ...
    private func discoverTmpSocketCandidates(limit: Int) -> [String] {
        ...
        let matches = entries.filter { $0.hasPrefix("coterm") && $0.hasSuffix(".sock") }
