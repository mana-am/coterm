...
final class CommandPaletteIdentifierClipboardUITests: XCTestCase {
    ...
    func testCmdShiftPOpenCotermJSONOpensUserConfigFile() throws {
        let app = XCUIApplication()
        let capturePath = "/tmp/coterm-ui-test-open-coterm-json-\(UUID().uuidString).txt"
        ...
        app.launchArguments += ["-AppleLanguages", "(en)", "-AppleLocale", "en_US", "-menuBarOnly", "false"]
        app.launchEnvironment["COTERM_UI_TEST_MODE"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_CAPTURE_OPEN_PATH"] = capturePath
        launchAndActivate(app)
        ...
        searchField.typeText("open coterm json")

        let predicate = NSPredicate(
            format: "identifier BEGINSWITH %@ AND value == %@",
            "CommandPaletteResultRow.",
            "palette.openCotermSettingsFile"
        )
        let row = app.descendants(matching: .any)
            .matching(predicate)
            .firstMatch
        XCTAssertTrue(row.waitForExistence(timeout: 5.0), "Expected row for Open coterm.json")
        try? FileManager.default.removeItem(atPath: capturePath)
        row.click()
        ...
        let expectedPath = (loginHomeDirectoryPath() as NSString)
            .appendingPathComponent(".config/coterm/coterm.json")
