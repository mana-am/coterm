...
final class FeedSidebarUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        socketPath = "/tmp/coterm-debug-\(UUID().uuidString).sock"
        diagnosticsPath = "/tmp/coterm-feed-sidebar-\(UUID().uuidString).json"
        feedResultPath = "/tmp/coterm-feed-sidebar-result-\(UUID().uuidString).json"
        feedTUIReadyPath = "/tmp/coterm-feed-sidebar-tui-ready-\(UUID().uuidString).json"
        dockRenderReadyPath = "/tmp/coterm-dock-render-ready-\(UUID().uuidString).json"
        dockConfigPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-feed-sidebar-dock-\(UUID().uuidString).json")
            .path
    ...
    func testFeedReceivesAndResolvesPermissionRequest() throws {
        ...
        app.launchEnvironment["COTERM_ALLOW_SOCKET_OVERRIDE"] = "1"
        ...
        app.launchEnvironment["COTERM_UI_TEST_PORTAL_STATS"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_FEED_SIDEBAR_RESULT_PATH"] = feedResultPath
        app.launchEnvironment["COTERM_UI_TEST_FEED_SIDEBAR_REQUEST_ID"] = requestId
        app.launchEnvironment["COTERM_UI_TEST_FEED_TUI_READY_PATH"] = feedTUIReadyPath
        app.launchEnvironment["COTERM_UI_TEST_DOCK_CONFIG_PATH"] = dockConfigPath
    ...
    private func waitForFeedTUIReady(timeout: TimeInterval) -> Bool {
        return pollUntil(timeout: timeout, interval: 0.5) {
            ...
            guard payload["stage"] == "opentui-ready",
                  ...
                  let cwd = payload["cwd"],
                  !cwd.contains(".coterm/feed-tui-opentui") else {
                return false
            }
            return true
        }
    ...
    private func launchAndEnsureUsable(_ app: XCUIApplication) {
        ...
        XCTAssertTrue(
            ...
            "coterm failed to launch for Feed UI test. state=\(app.state.rawValue)"
        )
    }
    ...
    private func writeFeedDockConfig(bunPath: String?) throws {
        var env = ["COTERM_FEED_TUI_READY_PATH": feedTUIReadyPath]
        if let bunPath, !bunPath.isEmpty {
            env["COTERM_FEED_TUI_BUN_PATH"] = bunPath
        }
    ...
    private func writeDockRenderConfig() throws {
        ...
        let config: [String: Any] = [
            "controls": [[
                ...
                "command": command,
                "height": 320,
                "env": ["COTERM_DOCK_RENDER_READY_PATH": dockRenderReadyPath]
            ]]
        ]
        let data = try JSONSerialization.data(withJSONObject: config, options: [.prettyPrinted, .sortedKeys])
        try data.write(to: URL(fileURLWithPath: dockConfigPath), options: .atomic)
