...
/// `shortcuts.bindings` in coterm.json (the JSON config store), and the
...
/// - `SettingsKeyboardShortcutsOpenSettingsFileButton` — open coterm.json.
...
final class SettingsShortcutsBehaviorUITests: SettingsUITestCase {
    ...
    ///   - off: "Turn this on to show or hide all coterm windows from any app."
    ///   - on:  "Press the shortcut from any app to show or hide all coterm windows."
    ...
    func testEnableToggleSwapsSubtitleCopy() {
        ...
        let offSubtitle = "Turn this on to show or hide all coterm windows from any app."
        let onSubtitle = "Press the shortcut from any app to show or hide all coterm windows."
    ...
    /// starting state, so the test leaves coterm.json clean.
...
//   chord shows/hides all coterm windows from any app. That registration and
...
//   ("Open coterm.json") — Calls SettingsHostActions.openConfigInExternalEditor(),
//   which runs NSWorkspace.shared.open(configFileURL) to hand the coterm.json
...
//   the coterm UI surface to assert against. Not e2e-testable from XCUITest;
...
//   A SwiftUI Link to https://coterm.cc/docs/keyboard-shortcuts#shortcut-chords.
