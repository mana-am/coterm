...
final class SidebarHelpMenuUITests: XCTestCase {
    ...
    func testTutorialVideoSeenFlagSuppressesAutomaticPresentation() {
        let app = XCUIApplication()
        app.launchArguments += ["-cotermTutorialVideoSeen.v1", "YES"]
        app.launchEnvironment["COTERM_UI_TEST_MODE"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_TUTORIAL_VIDEO_AUTO_SHOW"] = "1"
        launchAndActivate(app)
    ...
    func testHelpMenuCheckForUpdatesTriggersSidebarUpdatePill() {
        ...
        app.launchEnvironment["COTERM_UI_TEST_FEED_MODE"] = "available"
    ...
    func testHelpMenuSendFeedbackOpensComposerSheet() {
        ...
        XCTAssertTrue(
            app.staticTexts[
                "A human will read this! You can also reach us at contact@emergent.inc."
...
final class FeedbackComposerShortcutUITests: XCTestCase {
    ...
    func testCmdOptionFWorksFromSettingsWindow() {
        let app = XCUIApplication()
        app.launchEnvironment["COTERM_UI_TEST_MODE"] = "1"
        app.launchEnvironment["COTERM_UI_TEST_SHOW_SETTINGS"] = "1"
        app.launch()
        app.activate()
...
final class CommandPaletteAllSurfacesUITests: XCTestCase {
    ...
    private let debugDefaultsDomain = "coterm.com.emergent.app.debug"
    private let hiddenSurfaceToken = "coterm-command-palette-hidden-surface"
    private let visibleSurfaceToken = "coterm-command-palette-visible-surface"
    private let noMatchWorkspaceQuery = "coterm-command-palette-no-match"

    override func setUp() {
        super.setUp()
        continueAfterFailure = false
        socketPath = "/tmp/coterm-ui-test-command-palette-\(UUID().uuidString).sock"
    ...
    func testMinimalModeToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/coterm-ui-test-settings-focus-\(UUID().uuidString).json"
        ...
        let diagnostics = waitForDiagnostics(
            ...
            data["keyWindowIdentifier"] == "coterm.settings" && data["settingsWindowIsKey"] == "1"
        }
    ...
    func testMenuBarOnlyToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/coterm-ui-test-menu-bar-only-focus-\(UUID().uuidString).json"
        try? FileManager.default.removeItem(atPath: diagnosticsPath)
