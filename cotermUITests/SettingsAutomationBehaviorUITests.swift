...
///
/// The Automation section (`AutomationSection` in `CotermSettingsUI`) exposes:
...
/// env injection, which `rg` binary Find resolves, the `COTERM_PORT` range a new
...
///     not exist in the default `Coterm processes only` mode, and choosing a
...
final class SettingsAutomationBehaviorUITests: SettingsUITestCase {
    ...
    private enum L {
        ...
        static let modeCotermOnly = "Coterm processes only"
    ...
    /// Default mode is `Coterm processes only`, so the Socket Password row must
    /// be absent. Selecting `Password mode` must reveal it (title + the
    /// "no password set" subtitle), and selecting `Coterm processes only`
    ...
    func testPasswordModeRevealsAndHidesSocketPasswordRow() {
        ...
        // Default (cotermOnly): no password row.
        XCTAssertFalse(
            passwordTitle.waitForExistence(timeout: 1.0),
            "Socket Password row should be hidden in the default coterm-only mode"
        )
        ...
        // Switch back to coterm-only → row disappears again.
        XCTAssertTrue(selectMode(window, app, L.modeCotermOnly), "could not select coterm-only mode")
        XCTAssertTrue(
            poll(timeout: 4.0) { !passwordTitle.exists },
    ...
    //
    // TIER 2 (needs runtime seam): Socket Control Mode (cotermOnly/automation/
    ...
    //   connection handler (consumed in AppDelegate/cotermApp socket setup), not
    ...
    //   toggles — when enabled, coterm injects session-tracking + notification
    //   hook env into newly launched `claude`/`cursor`/`gemini` processes
    //   inside coterm terminals. The observable consequence (sidebar session
    ...
    //   these select which `claude` / `rg` executable coterm resolves
    ...
    //   COTERM_PORT / COTERM_PORT_END environment range a new terminal inherits.
    //   The only true verification is reading the env of a freshly spawned
    //   terminal (e.g. `echo $COTERM_PORT`), which requires driving the Ghostty
