...
/// (`Packages/macOS/CotermSettingsUI/.../Sections/BrowserSection.swift`).
///
...
/// `COTERM_UI_TEST_BROWSER_IMPORT_FIXTURE` env seam, so we can launch with a
...
final class SettingsBrowserBehaviorUITests: SettingsUITestCase {
    ...
    private func makeLaunchedAppWithImportFixture() -> XCUIApplication {
        ...
        app.launchEnvironment["COTERM_UI_TEST_BROWSER_IMPORT_FIXTURE"] =
    ...
    // TIER 2 (needs runtime seam): Enable coterm Browser (BrowserEnabledToggle) —
    ...
    //   never writes (only the legacy `cotermApp.browserEnabledBinding`
    //   mirrors both keys). So toggling Enable coterm Browser in the new
    //   Settings UI produces no XCUITest-observable change (the command
    //   palette still shows "Disable coterm Browser"). Verifying this
    ...
    //   coterm without a warning prompt. The effect (warning prompt shown or
    ...
    //   only via dedicated COTERM_UI_TEST_BROWSER_IMPORT_HINT_* launch seams,
