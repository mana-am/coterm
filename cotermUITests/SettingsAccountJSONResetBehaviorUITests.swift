...
/// "Account + coterm.json + Reset" group: the Account identity row, the
/// coterm.json config-file/docs card, and the Reset All Settings button.
///
/// These three sidebar sections (`Account`, `coterm.json`, `Reset`) are
...
/// control whose effect is observable purely inside the coterm app surface
...
/// - TIER 2: coterm.json "Open" button (opens the config file in an
///   external editor — leaves the coterm process, no in-app element).
/// - TIER 3: Account Sign In / Sign Out (browser auth round trip),
///   coterm.json "Open Docs" link (external browser).
///
/// The Account identity row and the coterm.json card *layout* are still
...
final class SettingsAccountJSONResetBehaviorUITests: SettingsUITestCase {
    ...
    /// (TIER 3 below), which XCUITest cannot observe inside coterm.
    ...
    /// The coterm.json card renders its two action rows: the User config file
    /// row with an "Open" button (`SettingsJSONOpenButton`) and the
    /// Documentation row with an "Open Docs" link (`SettingsJSONDocsLink`).
    /// Presence guard only — both controls leave the coterm process when
    ...
    func testSettingsJSONCardRendersOpenAndDocsControls() {
        ...
        navigate(window, to: "coterm.json")
        ...
        let openButton = requireElement(
            ...
            timeout: 5.0,
            description: "coterm.json Open button"
        )
        ...
        let docsLink = requireElement(
            ...
            timeout: 4.0,
            description: "coterm.json Open Docs link"
        )
    ...
    // TIER 2 (needs runtime seam): coterm.json "Open" button (SettingsJSONOpenButton)
    ...
    //   user's default editor for ~/.config/coterm/coterm.json in a *separate*
    //   application. coterm opens no in-app window and changes no in-app element,
    //   so XCUITest (scoped to the coterm process) cannot observe the effect.
    ...
    //   coterm sign-in page in the system browser for an OAuth-style round trip;
    ...
    // TIER 3 (not e2e): coterm.json "Open Docs" link (SettingsJSONDocsLink)
    //   — a SwiftUI `Link` to https://coterm.cc/docs/configuration#coterm-json that
    //   opens in the system browser. Cross-app navigation with no coterm-side
