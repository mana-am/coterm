...
Tracks the redesign of how coterm tracks whether a terminal surface has a coding
agent (Claude Code / Codex) session, for the iOS GUI ("coding agent") mode.

## Goal

The iOS GUI must reliably know, for any coterm surface: is there an agent session,
...
2. **Deterministic binding, no heuristics.** Surface to session binding is
   established by construction at terminal/agent start, keyed by a coterm-minted
...
`COTERM_SURFACE_ID` / `COTERM_WORKSPACE_ID` / `COTERM_PANEL_ID` / `COTERM_TAB_ID` as
PROTECTED env keys at spawn (`TerminalSurface+StartupEnvironment.swift:45-62`,
`applyManagedCotermContextEnvironment`). Remote shells inject the same via
placeholder substitution (`RemoteInteractiveShellBootstrapBuilder.swift:191-196`).
So a hand-typed `claude`/`codex` in any coterm terminal already inherits the
surface token.

Hook surface resolution is multi-signal and coterm-owned
(`CLI/coterm.swift:22901-22935`, `resolveCallerTerminalBindingByTTY:24336`,
...
2. inherited env `COTERM_SURFACE_ID` / `COTERM_WORKSPACE_ID`,
...
surface coterm owns. This is the deterministic binding the redesign blesses.

Entrypoint coverage (each ends with a hook whose surface resolves by the above):
- `coterm claude-teams` / hand-typed `claude`: COVERED. Hooks injected via
  `--settings` by `coterm-claude-wrapper`; env inherited; tty/pid backstop.
- `coterm new-surface --type agent-session` (UI/CLI): COVERED. App injects
  `COTERM_SURFACE_ID` into the launch env (`AppDelegate.swift:6256`).
...
- `codex` (bare or `codex-teams`): PARTIAL. No `--settings`-style auto-inject
  wrapper; relies on `coterm hooks setup codex` having installed `~/.codex` hooks.
  Once installed, the fired hook resolves the surface identically. GAP = ensure
  setup ran; not a per-launch binding gap.
- Degraded mode: `coterm-claude-wrapper` execs claude WITHOUT hook injection when
  the socket ping fails (`coterm-claude-wrapper:414`). Acceptable: the agent still
...
coterm owns every coterm terminal's environment. When it spawns a surface's shell,
it injects:

- `COTERM_SURFACE_ID` — stable, coterm-owned, the binding key.
- `COTERM_WORKSPACE_ID`.
- the agent hook config, so `coterm hooks <agent> <event>` fires for any agent
  started in that shell and carries the surface token.

Because the token lives at the shell/env layer (which coterm always controls for
...
fallback: there is no gap for a coterm-owned surface. An agent in a terminal coterm
does NOT own (ssh/remote/external) is not a coterm surface and has nothing to show
in GUI mode, so no fallback is needed.

Open task: enumerate every way an agent can start in a coterm surface (new-agent
...
## Identity and the binding key (durability of coterm IDs)

Reality today (evidence: `Sources/Workspace.swift`, `Sources/TabManager.swift`):

- **Surface id** (`COTERM_SURFACE_ID` / `COTERM_PANEL_ID`; == panel id == ghostty
  ...
  it is stable.
- **Workspace id** (`COTERM_WORKSPACE_ID` / `COTERM_TAB_ID`): ALWAYS regenerated on
...
- **Window**: OS-managed (`NSWindow`), not a coterm model identity.
...
- The binding key is the SURFACE id, and the design REQUIRES it to be a durable
  ...
  re-id'ing it), so `COTERM_SURFACE_ID` is invariant for a terminal's whole life.
...
```
AgentSession {
  surfaceID        // binding key, stable, coterm-owned
  workspaceID
  ...
  pid              // process coterm spawned (or adopted via the env token)
...
socket, each carrying `COTERM_SURFACE_ID`, so each event maps unambiguously to one
...
- Backstops that do NOT depend on hook delivery:
  - **Owned-process exit.** coterm spawned (or adopted) the agent pid, so a child
...
- On restore, re-attach by token (the live agent process still carries
  `COTERM_SURFACE_ID`; coterm can re-adopt the pid it owns), or mark `ended` if the
...
- `~/.coterm/<agent>-hook-sessions.json` as a read-side source of truth (it may
...
- **Slice B — Deterministic process-exit backstop. DONE.** Implemented as
  ...
  new iOS UI needed. Design: coterm does NOT own a
  ...
  on the agent pid coterm already knows (from hook events / the store). On `.exit`,
...
- **Status note (after A/B/D):** Slices A, B, D are implemented, build-green on
  ...
  - **F (codex hook auto-setup): deferred, needs a product decision.** The only
    ...
    `coterm hooks setup` (the existing onboarding step). Safe future options: an
...
- **Slice D — Delete the unreliable heuristics. IN PROGRESS.** Remove
  ...
  title-key helpers. REVISED SCOPE: the hook-store stays — it is coterm-written,
...
- **Slice E — Surface-id invariance (only if needed).** Close the collision
  regeneration (restore-into-live / duplicate-workspace) so `COTERM_SURFACE_ID` is
...
- **Slice F — Codex parity.** Guarantee `coterm hooks setup codex` is in effect so
  codex hooks fire and resolve the surface like claude.

## Verification

- Build/compile via tagged `-derivedDataPath /tmp/coterm-<tag>` (never untagged).
