...
//
// Auth on every /v1 route is delegated to the pluggable CollabAuthProvider via
// auth-adapter.ts (replacing the upstream Stack Auth verifier). The worker
// resolves the team, derives the per-team Durable Object from the VERIFIED team
// id, and forwards; the DO never sees unauthenticated input.
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/v1/presence/subscribe") {
      ...
      // (credential expiry capped at MAX_SUBSCRIBE_AGE_MS) so a revoked token or
      ...
      const expiresAt = cacheDeadline(Date.now(), subscribeExpiryMs(request, env), MAX_SUBSCRIBE_AGE_MS);
