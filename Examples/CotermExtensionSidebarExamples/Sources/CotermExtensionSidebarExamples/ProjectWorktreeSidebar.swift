...
public struct ProjectWorktreeSidebar: CotermSidebarProvider {
    public let descriptor = CotermSidebarProviderDescriptor(
        id: "com.example.coterm.sidebar.project-worktrees",
    ...
    public func render(snapshot: CotermSidebarProviderSnapshot) -> CotermSidebarProviderRenderModel {
        var sections: [CotermSidebarProviderSection] = []
        ...
        var grouped: [String: [CotermSidebarProviderWorkspace]] = [:]
    ...
    private func branchSubtitle(_ workspace: CotermSidebarProviderWorkspace) -> CotermSidebarProviderText? {
        trimmed(workspace.branchSummary).map(CotermSidebarProviderText.plain)
    }
}
