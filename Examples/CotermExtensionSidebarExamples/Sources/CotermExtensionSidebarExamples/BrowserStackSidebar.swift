...
public struct BrowserStackSidebar: CotermMutableSidebarProvider {
    public static let stateDidLoadNotification = Notification.Name("CotermBrowserStackSidebarStateDidLoad")

    public let descriptor = CotermSidebarProviderDescriptor(
        id: "com.example.coterm.sidebar.browser-stack",
    ...
    public func handle(
        ...
        guard case .moveWorkspace(let move) = mutation else {
            return CotermSidebarProviderCommandResult(ok: false)
        }
        stateCache.moveWorkspace(move, snapshot: snapshot)
        return CotermSidebarProviderCommandResult(ok: true)
    }

    private func recentActivityText(_ workspace: CotermSidebarProviderWorkspace) -> CotermSidebarProviderText? {
        workspace.latestSubmittedAt.map { .relativeDate($0, style: .compact) }
    }

    private func browserIcon(_ workspace: CotermSidebarProviderWorkspace) -> CotermSidebarProviderIcon? {
        ...
        if title.contains("google") {
            return CotermSidebarProviderIcon(
                text: "G",
...
private final class BrowserStackSidebarStateCache: @unchecked Sendable {
    ...
    private let persistenceQueue = DispatchQueue(label: "coterm.browser-stack-sidebar.persistence")
    ...
    func state(for snapshot: CotermSidebarProviderSnapshot) -> BrowserStackSidebarState {
    ...
    func moveWorkspace(
        _ move: CotermSidebarProviderWorkspaceMove,
        snapshot: CotermSidebarProviderSnapshot
    ) {
        let scopeKey = Self.scopeKey(for: snapshot)
    ...
    private func startLoadIfNeeded(scopeKey: String, snapshot: CotermSidebarProviderSnapshot) {
    ...
    private static func scopeKey(for snapshot: CotermSidebarProviderSnapshot) -> String {
...
public struct BrowserStackSidebarStore: Sendable {
    ...
    public func load(scopeKey: String, snapshot: CotermSidebarProviderSnapshot) throws -> BrowserStackSidebarState {
        let url = scopedStateURL(scopeKey: scopeKey)
        if url != stateURL, FileManager.default.fileExists(atPath: url.path) {
            return try load(from: url)
        }
    ...
    private func loadBestScopedFallback(
        matching snapshot: CotermSidebarProviderSnapshot,
    ...
    public func reconciledState(for snapshot: CotermSidebarProviderSnapshot) throws -> BrowserStackSidebarState {
...
public struct BrowserStackSidebarState: Codable, Equatable, Sendable {
    ...
    public static func initial(snapshot: CotermSidebarProviderSnapshot) -> BrowserStackSidebarState {
    ...
    public func reconciled(with snapshot: CotermSidebarProviderSnapshot) -> BrowserStackSidebarState {
        let liveIds = Set(snapshot.workspaceIds)
        var seen = Set<UUID>()
    ...
    public mutating func moveWorkspace(_ move: CotermSidebarProviderWorkspaceMove) {
