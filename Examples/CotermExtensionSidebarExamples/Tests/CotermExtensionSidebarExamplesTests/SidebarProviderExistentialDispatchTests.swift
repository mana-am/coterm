...
/// Regression coverage for https://github.com/emergent-inc/coterm/issues/5173.
///
/// The host renders sidebar views through an `any CotermSidebarProvider`
/// existential (`CotermExtensionSidebarSelection.provider(for:)?.render(snapshot:)`).
...
final class SidebarProviderExistentialDispatchTests: XCTestCase {
    ...
    func testSuperCompactRendersIdenticallyThroughExistential() {
        ...
        let existential: any CotermSidebarProvider = concrete
    ...
    private static func snapshot(workspaceCount: Int) -> CotermSidebarProviderSnapshot {
