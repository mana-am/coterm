...
@testable import CotermExtensionSidebarExamples
import XCTest
...
final class BrowserStackSidebarTests: XCTestCase {
    ...
    func testReconcilePreservesUserStateWhileApplyingSnapshotMembership() throws {
        ...
        let reconciled = try store.reconciledState(for: CotermSidebarProviderSnapshot(
            sequence: 2,
    ...
    func testWindowScopedStateFallsBackToMatchingRelaunchScope() throws {
        ...
        preferredState.moveWorkspace(CotermSidebarProviderWorkspaceMove(
            workspaceId: movedWorkspace.id,
            sourceSectionId: "loose",
            targetSectionId: "group:reading-list",
            targetIndex: 0
        ...
        let relaunchedSnapshot = CotermSidebarProviderSnapshot(
            sequence: 2,
    ...
    func testBrowserStackRenderModelPreservesEmptyRequiredSections() {
        let snapshot = CotermSidebarProviderSnapshot(sequence: 1, selectedWorkspaceId: nil, workspaces: [])
        let sections = [
    ...
    func testAsyncStateLoadNotifiesHostAndUpdatesRenderModel() throws {
        ...
        let snapshot = CotermSidebarProviderSnapshot(
            sequence: 1,
            selectedWorkspaceId: nil,
    ...
    private func temporaryStateURL() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-browser-stack-\(UUID().uuidString)", isDirectory: true)
    ...
    private func snapshot(titles: [String], windowId: UUID? = nil) -> CotermSidebarProviderSnapshot {
    ...
    private func workspace(title: String) -> CotermSidebarProviderWorkspace {
