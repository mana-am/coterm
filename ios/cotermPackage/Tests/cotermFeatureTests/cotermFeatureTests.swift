...
import UIKit
#endif
@testable import cotermFeature

/// Test collector that mounts a surface's ``CotermMobileShellStore`` output stream
...
final class TerminalOutputCollector {
    ...
    func mount(store: CotermMobileShellStore, surfaceID: String) {
...
@Test func mobileDevStackAuthTokenProviderUsesExplicitEnvironmentOnly() {
    ...
    #expect(MobileShellDevStackAuthTokenProvider.token(environment: [
        MobileShellDevStackAuthTokenProvider.environmentKey: " coterm-dev-token "
    ]) == "coterm-dev-token")
}
#endif

// Auth error mapping + cached-session recovery are now owned and tested by
// CotermAuthRuntime (AuthErrorMapperTests). The display-safe error and
// cached-session-validation assertions moved there with the AuthCoordinator
// lift; see Packages/Shared/CotermAuthRuntime/Tests.

@Test func mobileRuntimeDefaultsToThirtySecondRPCTimeout() {
    let runtime = CotermMobileRuntime(
...
@Test func mobileRuntimeMapsTimedOutStackTokenToRequestTimeout() {
    guard case .requestTimedOut = CotermMobileRuntime.connectionError(forStackAuthError: AuthError.timedOut) else {
...
@Test func activeMacReconnectRouteSkipsUnsupportedLoopbackRoute() throws {
    ...
    let route = CotermMobileShellStore.firstReconnectHostPortRoute(
        [loopback, tailscale],
...
@Test func signInMovesToPairingUntilCodeConnects() {
    ...
    #expect(store.connectedHostName == "coterm-macbook")
    #expect(store.macConnectionStatus == .connected)
}

@MainActor
@Test func pairingURLUsesCotermMobileCorePayloadWithoutConcreteTransport() async throws {
...
@Test func macConnectionStatusMarksUnavailableWhenEventStreamCloses() async throws {
    ...
    let store = CotermMobileShellStore.preview(runtime: runtime)
...
@Test func connectPreviewHostIgnoresPairingURLsForTrackedAsyncPath() async {
    ...
    store.signIn()
    store.pairingCode = "coterm-ios://attach?v=1&payload=invalid"
    store.connectPreviewHost()
    await Task.yield()
...
@Test func expiredPairingURLPayloadIsRejectedBeforePreviewConnection() async throws {
    ...
    """
    let url = try #require(URL(string: "coterm-ios://pair?v=1&payload=\(base64URLEncode(Data(json.utf8)))"))
    let store = CotermMobileShellStore.preview()
...
@Test func versionWarningDoesNotClearExistingConnectionBeforeApproval() async throws {
    ...
    let store = CotermMobileShellStore(
        runtime: runtime,
        ...
        feedbackStampProvider: {
            MobileFeedbackStamp(
                ...
                appVersion: "0.65.0",
                appBuild: "10",
                bundleIdentifier: "dev.coterm.ios.test",
                osVersion: "iOS test",
                deviceModel: "test"
            )
        }
    ...
    let warningResult = await store.connectPairingURLResult(
        "coterm-ios://attach?v=2&pc=2&av=0.65.0&ab=9&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)"
    )
...
@Test func qrPairingURLStillConnectsTenMinutesAfterMint() async throws {
    ...
    let url = "coterm-ios://attach?v=\(ticket.version)&payload=\(base64URLEncode(payload))"
    let responses = ScriptedTransportResponses([
...
@Test func minimalPairingCodeConnectsAndAdoptsHostReportedIdentity() async throws {
    ...
    store.signIn()
    await store.connectPairingURL("coterm-ios://attach?v=2&pc=1&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)")
...
@Test func minimalPairingCodeRequiresMatchingEmailBeforeDialing() async throws {
    ...
    let result = await store.connectPairingURLResult(
        "coterm-ios://attach?v=2&ub=mac-user&pc=1&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)"
    )
...
@Test func minimalPairingCodeCompatibilityMismatchWarnsAndContinuesAfterAcceptance() async throws {
    ...
    store.signIn()
    let result = await store.connectPairingURLResult(
        "coterm-ios://attach?v=2&ub=phone-user&pc=2&av=0.65.0&ab=9&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)"
    )
...
@Test func minimalPairingCodeMissingCompatibilityWarnsBeforeDialing() async throws {
    ...
    store.signIn()
    let result = await store.connectPairingURLResult(
        "coterm-ios://attach?v=2&ub=phone-user&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)"
    )
...
@Test func minimalPairingCodeAppVersionMismatchDoesNotWarnWhenCompatibilityMatches() async throws {
    ...
    store.signIn()
    let result = await store.connectPairingURLResult(
        "coterm-ios://attach?v=2&ub=phone-user&pc=1&av=0.65.0&ab=95&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)"
    )
...
@Test func scannedLoopbackPairingCodeIsRejectedWithGuidance() async throws {
    ...
    store.signIn()
    let result = await store.connectPairingURLResult("coterm-ios://attach?v=2&r=127.0.0.1:\(CmxMobileDefaults.defaultHostPort)")
...
private func testRuntime(
    ...
    rpcRequestTimeoutNanoseconds: UInt64 = CotermMobileRuntime.defaultRPCRequestTimeoutNanoseconds,
    pairingRequestTimeoutNanoseconds: UInt64 = CotermMobileRuntime.defaultPairingRequestTimeoutNanoseconds,
...
private func attachURL(for ticket: CmxAttachTicket) throws -> URL {
    ...
    return try #require(URL(string: "coterm-ios://attach?v=\(ticket.version)&payload=\(payload)"))
}
...
private func waitForWorkspaceIDs(
    in store: CotermMobileShellStore,
...
@MainActor private func deeplinkTestStore() -> CotermMobileShellStore {
