import CotermMobileShellUI
import CotermMobileTransport

/// Adapts the transport layer's ``CotermMobileTransport/TailscaleStatusMonitor``
/// onto the shell UI's read-only ``CotermMobileShellUI/TailscaleStatusObserving``
/// port, so `CotermMobileShellUI` stays decoupled from the concrete system
