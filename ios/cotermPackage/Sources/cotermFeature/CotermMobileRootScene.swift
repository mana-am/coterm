...
private let mobileRootSceneLog = Logger(subsystem: "dev.coterm.ios", category: "mobile-root-scene")

/// Top-level mobile scene root.
///
/// Renders the live coterm mobile UI: a ``CotermMobileAppView`` backed by a fresh
/// ``CotermMobileShellStore`` and the injected ``AuthCoordinator``. In DEBUG
/// builds, setting the environment variable `COTERM_ZOOM_STRESS=1` instead mounts
/// the terminal zoom-stress repro harness (`MobileZoomStressView`).
///
/// The composition root (`cotermApp`) builds the ``CotermMobileRuntime`` and the
...
public struct CotermMobileRootScene: View {
    private let runtime: CotermMobileRuntime
    ...
    private var content: some View {
        ...
        } else if ProcessInfo.processInfo.environment["COTERM_ZOOM_STRESS"] == "1" {
            MobileZoomStressView()
        } else {
            CotermMobileAppView(store: makeStore(), onboardingStore: onboardingStore)
        }
        #else
        CotermMobileAppView(store: makeStore(), onboardingStore: onboardingStore)
        #endif
        #else
        CotermMobileAppView(store: makeStore())
        #endif
    }

    @MainActor
    private func makeStore() -> CotermMobileShellStore {
        let coordinator = auth.coordinator
