...
/// ``CotermAuthRuntime/AuthConfig`` from the environment + an injected
/// `LocalConfig.plist` override table, builds the ``CotermAuthRuntime/AuthCoordinator``
/// (with a `StackAuthClient`, persistence caches over an injected `UserDefaults`,
/// and an ``CotermAuthRuntime/AuthPresentationContextProvider``), and the
/// ``CotermAuthRuntime/PushRegistrationService``. Replaces `AuthManager.shared`,
...
public struct MobileAuthComposition {
    ...
    public init(
        ...
        let teamSelection = CotermAuthTeamSelectionStore(
            keyValueStore: defaults,
            key: "auth_selected_team"
        )
        let launch = AuthLaunchOptions(
            clearAuthRequested: environment["COTERM_UITEST_CLEAR_AUTH"] == "1",
