...
/// injected here by `cotermApp`.
final class CotermAppDelegate: NSObject, @preconcurrency UIApplicationDelegate, UNUserNotificationCenterDelegate {
    /// The app-root push coordinator, injected by `cotermApp` at launch.
    @MainActor var pushCoordinator: MobilePushCoordinator?
    /// The app-root analytics emitter, injected by `cotermApp` at launch.
    ...
    func application(
        ...
        // `analytics` is assigned in `cotermApp.init()` which runs before
    ...
    func application(
        ...
        NSLog("coterm.push registration failed: %@", error.localizedDescription)
    ...
    func userNotificationCenter(
        ...
        let ids = Self.cotermIDs(from: notification.request.content.userInfo)
    ...
    func userNotificationCenter(
        ...
        // A swipe/clear of a coterm banner delivers the custom dismiss action
        // (enabled via the `coterm.terminal` category's `.customDismissAction`).
        // Forward it to the Mac so the desktop banner + store entry clear too.
        let ids = Self.cotermIDs(from: request.content.userInfo)
    ...
    private nonisolated static func dismissedIDs(from userInfo: [AnyHashable: Any]) -> [String] {
        guard let coterm = userInfo["coterm"] as? [String: Any],
              let ids = coterm["dismissedIds"] as? [String] else {
            return []
        }
        return ids
    ...
    private nonisolated static func cotermIDs(
        from userInfo: [AnyHashable: Any]
    ) -> (workspaceId: String?, surfaceId: String?, macDeviceId: String?) {
        guard let coterm = userInfo["coterm"] as? [String: Any] else { return (nil, nil, nil) }
        return (
            coterm["workspaceId"] as? String,
            coterm["surfaceId"] as? String,
            coterm["macDeviceId"] as? String
        )
    }
    ...
    ///
    /// The `coterm.notificationId` payload key is authoritative: the Mac stamps the
    ...
    private nonisolated static func notificationID(from request: UNNotificationRequest) -> String? {
        guard let coterm = request.content.userInfo["coterm"] as? [String: Any],
              let id = (coterm["notificationId"] as? String)?.trimmingCharacters(in: .whitespaces),
              !id.isEmpty else {
            return nil
        }
        return id
