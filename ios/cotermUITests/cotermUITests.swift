...
final class cotermUITests: XCTestCase {
    ...
    func testAddDeviceManualHostValidationUsesStableIdentifiers() throws {
        let invalidHostApp = launchAddDeviceApp(environment: [
            "COTERM_UITEST_ADD_DEVICE_HOST": "dev/path.local"
        ])
        ...
        XCTAssertTrue(invalidHostApp.staticTexts["MobileAddDeviceSignedInAccount"].label.contains("uitest@coterm.local"))
        ...
        let invalidPortApp = launchAddDeviceApp(environment: [
            "COTERM_UITEST_ADD_DEVICE_HOST": "127.0.0.1",
            "COTERM_UITEST_ADD_DEVICE_PORT": "70000",
        ])
    ...
    func testManualHostConnectsAndNavigatesToWorkspace() async throws {
        ...
        assertTerminalRow(0, label: "$ coterm ios status", in: app)
    ...
    func testDeleteComputersVerifierPasses() throws {
        let app = launchApp(mockData: false, environment: [
            "COTERM_DELETE_COMPUTERS_VERIFIER": "1",
        ])
        defer { app.terminate() }
    ...
    func testWorkspaceMacPickerUsesComputerCopy() throws {
        let app = launchApp(mockData: false, environment: [
            "COTERM_UITEST_WORKSPACE_LIST_PREVIEW": "1",
        ])
        defer { app.terminate() }
    ...
    func testTerminalPreviewRenderBottomTracksSyntheticKeyboardViewport() throws {
        let app = launchApp(mockData: false, environment: [
            "COTERM_UITEST_TERMINAL_PREVIEW": "1",
            "COTERM_UITEST_FAKE_KEYBOARD_HEIGHT": "320",
        ])
    ...
    func testInlineWorkspaceTitleKeepsCompactHeightWithTallGlyphs() throws {
        let app = launchAgentChatInlinePreviewApp(environment: [
            "COTERM_UITEST_INLINE_WORKSPACE_TITLE": "✳️ Claude Code",
            "COTERM_UITEST_INLINE_WORKSPACE_SUBTITLE": "🧑🏽‍💻 Claude Code",
        ])
        let titleMenu = app.buttons["MobileWorkspaceTitleMenu"]
        let backButton = app.buttons["MobileWorkspaceBackButton"]
        let surfacePicker = app.buttons["AgentChatInlinePreviewTerminalPicker"]
    ...
    func testAgentChatMiddleKeyboardVideoEvidence() throws {
        let app = launchAgentChatInlinePreviewApp(environment: [
            "COTERM_UITEST_CHAT_AUTOFOCUS_DELAY": "14.0",
            "COTERM_UITEST_CHAT_AUTO_DISMISS_DELAY": "1.25",
        ])
        let table = app.tables["ChatTranscriptTableView"]
        XCTAssertTrue(table.waitForExistence(timeout: 8))
        let composerBar = app.otherElements["ChatComposerBar"]
    ...
    func testAgentChatMiddleKeyboardInterruptedShowDismissVideoEvidence() throws {
        let app = launchAgentChatInlinePreviewApp(environment: [
            "COTERM_UITEST_CHAT_AUTOFOCUS_DELAY": "14.0",
            "COTERM_UITEST_CHAT_AUTO_DISMISS_DELAY": "0.18",
        ])
        let table = app.tables["ChatTranscriptTableView"]
        XCTAssertTrue(table.waitForExistence(timeout: 8))
        let composerBar = app.otherElements["ChatComposerBar"]
    ...
    func testAgentChatMiddleKeyboardInterruptedRefocusVideoEvidence() throws {
        let app = launchAgentChatInlinePreviewApp(environment: [
            "COTERM_UITEST_CHAT_AUTOFOCUS_DELAY": "14.0",
            "COTERM_UITEST_CHAT_AUTO_DISMISS_DELAY": "1.05",
            "COTERM_UITEST_CHAT_AUTO_REFOCUS_AFTER_DISMISS_DELAY": "0.18",
        ])
        let table = app.tables["ChatTranscriptTableView"]
        XCTAssertTrue(table.waitForExistence(timeout: 8))
        let composerBar = app.otherElements["ChatComposerBar"]
    ...
    func testAgentChatMiddleKeyboardToggleVideoEvidence() throws {
        for refocusCase in [
            ...
            let app = launchAgentChatInlinePreviewApp(environment: [
                "COTERM_UITEST_CHAT_AUTOFOCUS_DELAY": "14.0",
                "COTERM_UITEST_CHAT_AUTO_DISMISS_DELAY": "1.05",
                "COTERM_UITEST_CHAT_AUTO_REFOCUS_AFTER_DISMISS_DELAY": String(refocusCase.delay),
            ])
            let table = app.tables["ChatTranscriptTableView"]
            XCTAssertTrue(table.waitForExistence(timeout: 8))
            let composerBar = app.otherElements["ChatComposerBar"]
    ...
    func testAgentChatTranscriptFastSwipeEvidence() throws {
        let app = launchAgentChatInlinePreviewApp(environment: [
            "COTERM_UITEST_CHAT_INITIAL_SCROLL": "middle",
        ])
        let table = app.tables["ChatTranscriptTableView"]
        XCTAssertTrue(table.waitForExistence(timeout: 8))
    ...
    private func launchConnectedApp(port: UInt16, assertStatusRows: Bool = true) throws -> XCUIApplication {
        ...
        let app = launchApp(mockData: true, environment: [
            "COTERM_UITEST_ATTACH_URL": attachURL.absoluteString,
        ])
    ...
    private func attachURL(port: UInt16) throws -> URL {
        ...
        guard let url = URL(string: "coterm-ios://attach?v=\(ticket.version)&payload=\(payload)") else {
    ...
    private func launchAgentChatPreviewApp() -> XCUIApplication {
        let app = launchApp(mockData: false, environment: [
            "COTERM_UITEST_AGENT_CHAT_PREVIEW": "1",
        ])
    ...
    private func launchAgentChatInlinePreviewApp(environment: [String: String] = [:]) -> XCUIApplication {
        var launchEnvironment = [
            "COTERM_UITEST_AGENT_CHAT_INLINE_PREVIEW": "1",
        ]
    ...
    private func launchApp(
        ...
        app.launchEnvironment["COTERM_UITEST_MOCK_DATA"] = mockData ? "1" : "0"
        for (key, value) in environment {
            app.launchEnvironment[key] = value
        }
        if clearAuth {
            app.launchEnvironment["COTERM_UITEST_CLEAR_AUTH"] = "1"
        }
        app.launch()
        return app
    }
...
private final class MobileSyncMockHostServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "dev.coterm.ios-ui-tests.mobile-sync-server")
        ...
        Workspace(
            ...
            terminals: [
                Terminal(
                    ...
                    currentDirectory: "~/coterm/docs",
                    lines: [
                        "$ rg CotermMobileCore docs",
                        "docs/ios-swift-mobile-plan.md:iOS shell depends on CotermMobileCore.",
                    ]
                ),
            ]
        ),
