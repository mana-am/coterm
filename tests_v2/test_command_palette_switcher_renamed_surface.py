...
def _rename_surface(client: coterm, surface_id: str, title: str) -> None:
    client._call(
        "surface.action",
        {
...
def _current_surface_id(client: coterm, workspace_id: str) -> str:
...
def main() -> int:
    with coterm(SOCKET_PATH) as client:
        ...
        if len(rows) < 2:
            raise cotermError(f"expected at least two surfaces after split: {payload}")
        ...
        if not left_surface_id:
            raise cotermError(f"failed to resolve left surface id: {payload}")
        ...
        if not result_rows:
            raise cotermError("switcher returned no rows for renamed surface query")
