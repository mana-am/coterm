...
def _assert_shortcut_window_scoped(client: coterm, shortcut: str, w1: str, w2: str) -> None:
    ...
    if _palette_visible(client, w2):
        raise cotermError(f"{shortcut} in window1 incorrectly opened palette in window2")
...
def _assert_cross_window_typing_after_mixed_shortcuts(client: coterm, w1: str, w2: str) -> None:
    _set_palette_visible(client, w1, False)
    _set_palette_visible(client, w2, False)
...
def main() -> int:
    with coterm(SOCKET_PATH) as client:
        ...
        if _palette_visible(client, w2):
            raise cotermError("window2 palette became visible when toggling window1")
        ...
        if _palette_visible(client, w1):
            raise cotermError("window1 palette became visible when toggling window2")
        client._call("debug.command_palette.toggle", {"window_id": w2})
        _wait_until(
            lambda: not _palette_visible(client, w2),
            timeout_s=3.0,
