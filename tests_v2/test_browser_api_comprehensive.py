...
def _expect_error(label: str, fn, code_substr: str) -> None:
    try:
        ...
        if code_substr in text:
            return
        raise cotermError(f"{label}: expected error containing {code_substr!r}, got: {text}")
    raise cotermError(f"{label}: expected error containing {code_substr!r}, but call succeeded")
...
def _expect_error_contains(label: str, fn, *needles: str) -> None:
    try:
        ...
        if missing:
            raise cotermError(f"{label}: missing expected substrings {missing!r} in error: {text}")
        return
    raise cotermError(f"{label}: expected failure, but call succeeded")
...
def _wait_with_fallback(c: coterm, surface_id: str, params: dict, pred, timeout_s: float, label: str) -> None:
...
<html>
  <head>
    <title>coterm-browser-comprehensive-1</title>
    <style>
...
<!doctype html>
<html>
  <head><title>coterm-browser-comprehensive-2</title></head>
  <body>
...
def main() -> int:
    ...
    with coterm(SOCKET_PATH) as c:
        ...
        _wait_until(
            lambda: "coterm-browser-comprehensive-1"
        ...
        _must(str(_value(out_text)) == "coterm", f"Expected status text to be coterm: {out_text}")
        ...
        _must(str(_value(name_val)) == "coterm-v2", f"Expected typed suffix in input value: {name_val}")
        ...
        _must("coterm-browser-comprehensive-1" in snapshot_text, f"Expected snapshot text for page1: {snap}")
        ...
        _wait_until(
            lambda: "coterm-browser-comprehensive-2"
            in str((c._call("browser.get.title", {"surface_id": target}) or {}).get("title") or ""),
            timeout_s=3.0,
        ...
        _wait_with_fallback(
            c,
            target,
            {"url_contains": "coterm-browser-comprehensive-1", "timeout_ms": 4000},
            lambda: "coterm-browser-comprehensive-1" in str((c._call("browser.url.get", {"surface_id": target}) or {}).get("url") or ""),
            timeout_s=5.0,
        ...
        _wait_with_fallback(
            c,
            target,
            {"url_contains": "coterm-browser-comprehensive-2", "timeout_ms": 4000},
            lambda: "coterm-browser-comprehensive-2" in str((c._call("browser.url.get", {"surface_id": target}) or {}).get("url") or ""),
            timeout_s=5.0,
