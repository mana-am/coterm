...
def _wait_for_terminal_focus(c: coterm, panel_id: str, timeout_s: float = 8.0) -> None:
    start = time.time()
...
def _wait_for_text(c: coterm, panel_id: str, needle: str, timeout_s: float = 2.5) -> None:
    start = time.time()
    last = ""
    while time.time() - start < timeout_s:
    ...
    raise cotermError(f"Timed out waiting for token in terminal text: {needle}\nLast tail:\n{tail}")


def _type_and_wait_visible(c: coterm, panel_id: str, cmd: str) -> bool:
...
def _wait_for_tmp_write(c: coterm, panel_id: str, tmp: str, token: str) -> None:
    ...
    raise cotermError(f"Timed out waiting for tmp file write: {tmp}")
...
def main() -> int:
    with coterm(SOCKET_PATH) as c:
        ...
        for i in range(6):
            ...
            token = f"COTERM_NEW_TAB_OK_{i}_{uuid.uuid4().hex[:10]}"
            tmp = f"/tmp/coterm_new_tab_{token}.txt"
