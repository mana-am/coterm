...
def _wait_for_text(c: coterm, workspace_id: str, needle: str, timeout_s: float = 8.0) -> str:
    ...
    while time.time() < deadline:
        ...
        if needle in last_text:
            return last_text
        time.sleep(0.1)
    raise cotermError(f"Timed out waiting for {needle!r} in panel text: {last_text!r}")
...
def test_horizontal_split(c: coterm) -> None:
...
def test_nested_splits(c: coterm) -> None:
...
def test_env_vars(c: coterm) -> None:
        ...
        ws = _create_and_get_id(c, {
            ...
            "layout": {
                "pane": {
                    "surfaces": [{
                        "type": "terminal",
                        "env": {"COTERM_LAYOUT_TEST_TOKEN": token},
                    }],
                },
            },
        })
...
def test_surface_commands(c: coterm) -> None:
...
def test_multi_surface_pane(c: coterm) -> None:
...
def test_malformed_layout_rejected(c: coterm) -> None:
    ...
    try:
        ...
        raise cotermError("Expected error for malformed layout, but call succeeded")
...
def test_empty_surfaces_rejected(c: coterm) -> None:
    ...
    try:
        ...
        raise cotermError("Expected error for empty surfaces, but call succeeded")
    except cotermError as e:
        if "invalid_params" in str(e) or "Invalid layout" in str(e):
            pass  # expected
        else:
...
def test_layout_overrides_initial_command_and_env(c: coterm) -> None:
    ...
    try:
        ...
        c.send_surface(
            surface_id,
            "printf 'LAYOUT_ENV=%s INITIAL_ENV=%s\\n' \"$COTERM_LAYOUT_TEST_TOKEN\" \"$SHOULD_NOT_EXIST\"\\n",
        )
