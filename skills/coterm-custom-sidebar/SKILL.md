---
name: coterm-custom-sidebar
description: "Build a custom coterm sidebar from a plain-language request. Use when the user asks for a custom sidebar, a sidebar that shows their workspaces/tabs/PRs/clock, a vibe-coded sidebar, or anything involving files in ~/.config/coterm/sidebars/. Covers authoring the interpreted SwiftUI-style file, enabling the beta flag, selecting it, and iterating with hot reload."
---

# coterm Custom Sidebar

coterm renders custom sidebars from a small SwiftUI-style file at runtime: no Xcode, no build step, no signing. The file hot-reloads on save, binds to live coterm state (workspaces, tabs, git, PRs, clock), and can run real coterm commands on tap.
...
```bash
coterm docs sidebars
curl -fsSL https://raw.githubusercontent.com/emergent-inc/coterm/main/docs/custom-sidebars.md
```
...
2. **Write a named file.** The name becomes the menu label; use short kebab-case:
   ```
   ~/.config/coterm/sidebars/<name>.swift
   ```
...
3. **Validate and select it:**
   ```bash
   coterm sidebar validate <name>   # parse/interpret check with real data shapes
   coterm sidebar select <name>     # switch the sidebar to it
   ```
   The user can also pick it manually: right-click the sidebar toggle button.
4. **Iterate.** Saving the file hot-reloads the sidebar in place (`coterm sidebar reload` forces it). Look at the result, fix what looks off, and verify rows show real data and taps do the right thing before declaring it done.

## Authoring rules

- **Default to live data.** Bind to the `workspaces` context instead of hard-coding text so the sidebar stays correct on its own.
- **Make it interactive by default.** Rows that represent something openable should run the matching `coterm(...)` action on tap. A list that just displays text is rarely what they wanted.
...
SWIFT
coterm sidebar validate mine && coterm sidebar select mine
```
...
A button or `.onTapGesture` body calls `coterm("<method>", param: value)`, dispatched through the same surface as the `coterm` CLI. Common methods: `workspace.select` (`workspace_id`), `surface.focus` (`surface_id`), `workspace.reorder` (`workspace_id` + `index`). `openURL("https://...")` opens links. Discover the full command surface with `coterm docs api`.
...
Not yet supported (write the natural Swift anyway; it degrades gracefully): `@State` and input controls (`TextField`, `Toggle`, `Slider`, `Picker`), custom `struct`/`View` definitions, navigation (`sheet`/`popover`), `AsyncImage`. Two-way editing does not work yet; taps that run `coterm(...)` do.

## Troubleshooting

- Sidebar missing from the right-click picker: the beta flag is off, or the file is not directly under `~/.config/coterm/sidebars/`.
- Blank or partial render: run `coterm sidebar validate <name>`; errors show inline in the sidebar with the failing location. A broken save keeps the last working render on screen, so re-save after fixing.
- Rows not tappable: wrap the row in `Button(action: { coterm(...) }) { ... }` or add `.onTapGesture { coterm(...) }`.
