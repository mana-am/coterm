...
# Absolute path
coterm markdown open /path/to/PLAN.md

# Target a specific workspace
coterm markdown open design.md --workspace workspace:2
```
...
When creating a plan or task list, write it to a `.md` file and open it in coterm:
...
```bash
# Open in the caller's workspace (default -- uses COTERM_WORKSPACE_ID)
coterm markdown open plan.md

# Open in a specific workspace
coterm markdown open plan.md --workspace workspace:2

# Open splitting from a specific surface
coterm markdown open plan.md --surface surface:5

# Open in a specific window
coterm markdown open plan.md --window window:1
```
