...
- tagged Debug app: `/tmp/coterm-debug-<tag>.log`

`reload.sh` writes the current path to `/tmp/coterm-last-debug-log-path`, so the most robust tail command is:
...
The package implementation lives in `Packages/macOS/CotermDebugLog/Sources/CotermDebugLog/DebugEventLog.swift`, and the app shim lives in `Sources/App/DebugLogging.swift`.

Call sites use:

```swift
#if DEBUG
cotermDebugLog("focus.panel ...")
#endif
```
...
The debug logger has a 500-entry ring buffer. `CotermDebugLog.DebugEventLog.shared.dump()` writes the full buffer to file. Use this when the interesting event occurred before you started tailing.
