...
target `coterm.json`. Preserve unrelated sections, then run `coterm reload-config`
when available.

Prefer project-local `.coterm/coterm.json` for team workflows and global
`~/.config/coterm/coterm.json` for personal app preferences.
...
```json
{
  "controls": [
    ...
    { "id": "feed", "title": "Feed", "command": "coterm feed tui --opentui", "height": 260 }
  ]
}
```
...
Use this when the user's normal environment is remote, or when local coterm should
                    ...
                    "surfaces": [
                      {
                        "type": "terminal",
                        "name": "Markdown",
                        "command": "coterm markdown open docs/README.md --direction right --focus false; exec ${SHELL:-/bin/zsh} -l"
                      }
                    ]
                  }
                }
