...
- `CMUX_SIDEBAR_EXTENSION_POINT_ID` (default `mosaic.com.emergent.app.cmux.sidebar`): the extension point identifier baked into Info.plist at build time.
...
- add `CMUX_SIDEBAR_EXTENSION_POINT_ID` (default `mosaic.com.emergent.app.cmux.sidebar`), `CMUX_BUNDLE_ID_SUFFIX` (default empty), and `CMUX_DISPLAY_NAME_SUFFIX` (default empty) build settings to the app and appex targets in all build configs.
