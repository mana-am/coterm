...
This reference expands coterm package extraction and refactor architecture rules.
...
- `CotermAppearanceMath`
- `CotermWorkspaceModel`
- `CotermFooFormatting`
- `CotermFooLogic`
- `CotermFooState`
...
5. Executable: `cotermApp` and `AppDelegate` as the composition root.
...
`@main` `cotermApp` and `AppDelegate` stay in the executable target. Do not move extensions of executable-owned types down into lower packages. A lower package cannot extend a higher-owned type without creating the wrong dependency direction.
...
`coterm.xcodeproj` lists package dependencies explicitly. Adding `Packages/CotermFoo` means mirroring existing package entries:
...
App-target packages link into both `coterm` and `coterm-unit`, so tests can import and inject them. A package linked by the app but not `coterm-unit` can make the app build pass while the test target fails.
