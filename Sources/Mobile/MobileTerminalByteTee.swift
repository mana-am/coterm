...
private let mobileTerminalByteTeeLog = Logger(
    subsystem: "dev.coterm",
    category: "mobile-terminal-byte-tee"
)

/// Captures raw PTY-output bytes from every coterm terminal surface and
...
/// callback (coterm fork addition). The callback fires on the IO read
...
final class MobileTerminalByteTee {
    ...
    private let publishQueue = DispatchQueue(
        label: "dev.coterm.mobile.byte-tee.publish",
        qos: .userInitiated
    )
...
/// pointer.
public let cotermMobileTerminalByteTeeCallback: @convention(c) (
