...
struct mosaicApp: App {
    ...
    init() {
        ...
        let configFileURL = MosaicConfigLocation().userConfigFile
        ...
        // apps" prompt; the password now lives in the non-protected mosaic state
        // directory (https://github.com/emergent-inc/mosaic/issues/5146). The app
        ...
        // Secrets live in their own 0600 files under the mosaic state directory,
        ...
        // Lift any plaintext socket-control password out of `mosaic.json` into the
        ...
        // (`MosaicSettingsFileStore`, loaded later during app launch) reads the
        ...
        // If invoked with CLI-style arguments (e.g. `mosaic hooks setup`), exec the
        // bundled CLI at Contents/Resources/bin/mosaic. The GUI binary and the CLI
        // share the name `mosaic`, so if the GUI's Contents/MacOS leaks onto $PATH
        // (which happens for any shell descended from this process), bare `mosaic`
        // resolves here instead of the CLI. See
        // https://github.com/emergent-inc/mosaic/issues/4678.
        // mosaic ships a universal binary so it still supports Intel Macs, but a
        ...
        // launches. See https://github.com/emergent-inc/mosaic/issues/753.
        ...
        if let stored = defaults.string(forKey: SocketControlSettings.appStorageKey) {
            ...
            defaults.set(legacy ? SocketControlMode.mosaicOnly.rawValue : SocketControlMode.off.rawValue,
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged Mosaic DEV; start with ./scripts/reload.sh --tag <name> (or set MOSAIC_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
    ...
    static func resolvedGhosttyResourcesDirectory(
        ...
        // Tagged mosaic builds may inherit GHOSTTY_RESOURCES_DIR from another running
        // mosaic instance. Prefer this app's bundled resources when they are present.
    ...
    var body: some Scene {
            ...
            MainWindowBootstrapView()
                ...
                .onAppear {
                    ...
                    if ProcessInfo.processInfo.environment["MOSAIC_UI_TEST_MODE"] == "1" {
                        AppDelegate.shared?.updateLog.append("ui test: mosaicApp onAppear")
                    }
#endif
        ...
        .commands {
            CommandGroup(replacing: .appSettings) {
                ...
                Button(String(localized: "menu.app.openMosaicSettingsFile", defaultValue: "Open mosaic.json")) {
                ...
                Button(String(localized: "menu.app.makeDefaultTerminal", defaultValue: "Make mosaic the Default Terminal")) {
                    DefaultTerminalUserAction.setAsDefault(debugSource: "menu.makeDefaultTerminal")
                }
            }

            CommandGroup(replacing: .appInfo) {
                Button(String(localized: "menu.app.about", defaultValue: "About mosaic")) {
                    showAboutPanel()
                }
            ...
            CommandGroup(replacing: .appTermination) {
                splitCommandButton(title: String(localized: "menu.quitMosaic", defaultValue: "Quit mosaic"), shortcut: menuShortcut(for: .quit)) {
                    NSApp.terminate(nil)
                }
            }
...
#if DEBUG
                        mosaicDebugLog("find.menu Cmd+F fired")
#endif
    ...
    private func closePanelOrWindow() {
        ...
        if let window, mosaicWindowShouldOwnCloseShortcut(window) { window.performClose(nil); return }
    ...
    var body: some View {
        Color.clear
            ...
            .background(WindowAccessor { window in
                window.identifier = NSUserInterfaceItemIdentifier("mosaic.bootstrap")
...
private let mosaicAuxiliaryWindowIdentifiers: Set<String> = [
    "mosaic.settings",
    "mosaic.about",
    "mosaic.licenses",
    "mosaic.browser-popup",
    "mosaic.browserProfilePopoverDebug",
    "mosaic.configEditor",
    "mosaic.defaultTerminalRegistrationError",
    "mosaic.feedButtonStyleDebug",
    "mosaic.feedPreview",
    "mosaic.feedTextEditorDebug",
    "mosaic.fileExplorerStyleDebug",
    "mosaic.folderDragIcon",
    "mosaic.pdfPreviewChromeDebug",
    "mosaic.recentlyClosedHistory",
    "mosaic.splitButtonLayoutDebug",
    "mosaic.tabBarBackdropLab",
    "mosaic.taskManager",
    "mosaic.aboutTitlebarDebug",
    "mosaic.debugWindowControls",
    "mosaic.browserImportHintDebug",
    "mosaic.extensionSidebarInspector",
    "mosaic.sidebarDebug",
    "mosaic.menubarDebug",
    "mosaic.backgroundDebug",
    "mosaic.startupAppearanceDebug",
    "mosaic.bonsplitTabBarDebug",
    "mosaic.titlebarLayoutDebug",
    "mosaic.devWindowDisplay",
    "mosaic.mobilePairingWindow",
]

/// Returns whether the given window should handle the standard close shortcut
/// as a standalone auxiliary window instead of routing it through workspace or
/// panel-close behavior.
func mosaicWindowShouldOwnCloseShortcut(_ window: NSWindow?) -> Bool {
    guard let identifier = window?.identifier?.rawValue else { return false }
    return mosaicAuxiliaryWindowIdentifiers.contains(identifier)
}
...
private final class DebugWindowControlsWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.debugWindowControls")
        window.center()
...
private final class BrowserImportHintDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.browserImportHintDebug")
        window.center()
...
private final class BrowserProfilePopoverDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.browserProfilePopoverDebug")
        window.center()
...
private final class AboutWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.about")
        window.center()
...
private final class AcknowledgmentsWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.licenses")
        window.center()
...
private struct AcknowledgmentsView: View {
    ...
    var body: some View {
        ScrollView {
            Text(content)
                .mosaicFont(.body, design: .monospaced)
...
private final class FileExplorerStyleDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.fileExplorerStyleDebug")
        window.center()
...
private final class SidebarDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.sidebarDebug")
        window.center()
...
private struct AboutPanelView: View {
    ...
    private var commit: String? {
        if let value = Bundle.main.infoDictionary?["MosaicCommit"] as? String, !value.isEmpty {
            return value
        }
        let env = ProcessInfo.processInfo.environment["MOSAIC_COMMIT"] ?? ""
            ...
            VStack(alignment: .center, spacing: 32) {
                ...
                VStack(spacing: 2) {
                    ...
                    let commitURL = commit.flatMap { hash in
                        URL(string: "https://github.com/emergent-inc/mosaic/commit/\(hash)")
                    }
...
private final class MenuBarExtraDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.menubarDebug")
        window.center()
...
private final class SplitButtonLayoutDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.splitButtonLayoutDebug")
        window.center()
...
private final class TabBarBackdropLabWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.tabBarBackdropLab")
        window.center()
    ...
    private func labSlider(
        ...
        VStack(alignment: .leading, spacing: 4) {
            Text("\(title) \(displayValue)")
                .mosaicFont(.caption, monospacedDigit: true)
                .lineLimit(1)
            Slider(value: value, in: range)
                .frame(width: width)
        }
...
private struct TabBarBackdropLabSample: View {
    ...
    private static func makeController(for variant: TabBarBackdropLabVariant) -> BonsplitController {
        ...
        let titles = [
            ...
            String(localized: "debug.tabBarBackdropLab.tab.terminalTransparency", defaultValue: "mosaic terminal transparency"),
...
private struct TabBarBackdropLabTitlebar: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(title)
                .mosaicFont(.caption2, weight: .medium)
                .lineLimit(1)
            Spacer(minLength: 0)
        }
...
private struct TabBarBackdropLabSidebar: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(title)
                .mosaicFont(.caption2, weight: .bold)
...
private struct TabBarBackdropLabTerminalPane: View {
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("\(String(localized: "debug.tabBarBackdropLab.terminal.prompt", defaultValue: "lawrence in ~/mosaic")) \(title)")
...
private final class BackgroundDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.backgroundDebug")
        window.center()
...
private final class StartupAppearanceDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("mosaic.startupAppearanceDebug")
        window.center()
...
enum AppIconSettings {
    ...
    private static let dockTileIconDidChangeNotification = Notification.Name("mosaic.com.emergent.appIconDidChange")
...
nonisolated enum BuildFlavor: String, Sendable {
    ...
    static func detect(bundleNames: [String], bundleIdentifier: String?) -> BuildFlavor {
        ...
        if normalizedBundleIdentifier == "mosaic.com.emergent.app.nightly"
            || normalizedBundleIdentifier?.hasPrefix("mosaic.com.emergent.app.nightly.") == true {
            return .nightly
        }
...
enum TelemetrySettings {
    ...
    // live in `MosaicSettings` (`AppCatalogSection().sendAnonymousTelemetry`) as the
...
@MainActor
func openMosaicSettingsFileInEditor() {
