...
@MainActor
struct CotermConfigExecutor {

    @discardableResult
    static func execute(
        command: CotermCommandDefinition,
        ...
        if let workspace = command.workspace {
            return authorizeProjectActionIfNeeded(
                ...
                PostHogAnalytics.shared.trackAction(
                    actionID: actionID ?? command.id,
                    surface: "coterm_config",
                    entrypoint: "workspace_command",
                    source: "CotermConfigExecutor.execute"
                )
                onExecuted?()
            }
    ...
    static func prepareShellInputIfAuthorized(
        ...
        confirm: Bool,
        actionID: String,
        target: CotermConfigTerminalCommandTarget,
        configSourcePath: String?,
        globalConfigPath: String,
        displayTitle: String? = nil,
        icon: CotermButtonIcon? = nil,
    ...
    static func authorizeProjectAutomationIfNeeded(
        descriptor: CotermActionTrustDescriptor,
        confirm: Bool,
        configSourcePath: String?,
        globalConfigPath: String,
    ...
    private static func authorizeProjectActionIfNeeded(
        ...
        let isTrusted = CotermActionTrust.shared.isTrusted(descriptor)
    ...
    private static func makeConfirmDialog(
        ...
        alert.messageText = (trimmedDisplayTitle?.isEmpty == false)
            ? trimmedDisplayTitle!
            : String(
                localized: "dialog.cotermConfig.confirmCommand.title",
                defaultValue: "Run Project Action?"
            )
        let messageFormat = String(
            localized: "dialog.cotermConfig.confirmCommand.messageWithCommand",
        ...
        alert.addButton(withTitle: String(
            localized: "dialog.cotermConfig.confirmCommand.run",
            defaultValue: "Run Once"
        ))
        alert.addButton(withTitle: String(
            localized: "dialog.cotermConfig.confirmCommand.trustAndRun",
            defaultValue: "Trust and Run"
        ))
        alert.addButton(withTitle: String(
            localized: "dialog.cotermConfig.confirmCommand.cancel",
            defaultValue: "Cancel"
        ))
        return alert
    }
    ...
    private static func handleConfirmDialogResponse(
        ...
        case .alertSecondButtonReturn:
            CotermActionTrust.shared.trust(descriptor)
            return true
        default:
            return false
        }
    ...
    private static func terminalTrustDescriptor(
        ...
        target: CotermConfigTerminalCommandTarget,
        configSourcePath: String?,
        icon: CotermButtonIcon?,
        iconSourcePath: String?,
        globalConfigPath: String
    ) -> CotermActionTrustDescriptor {
        CotermActionTrustDescriptor(
            actionID: actionID,
            ...
            projectRoot: configSourcePath.map { canonicalPath(CotermButtonIcon.projectRoot(forConfigPath: $0)) },
    ...
    static func isTrustedSurfaceButton(
        ...
        workspaceCommand: CotermResolvedCommand?,
        ...
        return CotermActionTrust.shared.isTrusted(descriptor)
    }
    ...
    private static func executeWorkspaceCommand(
        command: CotermCommandDefinition,
        workspace wsDef: CotermWorkspaceDefinition,
        tabManager: TabManager,
        baseCwd: String
    ) -> Bool {
        ...
        if let existing = tabManager.tabs.first(where: { $0.customTitle == workspaceName }) {
            ...
            case .confirm:
                ...
                alert.messageText = String(
                    localized: "dialog.cotermConfig.confirmRestart.title",
                    defaultValue: "Workspace Already Exists"
                )
                alert.informativeText = String(
                    localized: "dialog.cotermConfig.confirmRestart.message",
                    defaultValue: "A workspace with this name already exists. Close it and create a new one?"
                )
                alert.alertStyle = .warning
                alert.addButton(withTitle: String(
                    localized: "dialog.cotermConfig.confirmRestart.recreate",
                    defaultValue: "Recreate"
                ))
                alert.addButton(withTitle: String(
                    localized: "dialog.cotermConfig.confirmRestart.cancel",
                    defaultValue: "Cancel"
                ))
        ...
        let resolvedCwd = CotermConfigStore.resolveCwd(wsDef.cwd, relativeTo: baseCwd)
