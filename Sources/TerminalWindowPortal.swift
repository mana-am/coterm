...
private var cotermWindowTerminalPortalKey: UInt8 = 0
private var cotermWindowTerminalPortalCloseObserverKey: UInt8 = 0
...
#if COTERM_ISSUE_483_PORTAL_RECOVERY
...
#if DEBUG
        cotermDebugLog("portal.hideEntry hosted=\(portalDebugToken(entry.hostedView)) reason=workspaceUnmount")
#endif
    }
    ...
    private func synchronizeHostedView(withId hostedId: ObjectIdentifier, syncLayout: Bool = true) {
        ...
        guard let anchorView = entry.anchorView, let window else {
            ...
            if !hostedView.isHidden {
                cotermDebugLog("portal.hidden hosted=\(portalDebugToken(hostedView)) value=1 reason=missingAnchorOrWindow")
            }
#endif
            hostedView.isHidden = true
            if entry.visibleInUI {
...
enum TerminalWindowPortalRegistry {
    ...
    private static func installWindowCloseObserverIfNeeded(for window: NSWindow) {
        guard objc_getAssociatedObject(window, &cotermWindowTerminalPortalCloseObserverKey) == nil else { return }
        ...
        objc_setAssociatedObject(
            window,
            &cotermWindowTerminalPortalCloseObserverKey,
            observer,
    ...
    private static func removePortal(windowId: ObjectIdentifier, window: NSWindow?) {
        ...
        guard let window else { return }
        if let observer = objc_getAssociatedObject(window, &cotermWindowTerminalPortalCloseObserverKey) {
            NotificationCenter.default.removeObserver(observer)
        }
        objc_setAssociatedObject(window, &cotermWindowTerminalPortalCloseObserverKey, nil, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        objc_setAssociatedObject(window, &cotermWindowTerminalPortalKey, nil, .OBJC_ASSOCIATION_RETAIN)
    }
    ...
    private static func portal(for window: NSWindow, syncLayout: Bool = true) -> WindowTerminalPortal {
        if let existing = objc_getAssociatedObject(window, &cotermWindowTerminalPortalKey) as? WindowTerminalPortal {
        ...
        objc_setAssociatedObject(window, &cotermWindowTerminalPortalKey, portal, .OBJC_ASSOCIATION_RETAIN)
