...
nonisolated private let cotermEventLogLogger = Logger(subsystem: "coterm.com.emergent.app", category: "event-log")

// Sendable safety: pending state is protected by `lock`; file IO runs on `queue`.
final class CotermEventLogWriter: @unchecked Sendable {
    static let defaultMaxPendingLines = 1_024

    private static let queue = DispatchQueue(label: "com.coterm.event-log", qos: .utility)
    ...
    private func flushPendingLines() {
        while true {
            ...
            if pendingLines.isEmpty {
                ...
                lock.unlock()
                if droppedCount > 0 {
                    cotermEventLogLogger.warning("Dropped \(droppedCount, privacy: .public) coterm event log line(s) under disk backpressure")
                }
                return
            }
            lines = pendingLines
    ...
    private func append(_ lines: [String]) {
        ...
        do {
            ...
            cotermEventLogLogger.error("Failed to append coterm event log: \(String(describing: error), privacy: .private)")
        }
    }
