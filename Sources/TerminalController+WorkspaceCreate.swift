...
extension TerminalController {
    ...
    func v2WorkspaceCreate(
        ...
        // Decode optional layout param (same JSON schema as coterm.json layout field).
        // Validate before creating the workspace so malformed layouts fail fast.
        var layoutNode: CotermLayoutNode?
        if let rawLayout = params["layout"] {
            ...
            do {
                layoutNode = try JSONDecoder().decode(CotermLayoutNode.self, from: layoutData)
            } catch {
