...
struct CotermWorkspaceDefinition: Codable, Sendable {
    var name: String?
    var cwd: String?
    var color: String?
    /// User-defined environment variables inherited by every shell spawned in the
    /// workspace (issue #5995). Managed `COTERM_*` variables always win.
    var env: [String: String]?
    var layout: CotermLayoutNode?
    ...
    init(from decoder: Decoder) throws {
        ...
        layout = try container.decodeIfPresent(CotermLayoutNode.self, forKey: .layout)

        if let rawColor = try container.decodeIfPresent(String.self, forKey: .color) {
            let defaults = decoder.userInfo[.cotermWorkspaceColorDefaults] as? UserDefaults ?? .standard
