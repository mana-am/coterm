...
/// How ``runCotermModalAlert(_:presentingWindow:willPresent:)`` ended up
/// presenting an alert.
///
...
enum CotermModalAlertPresentation {
...
/// Returns whether `window` is one of coterm's main windows.
///
/// Main windows carry the identifier `coterm.main` or a `coterm.main.<id>`
...
@MainActor
func isCotermMainWindow(_ window: NSWindow) -> Bool {
    guard let raw = window.identifier?.rawValue else { return false }
    return raw == "coterm.main" || raw.hasPrefix("coterm.main.")
}

/// Returns the visible main coterm window best suited to host a modal sheet.
///
...
/// when no main coterm window is currently on screen, in which case callers
/// should fall back to an app-modal presentation.
///
/// - Parameter preferredWindow: A window to consider ahead of the
///   key/main/any search, used when it is visible and a coterm main window
///   (e.g. a `TabManager`'s own owning window).
@MainActor
func cotermMainWindowForModalPresentation(preferring preferredWindow: NSWindow? = nil) -> NSWindow? {
    if let preferredWindow, preferredWindow.isVisible, isCotermMainWindow(preferredWindow) {
        return preferredWindow
    }
    if let keyWindow = NSApp.keyWindow, keyWindow.isVisible, isCotermMainWindow(keyWindow) {
        return keyWindow
    }
    if let mainWindow = NSApp.mainWindow, mainWindow.isVisible, isCotermMainWindow(mainWindow) {
        return mainWindow
    }
    return NSApp.windows.first { $0.isVisible && isCotermMainWindow($0) }
}
...
/// alert as a sheet attached to the main coterm window when one is available,
/// falling back to an app-modal `runModal()` only when there is no eligible
/// host window.
///
/// - Parameters:
///   - alert: The configured alert to present.
///   - presentingWindow: An explicit host window. When `nil`, the main coterm
///     window is resolved via ``cotermMainWindowForModalPresentation(preferring:)``.
...
@MainActor
func runCotermModalAlert(
    _ alert: NSAlert,
    presentingWindow: NSWindow? = nil,
    willPresent: ((CotermModalAlertPresentation) -> Void)? = nil
) -> NSApplication.ModalResponse {
    if NSApp.activationPolicy() == .regular {
        NSApp.activate(ignoringOtherApps: true)
    }

    let hostWindow = presentingWindow ?? cotermMainWindowForModalPresentation()
    guard let hostWindow, hostWindow.attachedSheet == nil else {
        let presentation = CotermModalAlertPresentation.appModal(hostWindowHadAttachedSheet: hostWindow?.attachedSheet != nil)
        CotermModalAlertAnalytics.track(alert, presentation: presentation)
        willPresent?(presentation)
        return alert.runModal()
    }

    let presentation = CotermModalAlertPresentation.sheet(hostWindow)
    CotermModalAlertAnalytics.track(alert, presentation: presentation)
    willPresent?(presentation)
