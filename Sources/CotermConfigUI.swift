...
struct CotermConfigUIDefinition: Codable, Sendable, Hashable {
    var newWorkspace: CotermConfigButtonPlacement?
    var surfaceTabBar: CotermSurfaceTabBarUIDefinition?
}

struct CotermSurfaceTabBarUIDefinition: Codable, Sendable, Hashable {
    var buttons: [CotermSurfaceTabBarButton]?
}

struct CotermConfigButtonPlacement: Codable, Sendable, Hashable {
    var action: String?
    ...
    init(
        ...
        tooltip: String? = nil,
        contextMenu: [CotermConfigContextMenuItem]? = nil
    ) {
        self.action = action
        self.icon = icon
    ...
    init(from decoder: Decoder) throws {
        ...
        contextMenu = try container.decodeIfPresent([CotermConfigContextMenuItem].self, forKey: .contextMenu)
            ?? container.decodeIfPresent([CotermConfigContextMenuItem].self, forKey: .rightClick)
    }
...
struct CotermConfigContextMenuActionItem: Codable, Sendable, Hashable {
    var action: String
    var title: String?
    var icon: CotermButtonIcon?
    var tooltip: String?
    ...
    init(action: String, title: String? = nil, icon: CotermButtonIcon? = nil, tooltip: String? = nil) {
        self.action = action
        self.title = title
        self.icon = icon
        self.tooltip = tooltip
...
enum CotermConfigContextMenuItem: Codable, Sendable, Hashable {
    case action(CotermConfigContextMenuActionItem)
    case separator
    ...
    init(from decoder: Decoder) throws {
        if let container = try? decoder.singleValueContainer(),
           let rawAction = try? container.decode(String.self) {
            ...
            self = .action(CotermConfigContextMenuActionItem(action: trimmed))
            return
        }
        ...
        self = .action(try CotermConfigContextMenuActionItem(from: decoder))
    }
...
struct CotermResolvedConfigMenuAction: Identifiable, Sendable, Hashable {
    var id: String
    var title: String
    var icon: CotermButtonIcon?
    var iconSourcePath: String?
    var tooltip: String?
    var action: CotermResolvedConfigAction
}

enum CotermResolvedConfigContextMenuItem: Identifiable, Sendable, Hashable {
    case action(CotermResolvedConfigMenuAction)
...
enum CotermRestartBehavior: String, Codable, Sendable {
    case new
    case recreate
    case ignore
    case confirm
