...
nonisolated struct CotermTopProcessCPUSample: Sendable {
    let totalTimeTicks: UInt64
    let sampledAtNanoseconds: UInt64
}

private nonisolated struct CotermTopProcessCPUTrackerState: Sendable {
    var entries: [CotermTopProcessScopeCacheKey: CotermTopProcessCPUTrackerEntry] = [:]
    var latestPrunedAtNanoseconds: UInt64 = 0
}

private nonisolated struct CotermTopProcessCPUTrackerEntry: Sendable {
    let sample: CotermTopProcessCPUSample
    let cpuPercent: Double
    let parentKey: CotermTopProcessScopeCacheKey?
}

private nonisolated final class CotermTopProcessCPUTracker: @unchecked Sendable {
    private static let minimumSampleWindowNanoseconds: UInt64 = 1_000_000_000

    private let state = OSAllocatedUnfairLock(initialState: CotermTopProcessCPUTrackerState())
    ...
    func cpuPercentages(
        for currentSamples: [CotermTopProcessScopeCacheKey: CotermTopProcessCPUSample],
        activeKeys: Set<CotermTopProcessScopeCacheKey>,
        parentKeysByKey: [CotermTopProcessScopeCacheKey: CotermTopProcessScopeCacheKey],
        sampledAtNanoseconds: UInt64
    ) -> [CotermTopProcessScopeCacheKey: Double] {
        state.withLock { state in
            var percentages: [CotermTopProcessScopeCacheKey: Double] = [:]
            percentages.reserveCapacity(currentSamples.count)
            var heldKeys: Set<CotermTopProcessScopeCacheKey> = []
            ...
            for (key, sample) in currentSamples {
                ...
                if let existing {
                    ...
                    guard elapsedNanoseconds >= Self.minimumSampleWindowNanoseconds else {
                        ...
                        if existing.parentKey != parentKey {
                            state.entries[key] = CotermTopProcessCPUTrackerEntry(
                ...
                let cpuPercent = CotermTopProcessSnapshot.cpuPercent(
                    current: sample,
...
private nonisolated let cotermTopProcessCPUTracker = CotermTopProcessCPUTracker()
private nonisolated let cotermTopAbsoluteTimeNanosecondsRatio: Double? = {
...
nonisolated extension CotermTopProcessSnapshot {
    ...
    static func cpuPercentages(
        for samples: [CotermTopProcessScopeCacheKey: CotermTopProcessCPUSample],
        activeKeys: Set<CotermTopProcessScopeCacheKey>,
        parentKeysByKey: [CotermTopProcessScopeCacheKey: CotermTopProcessScopeCacheKey] = [:],
        sampledAtNanoseconds: UInt64
    ) -> [CotermTopProcessScopeCacheKey: Double] {
        cotermTopProcessCPUTracker.cpuPercentages(
            for: samples,
    ...
    static func cpuPercent(
        current: CotermTopProcessCPUSample,
        previous: CotermTopProcessCPUSample?
    ) -> Double {
        guard let previous,
    ...
    private static func absoluteTimeNanoseconds(_ ticks: UInt64) -> Double? {
        guard let ratio = cotermTopAbsoluteTimeNanosecondsRatio else { return nil }
