...
nonisolated private let rosettaRelaunchLogger = Logger(subsystem: "coterm.com.emergent.app", category: "RosettaRelaunch")

/// Launch-time self-heal that re-execs coterm natively when the process is
/// running translated under Rosetta on Apple Silicon.
///
/// coterm ships a universal (`x86_64 arm64`) binary so it keeps supporting Intel
...
/// `zsh`, and every tool launched in a coterm terminal — runs translated. macOS 26
...
enum RosettaNativeRelaunch {
    ...
    private static let guardKey = "COTERM_ROSETTA_RELAUNCHED"
    ...
    /// Re-exec coterm as a native arm64 process when it is running translated.
    ///
