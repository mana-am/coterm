...
struct CotermCLIPathInstaller {
    ...
    enum InstallerError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .bundledCLIMissing(let expectedPath):
                return "Bundled coterm CLI was not found at \(expectedPath)."
            ...
            case .installVerificationFailed(let path):
                return "Installed symlink at \(path) did not point to the bundled coterm CLI."
    ...
    init(
        ...
        destinationURL: URL = URL(fileURLWithPath: "/usr/local/bin/coterm"),
        bundledCLIURLProvider: @escaping () -> URL? = {
            CotermCLIPathInstaller.defaultBundledCLIURL()
        },
        expectedBundledCLIPath: String = CotermCLIPathInstaller.defaultBundledCLIExpectedPath(),
    ...
    private static func defaultBundledCLIURL(bundle: Bundle = .main) -> URL? {
        bundle.resourceURL?.appendingPathComponent("bin/coterm", isDirectory: false)
    }
