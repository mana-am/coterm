...
final class MainWindowHostingView<Content: View>: NSHostingView<Content> {
    ...
    override var fittingSize: NSSize { CotermMainWindow.minimumContentSize }
    override var intrinsicContentSize: NSSize { CotermMainWindow.minimumContentSize }
...
@MainActor
func configureCotermMainWindowDragBehavior(_ window: NSWindow) {
    window.isMovableByWindowBackground = false
    window.isMovable = false
}

@MainActor
final class CotermMainWindow: NSWindow {
    ...
    /// coterm creates its main window programmatically (never from a nib), so it
    ...
    /// `.fullScreenNone`) so a coterm main window can always enter a native
    ...
    /// coterm owns main-window placement: it persists and restores window frames
    ...
    /// arrangements. Because coterm never re-asserts the saved frame after wake,
