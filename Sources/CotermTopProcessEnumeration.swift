...
private nonisolated let cotermTopPIDPathBufferSize = 4096

nonisolated extension CotermTopProcessSnapshot {
    static func allProcesses(includeProcessDetails: Bool, includeCotermScope: Bool) -> [CotermTopProcessInfo] {
        let sampledProcesses = allBSDProcesses()
        guard !sampledProcesses.isEmpty else { return [] }

        var scopeKeyByPID: [Int: CotermTopProcessScopeCacheKey] = [:]
        ...
        var parentScopeKeys: [CotermTopProcessScopeCacheKey: CotermTopProcessScopeCacheKey] = [:]
        ...
        var currentCPUSamples: [CotermTopProcessScopeCacheKey: CotermTopProcessCPUSample] = [:]
        var processRecords: [(info: CotermTopProcessInfo, cpuSampleKey: CotermTopProcessScopeCacheKey?)] = []
        ...
        if includeCotermScope {
            pruneCotermScopeCache(activeKeys: activeScopeKeys)
        }
    ...
    private static func processInfo(
        ...
        includeCotermScope: Bool,
        sampledAtNanoseconds: UInt64,
        currentCPUSamples: inout [CotermTopProcessScopeCacheKey: CotermTopProcessCPUSample]
    ) -> (info: CotermTopProcessInfo, cpuSampleKey: CotermTopProcessScopeCacheKey?)? {
        ...
        let cotermScope = includeCotermScope
            ? cachedCotermScope(for: pid, cacheKey: cacheKey, nowNanoseconds: sampledAtNanoseconds)
            : nil
        ...
        let cpuSampleKey: CotermTopProcessScopeCacheKey?
        if let taskInfo {
        ...
        return (CotermTopProcessInfo(
            pid: pid,
            parentPID: Int(bsdInfo.pbi_ppid),
            name: name.isEmpty ? "pid-\(pid)" : name,
            path: path,
            ttyDevice: ttyDevice,
            cotermWorkspaceID: cotermScope?.workspaceID,
            cotermSurfaceID: cotermScope?.surfaceID,
            cotermAttributionReason: cotermScope?.attributionReason,
    ...
    private static func processPath(pid: Int) -> String? {
        var buffer = [CChar](repeating: 0, count: cotermTopPIDPathBufferSize)
