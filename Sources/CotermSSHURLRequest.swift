...
enum CotermSSHURLParseError: Error, Equatable {
...
struct CotermSSHURLRequest: Equatable {
    static let maxDestinationLength = 256
    static let maxTitleLength = 160
    static let supportedSchemes: Set<String> = ["coterm", "coterm-nightly", "coterm-dev"]
    ...
    ) -> Result<CotermSSHURLRequest?, CotermSSHURLParseError> {
    ...
    private static func parseStandardSSHURL(_ url: URL) -> Result<CotermSSHURLRequest?, CotermSSHURLParseError> {
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false) else {
            return .failure(.missingDestination)
        }
    ...
    private static func standardSSHURLPort(in components: URLComponents) -> Result<Int?, CotermSSHURLParseError> {
    ...
    private static func structuredSSHOptions(from queryItems: [URLQueryItem]) -> Result<[String], CotermSSHURLParseError> {
        var options: [String] = []
    ...
    private static func boundedInteger(_ value: String, parameter: String, range: ClosedRange<Int>) -> Result<Int, CotermSSHURLParseError> {
        guard !containsUnsafeHiddenCharacter(value),
    ...
    private static func normalizedBooleanValue(named name: String, in queryItems: [URLQueryItem]) -> Result<Bool, CotermSSHURLParseError> {
...
enum CotermNavigationURLParseError: Error, Equatable {
    case unsupportedURLShape
    case invalidIdentifier(String)
}

struct CotermNavigationURLRequest: Equatable {
    ...
    static func parse(
        _ url: URL,
        supportedSchemes: Set<String> = activeSupportedSchemes
    ) -> Result<CotermNavigationURLRequest?, CotermNavigationURLParseError> {
        guard isSupportedScheme(url.scheme, supportedSchemes: supportedSchemes) else {
            return .success(nil)
        }
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false) else {
        ...
        if route.count == 2 {
            return .success(CotermNavigationURLRequest(originalURL: url, target: .workspace(workspaceId)))
        }
...
enum CotermTextURLParseError: Error, Equatable {
    case missingText
...
struct CotermTextURLRequest: Equatable {
    ...
    static let maxTitleLength = 160
    static let supportedSchemes: Set<String> = CotermSSHURLRequest.supportedSchemes
    static var activeSupportedSchemes: Set<String> {
        CotermSSHURLRequest.activeSupportedSchemes
    }
    ...
    static func parse(
        _ url: URL,
        supportedSchemes: Set<String> = activeSupportedSchemes
    ) -> Result<CotermTextURLRequest?, CotermTextURLParseError> {
        guard isSupportedScheme(url.scheme, supportedSchemes: supportedSchemes) else {
            return .success(nil)
        }
        ...
        return .success(CotermTextURLRequest(
            originalURL: url,
            kind: kind,
            text: text,
            name: name,
    ...
    private static func normalizedBooleanValue(named name: String, in queryItems: [ParsedQueryItem]) -> Result<Bool, CotermTextURLParseError> {
        guard let item = queryItems.first(where: { $0.name.lowercased() == name }) else {
            return .success(false)
        }
        guard let rawValue = item.value else {
