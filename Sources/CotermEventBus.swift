...
struct CotermEventSubscriptionSnapshot {
    let subscription: CotermEventSubscription
    let replay: [[String: Any]]
    let ack: [String: Any]
}

// Sendable safety: every mutable field is protected by `lock`; `semaphore` only wakes `next(timeout:)`.
final class CotermEventSubscription: @unchecked Sendable {
    let id: UUID
...
// Sendable safety: event state is protected by `lock`; disk appends are delegated to `CotermEventLogWriter`.
final class CotermEventBus: @unchecked Sendable {
    static let shared = CotermEventBus(eventLogURL: defaultEventLogURL())
    static let protocolName = "coterm-events"
    ...
    static let defaultMaxPendingEventLogLines = CotermEventLogWriter.defaultMaxPendingLines
    ...
    private let eventLogWriter: CotermEventLogWriter?
    private let bootId = UUID().uuidString
    private var nextSequence: Int64 = 1
    private var retained: [[String: Any]] = []
    private var subscriptions: [UUID: CotermEventSubscription] = [:]

    init(
        retainedEventLimit: Int = CotermEventBus.defaultRetainedEventLimit,
        eventLogURL: URL? = nil,
        maxEventLogBytes: UInt64 = CotermEventBus.defaultMaxEventLogBytes,
        maxEventLineBytes: Int = CotermEventBus.defaultMaxEventLineBytes,
        maxPendingEventLogLines: Int = CotermEventBus.defaultMaxPendingEventLogLines,
        maxPendingEventsPerSubscription: Int = CotermEventBus.defaultMaxPendingEventsPerSubscription
    ) {
    ...
    func subscribe(
    ...
    ) -> CotermEventSubscriptionSnapshot {
        let subscription = CotermEventSubscription(
            names: names,
        ...
        let gapReason: String? = afterSequence.flatMap { after in
            ...
            if after > latestSequence {
                return "requested sequence is newer than this coterm process; coterm probably restarted"
            }
            return nil
        }
        ...
        return CotermEventSubscriptionSnapshot(subscription: subscription, replay: replay, ack: ack)
    }

    func unsubscribe(_ subscription: CotermEventSubscription) {
        lock.lock()
    ...
    private func removeSubscriptionIfStillActive(_ subscription: CotermEventSubscription) {
        lock.lock()
    ...
    func heartbeat(subscription: CotermEventSubscription) -> [String: Any] {
        [
            "type": "heartbeat",
            "protocol": Self.protocolName,
            "version": Self.protocolVersion,
    ...
    private static func sanitizedJSONValue(_ value: Any, depth: Int) -> Any {
        ...
        case let value as [String: Any]:
            ...
            if value.count > maxSanitizedObjectEntries {
                result["__coterm_truncated_entries"] = value.count - maxSanitizedObjectEntries
            }
            return result
        case let value as [Any]:
            ...
            if value.count > maxSanitizedArrayItems {
                result.append(["__coterm_truncated_items": value.count - maxSanitizedArrayItems])
            }
            return result
        default:
