...
extension Notification.Name {
    static let socketListenerDidStart = Notification.Name("coterm.socketListenerDidStart")
    // terminalSurfaceDidBecomeReady moved to Coterminal (posted by TerminalSurface).
    static let terminalSurfaceHostedViewDidMoveToWindow = Notification.Name("coterm.terminalSurfaceHostedViewDidMoveToWindow")
    static let mainWindowContextsDidChange = Notification.Name("coterm.mainWindowContextsDidChange")
    static let browserDownloadEventDidArrive = Notification.Name("coterm.browserDownloadEventDidArrive")
    static let reactGrabDidCopySelection = Notification.Name("coterm.reactGrabDidCopySelection")
}
...
nonisolated func remotePTYSessionListErrorIsUnsupportedDaemon(_ error: Error) -> Bool {
    ...
    guard nsError.domain == "coterm.remote.daemon.rpc", nsError.code == 14 else {
        return false
    }
...
nonisolated private func v2RemotePTYUserFacingErrorMessage(_ message: String) -> String {
    ...
    if lowered.contains("missing required capability") ||
        ...
        return "remote daemon does not support persistent SSH PTY sessions; reconnect the remote workspace to update coterm"
    }
...
class TerminalController {
    ...
    // Stateless Sendable structs from CotermControlSocket; injected at construction.
    // `transport` is internal so sibling-file extensions (CotermEventStream) can write through it.
    ...
    // Stateless sidebar-metadata/command argument parser (CotermSidebar).
    ...
    private nonisolated static let socketCommandFocusAllowanceStackKey = "coterm.socketCommandFocusAllowanceStack"
...
#if DEBUG
    private nonisolated static let socketCommandDebugLogEnvironmentKey = "COTERM_DEBUG_SOCKET_COMMAND_LOG"
    ...
    /// The main-actor RPC dispatch coordinator (CotermControlSocket). Owns the
    ...
    private nonisolated static let v2BrowserEvalEnvelopeTypeKey = "__coterm_t"
    private nonisolated static let v2BrowserEvalEnvelopeValueKey = "__coterm_v"
    ...
    /// diagnostics, failure classification) extracted to `CotermBrowser`.
    ...
    private nonisolated func handleClient(_ socket: Int32, peerPid: pid_t? = nil) {
        ...
        // In cotermOnly mode, verify the connecting process is a descendant of coterm.
        // In allowAll mode (env-var only), skip the ancestry check.
        if socketServer.accessMode == .cotermOnly {
            ...
            if let pid {
                guard isDescendant(pid) else {
                    _ = writeSocketResponse(
                        "ERROR: Access denied — only processes started inside coterm can connect",
                        to: socket
                    )
                    return
                }
    ...
    private func processCommand(_ command: String) -> String {
        ...
        return withSocketCommandPolicy(commandKey: cmd, isV2: false, params: policyParams) {
            // V1 domains migrated into CotermControlSocket's ControlCommandCoordinator
    ...
    private func processV2Command(_ jsonLine: String) -> String {
        ...
        return withSocketCommandPolicy(commandKey: method, isV2: true, params: params) {
            ...
            // Domains migrated into CotermControlSocket's ControlCommandCoordinator
    ...
    func v2Identify(params: [String: Any]) -> [String: Any] {
        ...
        if let cliPath = Bundle.main.resourceURL?
            ...
            .appendingPathComponent("coterm", isDirectory: false)
            .path {
    ...
    private func parseV2WindowRouting(params: [String: Any]) -> (routing: V2WindowRouting?, error: V2CallResult?) {
        ...
        if params["window_id"] != nil && requestedWindowId == nil {
            return (
                ...
                .err(
                    code: "invalid_params",
                    message: "Invalid window selector. Use --window <id|ref|index> to target one window, or run `coterm list-windows` to see available windows and retry.",
        ...
        if includeAllWindows, requestedWindowId != nil {
            return (
                ...
                .err(
                    code: "invalid_params",
                    message: "Choose either --window <id|ref|index> or --all-windows, not both. Run `coterm list-windows` to see available windows and retry.",
                    data: v2WindowSelectorDetails(params: params)
                )
            )
        }
    ...
    private func v2WindowNotFoundResult(params: [String: Any], windowId: UUID) -> V2CallResult {
        .err(
            code: "not_found",
            message: "Window not found. Run `coterm list-windows` to see available windows, then retry with --window <id|ref|index>.",
    ...
    func taskManagerTopPayload(includeProcesses: Bool) async throws -> [String: Any] {
        ...
        let processSnapshot = await withTaskGroup(
            of: CotermTopProcessSnapshot.self,
            returning: CotermTopProcessSnapshot.self
        ) { group in
            group.addTask(priority: .utility) {
                CotermTopProcessSnapshot.capture(includeProcessDetails: includeProcesses)
            }
    ...
    private nonisolated func processAggregates(
        from processSnapshot: CotermTopProcessSnapshot,
        totalPIDs: Set<Int>
    ...
    private nonisolated func v2SystemTop(params: [String: Any]) -> V2CallResult {
        ...
        let processSnapshot = CotermTopProcessSnapshot.capture(includeProcessDetails: includeProcesses)
        let browserPIDOccurrences = v2TopBrowserPIDOccurrences(in: windowNodes)
        let totalPIDs = v2AnnotateTopWindows(
            &windowNodes,
            processSnapshot: processSnapshot,
    ...
    private func v2TopWorkspaceNode(
        ...
        for (surfaceIndex, panel) in orderedPanels(in: workspace).enumerated() {
            ...
            if panel.panelType == .browser, let browserPanel = panel as? BrowserPanel {
                let webContentPID = CotermWebContentProcessIdentifier.pid(for: browserPanel.webView)
    ...
    /// CLI presentation concern (`coterm workspace env --mask`): the local control
    ...
    private func readTerminalTextFromVTExportForSnapshot(
        ...
        #if DEBUG
        cotermDebugLog("mobile.vtExport action=\(bindingAction) succeeded=\(actionSucceeded) hasPath=\(exportedPath != nil)")
        #endif
    ...
    private nonisolated func v2FeedPush(params: [String: Any]) -> V2CallResult {
        ...
        CotermEventBus.shared.publishWorkstreamEvent(event, phase: "received")
        ...
        CotermEventBus.shared.publishWorkstreamEvent(
            event,
            phase: "completed",
...
#if DEBUG
                cotermDebugLog("browser.jsKick.locateFailed surface=\(surfaceId.uuidString.prefix(5))")
#endif
                return false
            }
    ...
    private nonisolated func v2RunBrowserJavaScript(
        ...
        if let frameSelector = v2BrowserCurrentFrameSelector(surfaceId: surfaceId) {
            ...
            framePrelude = """
            let __cotermDoc = document;
            try {
              const __cotermFrame = document.querySelector(\(selectorLiteral));
              if (__cotermFrame && __cotermFrame.contentDocument) {
                __cotermDoc = __cotermFrame.contentDocument;
              }
            } catch (_) {}
            """
        } else {
            framePrelude = "const __cotermDoc = document;"
        }
        ...
        const __cotermMaybeAwait = async (__r) => {
        ...
        const __cotermEvalInFrame = async function() {
          const document = __cotermDoc;
          \(executionBlock)
          const __value = await __cotermMaybeAwait(__r);
          return {
            __coterm_t: (typeof __value === 'undefined') ? 'undefined' : 'value',
            __coterm_v: __value
          };
        };

        return await __cotermEvalInFrame();
        """
    ...
    private func v2BrowserEnsureInitScriptsApplied(surfaceId: UUID, browserPanel: BrowserPanel) {
        ...
        let injector = """
        (() => {
          window.__cotermInitScriptsApplied = window.__cotermInitScriptsApplied || { scripts: [], styles: [] };
          return true;
        })()
        """
        ...
        for css in styles {
            ...
            let styleScript = """
            (() => {
              const id = 'coterm-init-style-' + btoa(unescape(encodeURIComponent(\(cssLiteral)))).replace(/=+$/g, '');
    ...
    private func v2BrowserDisabledExternalOpenResult(
        ...
        guard let url else {
            return .err(code: "browser_disabled", message: "Coterm browser is disabled", data: nil)
        }
    ...
    private func v2BrowserOpenSplit(params: [String: Any]) -> V2CallResult {
        ...
        if let urlStr {
            ...
            if let navigable = resolveBrowserNavigableURL(urlStr) {
                ...
                // the trusted coterm-diff-viewer:// scheme, and external app/deep-link schemes
    ...
    private func v2IsDiffViewerURL(_ url: URL?) -> Bool {
        ...
        return url.scheme?.lowercased() == "http" &&
            url.host == "127.0.0.1" &&
            url.fragment == "coterm-diff-viewer"
    }

    private func v2RegisterDiffViewerURLIfNeeded(params: [String: Any], url: URL?) -> V2CallResult? {
        guard let url,
              url.scheme == CotermDiffViewerURLSchemeHandler.scheme else {
            return nil
        }
        guard let token = v2String(params, "diff_viewer_token"),
              ...
              !rawFiles.isEmpty,
              rawFiles.count <= CotermDiffViewerURLSchemeHandler.maxRegisteredFiles else {
            return .err(code: "invalid_params", message: "Missing or invalid trusted diff viewer allowlist", data: nil)
        }

        let files = rawFiles.compactMap(CotermDiffViewerURLSchemeHandler.registeredFile(from:))
        ...
        do {
            try CotermDiffViewerURLSchemeHandler.shared.register(token: token, files: files)
            return nil
        } catch {
            return .err(
                code: "invalid_params",
        ...
        case .evaluationFailed(let message):
            return .err(
                ...
                data: [
                    ...
                    "hint": "Verify the page loaded with 'Coterm browser <surface> get url' before waiting"
                ]
            )
        }
    }
    ...
    private nonisolated func v2BrowserDblClick(params: [String: Any]) -> V2CallResult {
        v2BrowserSelectorAction(params: params, actionName: "dblclick") { selectorLiteral in
            ...
            (() => {
              ...
              __cotermClick(el);
              __cotermClick(el);
              const c = __cotermCenter(el);
              __cotermMouse(el, 'dblclick', c, 0, 2);
              return { ok: true };
            })()
            """
        }
    ...
    /// `__cotermClick(el)`, `__cotermHover(el)`, `__cotermSetChecked(el, desired)`, and `__cotermKey(t,type,key)`.
    private nonisolated static let browserInputHelpers = """
    function __cotermCenter(el){const r=el.getBoundingClientRect();return {x:Math.floor(r.left+Math.min(r.width,r.width/2)),y:Math.floor(r.top+Math.min(r.height,r.height/2))};}
    function __cotermPointer(el,type,c,buttons,bubbles){try{el.dispatchEvent(new PointerEvent(type,{bubbles:(bubbles===false?false:true),cancelable:true,composed:true,view:window,pointerId:1,pointerType:'mouse',isPrimary:true,button:0,buttons:buttons,clientX:c.x,clientY:c.y,screenX:c.x,screenY:c.y}));}catch(e){}}
    function __cotermMouse(el,type,c,buttons,detail,bubbles){el.dispatchEvent(new MouseEvent(type,{bubbles:(bubbles===false?false:true),cancelable:true,composed:true,view:window,button:0,buttons:buttons,detail:detail||0,clientX:c.x,clientY:c.y,screenX:c.x,screenY:c.y}));}
    function __cotermClick(el){const c=__cotermCenter(el);
      __cotermPointer(el,'pointerover',c,0);__cotermMouse(el,'mouseover',c,0);
      __cotermPointer(el,'pointerenter',c,0,false);__cotermMouse(el,'mouseenter',c,0,0,false);
      __cotermPointer(el,'pointermove',c,0);__cotermMouse(el,'mousemove',c,0);
      __cotermPointer(el,'pointerdown',c,1);__cotermMouse(el,'mousedown',c,1,1);
      if(typeof el.focus==='function'){try{el.focus({preventScroll:true});}catch(e){try{el.focus();}catch(e2){}}}
      __cotermPointer(el,'pointerup',c,0);__cotermMouse(el,'mouseup',c,0,1);
      if(typeof el.click==='function'){el.click();}else{__cotermMouse(el,'click',c,0,1);}
    }
    function __cotermHover(el){const c=__cotermCenter(el);
      __cotermPointer(el,'pointerover',c,0);__cotermMouse(el,'mouseover',c,0);
      __cotermPointer(el,'pointerenter',c,0,false);__cotermMouse(el,'mouseenter',c,0,0,false);
      __cotermPointer(el,'pointermove',c,0);__cotermMouse(el,'mousemove',c,0);
    }
    function __cotermSetChecked(el,desired){
      ...
      else {const c=__cotermCenter(el); __cotermMouse(el,'click',c,0,1);}
    }
    function __cotermKeyMeta(key){
    ...
    function __cotermKey(target,type,key){
      const meta=__cotermKeyMeta(key);
    ...
    private nonisolated func v2BrowserPress(params: [String: Any]) -> V2CallResult {
        ...
        return v2BrowserWithPanelContext(params: params) { ctx in
            ...
            (() => {
              ...
              const kdNotPrevented = __cotermKey(target, 'keydown', k);
              ...
              if (k.length === 1 || k === 'Enter') { kpNotPrevented = __cotermKey(target, 'keypress', k); }
              __cotermKey(target, 'keyup', k);
    ...
    private nonisolated func v2BrowserCheck(params: [String: Any], checked: Bool) -> V2CallResult {
        v2BrowserSelectorAction(params: params, actionName: checked ? "check" : "uncheck") { selectorLiteral in
            ...
            (() => {
              ...
              __cotermSetChecked(el, \(checked ? "true" : "false"));
    ...
    private nonisolated func v2BrowserScreenshot(params: [String: Any]) -> V2CallResult {
        ...
        let screenshotsDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-browser-screenshots", isDirectory: true)
    ...
    private nonisolated func v2BrowserDialogRespond(params: [String: Any], accept: Bool) -> V2CallResult {
        return v2BrowserWithPanelContext(params: params) { ctx in
            ...
            let script = """
            (() => {
              const q = window.__cotermDialogQueue || [];
              ...
              if (entry.type === 'confirm') {
                ...
                window.__cotermDialogDefaults.confirm = \(acceptLiteral);
              }
              if (entry.type === 'prompt') {
                window.__cotermDialogDefaults = window.__cotermDialogDefaults || { confirm: false, prompt: null };
                if (\(acceptLiteral)) {
                  window.__cotermDialogDefaults.prompt = \(textLiteral);
                } else {
                  window.__cotermDialogDefaults.prompt = null;
                }
              }
    ...
    private nonisolated func v2WaitForDownloadFile(path: String, timeout: TimeInterval) -> V2DownloadFileWaitResult {
        ...
        let watcherQueue = DispatchQueue(label: "com.coterm.browser.download.wait.file")
        ...
        if params.keys.contains("destination_profile") {
            ...
            if let uuid = UUID(uuidString: query),
               profiles.contains(where: { $0.id == uuid }) {
                ...
                return .err(
                    code: "invalid_params",
                    message: "destination_profile does not match a Coterm browser profile",
                    data: ["param": "destination_profile"]
                )
            }
        } else {
    ...
    private nonisolated func v2BrowserConsoleList(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanelContext(params: params) { ctx in
            ...
            let script = """
            (() => {
              const items = Array.isArray(window.__cotermConsoleLog) ? window.__cotermConsoleLog.slice() : [];
              if (\(clearLiteral)) {
                window.__cotermConsoleLog = [];
              }
    ...
    private nonisolated func v2BrowserErrorsList(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanelContext(params: params) { ctx in
            ...
            let script = """
            (() => {
              const items = Array.isArray(window.__cotermErrorLog) ? window.__cotermErrorLog.slice() : [];
              if (\(clearLiteral)) {
                window.__cotermErrorLog = [];
              }
              return { ok: true, items };
            })()
            """
    ...
    private func helpText() -> String {
        ...
        Split & surface commands:
          ...
          reload_config                   - Reload Ghostty config, coterm settings, and refresh terminals
    ...
    private func dragPasteboardType(from token: String) -> NSPasteboard.PasteboardType? {
        ...
        case "sidebarreorder", "sidebar-reorder", "sidebar_tab_reorder",
            "com.coterm.sidebar-tab-reorder":
    ...
    func panelSnapshot(_ args: String) -> String {
        ...
        let outputDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-screenshots")
    ...
    private func parseSidebarMutationTabTarget(
        ...
        // `CotermSidebar.SidebarMutationTabTarget` cases this controller resolves, so
    ...
    /// `CotermFeedback` package), which caps the fields, rejects an oversized
    /// base64 blob without decoding, and writes a self-contained bundle
    /// directory under `~/.cache/coterm-dogfood-feedback/<ISO8601>_<shortid>/`
    ...
    /// initiated from the Mac for automation (`coterm mobile set-font <size>`).
    ///
    ...
    func v2MobileTerminalReplay(params: [String: Any]) -> V2CallResult {
        ...
        guard let resolved = mobileResolveWorkspaceAndSurface(params: params, requireTerminal: true),
            ...
            #if DEBUG
            cotermDebugLog("mobile.terminal.replay NOT_FOUND surface=\(v2RawString(params, "surface_id") ?? "nil")")
            #endif
        ...
        #if DEBUG
        cotermDebugLog("mobile.terminal.replay surface=\(surfaceId.uuidString.prefix(8)) renderGrid=\(renderGrid != nil) seq=\(seq) hasState=\(state != nil)")
        #endif
        var payload: [String: Any] = [
