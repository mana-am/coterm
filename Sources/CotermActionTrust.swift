...
struct CotermActionTrustDescriptor: Codable, Sendable {
    var schemaVersion: Int = 1
    var actionID: String
    var kind: String
    var command: String?
    var target: String?
    var workspaceCommand: CotermCommandDefinition?
...
final class CotermActionTrust {
    static let shared = CotermActionTrust()
    static let didChangeNotification = Notification.Name("coterm.actionTrustDidChange")
    ...
    private init() {
        let appSupport = FileManager.default.urls(
            ...
            in: .userDomainMask
        ).first!.appendingPathComponent("coterm")
    ...
    func isTrusted(_ descriptor: CotermActionTrustDescriptor) -> Bool {
        trustedFingerprints.contains(descriptor.fingerprint)
    }

    func trust(_ descriptor: CotermActionTrustDescriptor) {
