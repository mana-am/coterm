...
extension TerminalController {
    ...
    nonisolated func handleEventsStreamRequest(_ line: String, socket: Int32) {
        ...
        let afterSequence = CotermEventBus.int64(params["after_seq"] ?? params["after"])
        ...
        defer { CotermEventBus.shared.unsubscribe(snapshot.subscription) }
        ...
        while true {
            if let event = snapshot.subscription.next(timeout: CotermEventBus.defaultHeartbeatIntervalSeconds) {
                guard writeEventsStreamLine(event, socket: socket) else { return }
                ...
                if let reason = snapshot.subscription.closeReason {
                    _ = writeEventsStreamLine([
                        ...
                        "error": [
                            "code": "slow_consumer",
                            "message": reason,
                            "latest_seq": NSNumber(value: CotermEventBus.shared.latestSequence)
                        ]
                    ], socket: socket)
                }
                return
            } else if includeHeartbeats {
                let heartbeat = CotermEventBus.shared.heartbeat(subscription: snapshot.subscription)
    ...
    private nonisolated func writeEventsStreamLine(_ object: [String: Any], socket: Int32) -> Bool {
        autoreleasepool {
            guard let line = CotermEventBus.encodeLine(object) else { return false }
