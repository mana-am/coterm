...
nonisolated let cotermTopMemoryDiagnosticDefaultGroupLimit = 12

nonisolated struct CotermTopProcessAttribution: Hashable, Sendable {
...
nonisolated extension CotermTopProcessSnapshot {
    func memoryDiagnosticPayload(
        ...
        topGroupLimit: Int = cotermTopMemoryDiagnosticDefaultGroupLimit,
        attributionByPID: [Int: CotermTopProcessAttribution] = [:]
    ) -> [String: Any] {
        ...
        return [
            ...
            "app": [
                ...
                "memory_source": appProcess?.memorySource.rawValue ?? CotermTopProcessMemorySource.unavailable.rawValue,
                "resident_memory_source": appProcess?.residentMemorySource.rawValue ?? CotermTopProcessMemorySource.unavailable.rawValue
            ] as [String: Any],
            "children": [
                "root_pid": appPID,
    ...
    private struct MemoryDiagnosticGroupAccumulator {
        ...
        var processIDs: [Int] = []
        var attributions: [CotermTopProcessAttribution: MemoryDiagnosticAttributionAccumulator] = [:]

        mutating func append(
            process: CotermTopProcessInfo,
            attribution: CotermTopProcessAttribution?
        ) {
            rssBytes = CotermTopProcessSnapshot.clampedAdd(rssBytes, process.residentBytes)
    ...
    private struct MemoryDiagnosticAttributionAccumulator {
        ...
        mutating func append(process: CotermTopProcessInfo) {
            rssBytes = CotermTopProcessSnapshot.clampedAdd(rssBytes, process.residentBytes)
            processIDs.append(process.pid)
        }
    ...
    private func memoryDiagnosticGroups(
        ...
        for pid in pids.sorted() {
            ...
            groups[key]?.append(
                process: process,
                attribution: nearestCotermAttribution(
                    for: pid,
                    attributionByPID: attributionByPID
                )
            )
    ...
    private func nearestCotermAttribution(
        for pid: Int,
        attributionByPID: [Int: CotermTopProcessAttribution]
        ...
        while currentPID > 0, visited.insert(currentPID).inserted {
            ...
            if process.cotermWorkspaceID != nil || process.cotermSurfaceID != nil {
                return CotermTopProcessAttribution(
                    workspaceID: process.cotermWorkspaceID,
                    workspaceRef: nil,
                    paneID: nil,
                    paneRef: nil,
                    surfaceID: process.cotermSurfaceID,
                    surfaceRef: nil,
                    surfaceType: nil,
                    reason: process.cotermAttributionReason ?? "coterm-process-scope"
                )
            }
