...
struct cotermApp: App {
    ...
    init() {
        ...
        let configFileURL = CotermConfigLocation().userConfigFile
        ...
        // apps" prompt; the password now lives in the non-protected coterm state
        // directory (https://github.com/emergent-inc/coterm/issues/5146). The app
        ...
        // Secrets live in their own 0600 files under the coterm state directory,
        ...
        // Lift any plaintext socket-control password out of `coterm.json` into the
        ...
        // (`CotermSettingsFileStore`, loaded later during app launch) reads the
        ...
        // If invoked with CLI-style arguments (e.g. `coterm hooks setup`), exec the
        // bundled CLI at Contents/Resources/bin/coterm. The GUI binary and the CLI
        // share the name `coterm`, so if the GUI's Contents/MacOS leaks onto $PATH
        // (which happens for any shell descended from this process), bare `coterm`
        // resolves here instead of the CLI. See
        // https://github.com/emergent-inc/coterm/issues/4678.
        // coterm ships a universal binary so it still supports Intel Macs, but a
        ...
        // launches. See https://github.com/emergent-inc/coterm/issues/753.
        ...
        if let stored = defaults.string(forKey: SocketControlSettings.appStorageKey) {
            ...
            defaults.set(legacy ? SocketControlMode.cotermOnly.rawValue : SocketControlMode.off.rawValue,
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged Coterm DEV; start with ./scripts/reload.sh --tag <name> (or set COTERM_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
    ...
    static func resolvedGhosttyResourcesDirectory(
        ...
        // Tagged coterm builds may inherit GHOSTTY_RESOURCES_DIR from another running
        // coterm instance. Prefer this app's bundled resources when they are present.
    ...
    var body: some Scene {
            ...
            MainWindowBootstrapView()
                ...
                .onAppear {
                    ...
                    if ProcessInfo.processInfo.environment["COTERM_UI_TEST_MODE"] == "1" {
                        AppDelegate.shared?.updateLog.append("ui test: cotermApp onAppear")
                    }
#endif
        ...
        .commands {
            CommandGroup(replacing: .appSettings) {
                ...
                Button(String(localized: "menu.app.openCotermSettingsFile", defaultValue: "Open coterm.json")) {
                ...
                Button(String(localized: "menu.app.makeDefaultTerminal", defaultValue: "Make coterm the Default Terminal")) {
                    DefaultTerminalUserAction.setAsDefault(debugSource: "menu.makeDefaultTerminal")
                }
            }

            CommandGroup(replacing: .appInfo) {
                Button(String(localized: "menu.app.about", defaultValue: "About coterm")) {
                    showAboutPanel()
                }
            ...
            CommandGroup(replacing: .appTermination) {
                splitCommandButton(title: String(localized: "menu.quitCoterm", defaultValue: "Quit Coterm"), shortcut: menuShortcut(for: .quit)) {
                    NSApp.terminate(nil)
                }
            }
...
#if DEBUG
                        cotermDebugLog("find.menu Cmd+F fired")
#endif
    ...
    private func closePanelOrWindow() {
        ...
        if let window, cotermWindowShouldOwnCloseShortcut(window) { window.performClose(nil); return }
    ...
    var body: some View {
        Color.clear
            ...
            .background(WindowAccessor { window in
                window.identifier = NSUserInterfaceItemIdentifier("coterm.bootstrap")
...
private let cotermAuxiliaryWindowIdentifiers: Set<String> = [
    "coterm.settings",
    "coterm.about",
    "coterm.licenses",
    "coterm.browser-popup",
    "coterm.browserProfilePopoverDebug",
    "coterm.configEditor",
    "coterm.defaultTerminalRegistrationError",
    "coterm.feedButtonStyleDebug",
    "coterm.feedPreview",
    "coterm.feedTextEditorDebug",
    "coterm.fileExplorerStyleDebug",
    "coterm.folderDragIcon",
    "coterm.pdfPreviewChromeDebug",
    "coterm.recentlyClosedHistory",
    "coterm.splitButtonLayoutDebug",
    "coterm.tabBarBackdropLab",
    "coterm.taskManager",
    "coterm.aboutTitlebarDebug",
    "coterm.debugWindowControls",
    "coterm.browserImportHintDebug",
    "coterm.extensionSidebarInspector",
    "coterm.sidebarDebug",
    "coterm.menubarDebug",
    "coterm.backgroundDebug",
    "coterm.startupAppearanceDebug",
    "coterm.bonsplitTabBarDebug",
    "coterm.titlebarLayoutDebug",
    "coterm.devWindowDisplay",
    "coterm.mobilePairingWindow",
]

/// Returns whether the given window should handle the standard close shortcut
/// as a standalone auxiliary window instead of routing it through workspace or
/// panel-close behavior.
func cotermWindowShouldOwnCloseShortcut(_ window: NSWindow?) -> Bool {
    guard let identifier = window?.identifier?.rawValue else { return false }
    return cotermAuxiliaryWindowIdentifiers.contains(identifier)
}
...
private final class DebugWindowControlsWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("coterm.debugWindowControls")
        window.center()
...
private final class BrowserImportHintDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("coterm.browserImportHintDebug")
        window.center()
...
private final class BrowserProfilePopoverDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("coterm.browserProfilePopoverDebug")
        window.center()
...
private final class AboutWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("coterm.about")
        window.center()
...
private final class AcknowledgmentsWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("coterm.licenses")
        window.center()
...
private struct AcknowledgmentsView: View {
    ...
    var body: some View {
        ScrollView {
            Text(content)
                .cotermFont(.body, design: .monospaced)
...
private final class FileExplorerStyleDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("coterm.fileExplorerStyleDebug")
        window.center()
...
private final class SidebarDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("coterm.sidebarDebug")
        window.center()
...
private struct AboutPanelView: View {
    ...
    private let websiteURL = URL(string: "https://coterm.cc")

    private var version: String? { Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String }
    private var build: String? { Bundle.main.infoDictionary?["CFBundleVersion"] as? String }
    private var commit: String? {
        if let value = Bundle.main.infoDictionary?["CotermCommit"] as? String, !value.isEmpty {
            return value
        }
        let env = ProcessInfo.processInfo.environment["COTERM_COMMIT"] ?? ""
        ...
        VStack(alignment: .center) {
            ...
            VStack(alignment: .center, spacing: 32) {
                VStack(alignment: .center, spacing: 8) {
                    Text(String(localized: "about.appName", defaultValue: "coterm"))
                        .cotermFont(.title)
                        .bold()
                }
                .textSelection(.enabled)
                ...
                VStack(spacing: 2) {
                    ...
                    let commitURL = commit.flatMap { hash in
                        URL(string: "https://github.com/emergent-inc/coterm/commit/\(hash)")
                    }
...
private final class MenuBarExtraDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("coterm.menubarDebug")
        window.center()
...
private final class SplitButtonLayoutDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("coterm.splitButtonLayoutDebug")
        window.center()
...
private final class TabBarBackdropLabWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("coterm.tabBarBackdropLab")
        window.center()
    ...
    private func labSlider(
        ...
        VStack(alignment: .leading, spacing: 4) {
            Text("\(title) \(displayValue)")
                .cotermFont(.caption, monospacedDigit: true)
                .lineLimit(1)
            Slider(value: value, in: range)
                .frame(width: width)
        }
...
private struct TabBarBackdropLabSample: View {
    ...
    private static func makeController(for variant: TabBarBackdropLabVariant) -> BonsplitController {
        ...
        let titles = [
            ...
            String(localized: "debug.tabBarBackdropLab.tab.terminalTransparency", defaultValue: "coterm terminal transparency"),
...
private struct TabBarBackdropLabTitlebar: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(title)
                .cotermFont(.caption2, weight: .medium)
                .lineLimit(1)
            Spacer(minLength: 0)
        }
...
private struct TabBarBackdropLabSidebar: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(title)
                .cotermFont(.caption2, weight: .bold)
...
private struct TabBarBackdropLabTerminalPane: View {
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("\(String(localized: "debug.tabBarBackdropLab.terminal.prompt", defaultValue: "lawrence in ~/coterm")) \(title)")
...
private final class BackgroundDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("coterm.backgroundDebug")
        window.center()
...
private final class StartupAppearanceDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("coterm.startupAppearanceDebug")
        window.center()
...
private struct StartupAppearanceDebugView: View {
    ...
    private func applyAppearance(_ mode: StartupAppearancePreviewMode) {
        switch mode {
        case .stored:
            // coterm pins the app to a fixed dark appearance.
...
enum TelemetrySettings {
    ...
    // live in `CotermSettings` (`AppCatalogSection().sendAnonymousTelemetry`) as the
...
@MainActor
func openCotermSettingsFileInEditor() {
