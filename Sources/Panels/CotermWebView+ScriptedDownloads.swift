...
extension CotermWebView {
    private static let scriptedDownloadMessageHandlerName = "cotermScriptedDownload"
    ...
    (() => {
      try {
        if (window.__cotermScriptedDownloadInstalled) return true;
        window.__cotermScriptedDownloadInstalled = true;
    ...
    private func startScriptedWebKitDownload(
        ...
        guard let downloadDelegate = cotermDownloadDelegate else {
#if DEBUG
...
private final class ScriptedDownloadMessageHandler: NSObject, WKScriptMessageHandler {
    func userContentController(
        ...
        guard let webView = message.webView as? CotermWebView,
