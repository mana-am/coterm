...
extension WKWebView {
    nonisolated private static var cotermSetPageMutedSelector: Selector {
        NSSelectorFromString("_setPageMuted:")
    }

    nonisolated private static var cotermMediaMutedStateAudio: Int {
        1 << 0
    }

    @discardableResult
    func cotermSetPageAudioMuted(_ muted: Bool) -> Bool {
        let selector = Self.cotermSetPageMutedSelector
        ...
        function(self, selector, muted ? Self.cotermMediaMutedStateAudio : 0)
        return true
    }

    var cotermIsElementFullscreenActiveOrTransitioning: Bool {
    ...
    func cotermIsManagedByExternalFullscreenWindow(relativeTo expectedWindow: NSWindow?) -> Bool {
        guard cotermIsElementFullscreenActiveOrTransitioning else { return false }
...
/// Find-family shortcuts. The configured Find shortcut stays app-owned so coterm can
/// choose browser find or right-sidebar file search from the current focus owner.
final class CotermWebView: WKWebView {
    ...
    private static let pasteAsPlainTextFocusMessageHandlerName = "cotermPasteAsPlainTextFocus"
    private static let browserFocusModeContextMenuItemIdentifier =
        NSUserInterfaceItemIdentifier("coterm.browserFocusMode.toggle")
    ...
    const __cotermPasteAsPlainTextHelpers = (() => {
      const existing = window.__cotermPasteAsPlainTextHelpers;
      ...
      window.__cotermPasteAsPlainTextHelpers = helpers;
      return helpers;
    })();
    """
    ...
    (() => {
      try {
        if (window.__cotermPasteAsPlainTextFocusTrackerInstalled) return true;
        window.__cotermPasteAsPlainTextFocusTrackerInstalled = true;
        ...
        const publish = (canPaste) => {
          ...
          window.__cotermPasteAsPlainTextTargetAvailable = canPaste;
          try {
            handler?.postMessage({ canPaste });
          } catch (_) {}
        };

        window.__cotermCanPasteAsPlainTextIntoCurrentFocus = () => {
          return __cotermPasteAsPlainTextHelpers.canPasteAsPlainTextInto(document.activeElement);
        };

        const publishForElement = (el) => {
          publish(__cotermPasteAsPlainTextHelpers.canPasteAsPlainTextInto(el));
        };
        ...
        document.addEventListener("mousedown", (ev) => {
          ...
          if (!__cotermPasteAsPlainTextHelpers.canPasteAsPlainTextInto(target)) {
            publish(false);
          }
        }, true);
    ...
    private final class PasteAsPlainTextFocusMessageHandler: NSObject, WKScriptMessageHandler {
        func userContentController(
            ...
            guard let webView = message.webView as? CotermWebView else {
                return
            }
    ...
    static func hasRecentMiddleClickIntent(for webView: WKWebView) -> Bool {
        guard let webView = webView as? CotermWebView else { return false }
    ...
    private static func recordMiddleClickIntent(for webView: CotermWebView) {
    ...
    private static var cotermDownloadDelegateKey: UInt8 = 0
    ...
    var contextMenuLinkURLProvider: ((CotermWebView, NSPoint, @escaping (URL?) -> Void) -> Void)?
    ...
    var cotermDownloadDelegate: WKDownloadDelegate? {
        get {
            objc_getAssociatedObject(self, &Self.cotermDownloadDelegateKey) as? WKDownloadDelegate
        }
        set {
            objc_setAssociatedObject(
                self,
                &Self.cotermDownloadDelegateKey,
                newValue,
    ...
    private func pageCanAcceptPlainTextPaste() -> Bool {
        ...
        (() => {
            try {
                const fn = window.__cotermCanPasteAsPlainTextIntoCurrentFocus;
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        if let decision = AppDelegate.shared?.handleBrowserFocusModeKeyEvent(
            ...
            case .forwardToWebView:
                ...
                // While focus mode is active, the page gets the shortcut once and coterm/main-menu
    ...
    override func keyDown(with event: NSEvent) {
        ...
        // If this path reaches keyDown, forward it to WebKit instead of coterm.
    ...
    private func handleMouseNavigationButton(_ event: NSEvent) -> Bool {
        ...
        case 3:
#if DEBUG
            cotermDebugLog("browser.mouse.navigation web=\(ObjectIdentifier(self)) kind=back canGoBack=\(canGoBack ? 1 : 0)")
#endif
        ...
        case 4:
#if DEBUG
            cotermDebugLog("browser.mouse.navigation web=\(ObjectIdentifier(self)) kind=forward canGoForward=\(canGoForward ? 1 : 0)")
#endif
    ...
    /// and its lifecycle live in `CotermWebView+ContextMenuLinkCapture.swift`;
...
#if DEBUG
        cotermDebugLog(Self.redactedContextDownloadDebugMessage(message()))
#endif
    }
    ...
    private static let blockedDragTypes: Set<NSPasteboard.PasteboardType> = [
        ...
        NSPasteboard.PasteboardType("com.coterm.sidebar-tab-reorder"),
    ]
