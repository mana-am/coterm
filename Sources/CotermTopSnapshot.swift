...
nonisolated struct CotermTopResourceSummary: Sendable {
...
nonisolated enum CotermTopProcessMemorySource: String, Sendable {
...
nonisolated struct CotermTopProcessInfo: Sendable {
    let pid: Int
    let parentPID: Int
    let name: String
    let path: String?
    let ttyDevice: Int64?
    let cotermWorkspaceID: UUID?
    let cotermSurfaceID: UUID?
    let cotermAttributionReason: String?
    let processGroupID: Int?
    let terminalProcessGroupID: Int?
    var cpuPercent: Double
    let memoryBytes: Int64
    let memorySource: CotermTopProcessMemorySource
    let residentBytes: Int64
    let residentMemorySource: CotermTopProcessMemorySource
    ...
    init(
        ...
        path: String?,
        ttyDevice: Int64?,
        cotermWorkspaceID: UUID?,
        cotermSurfaceID: UUID?,
        cotermAttributionReason: String?,
        processGroupID: Int?,
        terminalProcessGroupID: Int?,
        cpuPercent: Double,
        memoryBytes: Int64? = nil,
        memorySource: CotermTopProcessMemorySource? = nil,
        residentBytes: Int64,
        residentMemorySource: CotermTopProcessMemorySource = .residentSize,
        virtualBytes: Int64,
        threadCount: Int
    ) {
        self.pid = pid
        ...
        self.cotermWorkspaceID = cotermWorkspaceID
        self.cotermSurfaceID = cotermSurfaceID
        self.cotermAttributionReason = cotermAttributionReason
...
nonisolated struct CotermTopProcessScope: Sendable, Equatable {
...
nonisolated final class CotermTopProcessSnapshot: @unchecked Sendable {
    let sampledAt: Date
    private let includesProcessDetails: Bool
    private let includesCotermScope: Bool
    let processesByPID: [Int: CotermTopProcessInfo]
    private let childrenByParentPID: [Int: [Int]]
    private let pidsByTTYDevice: [Int64: [Int]]
    private let pidsByCotermSurfaceID: [UUID: [Int]]
    private let residentMemorySources: [CotermTopProcessMemorySource]
    ...
    static func capture(
        ...
        CotermTopProcessSnapshot(
            ...
            includesCotermScope: includeCotermScope
        )
    }

    init(
        processes: [CotermTopProcessInfo],
        sampledAt: Date,
        includesProcessDetails: Bool,
        includesCotermScope: Bool = true
    ) {
        self.sampledAt = sampledAt
        self.includesProcessDetails = includesProcessDetails
        self.includesCotermScope = includesCotermScope
        var processMap: [Int: CotermTopProcessInfo] = [:]
        ...
        for process in processMap.values {
            ...
            if let cotermSurfaceID = process.cotermSurfaceID {
                cotermSurfaceMap[cotermSurfaceID, default: []].append(process.pid)
            }
        }
        self.childrenByParentPID = children.mapValues { $0.sorted() }
        self.pidsByTTYDevice = ttyMap.mapValues { $0.sorted() }
        self.pidsByCotermSurfaceID = cotermSurfaceMap.mapValues { $0.sorted() }
    }
    ...
    func samplePayload() -> [String: Any] {
        ...
        return [
            ...
            "memory_source": CotermTopProcessMemorySource.physicalFootprint.rawValue,
            "memory_fallback_source": CotermTopProcessMemorySource.residentSize.rawValue,
            ...
            "resident_memory_fallback_source": CotermTopProcessMemorySource.rusageResidentSize.rawValue,
            "process_details": includesProcessDetails,
            "coterm_scope": includesCotermScope
        ]
    }

    var hasCotermScope: Bool {
        includesCotermScope
    }

    private static func sortedMemorySources(
        in sources: [CotermTopProcessMemorySource]
    ) -> [CotermTopProcessMemorySource] {
        [
            .physicalFootprint,
            .residentSize,
            .rusageResidentSize,
    ...
    private static func summaryMemorySource(
        _ sources: [CotermTopProcessMemorySource]
    ) -> CotermTopProcessMemorySource {
    ...
    func pids(forCotermSurfaceID surfaceID: UUID) -> Set<Int> {
        Set(pidsByCotermSurfaceID[surfaceID] ?? [])
    }

    func cotermScopedProcesses() -> [CotermTopProcessInfo] {
        processesByPID.values
            .filter { $0.cotermWorkspaceID != nil && $0.cotermSurfaceID != nil }
            .sorted { $0.pid < $1.pid }
    }

    func process(pid: Int) -> CotermTopProcessInfo? {
        processesByPID[pid]
    }
    ...
    func summary(for pids: Set<Int>, rootPIDs: Set<Int> = []) -> CotermTopResourceSummary {
        let sortedPIDs = pids.filter { $0 > 0 }.sorted()
        var summary = CotermTopResourceSummary()
    ...
    func programSummaryPayload(for pids: Set<Int>) -> [[String: Any]] {
        var aggregates: [String: CotermProgramProcessAggregate] = [:]
        ...
        for pid in pids.sorted() {
            ...
            if aggregates[key] == nil {
                aggregates[key] = CotermProgramProcessAggregate(id: key, title: title)
            }
    ...
    func codingAgentSummaryPayload(for pids: Set<Int>) -> [[String: Any]] {
        var aggregates: [String: CotermCodingAgentProcessAggregate] = [:]

        for pid in pids.sorted() {
            guard let process = processesByPID[pid] else { continue }
            let processArguments = Self.processArgumentsIfNeeded(for: process)
            guard let definition = CotermTaskManagerCodingAgentDefinition.matchingDefinition(
            ...
            if aggregates[definition.id] == nil {
                aggregates[definition.id] = CotermCodingAgentProcessAggregate(definition: definition)
            }
            aggregates[definition.id]?.append(process)
        }

        return CotermTaskManagerCodingAgentDefinition.builtIns.compactMap { definition in
            guard let aggregate = aggregates[definition.id] else { return nil }
            return aggregate.payload()
        }
    }

    private static func processArgumentsIfNeeded(for process: CotermTopProcessInfo) -> CotermTopProcessArguments? {
        guard CotermTaskManagerCodingAgentDefinition.shouldReadArguments(
            processName: process.name,
            processPath: process.path
        ) else { return nil }
    ...
    private struct CotermProgramProcessAggregate {
        let id: String
        let title: String
        var cpuPercent: Double = 0
        var memoryBytes: Int64 = 0
        ...
        mutating func append(_ process: CotermTopProcessInfo) {
            guard seenProcessIds.insert(process.pid).inserted else { return }
            cpuPercent += process.cpuPercent
            memoryBytes = CotermTopProcessSnapshot.clampedAdd(memoryBytes, process.memoryBytes)
            residentBytes = CotermTopProcessSnapshot.clampedAdd(residentBytes, process.residentBytes)
        ...
        func payload() -> [String: Any] {
            ...
            return [
                "id": id,
                "name": title,
                "resources": CotermTopResourceSummary(
    ...
    private struct CotermCodingAgentProcessAggregate {
        let definition: CotermTaskManagerCodingAgentDefinition
        var cpuPercent: Double = 0
        var memoryBytes: Int64 = 0
        var residentBytes: Int64 = 0
        var processIds: [Int] = []
    ...
    private func processTreeNode(
        ...
        if let cotermWorkspaceID = process.cotermWorkspaceID {
            payload["coterm_workspace_id"] = cotermWorkspaceID.uuidString
        } else {
            payload["coterm_workspace_id"] = NSNull()
        }
        if let cotermSurfaceID = process.cotermSurfaceID {
            payload["coterm_surface_id"] = cotermSurfaceID.uuidString
        } else {
            payload["coterm_surface_id"] = NSNull()
        }
    ...
    private func attributionReason(
        for process: CotermTopProcessInfo,
        allowedPIDs: Set<Int>,
        rootPIDs: Set<Int>
    ) -> String {
        if let reason = process.cotermAttributionReason {
            return reason
        }
    ...
    private func isWebKitWebContentProcess(_ process: CotermTopProcessInfo) -> Bool {
