#!/usr/bin/env python3
"""
coterm Python Client

A client library for programmatically controlling coterm via Unix socket.
...
# The control socket, markers, and password live in the non-TCC coterm state
# directory (~/.local/state/coterm), not Application Support, so the separately
# signed coterm CLI can touch them without the macOS "access data from other apps"
# prompt (https://github.com/emergent-inc/coterm/issues/5146).
...
def _is_known_coterm_bundle_id(bundle_id: str) -> bool:
    return (
...
def _default_bundle_id() -> str:
    override = (os.environ.get("COTERM_BUNDLE_ID") or "").strip()
    if override and _is_known_coterm_bundle_id(override):
        return override

    tag = os.environ.get("COTERM_TAG")
...
def _socket_variant() -> Tuple[str, Optional[str]]:
    ...
    if bundle_id == _DEFAULT_DEBUG_BUNDLE_ID:
        tag = os.environ.get("COTERM_TAG", "").strip()
...
def _last_socket_path_files() -> List[str]:
    ...
    if variant == "nightly":
        ...
        tmp_marker = f"/tmp/coterm-nightly-{slug}-last-socket-path" if slug else "/tmp/coterm-nightly-last-socket-path"
    elif variant == "staging":
        marker = f"staging-{slug}-last-socket-path" if slug else "staging-last-socket-path"
        tmp_marker = f"/tmp/coterm-staging-{slug}-last-socket-path" if slug else "/tmp/coterm-staging-last-socket-path"
    elif variant == "dev":
        marker = f"dev-{slug}-last-socket-path" if slug else "dev-last-socket-path"
        tmp_marker = f"/tmp/coterm-dev-{slug}-last-socket-path" if slug else "/tmp/coterm-dev-last-socket-path"
...
def _variant_socket_candidates() -> List[str]:
    variant, slug = _socket_variant()
    if variant == "nightly":
        return [f"/tmp/coterm-nightly-{slug}.sock"] if slug else ["/tmp/coterm-nightly.sock"]
    if variant == "staging":
        return [f"/tmp/coterm-staging-{slug}.sock"] if slug else ["/tmp/coterm-staging.sock"]
    if variant == "dev":
        if slug:
            return [f"/tmp/coterm-debug-{slug}.sock"]
        return ["/tmp/coterm-debug.sock"]
...
def _default_socket_path() -> str:
    ...
    if bundle_id == _DEFAULT_DEBUG_BUNDLE_ID:
        tagged = glob.glob("/tmp/coterm-debug-*.sock")
        tagged.extend(glob.glob(os.path.join(_STATE_DIR, "coterm*.sock")))
        tagged = [
            p for p in tagged
            if os.path.exists(p)
            and os.path.basename(p).startswith("coterm-debug-")
        ]
        if tagged:
...
class coterm:
    """Client for controlling coterm via Unix socket"""
    ...
    def _send_command(self, command: str) -> str:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if time.time() - start >= 5.0:
                        raise cotermError("Command timed out")
                    continue
                if not chunk:
                    break
                data += chunk.decode()
            ...
            raise cotermError(f"Socket error: {e}")
    ...
    def seed_drag_pasteboard_types(self, types: List[str]) -> None:
        ...
        if not types:
            raise cotermError("seed_drag_pasteboard_types requires at least one type")
        payload = ",".join(t.strip() for t in types if t and t.strip())
        if not payload:
            raise cotermError("seed_drag_pasteboard_types requires at least one non-empty type")
    ...
    def layout_debug(self) -> dict:
        ...
        try:
            ...
            raise cotermError(f"layout_debug JSON decode failed: {e}: {payload[:200]}")
    ...
    def render_stats(self, panel: Union[str, int, None] = None) -> dict:
        ...
        try:
            ...
            raise cotermError(f"render_stats JSON decode failed: {e}: {payload[:200]}")
    ...
    def panel_snapshot(self, panel: Union[str, int], label: str = "") -> dict:
        ...
        if len(parts) != 5:
            raise cotermError(f"panel_snapshot parse failed: {response}")
...
def main():
    """CLI interface for coterm"""
    import sys
    import argparse

    parser = argparse.ArgumentParser(description="coterm CLI")
    ...
    try:
        with coterm(args.socket) as client:
            if not args.command:
                # Interactive mode
                print("coterm CLI (type 'help' for commands, 'quit' to exit)")
                while True:
                    try:
                        line = input("> ").strip()
