...
Run against a Debug app socket (typically with COTERM_SOCKET_MODE=allowAll).
"""
...
def _wait_url_contains(client: coterm, panel_id: str, needle: str, timeout_s: float = 20.0) -> None:
    def _matches() -> bool:
    ...
    if not _wait_until(_matches, timeout_s=timeout_s, interval_s=0.1):
        ...
        raise cotermError(f"Timed out waiting for browser URL containing '{needle}', got: {current}")


def _capture_screenshot(client: coterm, label: str) -> str:
...
def _focused_terminal_ready(client: coterm, panel_id: str) -> bool:
    try:
...
def _drag_hit_chain(client: coterm, nx: float, ny: float) -> str:
    ...
    with coterm(SOCKET_PATH) as client:
        ...
        if not ping_ok:
            raise cotermError(
                ...
                "Launch Debug app with COTERM_SOCKET_MODE=allowAll for this test."
            )
        ...
        try:
            ...
            if not _wait_until(lambda: _focused_terminal_ready(client, terminal_id), timeout_s=4.0):
                raise cotermError(f"Terminal did not become first responder before Cmd+W: {terminal_id}")
