...
def _wait_portal_terminals(c: coterm, expected: int = 2, timeout: float = 8.0):
    ...
    cotermError with the last-seen snapshot if it never settles within `timeout`.
    """
    start = time.time()
    terminals: list = []
    ...
    with coterm(SOCKET_PATH) as c:
        ...
        for row in terminals:
            if not row.get("in_window", False):
                raise cotermError(f"terminal not attached to window: {row}")
            if row.get("portal") is not True:
                raise cotermError(f"terminal is not portal-hosted: {row}")
            depth = row.get("view_depth")
            if not isinstance(depth, int):
                raise cotermError(f"missing view_depth in surface_health: {row}")
            if depth > 8:
                raise cotermError(f"terminal view depth too deep ({depth}): {row}")
