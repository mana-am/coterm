#!/usr/bin/env python3
"""
Regression test: `coterm omx` preserves fallback provider dirs in PATH.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="coterm-omx-fallback-path-") as td:
        root = Path(td)
...
""",
        ...
        if proc.returncode != 0:
            print("FAIL: `coterm omx --version` failed with omx in a fallback dir")
        ...
        expected_helper = str(fallback_bin / "omx-node-helper")
        expected = [
            expected_helper,
    ...
    print("PASS: coterm omx preserves fallback provider dirs in PATH")
    return 0
