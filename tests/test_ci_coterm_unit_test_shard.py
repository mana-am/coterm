#!/usr/bin/env python3
"""Behavioral guards for cotermTests CI sharding."""
...
HELPER = ROOT / "scripts" / "ci" / "coterm_unit_test_shard.py"
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        tmp_root = Path(tmp)
        test_root = tmp_root / "cotermTests"
        test_root.mkdir()
    ...
    extension_selector = "-only-testing:cotermTests/LargeSuiteTests/testExtensionRegression"
    ...
    suite_selector = "-only-testing:cotermTests/LargeSuiteTests"
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for shard in range(1, 5):
            ...
            for focused_selector in (
                "-only-testing:cotermTests/BrowserSystemProxyMirrorTests",
                "-only-testing:cotermTests/GhosttyOptionAsAltModsTests",
            ):
    ...
    print("PASS: cotermTests sharding covers extension methods and leaves focused gates explicit")
    return 0
