...
SCRIPT="$ROOT_DIR/Resources/bin/start-coterm-profiling"
TMP_DIR="$(mktemp -d)"
...
EOF
chmod +x "$plist_buddy"
export COTERM_PROFILE_PLIST_BUDDY="$plist_buddy"
...
export COTERM_PROFILE_DEFAULTS="$defaults_bin"
...
export COTERM_PROFILE_SYSTEM_PROFILER="$system_profiler_bin"

ps_file="$TMP_DIR/ps.txt"
cat > "$ps_file" <<EOF
101 $stable_app/Contents/MacOS/Coterm
202 $nightly_app/Contents/MacOS/Coterm
303 $dev_app/Contents/MacOS/Coterm
EOF
...
if "$SCRIPT" --dry-run --test-ps-file "$ps_file" --out "$TMP_DIR/ambiguous" >/tmp/coterm-profile-ambiguous.log 2>&1; then
  echo "FAIL: unqualified selection should reject multiple coterm processes" >&2
  exit 1
fi
if ! grep -Fq "multiple coterm processes are running" /tmp/coterm-profile-ambiguous.log; then
  echo "FAIL: ambiguous selection did not explain how to discriminate instances" >&2
  cat /tmp/coterm-profile-ambiguous.log >&2
  exit 1
fi
...
HOME="$TMP_DIR" PATH="$fake_bin:$PATH" COTERM_PROFILE_TOC_TIMEOUT_SECONDS=1 "$SCRIPT" \
...
if ! grep -Fq "System:" "$timeout_out/summary.md" ||
   ! grep -Fq "App: ~/Coterm DEV dog.app" "$timeout_out/summary.md" ||
...
if grep -Fq "$TMP_DIR/Coterm DEV dog.app" "$timeout_out/summary.md" ||
   grep -Fq "$TMP_DIR/Coterm DEV dog.app" "$timeout_out/system-info.txt"; then
...
PATH="$fake_bin:$PATH" COTERM_PROFILE_SYSTEM_PROFILER="$failing_system_profiler" "$SCRIPT" \
  --test-ps-file "$ps_file" \
  --channel dev \
  --tag dog \
  --duration 1 \
...
PATH="$fake_bin:$PATH" COTERM_PROFILE_SYSTEM_PROFILER="$sleep_system_profiler" COTERM_PROFILE_SYSTEM_PROFILER_TIMEOUT_SECONDS=1 "$SCRIPT" \
  --test-ps-file "$ps_file" \
  --channel dev \
  --tag dog \
  --duration 1 \
...
if PATH="$fail_bin:$PATH" "$SCRIPT" \
  ...
  --template "Time Profiler" \
  --no-submit \
  --out "$all_failed_out" >/tmp/coterm-profile-all-failed.log 2>&1; then
  echo "FAIL: all-failed profiling run should exit nonzero" >&2
  exit 1
fi
if ! grep -Fq "all profiling templates failed" /tmp/coterm-profile-all-failed.log ||
  ...
  cat /tmp/coterm-profile-all-failed.log >&2
...
"$ROOT_DIR/Resources/bin/submit-coterm-profile" --dry-run --profile "$timeout_out" --target-name "Coterm DEV dog" >/dev/null
...
EOF
chmod +x "$sleep_ditto"
COTERM_PROFILE_DITTO="$sleep_ditto" "$ROOT_DIR/Resources/bin/submit-coterm-profile" \
  --profile "$timeout_out" \
  --package-only &
...
package_output="$(COTERM_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-coterm-profile" --profile "$timeout_out" --package-only)"
...
COTERM_PROFILE_OSASCRIPT="$cancel_bin" COTERM_PROFILE_OPEN="$open_bin" COTERM_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-coterm-profile" \
  --profile "$timeout_out" \
...
COTERM_PROFILE_OSASCRIPT="$sleep_osascript" COTERM_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-coterm-profile" \
  --profile "$timeout_out" \
  --target-name "Coterm DEV dog" \
  --target-pid 303 \
  --channel dev \
...
if COTERM_PROFILE_OSASCRIPT="$cancel_bin" COTERM_PROFILE_OPEN="$open_bin" COTERM_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-coterm-profile" \
  --profile "$timeout_out" \
  --target-name "Coterm DEV dog" \
  --target-pid 303 \
  --channel dev \
...
if ! grep -Fq "User canceled" /tmp/coterm-profile-send-cancel.log; then
  echo "FAIL: submit helper send cancellation did not preserve the AppleScript error" >&2
  cat /tmp/coterm-profile-send-cancel.log >&2
  exit 1
fi
...
HOME="$TMP_DIR" COTERM_PROFILE_FEEDBACK_EMAIL=wrong@example.com COTERM_PROFILE_LOCALE=ja_JP COTERM_PROFILE_OSASCRIPT="$capture_osascript" COTERM_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-coterm-profile" \
  --profile "$timeout_out" \
  --target-name "Coterm DEV dog" \
  --target-pid 303 \
  --channel dev \
  ...
  --note-file "$note_file" \
  --send
if ! grep -Fq "cotermプロファイルを送信" "$captured_args" ||
   ! grep -Fq "下書きを開く" "$captured_args" ||
   ! grep -Fq "cotermプロファイリングキャプチャ" "$captured_args" ||
...
echo "PASS: start-coterm-profiling target selection and default templates"
