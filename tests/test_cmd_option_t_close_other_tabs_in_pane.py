...
Run this against an app launched with COTERM_SOCKET_MODE=allowAll.
"""
...
def _pane_state(client: coterm) -> list[dict]:
    rows: list[dict] = []
...
def main() -> int:
    with coterm(SOCKET_PATH) as client:
        ...
        try:
            ...
            if len(before) < 3:
                raise cotermError(f"Expected >=3 tabs before shortcut, got {before}")

            selected_rows = [row for row in before if row["selected"]]
            if len(selected_rows) != 1:
                raise cotermError(f"Expected exactly one selected tab before shortcut, got {before}")
