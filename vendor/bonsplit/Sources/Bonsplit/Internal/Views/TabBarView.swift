...
enum TabBarStyling {
    ...
    static func inlineSplitButtons(
        from buttons: [BonsplitConfiguration.SplitActionButton]
    ) -> [BonsplitConfiguration.SplitActionButton] {
        guard let newTerminalIndex = buttons.firstIndex(where: { $0.action == .newTerminal }) else {
            return []
        }
        return [buttons[newTerminalIndex]]
    }

    static func trailingSplitButtons(
        from buttons: [BonsplitConfiguration.SplitActionButton]
    ) -> [BonsplitConfiguration.SplitActionButton] {
        var didMoveNewTerminalInline = false
        return buttons.filter { button in
            guard button.action == .newTerminal else { return true }
            if didMoveNewTerminalInline {
                return true
            }
            didMoveNewTerminalInline = true
            return false
        }
    }
...
struct TabBarView: View {
    ...
    private var tabBarLayout: TabBarLayout {
        TabBarLayout(
            ...
            splitButtonCount: trailingSplitButtons.count,
            splitButtonLaneVisible: shouldShowTrailingSplitButtons,
            reservesSplitButtonLane: shouldRenderTrailingSplitButtons,
    ...
    private var inlineSplitButtons: [BonsplitConfiguration.SplitActionButton] {
        TabBarStyling.inlineSplitButtons(from: visibleSplitButtons)
    }

    private var trailingSplitButtons: [BonsplitConfiguration.SplitActionButton] {
        TabBarStyling.trailingSplitButtons(from: visibleSplitButtons)
    }

    private var shouldRenderInlineSplitButtons: Bool {
        !inlineSplitButtons.isEmpty
    }

    private var shouldRenderTrailingSplitButtons: Bool {
        !trailingSplitButtons.isEmpty
    }

    private var shouldShowSplitButtons: Bool {
        (shouldRenderInlineSplitButtons || shouldRenderTrailingSplitButtons) && (!isMinimalMode || isHoveringTabBar)
    }

    private var shouldShowInlineSplitButtons: Bool {
        shouldRenderInlineSplitButtons && shouldShowSplitButtons
    }

    private var shouldShowTrailingSplitButtons: Bool {
        shouldRenderTrailingSplitButtons && shouldShowSplitButtons
    }
    ...
    private var tabScrollContent: some View {
        HStack(spacing: TabBarMetrics.tabSpacing) {
            ...
            inlineSplitButtonChrome

            // Unified drop zone after the last tab.
            dropZoneAfterTabs

            Color.clear
                .frame(width: trailingTabContentInset, height: tabBarHeight)
        }
    ...
    @ViewBuilder
    private var inlineSplitButtonChrome: some View {
        if shouldRenderInlineSplitButtons {
            splitButtonRow(
                buttons: inlineSplitButtons,
                registersHitRegions: true,
                showsHoverHighlight: true
            )
                .saturation(tabBarSaturation)
                .opacity(shouldShowInlineSplitButtons ? 1 : 0)
                .allowsHitTesting(shouldShowInlineSplitButtons)
    ...
    private var splitButtonChrome: some View {
        if shouldRenderTrailingSplitButtons {
            splitButtons(buttons: trailingSplitButtons)
                ...
                .clipped()
                .saturation(tabBarSaturation)
                .opacity(shouldShowTrailingSplitButtons ? 1 : 0)
                .allowsHitTesting(shouldShowTrailingSplitButtons)
    ...
    private var splitButtonBackdropChrome: some View {
        ...
        if shouldRenderTrailingSplitButtons && snapshot.drawsActionLaneSeparator {
            GeometryReader { geometry in
    ...
    private func updateTabScrollContent(frame: CGRect) {
        ...
        tabContentWidthExcludingSplitButtonLane = frame.width
    }

    @ViewBuilder
    private func splitButtonRow(
        buttons: [BonsplitConfiguration.SplitActionButton],
        registersHitRegions: Bool = false,
        showsHoverHighlight: Bool = false
    ) -> some View {
