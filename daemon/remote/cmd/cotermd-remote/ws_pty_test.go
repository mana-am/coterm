...
// for https://github.com/emergent-inc/coterm/issues/5185, where the failure
...
func TestWebSocketPTYRequiresSessionMatchAndConsumesLeaseOnce(t *testing.T) {
	...
	writeTestLease(t, leasePath, "coterm-secret", "sess-good", true, time.Now().Add(time.Minute))
	conn := dialPTY(t, ctx, server.URL)
	sendAuth(t, ctx, conn, "coterm-secret", "sess-other", 80, 24)
	_, _, err := conn.Read(ctx)
	if websocket.CloseStatus(err) != websocket.StatusPolicyViolation {
	...
	conn = dialPTY(t, ctx, server.URL)
	sendAuth(t, ctx, conn, "coterm-secret", "sess-good", 100, 30)
...
func TestWebSocketPTYRunsShellOverBinaryFrames(t *testing.T) {
	...
	output := waitForBinaryContains(t, ctx, conn, "COTERM_WS_OK", 15*time.Second)
...
func TestWebSocketPTYReconnectKeepsSessionProcess(t *testing.T) {
	...
	if err := conn.Write(ctx, websocket.MessageBinary, []byte("COTERM_RECONNECT_MARKER=alive; export COTERM_RECONNECT_MARKER; printf 'first-ready\\n'\r")); err != nil {
...
func TestWebSocketPTYStressSessionCleanupAndBoundedScrollback(t *testing.T) {
	...
	for i := 0; i < 25; i++ {
		...
		waitForBinaryContainsLabel(t, ctx, conn, "stress session "+sessionID+" hold marker", "COTERM_HOLD", 10*time.Second)
...
func TestWebSocketPTYAnonymousAttachesAreIsolated(t *testing.T) {
	...
	readReady(t, ctx, a)
	if err := a.Write(ctx, websocket.MessageBinary, []byte("COTERM_ANON_MARK=one; export COTERM_ANON_MARK; printf 'A_READY\\n'\r")); err != nil {
...
func TestWebSocketPTYAnonymousSessionKeyCannotBeForged(t *testing.T) {
	...
	if err := anon.Write(ctx, websocket.MessageBinary, []byte("COTERM_FORGE_MARK=anon; export COTERM_FORGE_MARK; printf 'ANON_FORGE_READY\\n'\r")); err != nil {
...
func TestWebSocketPTYAttachmentWithoutSessionIDIsAnonymous(t *testing.T) {
	...
	readReady(t, ctx, a)
	if err := a.Write(ctx, websocket.MessageBinary, []byte("COTERM_NO_SESSION_MARK=one; export COTERM_NO_SESSION_MARK; printf 'NO_SESSION_A_READY\\n'\r")); err != nil {
...
func TestWebSocketPTYSeedsUTF8LocaleAndTerminalEnv(t *testing.T) {
	...
	command := "printf '%s\\n' \"$LANG|$LC_CTYPE|$LC_ALL|$TERM|$COLORTERM|$TERM_PROGRAM|$COTERM_REMOTE_TRANSPORT\"; locale charmap; exit\r"
