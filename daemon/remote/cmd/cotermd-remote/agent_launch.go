...
const claudeNodeOptionsRestoreModuleScript = `const hadOriginalNodeOptions = process.env.COTERM_ORIGINAL_NODE_OPTIONS_PRESENT === "1";
if (hadOriginalNodeOptions) {
  process.env.NODE_OPTIONS = process.env.COTERM_ORIGINAL_NODE_OPTIONS ?? "";
} else {
  delete process.env.NODE_OPTIONS;
}
delete process.env.COTERM_ORIGINAL_NODE_OPTIONS;
delete process.env.COTERM_ORIGINAL_NODE_OPTIONS_PRESENT;
`

// runClaudeTeamsRelay implements `coterm claude-teams` on the remote side.
...
func runClaudeTeamsRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "coterm claude-teams: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	configureAgentEnvironment(agentConfig{
		...
		focused:        focused,
		tmuxPathPrefix: "coterm-claude-teams",
		cotermBinEnvVar:  "COTERM_CLAUDE_TEAMS_COTERM_BIN",
		termEnvVar:     "COTERM_CLAUDE_TEAMS_TERM",
	...
	if claudePath == "" {
		fmt.Fprintf(os.Stderr, "coterm claude-teams: claude not found in PATH\n")
		return 1
	}
	argv := append([]string{claudePath}, launchArgs...)
	execErr := syscall.Exec(claudePath, argv, os.Environ())
	fmt.Fprintf(os.Stderr, "coterm claude-teams: exec failed: %v\n", execErr)
	return 1
}

// runOMORelay implements `coterm omo` on the remote side.
func runOMORelay(socketPath string, args []string, refreshAddr func() string) int {
	rc := &rpcContext{socketPath: socketPath, refreshAddr: refreshAddr}

	shimDir, err := createOMOShimDir()
	if err != nil {
		fmt.Fprintf(os.Stderr, "coterm omo: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if opencodePath == "" {
		fmt.Fprintf(os.Stderr, "coterm omo: opencode not found in PATH\n"+
	...
	if err := omoEnsurePlugin(originalPath); err != nil {
		fmt.Fprintf(os.Stderr, "coterm omo: plugin setup: %v\n", err)
		return 1
	}
	...
	configureAgentEnvironment(agentConfig{
		...
		socketPath:     socketPath,
		focused:        focused,
		tmuxPathPrefix: "coterm-omo",
		cotermBinEnvVar:  "COTERM_OMO_COTERM_BIN",
		termEnvVar:     "COTERM_OMO_TERM",
		extraEnv:       map[string]string{},
	})
	...
	fmt.Fprintf(os.Stderr, "coterm omo: exec failed: %v\n", execErr)
	return 1
}

// runOMXRelay implements `coterm omx` on the remote side.
func runOMXRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "coterm omx: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if omxPath == "" {
		fmt.Fprintf(os.Stderr, "coterm omx: omx not found in PATH\n"+
	...
	configureAgentEnvironment(agentConfig{
		...
		socketPath:     socketPath,
		focused:        focused,
		tmuxPathPrefix: "coterm-omx",
		cotermBinEnvVar:  "COTERM_OMX_COTERM_BIN",
		termEnvVar:     "COTERM_OMX_TERM",
		extraEnv:       map[string]string{},
	})

	launchPath, launchArgv := resolveNodeScriptExec(omxPath, args, originalPath, shimDir)
	execErr := syscall.Exec(launchPath, launchArgv, os.Environ())
	fmt.Fprintf(os.Stderr, "coterm omx: exec failed: %v\n", execErr)
	return 1
}

// runOMCRelay implements `coterm omc` on the remote side.
func runOMCRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "coterm omc: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if omcPath == "" {
		fmt.Fprintf(os.Stderr, "coterm omc: omc not found in PATH\n"+
	...
	configureAgentEnvironment(agentConfig{
		...
		socketPath:     socketPath,
		focused:        focused,
		tmuxPathPrefix: "coterm-omc",
		cotermBinEnvVar:  "COTERM_OMC_COTERM_BIN",
		termEnvVar:     "COTERM_OMC_TERM",
		extraEnv:       map[string]string{},
	})
	...
	if restoreModulePath, err := ensureClaudeNodeOptionsRestoreModule(); err == nil {
		...
		fmt.Fprintf(os.Stderr, "coterm omc: warning: failed to create NODE_OPTIONS restore module: %v\n", err)
	}

	launchPath, launchArgv := resolveNodeScriptExec(omcPath, args, originalPath, shimDir)
	execErr := syscall.Exec(launchPath, launchArgv, os.Environ())
	fmt.Fprintf(os.Stderr, "coterm omc: exec failed: %v\n", execErr)
	return 1
}
...
set -euo pipefail
exec "${COTERM_CLAUDE_TEAMS_COTERM_BIN:-coterm}" __tmux-compat "$@"
`
...
exec "${COTERM_OMO_COTERM_BIN:-coterm}" __tmux-compat "$@"
`
...
exec "${COTERM_OMX_COTERM_BIN:-coterm}" __tmux-compat "$@"
`
...
exec "${COTERM_OMC_COTERM_BIN:-coterm}" __tmux-compat "$@"
`

const omoNotifierShimScript = `#!/usr/bin/env bash
# Intercept terminal-notifier calls and route through coterm notify.
TITLE="" BODY=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
exec "${COTERM_OMO_COTERM_BIN:-coterm}" notify --title "${TITLE:-OpenCode}" --body "${BODY:-}"
`
...
func createTmuxShimDir(dirName string, tmuxScript string) (string, error) {
	...
	dir := filepath.Join(home, ".coterm", dirName)
...
func ensureClaudeNodeOptionsRestoreModule() (string, error) {
	dir := filepath.Join(os.TempDir(), "coterm-claude-node-options")
	if err := os.MkdirAll(dir, 0755); err != nil {
		return "", err
	}
...
func configureClaudeNodeOptions(restoreModulePath string) {
	...
	if hadExisting {
		os.Setenv("COTERM_ORIGINAL_NODE_OPTIONS_PRESENT", "1")
		os.Setenv("COTERM_ORIGINAL_NODE_OPTIONS", existing)
	} else {
		os.Setenv("COTERM_ORIGINAL_NODE_OPTIONS_PRESENT", "0")
		os.Unsetenv("COTERM_ORIGINAL_NODE_OPTIONS")
	}
...
func configureAgentEnvironment(cfg agentConfig) {
	...
	os.Setenv(cfg.cotermBinEnvVar, selfPath)
	...
	// Socket path
	os.Setenv("COTERM_SOCKET_PATH", cfg.socketPath)
	os.Unsetenv("COTERM_SOCKET")
	...
	if cfg.focused != nil {
		os.Setenv("COTERM_WORKSPACE_ID", cfg.focused.workspaceId)
		if cfg.focused.surfaceId != "" {
			os.Setenv("COTERM_SURFACE_ID", cfg.focused.surfaceId)
		}
	}
...
func omoShadowConfigDir() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".coterm", "omo-config")
}
