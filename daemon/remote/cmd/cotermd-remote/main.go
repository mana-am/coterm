...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "cotermd-remote") || len(args) == 0 {
		return false
	}
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  cotermd-remote version")
	_, _ = fmt.Fprintln(w, "  cotermd-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  cotermd-remote serve --stdio --persistent --slot <slot>")
	_, _ = fmt.Fprintln(w, "  cotermd-remote serve --ws --auth-lease-file <path> [--rpc-auth-lease-file <path>] [--listen 127.0.0.1:7777]")
	_, _ = fmt.Fprintln(w, "  cotermd-remote cli <command> [args...]")
}
...
const (
	...
	persistentDaemonReadyFDEnv    = "COTERM_REMOTE_DAEMON_READY_FD"
...
func persistentDaemonPathsForSlot(rawSlot string) (persistentDaemonPaths, error) {
	...
	rootBase := strings.TrimSpace(os.Getenv("COTERM_REMOTE_DAEMON_ROOT"))
	if rootBase == "" {
		home, homeErr := os.UserHomeDir()
		if homeErr != nil || strings.TrimSpace(home) == "" {
			return persistentDaemonPaths{}, errors.New("cannot resolve remote home directory")
		}
		rootBase = filepath.Join(home, ".coterm", "daemon")
	}
...
func persistentDaemonSocketPath(root string, slot string) string {
	...
	if !overrideSet {
		socketBase = filepath.Join("/tmp", fmt.Sprintf("cotermd-remote-%d", os.Getuid()))
	}
	digest := sha256.Sum256([]byte(root + "\x00" + slot))
	return filepath.Join(socketBase, "cotermd-"+hex.EncodeToString(digest[:8])+".sock")
}

func persistentDaemonSocketBase() (string, bool) {
	socketBase := strings.TrimSpace(os.Getenv("COTERM_REMOTE_DAEMON_SOCKET_DIR"))
	if socketBase == "" {
		return "", false
	}
	return filepath.Join(socketBase, fmt.Sprintf("cotermd-remote-%d", os.Getuid())), true
}
...
func createPersistentDaemonFallbackSocketDir(root string) (string, error) {
	for attempt := 0; attempt < 8; attempt++ {
		...
		socketDir := filepath.Join(
			os.TempDir(),
			fmt.Sprintf("cotermd-remote-%d-%s", os.Getuid(), hex.EncodeToString(raw)),
		)
...
func signalPersistentDaemonReady() {
	...
	file := os.NewFile(uintptr(fd), "coterm-persistent-daemon-ready")
	if file == nil {
		return
	}
...
func (s *rpcServer) handleNotificationResponse(req rpcRequest, resp rpcResponse) error {
	...
	if s.frameWriter == nil {
		...
		_, _ = fmt.Fprintf(os.Stderr, "cotermd-remote: %s notification failed without response writer: %s\n", req.Method, detail)
		return nil
	}
