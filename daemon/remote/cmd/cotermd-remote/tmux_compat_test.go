...
func TestTmuxWaitForSignalPath(t *testing.T) {
	...
	if !strings.HasPrefix(path, "/tmp/coterm-wait-for-") {
...
func TestTmuxDisplayReporterFormatFields(t *testing.T) {
	origHome := os.Getenv("HOME")
	origWorkspace := os.Getenv("COTERM_WORKSPACE_ID")
	origSurface := os.Getenv("COTERM_SURFACE_ID")
	origPane := os.Getenv("TMUX_PANE")
	os.Setenv("HOME", t.TempDir())
	os.Setenv("COTERM_WORKSPACE_ID", "workspace:1")
	os.Setenv("COTERM_SURFACE_ID", "surface:1")
	leaderPaneToken := "%" + tmuxStableNumericId("33333333-3333-4333-8333-333333333333")
	os.Setenv("TMUX_PANE", leaderPaneToken)
	defer func() {
		os.Setenv("HOME", origHome)
		if origWorkspace != "" {
			os.Setenv("COTERM_WORKSPACE_ID", origWorkspace)
		} else {
			os.Unsetenv("COTERM_WORKSPACE_ID")
		}
		if origSurface != "" {
			os.Setenv("COTERM_SURFACE_ID", origSurface)
		} else {
			os.Unsetenv("COTERM_SURFACE_ID")
		}
		if origPane != "" {
	...
	if values["session_name"] != "coterm" {
		t.Fatalf("session_name = %q, want coterm", values["session_name"])
	}
...
func TestConfigureAgentEnvironment(t *testing.T) {
	...
	envKeys := []string{
		"COTERM_CLAUDE_TEAMS_COTERM_BIN", "PATH", "TMUX", "TMUX_PANE",
		"TERM", "COTERM_SOCKET_PATH", "TERM_PROGRAM",
		"COTERM_WORKSPACE_ID", "COTERM_SURFACE_ID",
	...
	// Verify socket path
	if os.Getenv("COTERM_SOCKET_PATH") != "127.0.0.1:54321" {
		t.Errorf("COTERM_SOCKET_PATH = %q", os.Getenv("COTERM_SOCKET_PATH"))
	}
	...
	if os.Getenv("COTERM_WORKSPACE_ID") != "ws-abc" {
		t.Errorf("COTERM_WORKSPACE_ID = %q", os.Getenv("COTERM_WORKSPACE_ID"))
	}
	if os.Getenv("COTERM_SURFACE_ID") != "surf-789" {
		t.Errorf("COTERM_SURFACE_ID = %q", os.Getenv("COTERM_SURFACE_ID"))
	}
	// Verify extra env
