...
// runCLI is the entry point for the "cli" subcommand (or busybox "coterm" invocation).
func runCLI(args []string) int {
	socketPath := os.Getenv("COTERM_SOCKET_PATH")
	...
	for i := 0; i < len(args); i++ {
		...
		case "--socket":
			if i+1 >= len(args) {
				fmt.Fprintln(os.Stderr, "coterm: --socket requires a path")
				return 2
			}
			socketPath = args[i+1]
			i++
...
doneFlags:
	...
	if socketPath == "" {
		fmt.Fprintln(os.Stderr, "coterm: COTERM_SOCKET_PATH not set and --socket not provided")
		return 1
	}
	...
	if cmdName == "workspace" {
		...
		fmt.Fprintln(os.Stderr, "coterm workspace: only the \"group\" subcommand is supported here. Use list-workspaces, new-workspace, close-workspace, or select-workspace for workspace operations.")
		return 2
	}
	...
	if !ok {
		fmt.Fprintf(os.Stderr, "coterm: unknown command %q\n", cmdName)
		return 2
	}
	...
	default:
		fmt.Fprintf(os.Stderr, "coterm: internal error: unknown protocol for %q\n", cmdName)
		return 1
	}
}
...
func execV1(socketPath string, spec *commandSpec, args []string, refreshAddr func() string) int {
	...
	if !spec.noParams {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "coterm: %v\n", err)
			return 2
		}
...
func runRPC(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "coterm rpc: requires a method name")
		return 2
	}
	method := args[0]
	...
	if len(args) > 1 {
		if err := json.Unmarshal([]byte(args[1]), &params); err != nil {
			fmt.Fprintf(os.Stderr, "coterm rpc: invalid JSON params: %v\n", err)
			return 2
		}
	}
...
// runWorkspaceGroupRelay handles "coterm workspace group <sub>" (and the
// "workspace-group" alias) by mapping each subcommand to its
// workspace.group.* v2 method, mirroring the macOS coterm CLI flags.
func runWorkspaceGroupRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	...
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "coterm workspace group: requires a subcommand (%s)\n", subcommandHint)
		return 2
	}
	sub := args[0]
	flagKeys, ok := workspaceGroupFlagKeys[sub]
	if !ok {
		fmt.Fprintf(os.Stderr, "coterm workspace group: unknown subcommand %q (%s)\n", sub, subcommandHint)
		return 2
	}

	fail := func(format string, a ...any) int {
		fmt.Fprintf(os.Stderr, "coterm workspace group %s: %s\n", sub, fmt.Sprintf(format, a...))
		return 2
	}
...
// runBrowserRelay handles "Coterm browser <subcommand>" by mapping to browser.* v2 methods.
func runBrowserRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "Coterm browser: requires a subcommand (%s)\n", browserSubcommandHint())
		return 2
	}
	...
	if !ok {
		fmt.Fprintf(os.Stderr, "Coterm browser: unknown subcommand %q\n", sub)
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Coterm browser: %v\n", err)
		return 2
	}
	for _, key := range spec.flagKeys {
		if val, ok := parsed.flags[key]; ok {
...
func applyWorkspaceEnvFallback(params map[string]any) {
	...
	if envWs := os.Getenv("COTERM_WORKSPACE_ID"); envWs != "" {
...
func applySurfaceEnvFallback(params map[string]any) {
	...
	if envSf := os.Getenv("COTERM_SURFACE_ID"); envSf != "" {
...
// readSocketAddrFile reads the socket address from ~/.coterm/socket_addr as a fallback
// when COTERM_SOCKET_PATH is not set. Written by the coterm app after the relay establishes.
func readSocketAddrFile() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	data, err := os.ReadFile(filepath.Join(home, ".coterm", "socket_addr"))
	if err != nil {
		return ""
	}
...
func readRelayAuthFile(socketPath string) *relayAuthState {
	if strings.Contains(socketPath, ":") && !strings.HasPrefix(socketPath, "/") {
		...
		data, err := os.ReadFile(filepath.Join(home, ".coterm", "relay", port+".auth"))
		if err != nil {
			return nil
		}
...
func currentRelayAuth(socketPath string) *relayAuthState {
	relayID := strings.TrimSpace(os.Getenv("COTERM_RELAY_ID"))
	relayToken := strings.TrimSpace(os.Getenv("COTERM_RELAY_TOKEN"))
...
// dialSocket connects to the coterm socket. If addr contains a colon and doesn't
...
func authenticateRelayConn(conn net.Conn, auth *relayAuthState) error {
	...
	if challenge.Protocol != "coterm-relay-auth" || challenge.Version != 1 || challenge.RelayID != auth.RelayID || challenge.Nonce == "" {
...
func cliUsage() {
	fmt.Fprintln(os.Stderr, "Usage: coterm [--socket <path>] [--json] <command> [args...]")
	...
	fmt.Fprintln(os.Stderr, "  browser <sub>             Browser commands through the local Coterm browser relay")
	fmt.Fprintln(os.Stderr, "  claude-teams [args...]     Launch Claude Code in teammate mode")
	fmt.Fprintln(os.Stderr, "  omo [args...]              Launch OpenCode with coterm integration")
	fmt.Fprintln(os.Stderr, "  omx [args...]              Launch Oh My Codex with coterm integration")
	fmt.Fprintln(os.Stderr, "  omc [args...]              Launch Oh My Claude Code with coterm integration")
