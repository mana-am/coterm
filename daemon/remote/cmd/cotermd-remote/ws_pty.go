...
func runWebSocketPTYServer(ctx context.Context, cfg wsPTYServerConfig, stderr io.Writer) error {
	...
	_, _ = fmt.Fprintf(stderr, "cotermd-remote ws listening on %s\n", listener.Addr().String())
...
func defaultWebSocketPTYEnv(shellPath string) []string {
	...
	set("COTERM_REMOTE_TRANSPORT", "ws")
...
func (h *wsPTYHub) startSessionLocked(sessionKey wsPTYSessionKey, sessionID string, cols int, rows int, command string) (*wsPTYSession, error) {
	...
	if trimmedCommand == "" {
		...
		f, err := os.CreateTemp("", "cotermd-startup-*.sh")
		if err != nil {
...
func newPTYAllocationError(err error) error {
	...
	if isPermissionDeniedErr(err) {
		hint = "; the remote devpts denies /dev/ptmx (e.g. mounted ptmxmode=000): remount it writable with `sudo mount -o remount,ptmxmode=0666 /dev/pts` or expose a writable /dev/ptmx so the coterm daemon can open a terminal"
	}
