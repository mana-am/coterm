...
final class TaskManagerResourcesTests: XCTestCase {
    func testGrokCodingAgentDefinitionMatchesSymlinkLaunch() throws {
        let definition = try XCTUnwrap(CotermTaskManagerCodingAgentDefinition.matchingDefinition(
            processName: "grok",
    ...
    func testParsesTypedIntPIDArrayFromSummaryPayload() {
        ...
        let resources = CotermTaskManagerResources(payload)
    ...
    func testSortOrderSortsSiblingRowsByCPUWhilePreservingHierarchy() {
        ...
        let sortedRows = CotermTaskManagerSortOrder.defaultOrder.sortedRows(rows)
    ...
    func testSortOrderUsesAscendingNameSortWhenNameColumnIsSelected() {
        ...
        let sortedRows = CotermTaskManagerSortOrder(
            column: .name,
            direction: .ascending
        ).sortedRows(rows)
    ...
    func testSortOrderSortsMemoryAndProcessColumnsDescending() {
        ...
        let memorySortedRows = CotermTaskManagerSortOrder(
            column: .memory,
            direction: .descending
        ).sortedRows(rows)
        let processSortedRows = CotermTaskManagerSortOrder(
            column: .processes,
            direction: .descending
        ).sortedRows(rows)
    ...
    func testSortOrderTogglesCurrentColumnAndUsesMetricDefaultsForNewColumns() {
        let cpuDescending = CotermTaskManagerSortOrder.defaultOrder
        ...
        XCTAssertEqual(cpuAscending, CotermTaskManagerSortOrder(column: .cpu, direction: .ascending))
        XCTAssertEqual(memoryDescending, CotermTaskManagerSortOrder(column: .memory, direction: .descending))
        XCTAssertEqual(nameAscending, CotermTaskManagerSortOrder(column: .name, direction: .ascending))
    }

    func testSnapshotBuildsProgramAggregateRowsForRepeatedProcessNames() throws {
        let snapshot = CotermTaskManagerSnapshot(
            rows: [
                taskManagerRow(
                    "node-101-a",
                    kind: .process,
    ...
    func testSnapshotParsesProgramTotalsFromTopPayloadWithoutProcessRows() throws {
        let snapshot = CotermTaskManagerSnapshot(payload: [
    ...
    func testSnapshotOnlyCountsAsLoadedAfterSamplingOrRowsArrive() {
        XCTAssertFalse(CotermTaskManagerSnapshot.empty.hasLoadedResourceUsage)

        let sampledSnapshot = CotermTaskManagerSnapshot(
            rows: [],
            total: .zero,
    ...
    func testCodingAgentMatcherUsesSupportedAgentNamesAndLaunchMetadata() {
        XCTAssertEqual(
            CotermTaskManagerCodingAgentDefinition.matchingDefinition(
                processName: "node",
                processPath: nil,
        ...
        XCTAssertTrue(CotermTaskManagerCodingAgentDefinition.shouldReadArguments(
            processName: "2.1.140",
        ...
        XCTAssertNil(CotermTaskManagerCodingAgentDefinition.matchingDefinition(
            processName: "2.1.140",
            processPath: nil,
            arguments: [],
            environment: [:]
        ...
        XCTAssertEqual(
            CotermTaskManagerCodingAgentDefinition.matchingDefinition(
                ...
                environment: ["COTERM_AGENT_LAUNCH_KIND": "claudeTeams"]
            )?.id,
            "claude"
        )
        XCTAssertEqual(
            CotermTaskManagerCodingAgentDefinition.matchingDefinition(
                ...
                environment: ["COTERM_AGENT_LAUNCH_KIND": "omo"]
            )?.id,
            "opencode"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            CotermTaskManagerCodingAgentDefinition.matchingDefinition(
                ...
                processPath: nil,
                arguments: ["node", "agent.js"],
                environment: ["COTERM_AGENT_LAUNCH_KIND": "omx"]
            )?.id,
            "codex"
        )
        XCTAssertNil(CotermTaskManagerCodingAgentDefinition.matchingDefinition(
    ...
    private func resourceSummary() -> CotermTopResourceSummary {
    ...
    private func taskManagerRow(
        _ id: String,
        kind: CotermTaskManagerRow.Kind = .workspace,
        level: Int,
        title: String? = nil,
        cpuPercent: Double = 0,
