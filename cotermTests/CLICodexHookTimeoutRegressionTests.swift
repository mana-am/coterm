...
struct CLICodexHookTimeoutRegressionTests {
    ...
    @Test func codexHookInstallReplacesSynchronousBundledHook() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-sync-hook-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let previousCommand = "coterm_cli=\"${COTERM_BUNDLED_CLI_PATH:-}\"; if [ -z \"$coterm_cli\" ] || [ ! -x \"$coterm_cli\" ]; then coterm_cli=\"$(command -v coterm 2>/dev/null || true)\"; fi; if [ -n \"$COTERM_SURFACE_ID\" ] && [ \"$COTERM_CODEX_HOOKS_DISABLED\" != \"1\" ] && [ -n \"$coterm_cli\" ]; then { if [ -n \"${COTERM_SOCKET_PATH:-}\" ]; then \"$coterm_cli\" --socket \"$COTERM_SOCKET_PATH\" hooks codex prompt-submit; else \"$coterm_cli\" hooks codex prompt-submit; fi; } || echo '{}'; else echo '{}'; fi"
        ...
        #expect(sessionStartCommands.allSatisfy { $0.contains("agent_pid=") && $0.contains("COTERM_CODEX_PID=") })
        ...
        #expect(promptCommands.allSatisfy { $0.contains("agent_pid=") && $0.contains("COTERM_CODEX_PID=") })
    ...
    @Test func codexInstalledHookReturnsBeforeSlowCotermCommandFinishes() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-hook-async-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("coterm", isDirectory: false)
        ...
        try makeExecutableShellFile(at: fakeCLI, lines: [
            "#!/bin/sh",
            "printf '%s\\n' \"$*\" > \"$COTERM_TEST_ARGS\"",
            "printf '%s\\n' \"$COTERM_CODEX_PID\" > \"$COTERM_TEST_PID\"",
            "cat > \"$COTERM_TEST_STDIN\"",
            "sleep 2",
            "printf done > \"$COTERM_TEST_DONE\"",
        ])
        ...
        let run = runProcess(
            ...
            environment: [
                ...
                "TMPDIR": root.path,
                "COTERM_SURFACE_ID": "surface-123",
                "COTERM_SOCKET_PATH": "/tmp/coterm-test.sock",
                "COTERM_BUNDLED_CLI_PATH": fakeCLI.path,
                "COTERM_CODEX_PID": "4242",
                "COTERM_TEST_STDIN": capturedStdin.path,
                "COTERM_TEST_ARGS": capturedArgs.path,
                "COTERM_TEST_PID": capturedPID.path,
                "COTERM_TEST_DONE": doneFile.path,
            ],
        ...
        #expect(waitForFile(capturedArgs, containing: "--socket /tmp/coterm-test.sock hooks codex prompt-submit", timeout: 1))
    ...
    @Test func codexPromptSubmitDoesNotReviveStoppedTurn() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-stale-prompt-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexSessionStartDoesNotOverwriteExistingTurnState() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-stale-start-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexSessionStartRefreshesCompletedPriorTurn() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-fresh-start-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexSessionStartDoesNotReviveCompletedTurnFromSamePID() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-same-pid-start-\(UUID().uuidString)", isDirectory: true)
    ...
    private func bindUnixSocket(at path: String) throws -> Int32 {
        ...
        guard fd >= 0 else {
            throw NSError(domain: "coterm.tests", code: Int(errno))
        }
        ...
        guard utf8.count < maxPathLength else {
            Darwin.close(fd)
            throw NSError(domain: "coterm.tests", code: Int(ENAMETOOLONG))
        }
        ...
        guard bindResult == 0, Darwin.listen(fd, 8) == 0 else {
            let code = errno
            Darwin.close(fd)
            throw NSError(domain: "coterm.tests", code: Int(code))
        }
        return fd
    }
