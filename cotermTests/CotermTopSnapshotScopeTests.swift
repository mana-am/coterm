...
final class CotermTopSnapshotScopeTests: XCTestCase {
    ...
    func testWindowRollupMatchesPSForApplicationProcessTree() throws {
        ...
        let snapshot = CotermTopProcessSnapshot.capture(includeProcessDetails: false)
    ...
    func testApplicationProcessTreeIsExposedAtWindowLevel() throws {
        ...
        let snapshot = CotermTopProcessSnapshot.capture(includeProcessDetails: true)
        var windows: [[String: Any]] = [[
            "kind": "window",
            "id": UUID().uuidString,
            "index": 0,
    ...
    func testSamplePayloadDescribesPhysicalFootprintFallbackSource() {
        let sample = CotermTopProcessSnapshot.capture(includeProcessDetails: false).samplePayload()

        XCTAssertEqual(
            sample["memory_source"] as? String,
            CotermTopProcessMemorySource.physicalFootprint.rawValue
        )
        XCTAssertEqual(
            sample["memory_fallback_source"] as? String,
            CotermTopProcessMemorySource.residentSize.rawValue
        )
        XCTAssertEqual(
            sample["resident_memory_fallback_source"] as? String,
            CotermTopProcessMemorySource.rusageResidentSize.rawValue
        )
        XCTAssertEqual(sample["coterm_scope"] as? Bool, true)

        let unscopedSnapshot = CotermTopProcessSnapshot(
            processes: [],
            sampledAt: Date(timeIntervalSince1970: 0),
            includesProcessDetails: false,
            includesCotermScope: false
        )
        XCTAssertEqual(unscopedSnapshot.samplePayload()["coterm_scope"] as? Bool, false)

        let fallbackSnapshot = CotermTopProcessSnapshot(
            processes: [
                CotermTopProcessInfo(
                    ...
                    path: nil,
                    ttyDevice: nil,
                    cotermWorkspaceID: nil,
                    cotermSurfaceID: nil,
                    cotermAttributionReason: nil,
                    processGroupID: nil,
    ...
    func testUnavailableMemorySourcesAreExposedInAggregatePayloads() throws {
        ...
        let snapshot = CotermTopProcessSnapshot(
            processes: [
                CotermTopProcessInfo(
                    pid: unavailablePID,
                    parentPID: 0,
    ...
    func testKernProcArgsWorkspaceID() {
        ...
        let scope = CotermTopProcessSnapshot.cotermScope(fromKernProcArgs: bytes)
    ...
    func testKernProcArgsTabIDFallback() {
        ...
        let bytes = kernProcArgs(environment: [
            "COTERM_TAB_ID=\(tabID.uuidString)"
        ])
    ...
    func testKernProcArgsSurfaceID() {
        ...
        let bytes = kernProcArgs(environment: [
            "COTERM_SURFACE_ID=\(surfaceID.uuidString)"
        ])
    ...
    func testCodexMonitorArgumentsSupportJoinedUUIDOptions() throws {
        ...
        let scope = try XCTUnwrap(CotermTopProcessSnapshot.cotermScope(
            arguments: [
        ...
        XCTAssertEqual(scope.workspaceID, workspaceID)
        XCTAssertEqual(scope.surfaceID, surfaceID)
        XCTAssertEqual(scope.attributionReason, "coterm-hook-arguments")
    }

    func testCodexMonitorArgumentsIgnorePathValuedSubcommandLookalikes() {
        let workspaceID = UUID(uuidString: "55555555-5555-5555-5555-555555555555")!

        let scope = CotermTopProcessSnapshot.cotermScope(
            arguments: [
                "/Applications/coterm.app/Contents/Resources/bin/coterm",
                "other",
                "/tmp/hooks",
    ...
    func testCodexMonitorArgumentsRequireCotermExecutable() {
        let workspaceID = UUID(uuidString: "55555555-5555-5555-5555-555555555555")!
    ...
    func testLaunchdParentedCodexMonitorArgumentsAttachToOwningSurface() throws {
        ...
        let scope = try XCTUnwrap(CotermTopProcessSnapshot.cotermScope(fromKernProcArgs: bytes))
        XCTAssertEqual(scope.workspaceID, workspaceID)
        XCTAssertEqual(scope.surfaceID, surfaceID)
        ...
        let snapshot = CotermTopProcessSnapshot(
            processes: [
                CotermTopProcessInfo(
                    ...
                    parentPID: 1,
                    name: "coterm",
                    path: "/Applications/coterm.app/Contents/Resources/bin/coterm",
                    ttyDevice: nil,
                    cotermWorkspaceID: scope.workspaceID,
                    cotermSurfaceID: scope.surfaceID,
                    cotermAttributionReason: scope.attributionReason,
                    processGroupID: nil,
                    terminalProcessGroupID: nil,
                    cpuPercent: 0,
        ...
        XCTAssertEqual(monitorProcess["attribution_reason"] as? String, "coterm-hook-arguments")
    ...
    private struct SpawnedProcessTree {
        ...
        static func start() throws -> SpawnedProcessTree {
            let directory = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("coterm-top-\(UUID().uuidString)", isDirectory: true)
