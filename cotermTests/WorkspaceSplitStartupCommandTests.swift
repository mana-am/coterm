...
private func waitForWorkspaceSplitView(
    ...
    XCTFail(
        "Timed out waiting for rendered coterm.json split ratio \(expectedDividerPosition); last ratio: \(lastRatioDescription)",
        file: file,
        line: line
    )
...
final class WorkspaceSplitStartupCommandTests: XCTestCase {
    func testCustomLayoutSplitRatioSurvivesInitialBonsplitViewLayout() throws {
        ...
        let layout = CotermLayoutNode.split(CotermSplitDefinition(
            ...
            children: [
                ...
                .pane(CotermPaneDefinition(surfaces: [
                    CotermSurfaceDefinition(type: .terminal, name: "Right")
                ]))
            ]
        ))
        ...
        XCTAssertEqual(
            ...
            accuracy: 0.000_1,
            "coterm.json split ratio should be applied to the Bonsplit model before rendering"
        )
        ...
        XCTAssertEqual(
            ...
            expectedDividerPosition,
            accuracy: 0.000_1,
            "Bonsplit initial view layout should not rewrite the coterm.json split ratio back to 0.5"
        )
    }
    ...
    func testTabManagerSplitCarriesRequestedWorkingDirectoryAndStartupCommand() {
        ...
        let requestedDirectory = "/tmp/coterm-split-startup-\(UUID().uuidString)"
        let startupCommand = "/tmp/coterm-tmux-command-\(UUID().uuidString).sh"
    ...
    func testNewTerminalSurfaceCarriesRequestedWorkingDirectoryAndStartupCommand() {
        ...
        let requestedDirectory = "/tmp/coterm-surface-startup-\(UUID().uuidString)"
        let startupCommand = "/tmp/coterm-surface-command-\(UUID().uuidString).sh"
        let tmuxStartCommand = "node /opt/oh-my-codex/dist/omx.js hud --watch"
    ...
    func testRespawnTerminalSurfacePreservesPaneTabAndSurfaceIdentity() throws {
        ...
        let requestedDirectory = "/tmp/coterm-respawn-\(UUID().uuidString)"
        let startupEnvironment = [
            "COTERM_OMO_SUBAGENT": "1",
        ...
        XCTAssertTrue(
            ...
            "Respawn should replace the registered terminal surface for the existing coterm surface id"
        )
    }
    ...
    func testSessionRestoreRelaunchesOMXHudTmuxStartCommand() throws {
        let workspace = Workspace()
        let sourcePanelId = try XCTUnwrap(workspace.focusedPanelId)
        let requestedDirectory = "/tmp/coterm-hud-restore-\(UUID().uuidString)"
        let originalStartupScript = "/tmp/coterm-tmux-command-\(UUID().uuidString).sh"
        ...
        XCTAssertTrue(restoredStartupScript.contains("coterm-session-terminal-command"))
    ...
    func testSessionSnapshotDoesNotPersistGenericTmuxStartCommand() throws {
        ...
        let panel = try XCTUnwrap(workspace.newTerminalSurface(
            inPane: paneId,
            focus: false,
            initialCommand: "/tmp/coterm-command-\(UUID().uuidString).sh",
