...
struct GhosttyDECCKMArrowKeyTests {
    ...
    func windowKeyEquivalentTerminalArrowsEncodeDECCKMBytes() throws {
        ...
        let captureReadyMarker = "COTERM_DECCKM_READY_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let captureMarker = "COTERM_DECCKM_HEX_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-decckm-arrow-capture-\(UUID().uuidString).py")
        let script = """
        import os
        import select
        import sys
