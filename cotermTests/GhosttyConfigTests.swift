...
// The app target still declares legacy duplicates of these CotermSettings
// value types; with CotermSettings imported unconditionally the names are
// ambiguous. These tests exercise the app-side paths, so pin the app types.
private typealias BrowserThemeMode = Coterm_DEV.BrowserThemeMode
#elseif canImport(Coterm)
@testable import Coterm
private typealias BrowserThemeMode = Coterm.BrowserThemeMode
#endif
...
final class GhosttyConfigTests: XCTestCase {
    ...
    func testLaunchGhosttyResourcesPreferCurrentBundleOverInheritedEnvironment() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-launch-resources-\(UUID().uuidString)")
        ...
        let resolved = cotermApp.resolvedGhosttyResourcesDirectory(
    ...
    func testLaunchGhosttyResourcesKeepInheritedEnvironmentWhenBundleHasNoResources() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-launch-resource-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLaunchGhosttyResourcesKeepInheritedEnvironmentWhenBundleLacksThemes() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-launch-incomplete-resource-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLaunchGhosttyResourcesUseIncompleteBundleOnlyAsLastFallback() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-launch-incomplete-resource-last-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testThemeSearchPathsIncludeXDGDataDirsThemes() {
        let pathA = "/tmp/coterm-theme-a"
        let pathB = "/tmp/coterm-theme-b"
    ...
    func testThemeSearchPathsIncludeCotermUserThemesDirectory() {
        let paths = GhosttyConfig.themeSearchPaths(
        ...
        XCTAssertTrue(
            paths.contains(
                "\(NSHomeDirectory())/Library/Application Support/coterm.com.emergent.app/themes/Zag Light"
            )
        )
    }

    func testThemeSearchPathsIncludeCotermUserThemesDirectoryFromFixedHome() {
        let fixedHome = "/tmp/coterm-fixed-home-\(UUID().uuidString)"
        let paths = GhosttyConfig.themeSearchPaths(
            forThemeName: "Zag Light",
        ...
        XCTAssertTrue(
            paths.contains(
                "\(fixedHome)/Library/Application Support/coterm.com.emergent.app/themes/Zag Light"
            )
        )
    }

    func testThemesListIncludesCotermUserThemesDirectory() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-user-theme-list-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }

        let themesDirectory = root
            .appendingPathComponent("Library/Application Support/coterm.com.emergent.app/themes", isDirectory: true)
    ...
    func testCotermDefaultThemeConfigContentsSkipsInvalidUTF8Candidate() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-managed-theme-search-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let contents = GhosttyConfig.cotermDefaultThemeConfigContents(
            preferredColorScheme: .light,
    ...
    func testLoadReadsSymlinkedGhosttyConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-config-symlink-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadEnforcesCursorDarkOverRecursiveColorConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-config-recursive-background-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        XCTAssertEqual(loaded.theme, GhosttyConfig.cotermEnforcedThemeName)
    ...
    func testLoadDoesNotReparseTopLevelConfigReferencedByConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-config-top-level-cycle-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadAllowsRecursiveConfigFileToReloadTopLevelConfigAsFinalOverride() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-config-top-level-reload-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadReadsOptionalQuotedConfigFilePath() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-config-optional-quoted-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadIgnoresLegacyAppSupportConfigWhenConfigGhosttyIsNonEmpty() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-app-support-legacy-skip-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadUsesLegacyAppSupportConfigWhenConfigGhosttyIsEmpty() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-app-support-legacy-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadEnforcesCursorDarkAfterUserThemeAndCursorColorOverride() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-theme-order-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadThemeReadsAbsoluteThemeFilePath() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-absolute-theme-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadThemeResolvesPairedThemeValueByColorScheme() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-theme-pair-\(UUID().uuidString)")
    ...
    func testLoadThemeResolvesBuiltinAliasFromGhosttyResourcesDir() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-themes-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedLightAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-solarized-light-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedDarkAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ghostty-solarized-dark-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testUnparsedAppearanceFallbackIgnoresNativeLegacyBaselineWhenCurrentConfigExists() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-native-legacy-baseline-\(UUID().uuidString)")
    ...
    func testUnparsedAppearanceFallbackKeepsNativeLegacyBaselineWhenCurrentConfigIsMissingOrEmpty() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-native-legacy-baseline-empty-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
    ...
    func testRuntimeColorSchemeForCotermSingleThemeReloadKeepsResolvedSchemeDuringConfigLoad() {
        XCTAssertEqual(
            GhosttyApp.runtimeColorSchemeForConfigLoad(
                ...
                cotermThemeValue: "light:3024 Day,dark:3024 Day"
            ),
            .light
        )
        XCTAssertEqual(
    ...
    func testRuntimeColorSchemeForPairedThemeReloadUsesAppearanceDuringConfigLoad() {
        XCTAssertEqual(
            GhosttyApp.runtimeColorSchemeForConfigLoad(
                ...
                requestedColorScheme: .dark,
                effectiveTerminalColorScheme: .light,
                cotermThemeValue: "light:3024 Day,dark:3024 Night"
            ),
            .dark
        )
        XCTAssertEqual(
    ...
    func testClaudeCodeIntegrationDefaultsToEnabledWhenUnset() {
        let suiteName = "coterm.tests.claude-hooks.\(UUID().uuidString)"
    ...
    func testKiroIntegrationDefaultsToEnabledWithStandardNotificationsWhenUnset() {
        let suiteName = "coterm.tests.kiro-hooks.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
    ...
    func testSubagentNotificationSuppressionDefaultsToEnabledWhenUnset() {
        let suiteName = "coterm.tests.subagent-notifications.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
    ...
    func testTelemetryDefaultsToEnabledWhenUnset() {
        let suiteName = "coterm.tests.telemetry.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
...
// cotermShouldApplyWindowGlass) were lifted out of the app target into
// CotermWorkspaceWindow's WindowBackgroundPolicy by the window-chrome tranche, and
// equivalent coverage now lives in
// Packages/macOS/CotermWorkspaceWindow/Tests/CotermWorkspaceWindowTests/WindowBackgroundPolicyTests.swift.
// The stale app-side test was left referencing the removed symbols, which broke
// the cotermTests compile on the Swift 6 depot toolchain.
...
final class WorkspaceRemoteDaemonManifestTests: XCTestCase {
    func testParsesEmbeddedRemoteDaemonManifestJSON() throws {
        let manifestJSON = """
        {
          ...
          "appVersion": "0.62.0",
          "releaseTag": "v0.62.0",
          "releaseURL": "https://github.com/emergent-inc/coterm/releases/tag/v0.62.0",
          "checksumsAssetName": "cotermd-remote-checksums.txt",
          "checksumsURL": "https://github.com/emergent-inc/coterm/releases/download/v0.62.0/cotermd-remote-checksums.txt",
          "entries": [
            {
              "goOS": "linux",
              "goArch": "amd64",
              "assetName": "cotermd-remote-linux-amd64",
              "downloadURL": "https://github.com/emergent-inc/coterm/releases/download/v0.62.0/cotermd-remote-linux-amd64",
              "sha256": "abc123"
            }
          ]
        }
        ...
        XCTAssertEqual(manifest?.entry(goOS: "linux", goArch: "amd64")?.assetName, "cotermd-remote-linux-amd64")
    }
    ...
    func testRemoteDaemonCachePathIsVersionedByPlatform() throws {
        ...
        XCTAssertTrue(url.path.contains("/.local/state/coterm/remote-daemons/0.62.0/linux-arm64/"))
        XCTAssertEqual(url.lastPathComponent, "cotermd-remote")
    }
}
...
final class RemoteLoopbackHTTPRequestRewriterTests: XCTestCase {
    func testRewritesLoopbackAliasHostHeadersToLocalhost() {
        let original = Data(
            (
                "GET /demo HTTP/1.1\r\n" +
                "Host: coterm-loopback.localtest.me:3000\r\n" +
                "Origin: http://coterm-loopback.localtest.me:3000\r\n" +
                "Referer: http://coterm-loopback.localtest.me:3000/app\r\n" +
                "\r\n"
            ).utf8
        )
        ...
        let rewritten = RemoteLoopbackHTTPRequestRewriter.rewriteIfNeeded(
            data: original,
            aliasHost: "coterm-loopback.localtest.me"
        )
        ...
        XCTAssertFalse(text.contains("coterm-loopback.localtest.me"))
    }
    ...
    func testRewritesLoopbackSubdomainAliasHostHeadersToOriginalLocalhostSubdomain() {
        let original = Data(
            (
                "GET /demo HTTP/1.1\r\n" +
                "Host: api.coterm-loopback.localtest.me:3000\r\n" +
                "Origin: http://api.coterm-loopback.localtest.me:3000\r\n" +
                "Referer: http://api.coterm-loopback.localtest.me:3000/app\r\n" +
                "\r\n"
            ).utf8
        )
        ...
        XCTAssertFalse(text.contains("api.coterm-loopback.localtest.me"))
    }

    func testRewritesAbsoluteFormRequestLineForLoopbackAlias() {
        let original = Data(
            (
                "GET http://coterm-loopback.localtest.me:3000/demo HTTP/1.1\r\n" +
                "Host: coterm-loopback.localtest.me:3000\r\n" +
                "\r\n"
            ).utf8
        )
    ...
    func testRewritesLoopbackResponseHeadersBackToAlias() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Location: http://coterm-loopback.localtest.me:3000/login"))
        XCTAssertTrue(text.contains("Access-Control-Allow-Origin: http://coterm-loopback.localtest.me:3000"))
        XCTAssertTrue(text.contains("Set-Cookie: sid=1; Domain=coterm-loopback.localtest.me; Path=/"))
    }
    ...
    func testRewritesLoopbackSubdomainResponseHeadersBackToAliasSubdomain() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Location: http://api.coterm-loopback.localtest.me:3000/login"))
        XCTAssertTrue(text.contains("Access-Control-Allow-Origin: http://api.coterm-loopback.localtest.me:3000"))
        XCTAssertTrue(text.contains("Set-Cookie: sid=1; Domain=api.coterm-loopback.localtest.me; Path=/"))
    }
    ...
    func testRewritesLeadingDotLoopbackCookieDomainsBackToAliasDomains() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Set-Cookie: root=1; Domain=.coterm-loopback.localtest.me; Path=/"))
        XCTAssertTrue(text.contains("Set-Cookie: api=1; Domain=.api.coterm-loopback.localtest.me; Path=/"))
    }
}
...
final class BrowserPanelWebViewLifecycleTests: XCTestCase {
    func testHiddenDiscardPolicyReadsUserDefaults() throws {
        let suiteName = "coterm.browserHiddenDiscardPolicyTests.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer {
            defaults.removePersistentDomain(forName: suiteName)
        }
        let hasEnabledEnvironmentOverride =
            ProcessInfo.processInfo.environment["COTERM_BROWSER_HIDDEN_WEBVIEW_DISCARD_ENABLED"] != nil
        let hasDelayEnvironmentOverride =
            ProcessInfo.processInfo.environment["COTERM_BROWSER_HIDDEN_WEBVIEW_DISCARD_DELAY_SECONDS"] != nil
...
final class BrowserDefaultsNormalizationTests: XCTestCase {
    ...
    func testNormalizeRewritesOutOfRangeAndLegacyValues() throws {
        let suiteName = "coterm.browserDefaultsNormalizationTests.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
    ...
    func testNormalizePreservesValidValues() throws {
        ...
        // `CotermSettings`, which declares a same-named enum.
...
final class BrowserNewTabNavigationSeedTests: XCTestCase {
    func testPreservesOriginalRequestHeadersMethodBodyAndBypassHost() throws {
        ...
        request.setValue("keep-me", forHTTPHeaderField: "X-Coterm-Test")
        ...
        XCTAssertEqual(
            seed.initialRequest.value(forHTTPHeaderField: "X-Coterm-Test"),
            "keep-me"
        )
...
final class BrowserPanelRemoteStoreTests: XCTestCase {
    ...
    func testRemoteWorkspaceDefersInitialNavigationUntilProxyEndpointIsReady() {
        ...
        XCTAssertEqual(panel.preferredURLStringForOmnibar(), url.absoluteString)
        XCTAssertEqual(panel.webView.url?.host, "coterm-loopback.localtest.me")
    }
    ...
    func testRemoteWorkspacePreservesLocalhostSubdomainWhenAliasingLoopbackURL() {
        ...
        XCTAssertEqual(panel.preferredURLStringForOmnibar(), url.absoluteString)
        XCTAssertEqual(panel.webView.url?.host, "api.coterm-loopback.localtest.me")
    }
    ...
    func testRemoteWorkspaceRuntimeBridgeAliasesMultipleLoopbackPortsFromSamePage() async throws {
        ...
        let baseURL = try XCTUnwrap(URL(string: "http://coterm-loopback.localtest.me:3000/"))
        ...
        let result = try await panel.evaluateJavaScript(
            """
            (() => {
              const rewrite = window.__cotermRewriteRemoteLoopbackURL;
        ...
        XCTAssertEqual(
            result,
            #"["http://coterm-loopback.localtest.me:3000/frontend","http://coterm-loopback.localtest.me:8000/api","http://api.coterm-loopback.localtest.me:8000/v1","ws://coterm-loopback.localtest.me:5173/hmr","wss://localhost:5173/hmr","https://localhost:9443/secure"]"#
        )
    }
    ...
    func testBrowserMoveIntoRemoteWorkspaceRebuildsWebsiteDataStoreScope() throws {
        ...
        destination.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/coterm-store-dest.sock",
                terminalStartupCommand: "ssh coterm-macmini"
            ),
            autoConnect: false
        )
    ...
    func testBrowserMoveOutOfRemoteWorkspaceRestoresDefaultWebsiteDataStore() throws {
        ...
        source.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/coterm-store-source.sock",
                terminalStartupCommand: "ssh coterm-macmini"
            ),
            autoConnect: false
        )
    ...
    func testNewTerminalSurfaceStaysRemoteWhileBrowserPanelsKeepWorkspaceRemote() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayID: "relay-test",
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/coterm-test.sock",
            terminalStartupCommand: "ssh coterm-macmini"
        )
...
final class WorkspaceRemoteConfigurationTransportKeyTests: XCTestCase {
    func testProxyBrokerTransportKeyIgnoresControlPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/coterm-ssh-501-64000-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64000,
            relayID: "relay-a",
            relayToken: "token-a",
            localSocketPath: "/tmp/coterm-a.sock",
            terminalStartupCommand: "ssh coterm-macmini"
        )
        let second = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/coterm-ssh-501-64001-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64001,
            relayID: "relay-b",
            relayToken: "token-b",
            localSocketPath: "/tmp/coterm-b.sock",
            terminalStartupCommand: "ssh coterm-macmini"
        )
    ...
    func testProxyBrokerTransportKeyIgnoresEphemeralAgentSocketPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/coterm-a.sock",
            terminalStartupCommand: "ssh coterm-macmini",
            agentSocketPath: "/tmp/coterm-agent-a.sock"
        )
        let second = WorkspaceRemoteConfiguration(
            destination: "coterm-macmini",
            port: 22,
            ...
            localSocketPath: "/tmp/coterm-b.sock",
            terminalStartupCommand: "ssh coterm-macmini",
            agentSocketPath: "/tmp/coterm-agent-b.sock"
        )
...
final class WorkspaceRemoteSSHCleanupTests: XCTestCase {
    func testOrphanedCotermRemoteSSHPIDsMatchesOnlyParentOneRelayAndDaemonTransports() {
        let psOutput = """
          101 1 /usr/bin/ssh -N -T -S none -o ControlPath=/tmp/coterm-ssh-501-56080-%C -R 127.0.0.1:56080:127.0.0.1:64048 coterm-macmini
          102 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec .coterm/bin/cotermd-remote/0.63.1/darwin-arm64/cotermd-remote serve --stdio'
          107 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec .coterm/bin/cotermd-remote/0.63.1/darwin-arm64/cotermd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-test''
          103 999 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56081:127.0.0.1:64049 coterm-macmini
          104 1 /usr/bin/ssh -tt coterm-macmini
          105 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56082:127.0.0.1:64050 other-host
          106 1 /usr/bin/ssh -T -S none coterm-macmini /bin/sh
        """

        XCTAssertEqual(
            RemoteSessionCoordinator.orphanedCotermRemoteSSHPIDs(
                psOutput: psOutput,
                destination: "coterm-macmini"
            ),
            [101, 102, 107]
    ...
    func testOrphanedCotermRemoteSSHPIDsCanRestrictCleanupToSpecificRelayPort() {
        let psOutput = """
          201 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56080:127.0.0.1:64048 coterm-macmini
          202 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56081:127.0.0.1:64049 coterm-macmini
          203 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec .coterm/bin/cotermd-remote/0.63.1/darwin-arm64/cotermd-remote serve --stdio'
          204 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec .coterm/bin/cotermd-remote/0.63.1/darwin-arm64/cotermd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-test''
          205 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec .coterm/bin/cotermd-remote/0.63.1/darwin-arm64/cotermd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-other''
        """
    ...
    func testOrphanedCotermRemoteSSHPIDsMatchesBackslashEscapedPersistentDaemonSlot() {
        let psOutput = """
          211 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec '\''.coterm/bin/cotermd-remote/0.63.1/darwin-arm64/cotermd-remote'\'' '\''serve'\'' '\''--stdio'\'' '\''--persistent'\'' '\''--slot'\'' '\''ssh-test'\'''
          212 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec '\''.coterm/bin/cotermd-remote/0.63.1/darwin-arm64/cotermd-remote'\'' '\''serve'\'' '\''--stdio'\'' '\''--persistent'\'' '\''--slot'\'' '\''ssh-other'\'''
        """
    ...
    func testOrphanedCotermRemoteSSHPIDsMatchesEqualsQuotedPersistentDaemonSlot() {
        let psOutput = """
          221 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec .coterm/bin/cotermd-remote serve --stdio --persistent --slot='ssh-test''
          222 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec .coterm/bin/cotermd-remote serve --stdio --persistent --slot="ssh-other"'
        """
    ...
    func testOrphanedCotermRemoteSSHPIDsWithSlotAndNoRelayKeepsGenericCleanup() {
        let psOutput = """
          301 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56080:127.0.0.1:64048 coterm-macmini
          302 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec .coterm/bin/cotermd-remote/0.63.1/darwin-arm64/cotermd-remote serve --stdio'
          303 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec .coterm/bin/cotermd-remote/0.63.1/darwin-arm64/cotermd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-test''
          304 1 /usr/bin/ssh -T -S none -o RequestTTY=no coterm-macmini sh -c 'exec .coterm/bin/cotermd-remote/0.63.1/darwin-arm64/cotermd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-other''
          305 1 /usr/bin/ssh -T -S none -o RequestTTY=no other-host sh -c 'exec .coterm/bin/cotermd-remote/0.63.1/darwin-arm64/cotermd-remote serve --stdio'
        """
...
final class SocketControlSettingsTests: XCTestCase {
    func testMigrateModeSupportsExpandedSocketModes() {
        ...
        XCTAssertEqual(SocketControlSettings.migrateMode("cotermOnly"), .cotermOnly)
    ...
    func testSocketModePermissions() {
        ...
        XCTAssertEqual(SocketControlMode.cotermOnly.socketFilePermissions, 0o600)
    ...
    func testInvalidEnvSocketModeDoesNotOverrideUserMode() {
        XCTAssertNil(
            SocketControlSettings.envOverrideMode(
                environment: ["COTERM_SOCKET_MODE": "definitely-not-a-mode"]
            )
        )
        XCTAssertEqual(
    ...
    func testStableReleaseIgnoresAmbientSocketOverrideByDefault() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "COTERM_TAG": "stray-tag",
                "COTERM_SOCKET_PATH": "/tmp/coterm-debug-issue-153-tmux-compat.sock",
            ],
    ...
    func testTaggedDebugLaunchUsesTagDefaultWhenNoOverrideIsProvided() {
        ...
        XCTAssertEqual(path, "/tmp/coterm-debug-my-tag.sock")
    }
    ...
    func testTaggedDebugLaunchStillHonorsSocketOverride() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "COTERM_TAG": "my-tag",
                "COTERM_SOCKET_PATH": "/tmp/coterm-debug-forced.sock",
            ],
            bundleIdentifier: "coterm.com.emergent.app.debug",
            isDebugBuild: true
        )

        XCTAssertEqual(path, "/tmp/coterm-debug-forced.sock")
    }
    ...
    func testNightlyReleaseUsesDedicatedDefaultAndIgnoresAmbientSocketOverride() {
        ...
        XCTAssertEqual(path, "/tmp/coterm-nightly.sock")
    }

    func testTaggedDebugBundleKeepsMatchingSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "COTERM_SOCKET_PATH": "/tmp/coterm-debug-my-tag.sock",
            ],
            bundleIdentifier: "coterm.com.emergent.app.debug.my-tag",
            isDebugBuild: false
        )
    ...
    func testTaggedDebugBundleIgnoresSocketOverrideInheritedFromDifferentCotermBundle() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "COTERM_BUNDLE_ID": "coterm.com.emergent.app.nightly",
                "COTERM_SOCKET_PATH": "/tmp/coterm-nightly.sock",
            ],
            bundleIdentifier: "coterm.com.emergent.app.debug.issue.4355.coterm.themes.set.state.dependent",
            isDebugBuild: true
        )

        XCTAssertEqual(path, "/tmp/coterm-debug-issue-4355-coterm-themes-set-state-dependent.sock")
    }
    ...
    func testTaggedDebugBundleIgnoresMismatchedInheritedSocketOverride() {
        let path = SocketControlSettings.socketPath(
            ...
            bundleIdentifier: "coterm.com.emergent.app.debug.fix-grok-notifications",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/coterm-debug-fix-grok-notifications.sock")
    }
    ...
    func testTaggedDebugBundleRefusesStableSocketOverrideEvenWithOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "COTERM_SOCKET_PATH": SocketControlSettings.stableDefaultSocketPath,
                "COTERM_ALLOW_SOCKET_OVERRIDE": "1",
            ],
            bundleIdentifier: "coterm.com.emergent.app.debug.sockguard",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/coterm-debug-sockguard.sock")
    }
    ...
    func testTaggedDebugBundleRefusesUserScopedStableSocketOverrideEvenWithOptInFlag() {
        let aliases = [
            ...
            "/private/tmp/coterm-501.sock",
        ]

        for alias in aliases {
            let path = SocketControlSettings.socketPath(
                environment: [
                    "COTERM_SOCKET_PATH": alias,
                    "COTERM_ALLOW_SOCKET_OVERRIDE": "1",
                ],
                bundleIdentifier: "coterm.com.emergent.app.debug.sockguard",
                isDebugBuild: false,
            ...
            XCTAssertEqual(path, "/tmp/coterm-debug-sockguard.sock", alias)
        }
    }
    ...
    func testTaggedDebugBundleRefusesCanonicalLegacyStableSocketAliasEvenWithOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "COTERM_SOCKET_PATH": "/private/tmp/coterm.sock",
                "COTERM_ALLOW_SOCKET_OVERRIDE": "1",
            ],
            bundleIdentifier: "coterm.com.emergent.app.debug.sockguard",
            isDebugBuild: false
    ...
    func testTaggedDebugBundleRefusesCaseVariantStableSocketAliasesEvenWithOptInFlag() {
        let aliases = [
            ...
            "/private/tmp/COTERM.sock",
            SocketControlSettings.userScopedStableSocketPath(currentUserID: 501)
                .replacingOccurrences(of: "coterm-501.sock", with: "COTERM-501.sock"),
            SocketControlSettings.legacyUserScopedStableSocketPath(currentUserID: 501)
                .replacingOccurrences(of: "coterm-501.sock", with: "COTERM-501.sock"),
        ]
    ...
    func testTaggedDebugBundleRefusesLeafSymlinkToStableSocketEvenWithOptInFlag() throws {
        let alias = "/tmp/coterm-stable-alias-\(UUID().uuidString).sock"
    ...
    func testTaggedDebugBundleRefusesExcessiveSymlinkChainEvenWithOptInFlag() throws {
        let root = "/tmp/coterm-stable-chain-\(UUID().uuidString)"
    ...
    func testStagingBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "COTERM_SOCKET_PATH": "/tmp/coterm-staging-my-tag.sock",
            ],
            bundleIdentifier: "coterm.com.emergent.app.staging.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/coterm-staging-my-tag.sock")
    }
    ...
    func testDefaultSocketPathByChannel() {
        ...
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "coterm.com.emergent.app.nightly.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/coterm-nightly-tag.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
        ...
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "coterm.com.emergent.app.staging.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/coterm-staging-tag.sock"
        )
    }
    ...
    func testInitialStableLaunchTreatsPrivateTmpLegacyStableAliasAsStablePath() {
        let path = SocketControlSettings.initialSocketPathBeforeListenerStart(
            preferredPath: "/private/tmp/coterm.sock",
            bundleIdentifier: "coterm.com.emergent.app",
            isDebugBuild: false,
            currentUserID: 501,
            probeStableDefaultPathEntry: { socketPath in
                XCTAssertEqual(socketPath, "/private/tmp/coterm.sock")
    ...
    func testInitialSocketPathDoesNotProbeForTaggedDebugBuild() {
        let debugPath = "/tmp/coterm-debug-tag.sock"
        let path = SocketControlSettings.initialSocketPathBeforeListenerStart(
    ...
    func testUntaggedDebugBundleAllowedWithLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["COTERM_TAG": "tests-v1"],
                bundleIdentifier: "coterm.com.emergent.app.debug",
                isDebugBuild: true
            )
        )
    ...
    func testTaggedDebugBundleAllowedWithoutLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "coterm.com.emergent.app.debug.tests-v1",
                isDebugBuild: true
            )
        )
    }
    ...
    func testXCUITestLaunchEnvironmentIgnoresLaunchTagGate() {
        ...
        // The app receives COTERM_UI_TEST_* vars via XCUIApplication.launchEnvironment.
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["COTERM_UI_TEST_MODE": "1"],
                bundleIdentifier: "coterm.com.emergent.app.debug",
                isDebugBuild: true
            )
        )
...
final class UITestLaunchManifestTests: XCTestCase {
    func testManifestPathReadsArgumentValue() {
        XCTAssertEqual(
            UITestLaunchManifest.manifestPath(
                from: ["coterm", "-cotermUITestLaunchManifest", "/tmp/coterm-ui-test-launch.json"]
            ),
            "/tmp/coterm-ui-test-launch.json"
        )
    }
    ...
    func testManifestPathReturnsNilWithoutValue() {
        XCTAssertNil(
            UITestLaunchManifest.manifestPath(
                from: ["coterm", "-cotermUITestLaunchManifest"]
            )
        )
    }
    ...
    func testApplyIfPresentDecodesEnvironmentPayload() {
        let payload = """
        {"environment":{"COTERM_TAG":"ui-tests-display","COTERM_SOCKET_PATH":"/tmp/coterm-ui-tests.sock"}}
        """.data(using: .utf8)!
        var applied: [String: String] = [:]

        UITestLaunchManifest.applyIfPresent(
            arguments: ["coterm", UITestLaunchManifest.argumentName, "/tmp/coterm-ui-test-launch.json"],
        ...
        XCTAssertEqual(applied["COTERM_TAG"], "ui-tests-display")
        XCTAssertEqual(applied["COTERM_SOCKET_PATH"], "/tmp/coterm-ui-tests.sock")
    }
}
...
final class GhosttyMouseFocusTests: XCTestCase {
    ...
    private func withTempConfig(
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-cjk-\(UUID().uuidString)")
    ...
    func testUserConfigContainsCJKCodepointMapReturnsFalseForMissingFiles() {
        let path = NSTemporaryDirectory() + "coterm-nonexistent-\(UUID().uuidString)/config"
        XCTAssertFalse(
    ...
    func testUserConfigContainsCJKCodepointMapFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-cjk-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapFollowsRelativeIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-cjk-rel-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesOptionalInclude() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-cjk-opt-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesCyclicIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-cjk-cycle-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-cjk-font-family-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainIgnoresDuplicateFamilies() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-cjk-font-family-duplicate-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainMatchesGhosttyIncludeLoadOrder() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-cjk-font-family-order-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainRespectsConfigFileReset() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-cjk-font-family-config-file-reset-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testLoadedCJKScanPathsIncludesNativeGhosttyAppSupportWhenTaggedConfigExists() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-cjk-app-support-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }

        let taggedDir = appSupport.appendingPathComponent("com.example.coterm-dev", isDirectory: true)
        ...
        let paths = GhosttyApp.loadedCJKScanPaths(
            currentBundleIdentifier: "com.example.coterm-dev",
            appSupportDirectory: appSupport
        )
    ...
    func testShouldApplyManagedDefaultAppearanceScansNativeGhosttyAppSupport() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-appearance-app-support-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
    ...
    func testLoadedGhosttyConfigScanPathsSkipsNativeLegacyConfigWhenCurrentConfigIsNonEmpty() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-appearance-app-support-current-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
    ...
    // Regression: https://github.com/emergent-inc/coterm/issues/3459
    // `coterm themes set` always encodes the selection with conditional
    ...
    // stay at the white defaults), so coterm must inject the resolved plain theme
    ...
    // Regression: https://github.com/emergent-inc/coterm/issues/3459
    // `coterm themes set --light X` encodes `theme = light:X`, which ghostty treats
    // as conditional config. coterm injects the resolved plain theme for the
    ...
    func testShouldApplyManagedDefaultAppearanceFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-theme-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testShouldApplyManagedDefaultAppearancePreservesQuotedQuestionMarkConfigFile() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-theme-quoted-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testShouldApplyManagedDefaultAppearanceProcessesIncludeQueuedAfterReset() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-test-theme-reset-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testStartupAppearanceFreshInstallPreviewUsesEnforcedTheme() {
        ...
        XCTAssertEqual(config.theme, GhosttyConfig.cotermEnforcedThemeName)
...
final class ZshShellIntegrationHandoffTests: XCTestCase {
    func testGhosttyPromptHooksLoadWhenCotermRequestsZshIntegration() throws {
        let output = try runInteractiveZsh(cotermLoadGhosttyIntegration: true)
    ...
    func testGhosttyPromptHooksDoNotLoadWithoutCotermHandoffFlag() throws {
        let output = try runInteractiveZsh(cotermLoadGhosttyIntegration: false)
    ...
    func testGhosttySemanticPatchRetriesAfterDeferredInitCreatesLiveHooks() throws {
        let output = try runInteractiveZsh(
            cotermLoadGhosttyIntegration: true,
            cotermLoadShellIntegration: true,
            command: """
    ...
    func testShellIntegrationWinchGuardDoesNotPrintSpacerLineOnResize() throws {
        let output = try runInteractiveZsh(
            cotermLoadGhosttyIntegration: false,
            cotermLoadShellIntegration: true,
            command: """
            print -r -- BEFORE
            TRAPWINCH
    ...
    func testShellIntegrationPreservesStartupTermForThemeSelectionBeforeRestoringManagedTerm() throws {
        let output = try runPromptInteractiveZsh(
            ...
            cotermLoadShellIntegration: true,
            command: """
            print -r -- "CMD=$TERM|${COTERM_ZSH_RESTORE_TERM-unset}" >> "$COTERM_TEST_OUTPUT"
            """,
            userZshRCContents: """
            export COTERM_STARTUP_THEME_TERM="$TERM"
            if [[ $TERM = (*256color|*rxvt*) ]]; then
              export COTERM_STARTUP_THEME_BRANCH=extended
            else
              export COTERM_STARTUP_THEME_BRANCH=basic
            fi

            coterm_test_ready() {
              [[ -e "$COTERM_TEST_READY" ]] && return 0
              print -r -- "PRE=$COTERM_STARTUP_THEME_TERM|$COTERM_STARTUP_THEME_BRANCH|$TERM|${COTERM_ZSH_RESTORE_TERM-unset}" > "$COTERM_TEST_OUTPUT"
              : > "$COTERM_TEST_READY"
              precmd_functions=(${precmd_functions:#coterm_test_ready})
            }
            precmd_functions+=(coterm_test_ready)
            """
        )
    ...
    func testShellIntegrationDoesNotSpoofManagedTermForInteractiveCommandMode() throws {
        let output = try runInteractiveZsh(
            ...
            cotermLoadShellIntegration: true,
            command: """
            print -r -- "$COTERM_STARTUP_TERM|$TERM|${COTERM_ZSH_RESTORE_TERM-unset}"
            """,
            userZshRCContents: """
            export COTERM_STARTUP_TERM="$TERM"
            """
        )
    ...
    func testShellIntegrationDoesNotSpoofManagedTermWhenIntegrationDisabled() throws {
        let output = try runInteractiveZsh(
            cotermLoadGhosttyIntegration: false,
            cotermLoadShellIntegration: false,
            command: """
            print -r -- "$COTERM_STARTUP_TERM|$TERM|${COTERM_ZSH_RESTORE_TERM-unset}"
            """,
            userZshRCContents: """
    ...
    func testShellIntegrationDoesNotSpoofManagedTermWhenUserZshEnvDisablesIntegration() throws {
        let output = try runInteractiveZsh(
            ...
            cotermLoadShellIntegration: true,
            command: """
            print -r -- "$COTERM_STARTUP_TERM|$TERM|${COTERM_ZSH_RESTORE_TERM-unset}|${COTERM_SHELL_INTEGRATION:-unset}"
            """,
            userZshEnvContents: """
            export COTERM_SHELL_INTEGRATION=0
            """,
            userZshRCContents: """
            export COTERM_STARTUP_TERM="$TERM"
            """
    ...
    func testShellIntegrationNormalizesClaudeConfigDirAfterUserZshrc() throws {
        let output = try runPromptInteractiveZsh(
            ...
            cotermLoadShellIntegration: true,
            command: """
            print -r -- "CMD=$CLAUDE_CONFIG_DIR" >> "$COTERM_TEST_OUTPUT"
            """,
            userZshRCContents: """
            ...
            coterm_test_ready() {
              [[ -e "$COTERM_TEST_READY" ]] && return 0
              print -r -- "PRE=$CLAUDE_CONFIG_DIR" > "$COTERM_TEST_OUTPUT"
              : > "$COTERM_TEST_READY"
              precmd_functions=(${precmd_functions:#coterm_test_ready})
            }
            precmd_functions+=(coterm_test_ready)
    ...
    func testShellIntegrationDoesNotRegisterPromptTimeTermRestoreHooks() throws {
        ...
        XCTAssertEqual(
            output,
            "_coterm_precmd,_coterm_fix_path",
            output
        )
    }
    ...
    func testShellIntegrationRestoresManagedTermDuringPreexec() throws {
        let output = try runInteractiveZsh(
            ...
            cotermLoadShellIntegration: true,
            command: """
            _coterm_preexec 'echo $TERM'
            print -r -- "$TERM|${COTERM_ZSH_RESTORE_TERM-unset}"
            """,
            extraEnvironment: [
                "TERM": "xterm-ghostty",
                "COTERM_ZSH_RESTORE_TERM": "xterm-256color",
            ]
        )
    ...
    func testShellIntegrationPublishesOnlyWorkspaceScopedCotermEnvironmentToTmuxServerAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-zsh-tmux-publish-\(UUID().uuidString)")
        ...
        _ = try runInteractiveZsh(
            cotermLoadGhosttyIntegration: false,
            cotermLoadShellIntegration: true,
            command: "_coterm_preexec tmux; print -r -- READY",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "COTERM_SOCKET_PATH": "/tmp/coterm-current.sock",
                "COTERM_TAG": "feat-tmux-notification-attention-state",
                "COTERM_WORKSPACE_ID": "11111111-1111-1111-1111-111111111111",
                "COTERM_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "COTERM_TAB_ID": "11111111-1111-1111-1111-111111111111",
                "COTERM_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )

        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -g COTERM_TAG feat-tmux-notification-attention-state"), log)
        XCTAssertTrue(log.contains("set-environment -g COTERM_SOCKET_PATH /tmp/coterm-current.sock"), log)
        XCTAssertTrue(log.contains("set-environment -g COTERM_WORKSPACE_ID 11111111-1111-1111-1111-111111111111"), log)
        XCTAssertFalse(log.contains("set-environment -g COTERM_SURFACE_ID"), log)
        XCTAssertFalse(log.contains("set-environment -g COTERM_PANEL_ID"), log)
    }

    func testShellIntegrationClearsStaleSurfaceScopedTmuxEnvironmentAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-zsh-tmux-clear-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("tmux.log", isDirectory: false)
        ...
        try writeExecutableScript(
            ...
            #!/bin/sh
            if [ "$1" = "show-environment" ] && [ "$2" = "-g" ]; then
              printf '%s\\n' 'COTERM_SURFACE_ID=99999999-9999-9999-9999-999999999999'
              printf '%s\\n' 'COTERM_PANEL_ID=99999999-9999-9999-9999-999999999999'
              exit 0
            fi
            printf '%s\\n' "$*" >> "\(logPath.path)"
            exit 0
        ...
        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -gu COTERM_SURFACE_ID"), log)
        XCTAssertTrue(log.contains("set-environment -gu COTERM_PANEL_ID"), log)
    }

    func testShellIntegrationRefreshesWorkspaceScopedCotermEnvironmentFromTmuxWithoutOverwritingSurfaceScope() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-zsh-tmux-refresh-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        ...
        let output = try runInteractiveZsh(
            cotermLoadGhosttyIntegration: false,
            cotermLoadShellIntegration: true,
            command: "_coterm_precmd; print -r -- \"$COTERM_TAG|$COTERM_SOCKET_PATH|$COTERM_WORKSPACE_ID|$COTERM_SURFACE_ID|$COTERM_PANEL_ID\"",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "TMUX": "/tmp/tmux-stale,123,0",
                "COTERM_SOCKET_PATH": "/tmp/coterm-stale.sock",
                "COTERM_TAG": "feat-tmux-integration-experiments",
                "COTERM_WORKSPACE_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "COTERM_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "COTERM_TAB_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "COTERM_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )
        ...
        XCTAssertEqual(
            output,
            "feat-tmux-notification-attention-state|/tmp/coterm-current.sock|11111111-1111-1111-1111-111111111111|22222222-2222-2222-2222-222222222222|22222222-2222-2222-2222-222222222222"
        )
    }
    ...
    func testShellIntegrationReportsTTYFromTmuxWithoutUsingPanelScope() throws {
        let output = try runInteractiveZsh(
            ...
            cotermLoadShellIntegration: true,
            command: """
            _COTERM_TTY_NAME=ttys999
            print -r -- "$(_coterm_report_tty_payload)"
            """,
            extraEnvironment: [
    ...
    func testShellIntegrationRelayReportTTYUsesWorkspaceIDInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-zsh-relay-report-tty-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
    ...
    func testShellIntegrationRelayPortsKickOmitsSurfaceIDUntilAvailableInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-zsh-relay-kick-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let output = try runInteractiveZsh(
            ...
            command: """
            : > "\(logPath.path)"
            _coterm_ports_kick_via_relay refresh
            repeat 20; do
    ...
    func testShellIntegrationRelayPromptRefreshUsesRefreshReasonInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-zsh-relay-precmd-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
    ...
    func testShellIntegrationRelayReportTTYUsesWorkspaceIDInBash() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-bash-relay-report-tty-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
    ...
    func testShellIntegrationRelayPreexecWorksBeforeSurfaceIDExistsInBash() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-bash-relay-preexec-no-surface-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let result = try runInteractiveBash(
            ...
            command: """
            : > "\(logPath.path)"
            _COTERM_TTY_NAME=ttys889
            _COTERM_TTY_REPORTED=0
            _coterm_preexec_command "python3 -m http.server 8899"
    ...
    func testShellIntegrationRelayPromptRefreshUsesRefreshReasonInBashWithoutPromptNoise() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-bash-relay-prompt-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        XCTAssertFalse(result.stderr.contains("_coterm_report_tmux_state"), result.stderr)
        XCTAssertTrue(
    ...
    func testBashNoGitWatchSkipsHeadTrackingAndPRClear() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-bash-no-git-watch-\(UUID().uuidString)")
        ...
        let socketPath = root.appendingPathComponent("coterm-test.sock", isDirectory: false)
        ...
        let result = try runInteractiveBash(
            ...
            _COTERM_GIT_HEAD_PATH="$PWD/.git/HEAD"
            _COTERM_GIT_HEAD_SIGNATURE="$(_coterm_git_head_signature "$_COTERM_GIT_HEAD_PATH")"
            printf '%s\\n' 'ref: refs/heads/old-cleared' > "$_COTERM_GIT_HEAD_PATH"
            cd "\(repoB.path)"
            _COTERM_PWD_LAST_PWD="$PWD"
            _COTERM_LAST_PR_ACTION="checkout"
            _COTERM_LAST_PR_TARGET="feature"
            _coterm_prompt_command
            ...
            """,
            extraEnvironment: [
                "COTERM_NO_GIT_WATCH": "1",
                "COTERM_SOCKET_PATH": socketPath.path,
                "COTERM_TAB_ID": "22222222-2222-2222-2222-222222222222",
                "COTERM_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )
    ...
    func testZshNoGitWatchSkipsHeadTrackingAndPRClear() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-zsh-no-git-watch-\(UUID().uuidString)")
        let repoA = root.appendingPathComponent("repo-a", isDirectory: true)
        let repoB = root.appendingPathComponent("repo-b", isDirectory: true)
        let logPath = root.appendingPathComponent("send.log", isDirectory: false)
        let socketPath = root.appendingPathComponent("coterm-test.sock", isDirectory: false)
    ...
    func testZshNoPullRequestWatchSkipsLegacyGhPRProbe() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-zsh-no-pr-watch-\(UUID().uuidString)")
        ...
        try writeExecutableScript(
            ...
            contents: """
            #!/bin/sh
            printf invoked > "$COTERM_GH_MARKER"
            printf '2746\\tOPEN\\thttps://github.com/emergent-inc/coterm/pull/2746\\n'
            """
        )
        let socketFD = try bindUnixSocket(at: socketPath.path)
        defer {
        ...
        let output = try runInteractiveZsh(
            ...
            _coterm_send() { :; }
            _coterm_send_bg() { :; }
            _coterm_report_pr_for_path "\(repoURL.path)" || true
            [[ -e "\(markerURL.path)" ]] && print MARKER=1 || print MARKER=0
            """,
            extraEnvironment: [
                "COTERM_NO_PR_WATCH": "1",
                "COTERM_GH_MARKER": markerURL.path,
                "COTERM_SOCKET_PATH": socketPath.path,
    ...
    func testBashNoPullRequestWatchSkipsLegacyGhPRProbe() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-bash-no-pr-watch-\(UUID().uuidString)")
        let repoURL = root.appendingPathComponent("repo", isDirectory: true)
        let fakeBinURL = root.appendingPathComponent("fake-bin", isDirectory: true)
        let markerURL = root.appendingPathComponent("gh-pr-invoked", isDirectory: false)
        let socketPath = root.appendingPathComponent("coterm-test.sock", isDirectory: false)
    ...
    func testZshPromptResetsTerminalKeyboardProtocols() throws {
        let output = try runInteractiveZsh(
            ...
            extraEnvironment: [
                "COTERM_SOCKET_PATH": "127.0.0.1:64011",
                "COTERM_BUNDLED_CLI_PATH": "/usr/bin/true",
                "COTERM_WORKSPACE_ID": "11111111-1111-1111-1111-111111111111",
                "COTERM_TAB_ID": "22222222-2222-2222-2222-222222222222",
                "COTERM_PANEL_ID": "22222222-2222-2222-2222-222222222222",
                "COTERM_TEST_FORCE_KITTY_RESET": "1",
            ]
        )
    ...
    private func runInteractiveZsh(cotermLoadGhosttyIntegration: Bool) throws -> String {
        try runInteractiveZsh(
            cotermLoadGhosttyIntegration: cotermLoadGhosttyIntegration,
            cotermLoadShellIntegration: false,
    ...
    private func runInteractiveZsh(
        cotermLoadGhosttyIntegration: Bool,
        cotermLoadShellIntegration: Bool,
        command: String,
        ...
        process.environment = [
            ...
            "COTERM_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
        ]
        if cotermLoadGhosttyIntegration {
            process.environment?["COTERM_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
        if cotermLoadShellIntegration {
            process.environment?["COTERM_SHELL_INTEGRATION"] = "1"
            process.environment?["COTERM_SHELL_INTEGRATION_DIR"] = cotermZdotdir.path
            process.environment?["COTERM_SOCKET_PATH"] = root.appendingPathComponent("coterm-test.sock").path
            process.environment?["COTERM_TAB_ID"] = "tab-test"
            process.environment?["COTERM_PANEL_ID"] = "panel-test"
        }
    ...
    private func runPromptInteractiveZsh(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-zsh-prompt-integration-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        process.environment = [
            ...
            "COTERM_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
            "COTERM_TEST_READY": readyPath.path,
            "COTERM_TEST_OUTPUT": outputPath.path,
        ]
        if cotermLoadGhosttyIntegration {
            process.environment?["COTERM_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
    ...
    private func runInteractiveBash(
        ...
        let rcfileContents: String = {
            guard cotermLoadShellIntegration else { return ":\n" }
            return """
...
final class BrowserInstallDetectorTests: XCTestCase {
    ...
    private func makeTemporaryHome() -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("coterm-browser-detect-\(UUID().uuidString)")
    }
