...
/// Tests the pure request-shaping for `coterm remotes`: host:port parsing, the
...
@Suite struct RemotesClientTests {
    ...
    // Note: `CotermCLI.sanitizeForTerminal` (used by `remotes list`) lives in the
    // CLI executable target, which `cotermTests` does not link, so it cannot be
    // unit-tested here (referencing `CotermCLI` breaks the test-target compile).
