...
final class WorkspaceRemoteConnectionTests: XCTestCase {
    ...
    private func runRelayZshHistfile(
        ...
        let home = fileManager.temporaryDirectory.appendingPathComponent("coterm-relay-zsh-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".coterm/relay/64011.shell")
    ...
    private func runGeneratedBashBootstrapMarkers(startupFiles: [String: String]) throws -> [String] {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("coterm-relay-bash-\(UUID().uuidString)")
        ...
        let result = runProcess(
            ...
            arguments: [
                ...
                "TERM=xterm-256color",
                "USER=\(NSUserName())",
                "COTERM_BASH_MARKERS=\(markerFile.path)",
                "/bin/sh",
                "-c",
                script,
            ],
    ...
    func testGeneratedFallbackShellBootstrapPrependsCotermBinOnce() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-fallback-shell-bootstrap-\(UUID().uuidString)")
        ...
        let result = runProcess(
            ...
            arguments: [
                ...
                "TERM=xterm-256color",
                "USER=\(NSUserName())",
                "COTERM_CAPTURE_PATH=\(capturedPath.path)",
                "/bin/sh",
                "-c",
                script,
            ],
        ...
        let cotermBinEntries = path.split(separator: ":")
            .filter { $0 == "\(home.path)/.coterm/bin" }
        XCTAssertEqual(cotermBinEntries.count, 1, path)
    }

    func testRemoteRelayMetadataCleanupScriptRemovesMatchingSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("coterm-relay-cleanup-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".coterm/relay")
        let socketAddrURL = home.appendingPathComponent(".coterm/socket_addr")
    ...
    func testRemoteRelayMetadataCleanupScriptPreservesDifferentSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("coterm-relay-cleanup-preserve-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".coterm/relay")
        let socketAddrURL = home.appendingPathComponent(".coterm/socket_addr")
    ...
    func testRemoteStaleRelayListenerCleanupScriptKillsMatchingPersistentRelayListener() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("coterm-stale-relay-cleanup-\(UUID().uuidString)")
        let bin = root.appendingPathComponent("bin")
        ...
        try writeExecutableShellFile(
            ...
            34057 33681 /Users/coterm/.coterm/bin/cotermd-remote/current/darwin-arm64/cotermd-remote serve --stdio --persistent --slot ssh-c4ba8ab1
            34058 33681 /bin/zsh
            EOF
            """
        )
        ...
        let result = runProcess(
            ...
            arguments: [
                "PATH=\(bin.path):/usr/bin:/bin",
                "COTERM_KILL_LOG=\(killLog.path)",
                "/bin/sh",
                "-c",
                """
                kill() { printf '%s\\n' "$*" >> "$COTERM_KILL_LOG"; return 0; }
                \(script)
                """,
            ],
            timeout: 5
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertTrue(result.stdout.contains("coterm_stale_relay_killed pid=33681 children=34057 port=50446"), result.stdout)
    ...
    func testRemoteStaleRelayListenerCleanupScriptPreservesDifferentPersistentSlot() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("coterm-stale-relay-preserve-\(UUID().uuidString)")
        let bin = root.appendingPathComponent("bin")
        let killLog = root.appendingPathComponent("kill.log")
        try fileManager.createDirectory(at: bin, withIntermediateDirectories: true)
        try "".write(to: killLog, atomically: true, encoding: .utf8)
        ...
        try writeExecutableShellFile(
            ...
            cat <<'EOF'
            33681 1 /usr/sbin/sshd-session
            34057 33681 /Users/coterm/.coterm/bin/cotermd-remote/current/darwin-arm64/cotermd-remote serve --stdio --persistent --slot ssh-other
            EOF
            """
        )
    ...
    func testRemoteStaleRelayListenerCleanupScriptMatchesPersistentSlotExactly() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("coterm-stale-relay-slot-prefix-\(UUID().uuidString)")
        let bin = root.appendingPathComponent("bin")
        let killLog = root.appendingPathComponent("kill.log")
        try fileManager.createDirectory(at: bin, withIntermediateDirectories: true)
        try "".write(to: killLog, atomically: true, encoding: .utf8)
        ...
        try writeExecutableShellFile(
            ...
            cat <<'EOF'
            33681 1 /usr/sbin/sshd-session
            34057 33681 /Users/coterm/.coterm/bin/cotermd-remote/current/darwin-arm64/cotermd-remote serve --stdio --persistent --slot ssh-ab
            EOF
            """
        )
    ...
    func testRemoteStaleRelayListenerCleanupScriptKillsMetadataMatchedListenerWithoutChild() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("coterm-stale-relay-metadata-\(UUID().uuidString)")
        let bin = root.appendingPathComponent("bin")
        let relayDir = root.appendingPathComponent(".coterm/relay")
        let killLog = root.appendingPathComponent("kill.log")
        try fileManager.createDirectory(at: bin, withIntermediateDirectories: true)
        try fileManager.createDirectory(at: relayDir, withIntermediateDirectories: true)
        try "/Users/coterm/.coterm/bin/cotermd-remote/current/darwin-arm64/cotermd-remote".write(
        ...
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertTrue(
            result.stdout.contains("coterm_stale_relay_killed pid=33681 children= port=50446 reason=metadata"),
            result.stdout
        )
    ...
    func testRemoteStaleRelayListenerCleanupScriptPreservesMetadataMatchedDifferentPersistentSlot() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("coterm-stale-relay-metadata-preserve-\(UUID().uuidString)")
        let bin = root.appendingPathComponent("bin")
        let relayDir = root.appendingPathComponent(".coterm/relay")
        let killLog = root.appendingPathComponent("kill.log")
        try fileManager.createDirectory(at: bin, withIntermediateDirectories: true)
    ...
    func testDaemonSocketForwardArgumentsTargetBakedVMSocket() {
        ...
        let arguments = configuration.daemonSocketForwardArguments(
            localPort: 64123,
            remoteSocketPath: "/run/cotermd-remote.sock"
        )
        ...
        XCTAssertTrue(arguments.contains("127.0.0.1:64123:/run/cotermd-remote.sock"))
        XCTAssertEqual(arguments.last, "coterm-macmini")
    }
    ...
    func testProxyBrokerTransportKeySeparatesVMBakedSSHFromStandardSSH() {
        let standard = WorkspaceRemoteConfiguration(
            ...
            port: 2222,
            identityFile: "/Users/test/.ssh/id_ed25519",
            sshOptions: ["ControlPath /tmp/coterm-ssh-%C"],
            localProxyPort: nil,
            relayPort: 64099,
            relayID: "relay-a",
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/coterm.sock",
        ...
        let vmWebSocket = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: nil,
            terminalStartupCommand: "coterm vm-pty-attach --id abcd1234",
    ...
    func testWebSocketVMWithoutDaemonEndpointSkipsProxyStartup() {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: nil,
            terminalStartupCommand: "coterm vm-pty-attach --id test-no-daemon",
            skipDaemonBootstrap: true
        )
    ...
    func testRemoteDaemonCapabilityErrorsUseUserFacingMessage() {
        ...
        XCTAssertEqual(
            message,
            "remote daemon does not support persistent SSH PTY sessions; reconnect the remote workspace to update coterm"
        )
        ...
        let rawError = NSError(domain: "coterm.remote.daemon", code: 43, userInfo: [
    ...
    func testWebSocketVMWithDaemonEndpointStartsProxyCapableConnection() {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: nil,
            terminalStartupCommand: "coterm vm-pty-attach --id test-with-daemon",
            daemonWebSocketEndpoint: WorkspaceRemoteWebSocketDaemonEndpoint(
    ...
    func testRemoteTerminalSurfaceLookupTracksOnlyActiveSSHSurfaces() throws {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/coterm-debug-test.sock",
            terminalStartupCommand: "ssh coterm-macmini"
        )
    ...
    func testWebSocketRemoteTerminalEndLeavesConnectedStateWithinBoundedTime() throws {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: "/tmp/coterm-debug-test.sock",
            terminalStartupCommand: "coterm vm-pty-attach --id issue-4509",
            daemonWebSocketEndpoint: WorkspaceRemoteWebSocketDaemonEndpoint(
    ...
    func testWebSocketRemoteTerminalEndWithoutStartupCommandStillDisconnects() throws {
        ...
        let initialConfig = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: "/tmp/coterm-debug-test.sock",
            terminalStartupCommand: "coterm vm-pty-attach --id issue-4509-no-startup",
            daemonWebSocketEndpoint: WorkspaceRemoteWebSocketDaemonEndpoint(
                url: "wss://vm.example.invalid/daemon",
                headers: [:],
                token: "token",
    ...
    func testRemoteTerminalSessionEndPublishesDisconnectedDetailWithoutTransientNil() throws {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to coterm-macmini",
            target: "coterm-macmini"
        )
    ...
    func testRemoteReconnectingStateIsExposedInStatusPayload() {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .reconnecting,
            detail: "Reconnecting to coterm-macmini",
            target: "coterm-macmini"
        )
    ...
    func testRemoteTerminalSessionEndRequestsControlMasterCleanupAndLeavesWorkspaceDisconnected() throws {
        ...
        XCTAssertEqual(
            ...
            [
                ...
                "-o", "ControlPath=/tmp/coterm-ssh-%C",
    ...
    func testTeardownRemoteConnectionRequestsControlMasterCleanupWhileStillConnecting() {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connecting,
            detail: "Connecting to coterm-macmini",
            target: "coterm-macmini"
        )
    ...
    func testTransferredRemoteSurfaceCleansUpControlMasterWhenSessionEndsInLocalWorkspace() throws {
        ...
        XCTAssertEqual(cleanupArguments.first?.suffix(2), ["exit", "coterm-macmini"])
    }
    ...
    func testRemoteDropPathUsesLowercasedExtensionAndProvidedUUID() throws {
        ...
        XCTAssertEqual(remotePath, "/tmp/coterm-drop-12345678-1234-1234-1234-1234567890ab.png")
    }
    ...
    func testDaemonBootstrapUploadUsesAbsoluteHomePathForScpDestination() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-remote-daemon-upload-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: directoryURL) }

        let fakeDaemonURL = directoryURL.appendingPathComponent("cotermd-remote", isDirectory: false)
        ...
        let previousAllowLocalBuild = getenv("COTERM_REMOTE_DAEMON_ALLOW_LOCAL_BUILD").map { String(cString: $0) }
        let previousDaemonBinary = getenv("COTERM_REMOTE_DAEMON_BINARY").map { String(cString: $0) }
        setenv("COTERM_REMOTE_DAEMON_ALLOW_LOCAL_BUILD", "1", 1)
        setenv("COTERM_REMOTE_DAEMON_BINARY", fakeDaemonURL.path, 1)
        defer {
            if let previousAllowLocalBuild {
                setenv("COTERM_REMOTE_DAEMON_ALLOW_LOCAL_BUILD", previousAllowLocalBuild, 1)
            } else {
                unsetenv("COTERM_REMOTE_DAEMON_ALLOW_LOCAL_BUILD")
            }
            if let previousDaemonBinary {
                setenv("COTERM_REMOTE_DAEMON_BINARY", previousDaemonBinary, 1)
            } else {
                unsetenv("COTERM_REMOTE_DAEMON_BINARY")
            }
        }
            ...
            if executable == "/usr/bin/ssh" {
                ...
                if command.contains("uname -s") {
                    return (
                        status: 0,
                        stdout: """
                        __COTERM_REMOTE_HOME__=/home/test
                        __COTERM_REMOTE_OS__=Linux
                        __COTERM_REMOTE_ARCH__=x86_64
                        __COTERM_REMOTE_EXISTS__=no
                        """,
                        stderr: ""
                    )
                }
        ...
        XCTAssertTrue(
            destination.hasPrefix("test@hpc.example:/home/test/.coterm/bin/cotermd-remote/"),
            "expected scp to target an absolute path under remote HOME, got \(destination)"
        )
        XCTAssertTrue(
            destination.contains("/linux-amd64/cotermd-remote.tmp-"),
        ...
        let remoteProcessScript: RemoteProcessScript = { executable, arguments, _, _ in
            ...
            if executable == "/usr/bin/ssh" {
                ...
                if command.contains("serve --stdio") {
                    return (
                        status: 0,
                        stdout: #"{"id":1,"ok":true,"result":{"name":"cotermd-remote","version":"old","capabilities":["proxy.stream.push"]}}"# + "\n",
                        stderr: ""
                    )
                }
                if command.contains("mkdir -p") {
    ...
    func testPersistentReverseRelayCancelsStaleControlMasterForwardBeforeReusingRelayPort() throws {
        ...
        let remoteProcessScript: RemoteProcessScript = { executable, arguments, _, _ in
            ...
            if command.contains("serve --stdio") {
                return (
                    status: 0,
                    stdout: #"{"id":1,"ok":true,"result":{"name":"cotermd-remote","version":"dev","capabilities":["proxy.stream.push","pty.session","pty.session.token","pty.session.persistent_daemon"]}}"# + "\n",
                    stderr: ""
                )
            }
            return (status: 0, stdout: "", stderr: "")
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "ControlMaster=auto",
                "ControlPersist=600",
                "ControlPath=/tmp/coterm-ssh-\(getuid())-64044-%C",
                "StrictHostKeyChecking=accept-new",
            ],
            localProxyPort: nil,
            relayPort: 64044,
            ...
            localSocketPath: "/tmp/coterm-stale-forward-test.sock",
            terminalStartupCommand: "ssh-pty-attach",
            preserveAfterTerminalExit: true,
    ...
    func testPersistentReverseRelayCleansStaleRemoteListenerAndRetriesControlMasterForward() throws {
        ...
        let remoteProcessScript: RemoteProcessScript = { executable, arguments, _, _ in
            ...
            let command = arguments.last ?? ""
            if command.contains("coterm_stale_relay_listener_cleanup=1") {
                lock.lock()
                cleanupInvoked = true
                ...
                return (
                    status: 0,
                    stdout: "coterm_stale_relay_killed pid=33681 children=34057 port=64045\n",
                    stderr: ""
                )
            }
            if command.contains("uname -s") {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "ControlMaster=auto",
                "ControlPersist=600",
                "ControlPath=/tmp/coterm-ssh-\(getuid())-64045-%C",
                "StrictHostKeyChecking=accept-new",
            ],
            localProxyPort: nil,
            relayPort: 64045,
            ...
            localSocketPath: "/tmp/coterm-stale-forward-retry.sock",
            terminalStartupCommand: "ssh-pty-attach",
            preserveAfterTerminalExit: true,
    ...
    func testDetachAttachDoesNotAdoptPersistentPTYSessionIDAcrossNilRelayWorkspaces() throws {
        ...
        let movedPanel = try XCTUnwrap(
            source.newTerminalSurface(
                inPane: sourcePaneID,
                focus: true,
                initialCommand: "coterm ssh-pty-attach",
                remotePTYSessionID: sessionID
            )
        )
    ...
    func testDetectedSSHUploadFailureCleansUpEarlierRemoteUploads() throws {
        let fileManager = FileManager.default
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-detected-ssh-upload-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: directoryURL) }
    ...
    func testDetectsForegroundSSHSessionWithShortControlPathFlag() {
        ...
        XCTAssertEqual(session?.controlPath, "/tmp/coterm-ssh-%C")
        let scpArgs = session?.scpArgumentsForTesting(
            localPath: "/tmp/local.png",
            remotePath: "/tmp/coterm-drop-123.png"
        ) ?? []
        XCTAssertTrue(scpArgs.contains("ControlPath=/tmp/coterm-ssh-%C"))
    ...
    func testDetectsEternalTerminalSessionWithoutTreatingETPortAsSSHPort() {
        ...
        XCTAssertEqual(scpArgs.last, "lawrence@example.com:/tmp/coterm-drop-123.png")
    }
    ...
    func testDetectsEternalTerminalSessionWithBracketedIPv6ServerPortForSCP() {
        ...
        XCTAssertNil(session?.port)
        XCTAssertFalse(scpArgs.contains("-P"))
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8::1]:/tmp/coterm-drop-123.png")
    }
    ...
    func testDetectsEternalTerminalSessionWithFullIPv6ServerPortForSCP() {
        ...
        XCTAssertNil(session?.port)
        XCTAssertFalse(scpArgs.contains("-P"))
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8:0:0:0:0:0:1]:/tmp/coterm-drop-123.png")
    }
    ...
    func testDetectsEternalTerminalSessionPreservesAmbiguousCompressedIPv6LiteralForSCP() {
        ...
        XCTAssertNil(session?.port)
        XCTAssertFalse(scpArgs.contains("-P"))
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8::1:2022]:/tmp/coterm-drop-123.png")
    }
        ...
        let session = TerminalSSHSessionDetector.detectForTesting(
            ...
            argumentsByPID: [
                2145: [
                    ...
                    "--ssh-option", "ControlPath=/tmp/coterm-ssh-%C",
    ...
    func testDaemonTransportArgumentsReuseConfiguredControlPath() {
        ...
        let arguments = configuration.daemonTransportArguments(
            remotePath: "/remote/cotermd-remote"
        )

        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains(where: { $0 == "ControlPath /tmp/coterm-ssh-%C" || $0 == "ControlPath=/tmp/coterm-ssh-%C" }))
        XCTAssertTrue(arguments.contains("coterm-macmini"))
        XCTAssertTrue(arguments.last?.contains("/remote/cotermd-remote") ?? false)
    }
    ...
    func testReverseRelayControlMasterArgumentsReuseConfiguredControlSocket() throws {
        ...
        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains("ControlPath=/tmp/coterm-ssh-%C"))
    ...
    func testProxyOnlyErrorsKeepSSHWorkspaceConnectedAndLoggedInSidebar() {
        ...
        let proxyError = "Remote proxy to coterm-macmini unavailable: Failed to start local daemon proxy: daemon RPC timeout waiting for hello response (retry in 3s)"
        workspace.applyRemoteConnectionStateUpdate(.error, detail: proxyError, target: "coterm-macmini")

        XCTAssertEqual(workspace.remoteConnectionState, .connected)
        XCTAssertEqual(workspace.remoteConnectionDetail, proxyError)
        XCTAssertEqual(
            workspace.statusEntries["remote.error"]?.value,
            "Remote proxy unavailable (coterm-macmini): \(proxyError)"
        )
        ...
        workspace.applyRemoteConnectionStateUpdate(.connecting, detail: "Connecting to coterm-macmini", target: "coterm-macmini")
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to coterm-macmini via shared local proxy 127.0.0.1:9999",
            target: "coterm-macmini"
        )
...
final class CLINotifyProcessIntegrationTests: XCTestCase {
    ...
    private func cliTestEnvironment() -> [String: String] {
        ...
        environment["COTERM_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        return environment
    }
    ...
    func testAgentHookLaunchEnvironmentDoesNotPersistPathOrShell() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-hook-\(UUID().uuidString)", isDirectory: true)
        ...
        environment["COTERM_AGENT_LAUNCH_ARGV_B64"] = base64NULSeparated([
            "/usr/local/bin/codex",
            "--model",
            "gpt-5.4",
            "old prompt"
        ])
        environment["COTERM_AGENT_LAUNCH_CWD"] = "/tmp/repo"
    ...
    func testCodexHookStopSetsRateLimitStatusFromTranscript() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-hook-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testCodexPromptSubmitRetiresPreviousMonitorLeaseForSameSession() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-monitor-leases-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testCodexHookMonitorSetsErrorStatusFromCompletedTranscriptWithoutAssistant() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-monitor-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testNotifyWithWorkspaceHandleKeepsCallerSurfaceFallback() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["COTERM_SOCKET_PATH"] = socketPath
        environment["COTERM_WORKSPACE_ID"] = "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA"
        environment["COTERM_SURFACE_ID"] = staleSurface
    ...
    func testTriggerFlashFallsBackFromStaleCallerWorkspaceAndSurfaceIDs() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["COTERM_SOCKET_PATH"] = socketPath
        environment["COTERM_WORKSPACE_ID"] = staleWorkspace
        environment["COTERM_SURFACE_ID"] = staleSurface
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment["COTERM_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testSSHCommandCreatesConfiguresAndSelectsRemoteWorkspaceViaCLI() throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=coterm-macmini state=disconnected\n")
        XCTAssertTrue(result.stderr.isEmpty, result.stderr)
        ...
        XCTAssertEqual(configureParams["destination"] as? String, "coterm-macmini")
        ...
        XCTAssertTrue(sshOptions.contains("ControlPath /tmp/coterm-ssh-%C"))
        XCTAssertTrue(sshOptions.contains("StrictHostKeyChecking=accept-new"))

        // `coterm ssh` should land the user in the new SSH workspace immediately.
    ...
    func testSSHCommandDoesNotDeferReconnectWhenWhitespaceControlMasterDisablesMultiplexing() throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=coterm-macmini state=connecting\n")
        XCTAssertTrue(result.stderr.isEmpty, result.stderr)
    ...
    func testNotifyPrefersCallerTTYOverFocusedSurfaceWhenCallerIDsAreStale() throws {
        ...
        environment["COTERM_WORKSPACE_ID"] = staleWorkspace
        environment["COTERM_SURFACE_ID"] = staleSurface
        environment["COTERM_CLI_TTY_NAME"] = callerTTY
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment["COTERM_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
