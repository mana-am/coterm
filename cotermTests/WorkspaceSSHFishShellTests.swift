...
final class WorkspaceSSHFishShellTests: XCTestCase {
    ...
    func testSSHBootstrapStartupCommandPassesRemoteInstallScriptAsSingleSSHCommand() throws {
        ...
        let tempRoot = fileManager.temporaryDirectory.appendingPathComponent("coterm-ssh-bootstrap-\(UUID().uuidString)")
        ...
        args = sys.argv[1:]
        with open(os.environ["COTERM_FAKE_SSH_LOG"], "a", encoding="utf-8") as handle:
        ...
        if local_command:
            ...
            subprocess.run([os.environ["COTERM_TEST_LOCAL_SHELL"], "-c", local_command], check=False, env=os.environ.copy())
        PY
        cat >/dev/null
        exit 0
        """
        ...
        startupEnvironment["COTERM_FAKE_SSH_LOG"] = fakeSSHLog.path
        startupEnvironment["COTERM_TEST_PYTHON3"] = python3Path
        startupEnvironment["COTERM_TEST_LOCAL_SHELL"] = fishExecutable
        startupEnvironment["COTERM_SOCKET_PATH"] = socketPath
        startupEnvironment["COTERM_WORKSPACE_ID"] = workspaceID
        startupEnvironment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        startupEnvironment["COTERM_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        ...
        let destinationIndex = try XCTUnwrap(firstInvocation.lastIndex(of: "coterm-macmini"))
        ...
        XCTAssertTrue(
            secondRemoteCommand.contains("coterm_bootstrap_tty=") || secondRemoteCommand.contains("coterm_bootstrap_tty\\\""),
