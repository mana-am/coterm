...
final class CotermConfigContextMenuTests: XCTestCase {
    private func decode(_ json: String) throws -> CotermConfigFile {
        try JSONDecoder().decode(CotermConfigFile.self, from: Data(json.utf8))
    }
    ...
    @MainActor
    private func loadStore(localJSON: String? = nil) throws -> CotermConfigStore {
        ...
        let localConfigURL = root.appendingPathComponent("coterm.json")
        if let localJSON {
    ...
    func testDefaultNewWorkspaceContextMenuIncludesCloudVM() throws {
        ...
        XCTAssertEqual(first.action.id, CotermSurfaceTabBarBuiltInAction.newWorkspace.configID)
        XCTAssertEqual(second.action.id, CotermSurfaceTabBarBuiltInAction.cloudVM.configID)
    ...
    func testCloudVMAliasesResolveToCanonicalBuiltInAction() throws {
        ...
        for item in store.newWorkspaceContextMenuItems {
            ...
            XCTAssertEqual(action.action.id, CotermSurfaceTabBarBuiltInAction.cloudVM.configID)
        }
        XCTAssertTrue(store.configurationIssues.isEmpty)
    }
    ...
    func testActionAliasesCannotOverrideSameBuiltInTwice() throws {
        ...
        {
          "actions": {
            "coterm.cloudvm": { "type": "command", "command": "echo canonical" },
        ...
        XCTAssertThrowsError(try decode(json)) { error in
            ...
            XCTAssertTrue(description.contains(CotermSurfaceTabBarBuiltInAction.cloudVM.configID))
        }
    }
    ...
    func testDefaultCloudVMMenuActionCanBeOverriddenByAlias() throws {
        ...
        XCTAssertEqual(item.action.id, CotermSurfaceTabBarBuiltInAction.cloudVM.configID)
    ...
    func testResolvedNewWorkspaceContextMenuSupportsBuiltInsAndActionOverrides() throws {
        ...
        if case .action(let first) = items[0] {
            XCTAssertEqual(first.action.id, CotermSurfaceTabBarBuiltInAction.newTerminal.configID)
        } else {
            XCTFail("Expected first context-menu item to be an action.")
        }
    ...
    func testNewWorkspaceContextMenuIconRendererSupportsAllIconTypes() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-context-menu-icons-\(UUID().uuidString)",
            isDirectory: true
        )
        ...
        let actions = store.newWorkspaceContextMenuItems.compactMap { item -> CotermResolvedConfigMenuAction? in
    ...
    func testContextMenuImageAssetsAreNormalizedToMenuSize() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-context-menu-large-icon-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let image = try XCTUnwrap(
            CotermButtonIcon.imagePath(iconURL.path).contextMenuImage(
    ...
    func testResolvedNewWorkspaceContextMenuFiltersInvalidWorkspaceCommandActions() throws {
        ...
        XCTAssertEqual(store.newWorkspaceContextMenuItems.count, 1)
        if case .action(let item) = store.newWorkspaceContextMenuItems.first {
            XCTAssertEqual(item.action.id, CotermSurfaceTabBarBuiltInAction.newTerminal.configID)
        } else {
