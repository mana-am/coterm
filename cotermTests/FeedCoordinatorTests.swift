...
struct FeedCoordinatorTests {
    @Test func codexTeamsResolvesExplicitWorkingDirectoryFlags() {
        let base = "/tmp/coterm-base"

        #expect(
            CodexTeamsApprovalBridge.resolvedWorkingDirectory(
                commandArgs: ["-C", "child", "prompt"],
                baseDirectory: base
            ) == "/tmp/coterm-base/child"
        )
        #expect(
            CodexTeamsApprovalBridge.resolvedWorkingDirectory(
                commandArgs: ["--cwd=/tmp/coterm-review", "--cd", "/tmp/coterm-final"],
                baseDirectory: base
            ) == "/tmp/coterm-final"
        )
        #expect(
    ...
    @Test func codexTeamsValidatesExplicitWorkingDirectoryExists() throws {
        let existing = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-teams-cwd-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexAppServerApprovalBuildsActionableFeedEvent() throws {
        let event = CodexTeamsApprovalBridge.feedEvent(
            ...
            params: [
                ...
                "command": "touch /tmp/coterm-security-review",
                "cwd": "/tmp/project",
                ...
                "commandActions": [[
                    "type": "write",
                    "path": "/tmp/coterm-security-review",
        ...
        #expect(toolInput["command"] as? String == "touch /tmp/coterm-security-review")
