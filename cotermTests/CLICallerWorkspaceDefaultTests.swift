...
/// workspace (the `COTERM_WORKSPACE_ID` the app injects into every terminal surface)
...
/// `--workspace "${COTERM_WORKSPACE_ID:-}"` expands to an empty value whenever a
...
struct CLICallerWorkspaceDefaultTests {
    ...
    /// dangerous case where `--workspace "${COTERM_WORKSPACE_ID:-}"` expands to an empty
    ...
    /// visibly retarget there. Pass `callerWorkspaceId: nil` to omit `COTERM_WORKSPACE_ID`.
    ...
    private func cliEnvironment(socketPath: String, callerWorkspaceId: String?) -> [String: String] {
        ...
        environment.removeValue(forKey: "COTERM_WINDOW_ID")
        if let callerWorkspaceId {
            environment["COTERM_WORKSPACE_ID"] = callerWorkspaceId
        } else {
