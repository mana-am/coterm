...
final class CommandPaletteSwitcherSearchIndexerTests: XCTestCase {
    func testKeywordsIncludeDirectoryBranchAndPortMetadata() {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/Users/example/dev/coterm-hq/worktrees/feat-cmd-palette"],
        ...
        XCTAssertTrue(keywords.contains("/Users/example/dev/coterm-hq/worktrees/feat-cmd-palette"))
    ...
    func testFuzzyMatcherMatchesDirectoryBranchAndPortMetadata() {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/tmp/coterm/worktrees/issue-123-switcher-search"],
    ...
    func testSurfaceDetailOutranksWorkspaceDetailForPathToken() throws {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/tmp/worktrees/coterm"],
        ...
        let workspaceScore = try XCTUnwrap(
            CommandPaletteFuzzyMatcher.score(query: "coterm", candidates: workspaceKeywords)
        )
        let surfaceScore = try XCTUnwrap(
            CommandPaletteFuzzyMatcher.score(query: "coterm", candidates: surfaceKeywords)
        )
