...
final class CotermEventBusTests: XCTestCase {
    ...
    func testSubscribeReplaysEventsAfterSequenceAndReportsAck() throws {
        let bus = CotermEventBus(retainedEventLimit: 4)
        bus.publish(
    ...
    func testSubscribeReportsGapWhenCursorFallsOutOfRetention() throws {
        let bus = CotermEventBus(retainedEventLimit: 2)
    ...
    func testSubscriptionFiltersLiveEventsByCategory() {
        let bus = CotermEventBus(retainedEventLimit: 8)
    ...
    func testSlowSubscriptionClosesWhenPendingQueueIsFull() {
        let bus = CotermEventBus(retainedEventLimit: 8, maxPendingEventsPerSubscription: 2)
    ...
    func testEventEncodingIsSingleLineJSON() throws {
        ...
        let line = try XCTUnwrap(CotermEventBus.encodeLine(event))
    ...
    func testEncodingPreservesZeroAndOneNumbers() throws {
        let line = try XCTUnwrap(CotermEventBus.encodeLine([
    ...
    func testStrictSequenceParsingRejectsBooleanAndFloatFrames() throws {
        XCTAssertEqual(CotermEventBus.int64(NSNumber(value: Int64(42))), 42)
        XCTAssertEqual(CotermEventBus.int64("42"), 42)
        XCTAssertNil(CotermEventBus.int64(true))
        XCTAssertNil(CotermEventBus.int64(NSNumber(value: 1.5)))
    }

    func testOversizedEventPayloadIsTruncatedBeforeRetention() throws {
        let bus = CotermEventBus(retainedEventLimit: 4, maxEventLineBytes: 1_024)
    ...
    func testWorkspaceReorderSocketMapperDoesNotDuplicateLifecycleEvent() throws {
        ...
        let snapshot = CotermEventBus.shared.subscribe(
            afterSequence: nil,
            names: ["workspace.reordered"],
            categories: []
        )
        defer {
            CotermEventBus.shared.unsubscribe(snapshot.subscription)
            CotermEventBus.shared.resetForTesting()
        }
        ...
        CotermSocketEventMapper.publish(command: command, response: response)
    ...
    func testPublishV2ReadTextResponseDoesNotAccumulateOnLongLivedThread() throws {
        ...
        XCTAssertTrue(CotermEventBus.shared.retainedSnapshot().isEmpty)
    }
    ...
    func testBulkNotificationClearPublishesClearedWithoutRemovedDuplicates() throws {
        ...
        let events = CotermEventBus.shared.retainedSnapshot()
    ...
    func testNotificationSocketParamsRedactTextFields() throws {
        let redacted = CotermSocketEventMapper.redactedNotificationParams([
            "title": "Secret title",
    ...
    func testV1NotifySurfacePublishesSurfaceIdWithoutWorkspaceId() throws {
        ...
        CotermSocketEventMapper.publish(command: "notify_surface \(surfaceId.uuidString) done", response: "OK")
    ...
    func testV1MapperIgnoresNonSuccessResponses() throws {
        ...
        CotermSocketEventMapper.publish(command: "notify title", response: "OKAY")
        CotermSocketEventMapper.publish(command: "notify title", response: "queued")
        CotermSocketEventMapper.publish(command: "notify title", response: "ERROR: failed")
    ...
    func testWorkstreamPayloadRedactsSensitiveFields() throws {
        ...
        let payload = CotermEventBus.workstreamPayload(event)
        ...
        let line = try XCTUnwrap(CotermEventBus.encodeLine(["payload": payload]))
        XCTAssertFalse(line.contains("secret"))
    }

    func testPublishAppendsDurableEventLog() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-event-log-\(UUID().uuidString)", isDirectory: true)
        let logURL = directory.appendingPathComponent("events.jsonl")
        let bus = CotermEventBus(retainedEventLimit: 4, eventLogURL: logURL)
    ...
    func testDurableEventLogDropsOldestPendingLinesUnderBackpressure() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-event-log-backpressure-\(UUID().uuidString)", isDirectory: true)
        let logURL = directory.appendingPathComponent("events.jsonl")
        let bus = CotermEventBus(
    ...
    func testDurableEventLogRotatesAtByteLimit() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-event-log-rotation-\(UUID().uuidString)", isDirectory: true)
        let logURL = directory.appendingPathComponent("events.jsonl")
        let bus = CotermEventBus(
