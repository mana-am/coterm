...
extension CLINotifyProcessIntegrationRegressionTests {
    func testVMSSHOpensManagedWorkspaceThroughSharedSSHPath() throws {
        ...
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=coterm@gateway.freestyle.sh state=connecting\n")
        ...
        XCTAssertEqual(configureParams["destination"] as? String, "coterm@gateway.freestyle.sh")
    ...
    func testSSHCommandGlobalWindowOverridesCallerEnvironment() throws {
        ...
        environment["COTERM_SURFACE_ID"] = callerSurfaceID
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment["COTERM_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testVMSSHInfoRemainsPrintOnly() throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertTrue(result.stdout.contains("ssh coterm@gateway.freestyle.sh -p 2222"), result.stdout)
