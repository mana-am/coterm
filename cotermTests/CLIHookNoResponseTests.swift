...
struct CLIHookNoResponseTests {
    ...
    @Test func nonActionableFeedHooksDoNotWaitForSocketResponseAcrossAgents() throws {
        let cases = [
            FeedHookCase(source: "codex", event: "PreToolUse", toolName: "apply_patch", pidKey: "COTERM_CODEX_PID"),
            FeedHookCase(source: "gemini", event: "PreToolUse", toolName: "read", pidKey: "COTERM_GEMINI_PID"),
            FeedHookCase(source: "kiro", event: "postToolUse", toolName: "fs_write", pidKey: "COTERM_KIRO_PID"),
            FeedHookCase(source: "hermes-agent", event: "pre_tool_call", toolName: "terminal", pidKey: "COTERM_HERMES_AGENT_PID"),
            FeedHookCase(source: "antigravity", event: "PostToolUse", toolName: "run_command", pidKey: "COTERM_ANTIGRAVITY_PID"),
        ]
        ...
        for testCase in cases {
            ...
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("coterm-feed-no-reply-\(testCase.source)-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func genericLifecycleFeedTelemetryDoesNotWaitForSocketResponse() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-kiro-lifecycle-no-response-\(UUID().uuidString)", isDirectory: true)
        ...
        let result = Self.runProcess(
            ...
            environment: [
                ...
                "COTERM_AGENT_LAUNCH_KIND": "kiro",
                "COTERM_AGENT_LAUNCH_EXECUTABLE": "/Users/example/.cargo/bin/kiro-cli",
                "COTERM_AGENT_LAUNCH_ARGV_B64": Self.base64NULSeparated([
                ...
                "COTERM_AGENT_LAUNCH_CWD": root.path,
                "COTERM_CLI_SENTRY_DISABLED": "1",
                "COTERM_SOCKET_PASSWORD": "test-password",
            ],
    ...
    @Test func nonActionableFeedHookDoesNotBlockWhenAcceptedSocketStopsReading() throws {
        ...
        let listenerFD = try Self.bindUnixSocket(at: socketPath, backlog: 1)
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-feed-no-read-\(UUID().uuidString)", isDirectory: true)
    ...
    private static func bundledCLIPath() throws -> String {
        ...
        while let item = enumerator?.nextObject() as? URL {
            guard item.lastPathComponent == "coterm",
                  item.path.contains(".app/Contents/Resources/bin/coterm") else {
                continue
            }
            return item.path
        }
        ...
        throw NSError(domain: "coterm.tests", code: 1, userInfo: [
            NSLocalizedDescriptionKey: "Bundled coterm CLI not found in \(appBundleURL.path)",
        ])
    }
    ...
    private static func runProcess(
        ...
        if let standardInput {
            let url = FileManager.default.temporaryDirectory
                .appendingPathComponent("coterm-test-stdin-\(UUID().uuidString).json")
            do {
    ...
    private static func posixError(_ message: String) -> NSError {
        NSError(domain: "coterm.tests", code: Int(errno), userInfo: [
