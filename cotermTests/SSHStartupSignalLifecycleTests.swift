...
extension CLINotifyProcessIntegrationRegressionTests {
    func testSSHPaneCloseSignalDoesNotReportSessionEndToSharedTransport() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ssh-pane-close-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("coterm")
        ...
        try writeShellFile(at: fakeCLI, lines: [
            "#!/bin/sh",
            "printf '%s\\n' \"$*\" >> \"${COTERM_TEST_SESSION_END_LOG}\"",
        ])
        try writeShellFile(at: fakeSSH, lines: [
            "#!/bin/sh",
            "kill -\"${COTERM_TEST_SIGNAL:?}\" \"${COTERM_SSH_STARTUP_PID:-$PPID}\"",
            "sleep 0.1",
            "exit 0",
        ])
        ...
        for signal in ["HUP", "INT", "TERM"] {
            ...
            environment["COTERM_BUNDLED_CLI_PATH"] = fakeCLI.path
            environment["COTERM_SOCKET_PATH"] = "/tmp/coterm-debug-test.sock"
            environment["COTERM_WORKSPACE_ID"] = "11111111-1111-1111-1111-111111111111"
            environment["COTERM_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
            environment["COTERM_TEST_SESSION_END_LOG"] = logFile.path
            environment["COTERM_TEST_SIGNAL"] = signal
    ...
    func testSSHPaneCloseSignalDoesNotTerminateWrappedSSHChild() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ssh-pane-close-child-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("coterm")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        ...
        try writeShellFile(at: fakeSSH, lines: [
            "#!/bin/sh",
            "trap 'printf \"%s\\n\" child-hup >> \"${COTERM_TEST_CHILD_SIGNAL_LOG}\"; exit 0' HUP",
            "trap 'printf \"%s\\n\" child-int >> \"${COTERM_TEST_CHILD_SIGNAL_LOG}\"; exit 0' INT",
            "trap 'printf \"%s\\n\" child-term >> \"${COTERM_TEST_CHILD_SIGNAL_LOG}\"; exit 0' TERM",
            "printf '%s\\n' child-started >> \"${COTERM_TEST_CHILD_SIGNAL_LOG}\"",
            "kill -\"${COTERM_TEST_SIGNAL:?}\" \"${COTERM_SSH_STARTUP_PID:-$PPID}\"",
            "sleep 0.2",
            "printf '%s\\n' child-completed >> \"${COTERM_TEST_CHILD_SIGNAL_LOG}\"",
            "exit 0",
        ])
        try fileManager.setAttributes([.posixPermissions: 0o700], ofItemAtPath: fakeCLI.path)
        try fileManager.setAttributes([.posixPermissions: 0o700], ofItemAtPath: fakeSSH.path)
        ...
        for signal in ["HUP", "INT", "TERM"] {
            ...
            environment["COTERM_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
            environment["COTERM_TEST_SESSION_END_LOG"] = logFile.path
            environment["COTERM_TEST_CHILD_SIGNAL_LOG"] = childSignalLog.path
            environment["COTERM_TEST_SIGNAL"] = signal
    ...
    func testSSHStartupRetriesTransientSSHExitBeforeReportingSessionEnd() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ssh-reconnect-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("coterm")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        ...
        try writeShellFile(at: fakeSSH, lines: [
            "#!/bin/sh",
            "count=0",
            "if [ -r \"${COTERM_TEST_ATTEMPT_FILE}\" ]; then count=$(cat \"${COTERM_TEST_ATTEMPT_FILE}\"); fi",
            "count=$((count + 1))",
            "printf '%s\\n' \"$count\" > \"${COTERM_TEST_ATTEMPT_FILE}\"",
        ...
        environment["COTERM_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
        environment["COTERM_TEST_SESSION_END_LOG"] = logFile.path
        environment["COTERM_TEST_ATTEMPT_FILE"] = attemptFile.path
        environment["COTERM_SSH_RECONNECT_DELAY_SECONDS"] = "0"
        ...
        // The regular `coterm ssh` bootstrap path runs one SSH command to install
    ...
    func testSSHStartupRemovesStaleCotermControlSocketBeforeLaunchingPaneSSH() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ssh-stale-control-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("coterm")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh.log")
        let staleControlPath = URL(fileURLWithPath: "/tmp", isDirectory: true)
            .appendingPathComponent("coterm-ssh-\(getuid())-\(UUID().uuidString.prefix(8)).sock")
        ...
        try writeShellFile(at: fakeSSH, lines: [
            ...
            "  previous_arg=\"$arg\"",
            "done",
            "if [ -e \"${COTERM_TEST_CONTROL_PATH}\" ]; then",
            "  printf 'ControlSocket %s already exists, disabling multiplexing\\n' \"${COTERM_TEST_CONTROL_PATH}\" >&2",
            "  exit 99",
            "fi",
            "exit 0",
        ])
        ...
        environment["COTERM_WORKSPACE_ID"] = "11111111-1111-1111-1111-111111111111"
        environment["COTERM_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
        environment["COTERM_TEST_CONTROL_PATH"] = staleControlPath.path
        environment["COTERM_TEST_SESSION_END_LOG"] = root.appendingPathComponent("ssh-session-end.log").path
        environment["COTERM_TEST_SSH_LOG"] = logFile.path
        environment["COTERM_SSH_RECONNECT_DELAY_SECONDS"] = "0"
    ...
    func testSSHStartupStopsAtConfiguredReconnectLimit() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ssh-retry-limit-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("coterm")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        let attemptFile = root.appendingPathComponent("ssh-attempts.txt")
        ...
        environment["COTERM_TEST_ATTEMPT_FILE"] = attemptFile.path
        environment["COTERM_SSH_RECONNECT_DELAY_SECONDS"] = "0"
        environment["COTERM_SSH_RECONNECT_LIMIT"] = "2"
    ...
    func testSSHStartupDoesNotRetryNonTransientSSHExit() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ssh-no-retry-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("coterm")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        let attemptFile = root.appendingPathComponent("ssh-attempts.txt")
    ...
    func testSSHSignalDerivedChildExitReportsSessionEnd() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ssh-child-signal-exit-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("coterm")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
    ...
    func testSSHSignalDuringReconnectDelayDoesNotStartAnotherSSH() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ssh-signal-during-reconnect-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("coterm")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        let attemptFile = root.appendingPathComponent("ssh-attempts.txt")
        ...
        try writeShellFile(at: fakeSSH, lines: [
            ...
            "printf '%s\\n' \"$count\" > \"${COTERM_TEST_ATTEMPT_FILE}\"",
            "if [ \"$count\" -eq 1 ]; then",
            "  ( sleep 0.2; kill -TERM \"${COTERM_SSH_STARTUP_PID:?}\" ) &",
            "  exit 255",
            "fi",
            "exit 0",
        ])
        ...
        environment["COTERM_TEST_SESSION_END_LOG"] = logFile.path
        environment["COTERM_TEST_ATTEMPT_FILE"] = attemptFile.path
        environment["COTERM_SSH_RECONNECT_DELAY_SECONDS"] = "2"
        environment["COTERM_SSH_RECONNECT_LIMIT"] = "2"
    ...
    func testSSHStartupPrintsFinalErrorBannerWhenStderrIsCaptured() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ssh-error-banner-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("coterm")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 1, result.stderr)
        XCTAssertTrue(result.stderr.contains("[coterm] ssh exited with status 1."), result.stderr)
        XCTAssertTrue(result.stderr.contains("[coterm] press Enter to close this pane."), result.stderr)
    }

    func testSSHStartupForwardsStdinToBackgroundedSSH() throws {
        // Regression test for coterm ssh sessions where output flowed back from
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-ssh-stdin-forward-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("coterm")
        let fakeSSH = root.appendingPathComponent("ssh")
        ...
        environment["COTERM_WORKSPACE_ID"] = "11111111-1111-1111-1111-111111111111"
        environment["COTERM_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
        environment["COTERM_TEST_SESSION_END_LOG"] = sessionEndLog.path
        environment["COTERM_TEST_STDIN_LOG"] = stdinCapture.path
        environment["COTERM_SSH_RECONNECT_DELAY_SECONDS"] = "0"
    ...
    private func generatedSSHStartupCommand(
        ...
        arguments.append("coterm-macmini")
