...
final class FinderFileDropRegressionTests: XCTestCase {
    ...
    func testLegacyFinderFilenameDropPlanInsertsEscapedLocalPath() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("coterm-test-legacy-filename-drop-\(UUID().uuidString)"))
    ...
    func testImageFileURLDropInsertsOriginalLocalImagePaths() throws {
        let imageDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm image file drop \(UUID().uuidString)")
        try FileManager.default.createDirectory(at: imageDirectory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: imageDirectory) }

        let firstURL = imageDirectory.appendingPathComponent("coterm ssh 2.png")
        let secondURL = imageDirectory.appendingPathComponent("coterm ssh.png")
        try make1x1PNG(color: .systemRed).write(to: firstURL)
        try make1x1PNG(color: .systemGreen).write(to: secondURL)

        let pasteboard = NSPasteboard(name: .init("coterm-test-image-file-url-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImageFileURLDropUploadsOriginalFilesForRemoteTerminal() throws {
        let imageDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm remote image file drop \(UUID().uuidString)")
        try FileManager.default.createDirectory(at: imageDirectory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: imageDirectory) }
        ...
        let pasteboard = NSPasteboard(name: .init("coterm-test-remote-image-file-url-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
        XCTAssertTrue(pasteboard.writeObjects([firstURL as NSURL, secondURL as NSURL]))
    ...
    func testImagePasteboardDropMaterializesEveryImageForLocalInsertion() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-multi-image-local-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
        let items = try [
    ...
    func testImagePasteboardItemWithDirectImageAndRTFDAttachmentMaterializesOnce() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-image-rtfd-duplicate-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImagePasteboardItemTIFFDropNormalizesToPNGForLocalInsertion() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-image-tiff-normalization-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImagePasteboardDropMaterializesEveryImageForRemoteUpload() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-multi-image-remote-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
        let items = try [
            makeImagePasteboardItem(color: .systemRed),
            makeImagePasteboardItem(color: .systemGreen),
    ...
    func testFileExplorerPathInsertionEscapesMultiplePathsLikeTerminalDrop() {
        let paths = [
            "/tmp/coterm path/one file.txt",
            "/tmp/coterm path/quote's file.txt"
        ]
    ...
    func testFileURLTextInsertionIsExtensionAgnostic() {
        let urls = [
            URL(fileURLWithPath: "/tmp/coterm drop/image.png"),
            URL(fileURLWithPath: "/tmp/coterm drop/report.pdf"),
            URL(fileURLWithPath: "/tmp/coterm drop/movie.mov"),
            URL(fileURLWithPath: "/tmp/coterm drop/archive.zip")
        ]
    ...
    func testFilePreviewTransferRoutesToTextEvenWhenTargetPasteboardOmitsFileURLType() throws {
        let filePath = "/tmp/coterm drop/from image pane.png"
        let dragId = UUID()
        ...
        let pasteboard = NSPasteboard(name: .init("coterm-test-file-preview-transfer-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testFileExplorerRelativePathInsertionStandardizesMacOSSymlinkedRoots() {
        XCTAssertEqual(
            FileExplorerTerminalPathInsertion.relativePath(
                for: "/private/tmp/coterm-project/Sources/App.swift",
                rootPath: "/tmp/coterm-project"
            ),
            "Sources/App.swift"
        )
    }
