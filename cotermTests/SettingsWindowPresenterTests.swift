...
struct SettingsWindowPresenterTests {
    @Test func configureWindowLeavesPendingNavigationForSettingsViews() async throws {
        try await withCleanSettingsWindows {
            ...
            let settingsWindow = makeWindow(identifier: "coterm.unconfiguredSettings.\(UUID().uuidString)")
            var didOpen = false
            defer {
    ...
    @Test func doesNotAttachSettingsAsChildOfPreferredMainWindow() async throws {
        try await withCleanSettingsWindows {
            let presenter = SettingsWindowPresenter()
            let parentWindow = makeWindow(identifier: "coterm.main.\(UUID().uuidString)")
            let settingsWindow = makeWindow(identifier: SettingsWindowPresenter.windowIdentifier)
            defer {
                settingsWindow.orderOut(nil)
    ...
    @Test func focusingSettingsKeepsItAsPeerWhenPreferredMainWindowChanges() async throws {
        try await withCleanSettingsWindows {
            let presenter = SettingsWindowPresenter()
            let firstParent = makeWindow(identifier: "coterm.main.\(UUID().uuidString)")
            let secondParent = makeWindow(identifier: "coterm.main.\(UUID().uuidString)")
            let settingsWindow = makeWindow(identifier: SettingsWindowPresenter.windowIdentifier)
    ...
    @Test func adoptCotermPeerWindowLevelBringsFloatingWindowToNormal() async throws {
        try await withCleanSettingsWindows {
            let window = makeWindow(identifier: "coterm.peer.\(UUID().uuidString)")
            defer {
                window.orderOut(nil)
                window.close()
            }
