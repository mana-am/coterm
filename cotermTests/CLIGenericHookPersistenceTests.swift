...
extension CLINotifyProcessIntegrationRegressionTests {
    ...
    func testGenericHookAgentsPersistSanitizedLaunchCommandsForSessionRestore() throws {
        let scenarios: [GenericHookPersistenceScenario] = [
            ...
            GenericHookPersistenceScenario(
                ...
                executable: "/tmp/coterm-agent-upstreams/copilot-install/bin/copilot",
                launchArguments: [
                    "/tmp/coterm-agent-upstreams/copilot-install/bin/copilot",
                    "--model",
                    "gpt-5.4",
                    "--resume=old-session",
                    "--allow-all-tools",
    ...
    func testAntigravityStopAndNotificationsUseGenericNotificationPath() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-antigravity-notification-\(UUID().uuidString)", isDirectory: true)
        ...
        let environment: [String: String] = [
            ...
            "PWD": root.path,
            "COTERM_SOCKET_PATH": socketPath,
            "COTERM_WORKSPACE_ID": workspaceId,
            "COTERM_SURFACE_ID": surfaceId,
            "COTERM_AGENT_HOOK_STATE_DIR": root.path,
        ...
        XCTAssertTrue(
            ...
            "Expected Antigravity permission notifications to publish through coterm, saw \(permissionCommands)"
        )
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            ...
            "Expected Antigravity Stop errors to publish through coterm, saw \(stopErrorCommands)"
        )
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            ...
            "Expected Antigravity error notifications to publish through coterm, saw \(errorCommands)"
        )
        XCTAssertTrue(
    ...
    func testHermesAgentNotificationsUseShellHookExtraPayload() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-hermes-notification-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testHermesAgentSessionEndIsTurnBoundaryButFinalizeTearsDown() throws {
        ...
        // separate `on_session_finalize` hook once at genuine teardown. coterm maps the
        ...
        // See https://github.com/emergent-inc/coterm/issues/5000.
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("hermes-session-end")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-hermes-session-end-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testAntigravityHookInstallUsesNativeHooksJSONShape() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-antigravity-hook-install-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        ...
        let result = runProcess(
            ...
            environment: [
                "HOME": root.path,
                "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
                "COTERM_BUNDLED_CLI_PATH": root.path,
                "COTERM_CLI_SENTRY_DISABLED": "1",
            ],
            timeout: 5
        )
        ...
        let cotermGroup = try XCTUnwrap(json["coterm"] as? [String: Any])
        let allCommands = cotermGroup.values
        ...
        XCTAssertTrue(
            allCommands.allSatisfy { $0.contains("coterm-antigravity-hook-v2") },
        ...
        XCTAssertFalse(
            ...
            "Directory-valued COTERM_BUNDLED_CLI_PATH must not be embedded as a hook executable, saw \(allCommands)"
        )
        XCTAssertFalse(
            allCommands.contains { $0.contains(#"[ -n "$COTERM_SURFACE_ID" ]"#) },
            "Antigravity hooks must still dispatch when agy does not preserve COTERM_SURFACE_ID, saw \(allCommands)"
        )

        let preToolUse = try XCTUnwrap(cotermGroup["PreToolUse"] as? [[String: Any]])
        ...
        let stop = try XCTUnwrap(cotermGroup["Stop"] as? [[String: Any]])
        XCTAssertTrue(
            stop.contains {
        ...
        XCTAssertNotNil(cotermGroup["SessionStart"])
        XCTAssertNotNil(cotermGroup["SessionEnd"])
        XCTAssertNotNil(cotermGroup["turn-completion"])
        XCTAssertNotNil(cotermGroup["Notification"])
        XCTAssertNotNil(cotermGroup["PostToolUse"])
    }

    func testKiroHookInstallUsesAgentConfigShapeAndPreservesDenyExit() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-kiro-hook-install-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertTrue(
            result.stdout.contains("kiro-cli chat --agent coterm"),
            "Expected Kiro install to print the --agent coterm activation hint, saw: \(result.stdout)"
        )
        ...
        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: hookURL)) as? [String: Any])
        XCTAssertEqual(json["name"] as? String, "coterm")
        XCTAssertNil(json["version"], "Kiro agent configs should not receive Cursor's hooks version field")
        XCTAssertEqual(
            json["tools"] as? [String], ["*"],
            "Kiro coterm agent must grant the full tool set so `--agent coterm` can run tools and fire preToolUse hooks"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected Kiro preToolUse feed hook to preserve coterm's exit status for deny decisions, saw \(preToolUse)"
        )
    ...
    func testKiroFeedDenyUsesPreToolUseExitCodeTwo() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-kiro-feed-deny-\(UUID().uuidString)", isDirectory: true)
        ...
        let result = runProcess(
            ...
            environment: [
                ...
                "COTERM_WORKSPACE_ID": workspaceId,
                "COTERM_SURFACE_ID": surfaceId,
                "COTERM_KIRO_PID": "525252",
                "COTERM_KIRO_NOTIFICATION_LEVEL": "standard",
                "COTERM_CLI_SENTRY_DISABLED": "1",
            ],
        ...
        XCTAssertTrue(result.stderr.contains("User denied permission via coterm Feed."), result.stderr)
    ...
    func testKiroFeedAllowModesProceedAndUnknownModeDenies() throws {
        func runKiroDecision(mode: String) throws -> ProcessRunResult {
            ...
            let state = MockSocketServerState()
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("coterm-kiro-feed-mode-\(UUID().uuidString)", isDirectory: true)
            try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
            defer {
                Darwin.close(listenerFD)
                unlink(socketPath)
            ...
            let result = runProcess(
                ...
                environment: [
                    ...
                    "PWD": root.path,
                    "COTERM_SOCKET_PATH": socketPath,
                    "COTERM_WORKSPACE_ID": "33333333-3333-3333-3333-333333333333",
                    "COTERM_SURFACE_ID": "44444444-4444-4444-4444-444444444444",
                    "COTERM_KIRO_PID": "525252",
                    "COTERM_KIRO_NOTIFICATION_LEVEL": "standard",
                    "COTERM_CLI_SENTRY_DISABLED": "1",
                ],
    ...
    func testKiroStandardLevelSuppressesReadOnlyToolFeedEvents() throws {
        func feedPushCount(forTool tool: String) throws -> Int {
            ...
            let state = MockSocketServerState()
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("coterm-kiro-suppress-\(UUID().uuidString)", isDirectory: true)
            try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
            defer {
                Darwin.close(listenerFD)
                unlink(socketPath)
    ...
    func testLowercaseGenericFeedToolsStayTelemetryOutsideKiro() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-generic-lowercase-feed-tool-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
        let surfaceId = "44444444-4444-4444-4444-444444444444"
    ...
    func testAntigravityFeedHookMissingSessionIdUsesStableFallback() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-antigravity-feed-stable-session-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
        let surfaceId = "44444444-4444-4444-4444-444444444444"
    ...
    func testGrokNotificationHookUsesPayloadMessageAndStopDoesNotSendGenericNotification() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-notification-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testGrokStopFallbackCompletionsFireForTwoConcurrentThreads() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-two-threads-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceIds = [
        ...
        func runGrokHook(_ subcommand: String, input: String, surfaceId: String) -> ProcessRunResult {
            ...
            environment["COTERM_SURFACE_ID"] = surfaceId
            let result = runProcess(
                executablePath: cliPath,
                arguments: ["hooks", "grok", subcommand],
                environment: environment,
    ...
    func testGrokStopNotificationFallsBackWhenTranscriptCwdIsUnavailable() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-stop-without-cwd-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testGrokNotificationStillFiresOnRepeatedPromptWhenFeedTelemetryDoesNotReply() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-repeat-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testGrokSessionEndDoesNotDropRoutingForLaterChatMessages() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-turns-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
        ...
        for index in 1...2 {
            ...
            XCTAssertTrue(
                ...
                "Expected Grok prompt \(index) to reuse the saved target without COTERM env, saw \(promptCommands)"
            )
            XCTAssertTrue(
    ...
    func testGrokCompletionDoesNotResetStatusWhileSiblingSessionRuns() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-sibling-status-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
    ...
    func testGrokCompletionResetsStatusWhenSiblingRunningRecordHasDeadPID() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-stale-sibling-status-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        ...
        let environment: [String: String] = [
            ...
            "COTERM_CLI_SENTRY_DISABLED": "1",
            "COTERM_WORKSPACE_ID": workspaceId,
            "COTERM_SURFACE_ID": completingSurfaceId,
        ]
    ...
    func testGrokHookInstallRoutesNotificationEventToNotificationSubcommand() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-hook-install-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
            ...
            "hooks": [
                "PostToolUse": [
                    [
                        "hooks": [
                            [
                                "command": "[ -n \"$COTERM_SURFACE_ID\" ] && [ \"$COTERM_GROK_HOOKS_DISABLED\" != \"1\" ] && command -v coterm >/dev/null 2>&1 && coterm hooks feed --source grok --event PostToolUse || echo '{}'",
                                "timeout": 120,
                                "type": "command",
                            ],
                        ],
                ...
                "Stop": [
                    [
                        "hooks": [
                            [
                                "command": "[ -n \"$COTERM_SURFACE_ID\" ] && [ \"$COTERM_GROK_HOOKS_DISABLED\" != \"1\" ] && command -v coterm >/dev/null 2>&1 && coterm hooks grok stop || echo '{}'",
                                "timeout": 5,
                                "type": "command",
                            ],
                        ],
        ...
        let hookURL = root
            .appendingPathComponent(".grok", isDirectory: true)
            .appendingPathComponent("hooks", isDirectory: true)
            .appendingPathComponent("coterm-session.json", isDirectory: false)
        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: hookURL)) as? [String: Any])
        let hooks = try XCTUnwrap(json["hooks"] as? [String: Any])
        ...
        XCTAssertTrue(
            notificationCommands.contains { $0.contains("coterm hooks grok notification") },
            "Expected Grok Notification to dispatch to the notification handler, saw \(notificationCommands)"
        )
        XCTAssertFalse(
            notificationCommands.contains { $0.contains("coterm hooks grok stop") },
        ...
        XCTAssertFalse(
            allCommands.contains { $0.contains("[ -n \"$COTERM_SURFACE_ID\" ]") },
            "Grok strips COTERM_* from hook subprocesses, so installed commands must not gate on COTERM_SURFACE_ID. Saw \(allCommands)"
        )
        XCTAssertFalse(
            allCommands.contains { $0.contains("$COTERM_") },
            "Grok treats $VAR references as required hook environment, so installed commands must avoid COTERM variable interpolation. Saw \(allCommands)"
        )
        XCTAssertFalse(
            FileManager.default.fileExists(atPath: legacyHookURL.path),
            "Expected setup to remove legacy coterm-owned Grok hook file"
        )
    }

    func testGrokHookInstallPinsInstallingCLIAndSocketWithoutCotermInterpolation() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-hook-pin-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let pinnedCLI = root.appendingPathComponent("coterm pinned dev cli", isDirectory: false)
        ...
        let socketPath = "/tmp/coterm-debug-grok-pin.sock"
        let result = runProcess(
            executablePath: cliPath,
            arguments: ["hooks", "grok", "install", "--yes"],
            environment: [
                "HOME": root.path,
                "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
                "COTERM_BUNDLED_CLI_PATH": pinnedCLI.path,
                "COTERM_SOCKET_PATH": socketPath,
                "COTERM_CLI_SENTRY_DISABLED": "1",
            ],
            timeout: 5
        ...
        XCTAssertFalse(allCommands.isEmpty)
        XCTAssertTrue(
            allCommands.allSatisfy { $0.contains("coterm-grok-hook-v2") },
        ...
        XCTAssertFalse(
            allCommands.contains { $0.contains("$COTERM_") },
            "Grok hook commands must not depend on COTERM environment interpolation, saw \(allCommands)"
        )
    }
    ...
    func testGrokHookInstallPreservesUserWrappedLegacyCommands() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-hook-preserve-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let preservedCommand = "bash -lc 'coterm hooks grok notification && ~/bin/after-grok'"
        let legacyHookJSON: [String: Any] = [
            "hooks": [
                "Notification": [
                    [
                        "hooks": [
                            ...
                            [
                                "command": "[ \"$COTERM_GROK_HOOKS_DISABLED\" != \"1\" ] && command -v coterm >/dev/null 2>&1 && coterm hooks grok notification || echo '{}'",
                                "timeout": 10,
                                "type": "command",
                            ],
                        ],
        ...
        XCTAssertFalse(
            commands.contains { $0.hasPrefix("[ \"$COTERM_GROK_HOOKS_DISABLED\"") },
            "Expected setup to remove only exact coterm-owned legacy commands, saw \(commands)"
        )
    }
    ...
    func testGrokHookInstallPreservesLegacyFileMetadataWhenPruningOwnedHooks() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-hook-metadata-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testCodexHookInstallPrefersLaunchingAppBundledCLI() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-hook-install-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let previousBundledHookCommand = "coterm_cli=\"${COTERM_BUNDLED_CLI_PATH:-}\"; if [ -z \"$coterm_cli\" ] || [ ! -x \"$coterm_cli\" ]; then coterm_cli=\"$(command -v coterm 2>/dev/null || true)\"; fi; [ -n \"$COTERM_SURFACE_ID\" ] && [ \"$COTERM_CODEX_HOOKS_DISABLED\" != \"1\" ] && [ -n \"$coterm_cli\" ] && \"$coterm_cli\" hooks codex prompt-submit || echo '{}'"
        let legacyHookJSON: [String: Any] = [
            "hooks": [
                "UserPromptSubmit": [
                    [
        ...
        XCTAssertTrue(
            commandBodies.contains {
                $0.contains("COTERM_BUNDLED_CLI_PATH")
                    && $0.contains("\"$coterm_cli\" --socket \"$COTERM_SOCKET_PATH\" hooks codex prompt-submit")
            },
            "Codex hooks should route through the launching app's bundled CLI, saw \(commandBodies)"
        )
        XCTAssertFalse(
            commandBodies.contains { $0.contains("command -v coterm >/dev/null 2>&1 && coterm hooks codex") },
            "Codex hooks must not use the reload-global coterm shim directly, saw \(commandBodies)"
        )
        XCTAssertFalse(
            commandBodies.contains { $0 == previousBundledHookCommand },
            "Codex setup should replace bundled-CLI hooks that did not pin COTERM_SOCKET_PATH, saw \(commandBodies)"
        )
        XCTAssertEqual(
            allCommands.filter { $0.contains("hooks codex prompt-submit") }.count,
            1,
            "Codex setup should collapse duplicate coterm-owned prompt hooks to one entry, saw \(allCommands)"
        )
    }
    ...
    func testGrokHookInstallRejectsFileAtHooksDirectory() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-hook-file-dir-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertTrue(
            result.stderr.contains("coterm could not create the hooks directory: a file exists at \(hooksPath.path); remove or rename the conflicting file and re-run `coterm hooks setup`"),
            result.stderr
        )
        XCTAssertFalse(
    ...
    func runGenericHookPersistenceScenario(_ scenario: GenericHookPersistenceScenario) throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-\(scenario.agent)-hook-\(UUID().uuidString)", isDirectory: true)
        ...
        var environment: [String: String] = [
            ...
            "COTERM_SURFACE_ID": surfaceId,
            "COTERM_AGENT_HOOK_STATE_DIR": root.path,
            "COTERM_AGENT_LAUNCH_KIND": scenario.agent,
            "COTERM_AGENT_LAUNCH_EXECUTABLE": scenario.executable,
            "COTERM_AGENT_LAUNCH_ARGV_B64": base64NULSeparated(scenario.launchArguments),
            "COTERM_AGENT_LAUNCH_CWD": workspace.path,
            "COTERM_CLI_SENTRY_DISABLED": "1",
        ]
        ...
        if scenario.agent == "kiro" {
            ...
            XCTAssertEqual(
                ...
                "cd -- '\(workspace.path)' 2>/dev/null || [ ! -d '\(workspace.path)' ] && '\(scenario.executable)' 'chat' '--resume-id' '\(scenario.sessionId)' '--agent' 'coterm' '--trust-tools' 'fs_read,fs_write'"
            )
    ...
    /// G2 (https://github.com/emergent-inc/coterm/issues/5350): plain `codex` under the subrouter account
    /// manager points CODEX_HOME at ~/.codex-accounts/<account>, not ~/.codex. When the launch argv
    /// can't be captured (no COTERM_AGENT_LAUNCH_ARGV_B64 and an exited PID), the session record used to
    ...
    func testCodexHookPreservesCodexHomeWhenLaunchCommandUnavailable() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-home-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
        ...
        environment["COTERM_WORKSPACE_ID"] = workspaceId
        environment["COTERM_SURFACE_ID"] = surfaceId
        environment["COTERM_CLI_TTY_NAME"] = ttyName
        environment["COTERM_AGENT_HOOK_STATE_DIR"] = root.path
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment["CODEX_HOME"] = codexHome
        for key in ["COTERM_AGENT_LAUNCH_KIND", "COTERM_AGENT_LAUNCH_EXECUTABLE", "COTERM_AGENT_LAUNCH_ARGV_B64", "COTERM_AGENT_LAUNCH_CWD"] {
    ...
    /// G3 (https://github.com/emergent-inc/coterm/issues/5333): the codex surface jumble. COTERM_SURFACE_ID
    ...
    func testCodexHookOverridesLeakedEnvSurfaceWithProcessTTYBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-surface-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let leakedSurfaceId = "22222222-2222-2222-2222-222222222222"   // env COTERM_SURFACE_ID (wrong)
        ...
        environment["COTERM_SOCKET_PATH"] = socketPath
        environment["COTERM_WORKSPACE_ID"] = workspaceId
        environment["COTERM_SURFACE_ID"] = leakedSurfaceId
        environment["COTERM_CLI_TTY_NAME"] = ttyName
        environment["COTERM_AGENT_HOOK_STATE_DIR"] = root.path
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment["COTERM_AGENT_LAUNCH_KIND"] = "codex"
        environment["COTERM_AGENT_LAUNCH_EXECUTABLE"] = "/usr/local/bin/codex"
        environment["COTERM_AGENT_LAUNCH_CWD"] = root.path
        environment["COTERM_AGENT_LAUNCH_ARGV_B64"] = base64NULSeparated(["/usr/local/bin/codex"])
        ...
        XCTAssertEqual(
            ...
            "PID/TTY ground truth must override the leaked env COTERM_SURFACE_ID; params=\(params)"
        )
    }

    /// G3 stale-env variant (https://github.com/emergent-inc/coterm/issues/5333): when the ambient
    /// COTERM_SURFACE_ID is stale/invalid (the surface was closed, or belongs to another workspace) it no
    ...
    func testCodexHookRecoversFromStaleEnvSurfaceViaProcessTTYBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-stale-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let staleSurfaceId = "22222222-2222-2222-2222-222222222222"   // env COTERM_SURFACE_ID, no longer exists
        ...
        environment["COTERM_SOCKET_PATH"] = socketPath
        environment["COTERM_WORKSPACE_ID"] = workspaceId
        environment["COTERM_SURFACE_ID"] = staleSurfaceId
        environment["COTERM_CLI_TTY_NAME"] = ttyName
        environment["COTERM_AGENT_HOOK_STATE_DIR"] = root.path
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment["COTERM_AGENT_LAUNCH_KIND"] = "codex"
        ...
        XCTAssertEqual(
            params["surface_id"] as? String, ttySurfaceId,
            "a stale ambient COTERM_SURFACE_ID must fall through to the TTY pane; params=\(params)"
        )
    }
    ...
    func testCodexHookDoesNotPersistEnvOnlyRecordForNonRestorableExec() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-exec-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
        ...
        environment["COTERM_AGENT_LAUNCH_ARGV_B64"] = base64NULSeparated(["/usr/local/bin/codex", "exec", "do a one-shot task"])
