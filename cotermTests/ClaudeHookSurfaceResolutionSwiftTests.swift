...
struct ClaudeHookSurfaceResolutionSwiftTests {
    @Test func claudeSessionStartOverridesLeakedEnvSurfaceWithTTYBinding() throws {
        ...
        let environment = [
            ...
            "COTERM_SURFACE_ID": leakedSurfaceId,
            "COTERM_CLI_TTY_NAME": ttyName,
            "COTERM_CLAUDE_PID": "42424",
            ...
            "COTERM_AGENT_LAUNCH_ARGV_B64": base64NULSeparated(["/usr/local/bin/claude"]),
        ]
        ...
        #expect(
            ...
            "Claude must persist the agent TTY surface, not the leaked ambient COTERM_SURFACE_ID; params=\(request)"
        )
        #expect(
    ...
    @Test func claudeSessionStartOverridesLeakedEnvWorkspaceAndSurfaceWithTTYBinding() throws {
        ...
        let environment = [
            ...
            "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
            "COTERM_SOCKET_PATH": context.socketPath,
            "COTERM_WORKSPACE_ID": leakedWorkspaceId,
            "COTERM_SURFACE_ID": leakedSurfaceId,
            "COTERM_CLI_TTY_NAME": ttyName,
            "COTERM_CLAUDE_HOOK_STATE_PATH": context.root.appendingPathComponent("claude-hook-sessions.json").path,
            "COTERM_CLI_SENTRY_DISABLED": "1",
        ...
        #expect(
            ...
            "Claude must persist the agent TTY workspace, not the leaked ambient COTERM_WORKSPACE_ID; params=\(request)"
        )
        #expect(
            request["surface_id"] as? String == ttySurfaceId,
            "Claude must persist the agent TTY surface, not the leaked ambient COTERM_SURFACE_ID; params=\(request)"
    ...
    @Test func claudeSessionStartOverridesLeakedEnvWithClaudePIDBindingWhenTTYMissing() throws {
        ...
        let environment = [
            ...
            "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
            "COTERM_SOCKET_PATH": context.socketPath,
            "COTERM_SOCKET_PASSWORD": socketPassword,
            "COTERM_WORKSPACE_ID": leakedWorkspaceId,
            "COTERM_SURFACE_ID": leakedSurfaceId,
            "COTERM_CLAUDE_PID": "\(claudePID)",
            "COTERM_CLAUDE_HOOK_STATE_PATH": context.root.appendingPathComponent("claude-hook-sessions.json").path,
            "COTERM_CLI_SENTRY_DISABLED": "1",
            "COTERM_CLAUDE_HOOK_SENTRY_DISABLED": "1",
            "COTERM_AGENT_LAUNCH_KIND": "claude",
        ...
        #expect(
            ...
            "Claude PID binding must beat leaked ambient COTERM_WORKSPACE_ID when no TTY marker exists; params=\(request)"
        )
        #expect(
            request["surface_id"] as? String == pidSurfaceId,
            "Claude PID binding must beat leaked ambient COTERM_SURFACE_ID when no TTY marker exists; params=\(request)"
        )
        #expect(
    ...
    @Test func claudeSessionStartIgnoresStaleTTYWorkspaceWhenBoundSurfaceIsGone() throws {
        ...
        #expect(
            ...
            "Stale TTY workspace must not beat valid ambient COTERM_WORKSPACE_ID; params=\(request)"
        )
        #expect(
    ...
    func claudeHookEnvironment(context: ClaudeHookContext, surfaceId: String, ttyName: String, storeURL: URL) -> [String: String] {
        [
            ...
            "COTERM_CLI_TTY_NAME": ttyName,
            "COTERM_CLAUDE_HOOK_STATE_PATH": storeURL.path,
            "COTERM_CLI_SENTRY_DISABLED": "1",
            "COTERM_CLAUDE_HOOK_SENTRY_DISABLED": "1",
        ]
    }
    ...
    private func assertPromptSubmitRoutes(
        ...
        #expect(request["surface_id"] as? String != ambientSurfaceId, "Raw ambient COTERM_SURFACE_ID must not win")
        #expect(
            context.state.snapshot().contains {
                $0.hasPrefix("set_status claude_code Running ")
