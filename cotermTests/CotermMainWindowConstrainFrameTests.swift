...
#if DEBUG
@MainActor
final class CotermMainWindowConstrainFrameTests: XCTestCase {
    ...
    // on-screen; coterm never re-asserts its saved frame afterward, so the window
    // creeps each sleep cycle. CotermMainWindow.constrainFrameRect must leave an
    ...
    func testPreservesFrameFullyInsideVisibleArea() {
        ...
        XCTAssertTrue(
            CotermMainWindow.shouldPreserveFrameDuringConstrain(frame, visibleFrames: [visible])
        )
    }
    ...
    func testDoesNotPreserveWhenNoScreensAvailable() {
        ...
        XCTAssertFalse(
            CotermMainWindow.shouldPreserveFrameDuringConstrain(frame, visibleFrames: [])
        )
    }
}
#endif
