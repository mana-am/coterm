...
/// and the managed `COTERM_*` variables can never be clobbered.
@Suite(.serialized)
@MainActor
struct WorkspaceEnvironmentTests {
    ...
    /// at `strdup`/Ghostty, so `"COTERM_SOCKET_PATH\0x"` would dodge the exact-match
    ...
    func sanitizedWorkspaceEnvironmentRejectsKeysThatTruncateAtTheCBoundary() {
        let result = Workspace.sanitizedWorkspaceEnvironment([
            "COTERM_SOCKET_PATH\u{0}x": "spoofed",  // NUL would truncate to COTERM_SOCKET_PATH
    ...
    // MARK: - Acceptance: managed COTERM_* variables cannot be clobbered
    ...
    @Test
    func workspaceEnvironmentCannotClobberProtectedCotermVariables() {
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: ["COTERM_WORKSPACE_ID": "real-id", "TERM": "xterm-ghostty"],
            protectedKeys: ["COTERM_WORKSPACE_ID", "TERM"],
            additionalEnvironment: [
                "COTERM_WORKSPACE_ID": "spoofed",   // must be ignored
                "TERM": "dumb",                    // must be ignored
                "AWS_PROFILE": "prod",             // must be applied
            ],
            initialEnvironmentOverrides: ["COTERM_WORKSPACE_ID": "also-spoofed"],
            ambientEnvironment: [:]
        )
        #expect(merged["COTERM_WORKSPACE_ID"] == "real-id")
    ...
    // MARK: - Config entry point (coterm.json)

    @Test
    func cotermWorkspaceDefinitionDecodesEnv() throws {
        let json = #"{"name":"Build","env":{"AWS_PROFILE":"prod","API_BASE":"https://api.example.com"}}"#
        let definition = try JSONDecoder().decode(CotermWorkspaceDefinition.self, from: Data(json.utf8))
        #expect(definition.env == ["AWS_PROFILE": "prod", "API_BASE": "https://api.example.com"])
    }

    @Test
    func cotermWorkspaceDefinitionEnvIsOptional() throws {
        let definition = try JSONDecoder().decode(CotermWorkspaceDefinition.self, from: Data(#"{"name":"Build"}"#.utf8))
