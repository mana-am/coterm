...
final class AgentSessionAutoResumeSettingsTests: XCTestCase {
    func testDefaultsKeyAndNotificationOnFlip() throws {
        let suiteName = "coterm-agent-session-auto-resume-\(UUID().uuidString)"
    ...
    /// (wasAgentRunning == false), coterm must NOT inject the resume command on restore.
    ...
    /// (wasAgentRunning == true), coterm MUST auto-resume as before.
    @MainActor
    func testAgentRunningAtSnapshotAutoResumesWhenSettingEnabled() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            // Simulate: agent was still running when coterm quit
    ...
    func testRemoteWorkspaceAutoResumeKeepsRemoteStartupCommand() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let source = Workspace()
            let remoteCommand = "ssh coterm-macmini"
            let expectedRestoredRemoteCommand = "ssh -tt coterm-macmini"
            source.configureRemoteConnection(
                WorkspaceRemoteConfiguration(
                    ...
                    localSocketPath: "/tmp/coterm-auto-resume-remote.sock",
            ...
            XCTAssertFalse(input.contains("coterm-agent-resume"), input)
        ...
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            source.configureRemoteConnection(
                WorkspaceRemoteConfiguration(
                    ...
                    localSocketPath: "/tmp/coterm-auto-resume-long-remote.sock",
                    terminalStartupCommand: remoteCommand
                ),
                autoConnect: false
            )
            ...
            XCTAssertFalse(restoredInput.contains("coterm-agent-resume"), restoredInput)
            XCTAssertEqual(
                restored.restoredAgentResumeStatesByPanelId[restoredPanelId],
                .awaitingAutoResumeCommand
            )
    ...
    func testResumeLauncherReturnsToLaunchCwdAfterAgentExits() {
        ...
        let exec = "exec -l \"$_coterm_resume_shell\""
    ...
    private func assertAgentAutoResumeUsesStartupCommand(
        ...
        XCTAssertTrue(script.contains("COTERM_SHELL_INTEGRATION_DIR"), script, file: file, line: line)
        XCTAssertTrue(script.contains("COTERM_ZSH_ZDOTDIR"), script, file: file, line: line)
        XCTAssertTrue(script.contains("\"$_coterm_resume_shell\" -lic"), script, file: file, line: line)
        XCTAssertTrue(script.contains("csh|tcsh) \"$_coterm_resume_shell\" -c"), script, file: file, line: line)
        XCTAssertTrue(script.contains("exec -l \"$_coterm_resume_shell\""), script, file: file, line: line)
    }
    ...
    private func makeRestorableAgentIndex(
        ...
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-auto-resume-\(UUID().uuidString)", isDirectory: true)
        let previousHookStateDir = getenv("COTERM_AGENT_HOOK_STATE_DIR").map { String(cString: $0) }
        setenv("COTERM_AGENT_HOOK_STATE_DIR", home.appendingPathComponent("hook-state", isDirectory: true).path, 1)
        defer {
...
final class TerminalCopyOnSelectSettingsTests: XCTestCase {
    func testDefaultsNotificationAndGhosttyConfigMapping() throws {
        let suiteName = "coterm-terminal-copy-on-select-\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
