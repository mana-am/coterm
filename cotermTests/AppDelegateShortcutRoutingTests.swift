...
import Combine
import SwiftUI
@testable import CotermSettingsUI
...
final class AppDelegateShortcutRoutingTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        originalSettingsFileStore = KeyboardShortcutSettings.installIsolatedTestFileStore(prefix: "coterm-shortcut-routing")
    ...
    func testSettingsFileChordDispatchesNewWorkspaceShortcut() throws {
        ...
        RunLoop.main.run(until: Date(timeIntervalSinceNow: 0.05))
        XCTAssertEqual(manager.tabs.count, initialCount + 1, "coterm.json chord should dispatch the configured shortcut")
    }
    ...
    func testWelcomeWindowSidebarShortcutsUseSharedToggleCommands() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("coterm.main.\(windowId.uuidString)")
    ...
    func testAddWorkspaceInPreferredMainWindowPrunesOrphanedContextWithoutLiveWindow() {
        ...
        autoreleasepool {
            ...
            orphanWindow?.identifier = NSUserInterfaceItemIdentifier("coterm.main.\(orphanWindowId.uuidString)")
            appDelegate.registerMainWindow(
                orphanWindow!,
    ...
    func testCmdDPropagatesWhenSplitRightShortcutIsCleared() {
        ...
        withTemporaryShortcut(action: .splitRight, shortcut: .unbound) {
            ...
            XCTAssertFalse(
                appDelegate.debugHandleCustomShortcut(event: event),
                "Cleared Cmd+D split shortcut should not be consumed by coterm"
            )
#else
            XCTFail("debugHandleCustomShortcut is only available in DEBUG")
#endif
    ...
    func testRemappedCloseTabDoesNotLetCmdWReachGhosttyCloseSurfaceFallback() throws {
        ...
        probeWindow.identifier = NSUserInterfaceItemIdentifier("coterm.browser-popup")
        ...
        withTemporaryShortcut(action: .closeTab, shortcut: remappedCloseTab) {
            ...
            XCTAssertFalse(
                ...
                "After Close Tab is remapped, Cmd+W must not match the coterm Close Tab action"
            )
            ...
            XCTAssertEqual(
                ...
                1,
                "Remapped-away Cmd+W should reach the terminal as input instead of closing through coterm"
            )
            ...
            XCTAssertTrue(
                ...
                "The remapped Cmd+Option+W shortcut should match the coterm Close Tab action"
            )
#if DEBUG
            XCTAssertTrue(
                appDelegate.debugHandleShortcutMonitorEvent(event: remappedCmdOptionW),
                "The remapped Cmd+Option+W shortcut should trigger the coterm Close Tab action"
            )
#else
            XCTFail("debugHandleShortcutMonitorEvent is only available in DEBUG")
#endif
    ...
    func testGhosttyConfigDoesNotRetainNumberedGotoTabFallback() throws {
        // Regression for https://github.com/emergent-inc/coterm/issues/5189.
        // coterm owns "Select Workspace 1…9" (default ⌘1–9) through KeyboardShortcutSettings.
        // Ghostty's built-in super+1…8 = goto_tab and super+9 = last_tab fallbacks must be
        // unbound — exactly like coterm already unbinds super+d / super+w — so the numbered
    ...
    func testRebindingSelectWorkspaceByNumberHonorsNewModifierAndDropsDefault() {
        ...
        // the coterm routing layer must drive the numbered shortcut from the configured
        // value, so a rebound modifier selects the workspace and the old ⌘ default no
        // longer routes through coterm.
        guard let appDelegate = AppDelegate.shared else {
            XCTFail("Expected AppDelegate.shared")
            return
        }
        ...
        withTemporaryShortcut(action: .selectWorkspaceByNumber, shortcut: rebound) {
            ...
            XCTAssertFalse(
                ...
                "After rebinding Select Workspace 1…9, the old ⌘1 must not be routed by coterm"
            )
#else
            XCTFail("debugHandleCustomShortcut is only available in DEBUG")
#endif
            ...
            XCTAssertTrue(
                ...
                "The rebound Ctrl+Option+1 shortcut should be routed by coterm"
            )
#else
            XCTFail("debugHandleCustomShortcut is only available in DEBUG")
#endif
    ...
    func testCmdWClosesAuxiliaryWindowInsteadOfMainTerminalPanel() throws {
        ...
        auxiliaryWindow.identifier = NSUserInterfaceItemIdentifier("coterm.about")
        ...
        XCTAssertNotEqual(NSApp.keyWindow?.identifier?.rawValue, "coterm.about", "Closed auxiliary window should not remain key")
    }
    ...
    func testCmdWClosesMobilePairingWindowInsteadOfTerminalTab() throws {
        ...
        // window's identifier ever drops out of cotermAuxiliaryWindowIdentifiers
    ...
    func testMinimalModeTitlebarPaddingOnlyCancelsHostingSafeArea() {
        XCTAssertEqual(
            ...
            accuracy: 0.5,
            "Standard mode should align terminal content with coterm's visual titlebar height even when AppKit reports a taller native titlebar zone"
        )
    ...
    func testAttachUpdateAccessoryHidesTitlebarAccessoryWhenMinimalModeEnabled() {
        ...
        let titlebarAccessory: () -> NSTitlebarAccessoryViewController? = {
            window.titlebarAccessoryViewControllers.first {
                $0.view.identifier?.rawValue == "coterm.titlebarControls"
            }
        }
    ...
    func testKeyboardShortcutSettingsSetShortcutPostsSpecificChangeNotification() {
        let notificationName = Notification.Name("coterm.keyboardShortcutSettingsDidChange")
    ...
    func testBrowserFirstFindShortcutRoutingExcludesAppOwnedFindCommands() {
        ...
        for testCase in cases {
            ...
            XCTAssertFalse(
                shouldRouteBrowserFindCommandEquivalentThroughWebContentFirst(event),
                "\(testCase.name) belongs to coterm find routing, not browser-first routing"
            )
        }
    }
    ...
    func testInlineVSCodeCommandPaletteShortcutDoesNotRouteForUntrackedLocalhostPage() {
        ...
        XCTAssertFalse(
            ...
            "A localhost page with a folder query must not steal coterm's command palette shortcut unless it is the tracked VS Code serve-web origin"
        )
    }
    ...
    func testInlineVSCodeCommandPaletteShortcutDoesNotRouteUnrelatedShortcut() {
        ...
        XCTAssertFalse(
            ...
            "Only the configured command palette shortcut should bypass coterm for inline VS Code"
        )
    }
    ...
    func testWindowPerformKeyEquivalentSuppressesRemappedCmdDStaleMenuShortcut() {
        ...
        withTemporaryShortcut(action: .splitRight, shortcut: remappedSplitRight) {
            XCTAssertFalse(
                probeWindow.performKeyEquivalent(with: event),
                "Remapped Cmd+D should not be consumed by stale coterm menu equivalents"
            )
        }
    ...
    func testCurrentGlobalSearchShortcutIsNotSuppressedAsStaleMenuShortcut() {
        ...
        withTemporaryShortcut(action: .globalSearch, shortcut: remappedGlobalSearch) {
            XCTAssertFalse(
                appDelegate.shouldSuppressStaleCotermMenuShortcut(event: event),
    ...
    func testTextBoxMentionFileSuggestionsUseCommandPaletteSearchIndex() async throws {
        ...
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-textbox-mentions-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testTextBoxMentionFileSuggestionsRefreshCachedMisses() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-textbox-mentions-refresh-\(UUID().uuidString)",
            isDirectory: true
        )
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testTextBoxMentionSkillSuggestionsUseTypedDollarTrigger() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-textbox-skills-\(UUID().uuidString)",
            isDirectory: true
        )
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testTextBoxSubmitUsesLocalPreviewPathForClaudeRemoteImage() throws {
        ...
        let remotePath = "/tmp/coterm-upload/moon.png"
        let attachment = TextBoxAttachment(
            localURL: previewURL,
    ...
    func testTextBoxPasteboardRestorationSkipsAfterUserClipboardChange() throws {
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("coterm.textbox.restore.\(UUID().uuidString)"))
        defer {
            pasteboard.clearContents()
...
#if DEBUG
        XCTAssertFalse(
            cotermCloseFocusedTerminalFindForEscape(event: escapeEvent, appDelegate: appDelegate),
    ...
    private func makeRegisteredShortcutRoutingWindow(id: UUID) -> NSWindow {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("coterm.main.\(id.uuidString)")
        return window
    }
    ...
    private func makeTemporaryPNGFile(named name: String) throws -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-textbox-attachment-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeBrowserFocusModeHarness(
        file: StaticString = #filePath,
        line: UInt = #line
    ) -> (windowId: UUID, window: NSWindow, panel: BrowserPanel, webView: CotermWebView)? {
        guard let appDelegate = AppDelegate.shared else {
            XCTFail("Expected AppDelegate.shared", file: file, line: line)
            return nil
        }
        ...
        guard let window = window(withId: windowId),
              ...
              let webView = browserPanel.webView as? CotermWebView else {
            closeWindow(withId: windowId)
    ...
    private func mainWindowIds() -> Set<UUID> {
        Set(NSApp.windows.compactMap { window in
            guard let raw = window.identifier?.rawValue,
                  raw.hasPrefix("coterm.main.") else {
                return nil
            }
            return UUID(uuidString: String(raw.dropFirst("coterm.main.".count)))
        })
    }
