...
struct ShellStartupMissingBundleTests {
    ...
    // The spawn-env call site only advertises COTERM_SHELL_INTEGRATION /
    // COTERM_SHELL_INTEGRATION_DIR when the bundled dir actually exists, so a
    ...
    func shellIntegrationDirectoryExistsRequiresARealDirectory() throws {
        ...
        let missingDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-tests-deleted-bundle-\(UUID().uuidString)")
    ...
    // zshrc": spawn a real interactive zsh with the environment coterm produces
    ...
    func zshStillLoadsUserZshrcWhenBundledIntegrationDirIsDeleted() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-zshrc-deleted-bundle-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        try fileManager.createDirectory(at: home, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        try "export COTERM_TEST_USER_ZSHRC=1\n"
        ...
        let arguments = ["-i"] + environment.map { "\($0.key)=\($0.value)" } + [
            "/bin/zsh",
            "-ic",
            "print -rn -- \"user_zshrc_loaded=${COTERM_TEST_USER_ZSHRC:-no}\"",
        ]
    ...
    func zshReturnShellLoadsUserZshrcWhenBundledIntegrationDirIsDeleted() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-return-zshrc-deleted-bundle-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        try fileManager.createDirectory(at: home, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }

        try """
        export COTERM_RETURN_TOP_MARKER=top
        alias coterm_return_bottom_alias='echo bottom'
        export COTERM_RETURN_BOTTOM_MARKER=bottom
        """
            .write(to: home.appendingPathComponent(".zshrc"), atomically: true, encoding: .utf8)
        ...
        print -r -- "child_top=${COTERM_RETURN_TOP_MARKER:-missing} child_bottom=${COTERM_RETURN_BOTTOM_MARKER:-missing}"
        if (( $+aliases[coterm_return_bottom_alias] )); then print -r -- child_alias=present; else print -r -- child_alias=missing; fi
        """
        try (
            "#!/bin/zsh\n"
        ...
        let launchCommand = [
            ...
            "PATH=/usr/bin:/bin",
            "TERM=xterm-256color",
            "COTERM_SHELL_INTEGRATION_DIR=\(missingIntegrationDir)",
        ...
        zpty -w p $'print -r -- "top=${COTERM_RETURN_TOP_MARKER:-missing} bottom=${COTERM_RETURN_BOTTOM_MARKER:-missing}"\\n'
        zpty -w p $'if (( $+aliases[coterm_return_bottom_alias] )); then print -r -- alias=present; else print -r -- alias=missing; fi\\n'
        zpty -w p $'exit\\n'
        local output
    ...
    private func makeBundledIntegrationDir(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-shell-missing-bundle-\(UUID().uuidString)")
