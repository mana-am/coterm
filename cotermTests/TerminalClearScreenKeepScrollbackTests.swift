...
struct TerminalClearScreenKeepScrollbackTests {
    ...
    func remappedDefaultShortcutDoesNotTriggerStaleMenuSuppression() throws {
        ...
        withTemporaryShortcut(action: .clearScreenKeepScrollback, shortcut: .unbound) {
            #expect(!appDelegate.shouldSuppressStaleCotermMenuShortcut(event: event))
        }
    }

    @Test
    func clearScreenKeepScrollbackDeliversFormFeedToForegroundProgram() throws {
        let readyMarker = "COTERM_CLEAR_READY_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let captureMarker = "COTERM_CLEAR_HEX_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-clear-keep-scrollback-\(UUID().uuidString).py")
        let script = """
        import os
        import select
        import sys
