...
struct AgentSessionAutoResumeSwiftTests {
    ...
    @Test func claudeAgentHookResumeBindingRestoresFromLaunchCwdWhenRuntimeCwdDrifted() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let launchCwd = "/tmp/coterm-claude-launch"
            let runtimeCwd = "/tmp/coterm-claude-runtime"
            let agent = SessionRestorableAgentSnapshot(
                ...
                launchCommand: AgentLaunchCommandSnapshot(
                    ...
                    workingDirectory: launchCwd,
                    environment: ["CLAUDE_CONFIG_DIR": "/tmp/coterm-claude-config"],
    ...
    @Test func cmdTAfterAgentResumeRestoreKeepsProjectCwdDespiteSpuriousHomePwdReport() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = FileManager.default.temporaryDirectory
                .appendingPathComponent("coterm-cmdt-resume-project-\(UUID().uuidString)", isDirectory: true)
                .path
    ...
    @Test func agentResumeRestoreAcceptsHomePwdReportWhenSavedDirectoryWasDeleted() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let deletedDir = FileManager.default.temporaryDirectory
                .appendingPathComponent("coterm-cmdt-deleted-project-\(UUID().uuidString)", isDirectory: true)
                .path
    ...
    @Test func claudeAgentHookResumeBindingIgnoresStaleRestoredAgentSnapshot() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let staleLaunchCwd = "/tmp/coterm-claude-stale-launch"
            let freshRuntimeCwd = "/tmp/coterm-claude-fresh-runtime"
    ...
    @Test func crossKindAgentHookResumeBindingDoesNotRetainStaleClaudeSnapshot() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let cwd = "/tmp/coterm-cross-kind-runtime"
    ...
    @Test func crossKindAgentHookResumeBindingIgnoresStaleClaudeHibernation() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let cwd = "/tmp/coterm-cross-kind-hibernation-runtime"
            let claudeAgent = SessionRestorableAgentSnapshot(
                kind: .claude,
                sessionId: claudeSessionId,
                workingDirectory: cwd,
    ...
    @Test func claudeRestorableIndexFindsNestedTranscriptWithoutTranscriptPath() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-claude-nested-index-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func claudeRestorableIndexMapsNestedTranscriptPathToProjectCwd() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-claude-nested-path-\(UUID().uuidString)", isDirectory: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    private func assertAgentAutoResumeUsesStartupCommand(
        ...
        #expect(script.contains("COTERM_SHELL_INTEGRATION_DIR"), Comment(rawValue: script))
        #expect(script.contains("COTERM_ZSH_ZDOTDIR"), Comment(rawValue: script))
        #expect(script.contains("\"$_coterm_resume_shell\" -lic"), Comment(rawValue: script))
        #expect(script.contains("csh|tcsh) \"$_coterm_resume_shell\" -c"), Comment(rawValue: script))
        #expect(script.contains("exec -l \"$_coterm_resume_shell\""), Comment(rawValue: script))
    }
}
