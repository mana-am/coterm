...
final class OpenCodeHookRegressionTests: XCTestCase {
    ...
    func testOpenCodeInstallHooksIsIdempotentForLegacySetupAlias() throws {
        ...
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("coterm-opencode-hooks-\(UUID().uuidString)", isDirectory: true)
        ...
        try #"{"plugin":["other-plugin","./plugins/coterm-session.js"]}"#.write(to: configURL, atomically: true, encoding: .utf8)
        ...
        let pluginURL = configDir.appendingPathComponent("plugins", isDirectory: true).appendingPathComponent("coterm-session.js", isDirectory: false)
        let pluginSource = try String(contentsOf: pluginURL, encoding: .utf8)
        XCTAssertTrue(pluginSource.contains("coterm-opencode-session-plugin-marker"))
        ...
        XCTAssertFalse(secondResult.stdout.contains("Will write OpenCode coterm plugin"), secondResult.stdout)
        XCTAssertTrue(secondResult.stdout.contains("OpenCode hooks already up to date"), secondResult.stdout)
        XCTAssertTrue(try String(contentsOf: configDir.appendingPathComponent("plugins/coterm-feed.js"), encoding: .utf8).contains("coterm-feed-plugin-marker"))

        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: try Data(contentsOf: configURL), options: []) as? [String: Any])
        XCTAssertEqual(try XCTUnwrap(json["plugin"] as? [String]), ["other-plugin", "./plugins/coterm-session.js"])
    }
