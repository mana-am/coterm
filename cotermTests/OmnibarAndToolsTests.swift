...
final class FinderServicePathResolverTests: XCTestCase {
    private func withTemporaryDirectory<T>(_ body: (URL) throws -> T) throws -> T {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-finder-service-tests-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testOrderedUniqueDirectoriesUsesParentForFilesAndDedupes() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/coterm-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/coterm-services/project/README.md", isDirectory: false),
            URL(fileURLWithPath: "/tmp/coterm-services/../coterm-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/coterm-services/other", isDirectory: true),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/coterm-services/project",
                "/tmp/coterm-services/other",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesPreservesFirstSeenOrder() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/coterm-services/b", isDirectory: true),
            URL(fileURLWithPath: "/tmp/coterm-services/a/file.txt", isDirectory: false),
            URL(fileURLWithPath: "/tmp/coterm-services/a", isDirectory: true),
            URL(fileURLWithPath: "/tmp/coterm-services/b/file.txt", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/coterm-services/b",
                "/tmp/coterm-services/a",
            ]
        )
    }

    func testOrderedUniqueDirectoriesSkipsBundleAndEmbeddedPathsWhenExcludingBundleRoot() {
        let bundleURL = URL(fileURLWithPath: "/Applications/Tools/../coterm.app", isDirectory: true)
        let input: [URL] = [
            bundleURL,
            URL(fileURLWithPath: "/Applications/coterm.app/Contents/MacOS/coterm", isDirectory: false),
            URL(fileURLWithPath: "/Applications/coterm.app/Contents/Resources/bin/coterm", isDirectory: false),
            URL(fileURLWithPath: "/Users/tester/Projects/coterm", isDirectory: true),
            URL(fileURLWithPath: "/Users/tester/Projects/coterm/README.md", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Users/tester/Projects/coterm",
            ]
        )
    }

    func testOrderedUniqueDirectoriesExclusionDoesNotFilterSiblingPaths() {
        let bundleURL = URL(fileURLWithPath: "/Applications/coterm.app", isDirectory: true)
        let input: [URL] = [
            URL(fileURLWithPath: "/Applications/coterm.app backup/project", isDirectory: true),
            URL(fileURLWithPath: "/Applications/coterm.app.beta/project/file.txt", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Applications/coterm.app backup/project",
                "/Applications/coterm.app.beta/project",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesResolvesSymlinksOnlyForExcludedRootComparison() throws {
        try withTemporaryDirectory { root in
            ...
            let actualBundle = applicationsDirectory.appendingPathComponent("coterm.app", isDirectory: true)
            let actualBinary = actualBundle.appendingPathComponent("Contents/MacOS/coterm", isDirectory: false)
            ...
            let directories = FinderServicePathResolver.orderedUniqueDirectories(
                from: [
                    aliasApplications.appendingPathComponent("coterm.app", isDirectory: true),
                    aliasApplications.appendingPathComponent("coterm.app/Contents/MacOS/coterm", isDirectory: false),
                    aliasWorkspace,
                ],
...
final class VSCodeServeWebURLBuilderTests: XCTestCase {
    ...
    func testOpenFolderURLAppendsFolderQueryWhilePreservingToken() {
        ...
        let url = VSCodeServeWebURLBuilder.openFolderURL(
            ...
            directoryPath: "/Users/tester/Projects/coterm"
        )

        let components = URLComponents(url: url!, resolvingAgainstBaseURL: false)
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "tkn" })?.value, "test-token")
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "folder" })?.value, "/Users/tester/Projects/coterm")
    }
...
final class BrowserOmnibarFieldEditorResolutionTests: XCTestCase {
    func testPanelIdResolutionUsesLiveOmnibarFieldWhenFieldEditorResponderChainIsStale() {
        ...
        let staleWebView = CotermWebView(frame: NSRect(x: 0, y: 0, width: 420, height: 80), configuration: WKWebViewConfiguration())
