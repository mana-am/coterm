...
@MainActor
@Suite("CotermMainWindow native fullscreen capability")
struct CotermMainWindowFullScreenCapabilityTests {
    // coterm creates its main window programmatically and never loaded fullscreen
    ...
    // macOS 26 (Tahoe) a freshly-created CotermMainWindow reports an empty
    ...
    //
    // A CotermMainWindow must therefore *declare* `.fullScreenPrimary` itself so
    ...
    @Test func canonicalBehaviorAddsFullScreenPrimaryToEmptyBehavior() {
        let result = CotermMainWindow.canonicalCollectionBehavior([])
    ...
    @Test func canonicalBehaviorDropsStaleFullScreenNone() {
        let result = CotermMainWindow.canonicalCollectionBehavior([.fullScreenNone])
        #expect(result.contains(.fullScreenPrimary))
        #expect(!result.contains(.fullScreenNone))
    }
    ...
    @Test func canonicalBehaviorPreservesUnrelatedBehaviorBits() {
        ...
        let result = CotermMainWindow.canonicalCollectionBehavior(base)
        #expect(result.contains(.fullScreenPrimary))
    ...
    @Test func canonicalBehaviorIsIdempotent() {
        let once = CotermMainWindow.canonicalCollectionBehavior([])
        let twice = CotermMainWindow.canonicalCollectionBehavior(once)
        #expect(once == twice)
    }
}
