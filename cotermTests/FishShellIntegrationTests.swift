...
struct FishShellIntegrationTests {
    ...
    func testApplyManagedFishStartupEnvironmentPreservesUserConfigHome() {
        ...
        expectEqual(environment["COTERM_FISH_INTEGRATION_FILE"], "/Applications/coterm.app/Contents/Resources/shell-integration/fish/config.fish")
        expectEqual(environment["COTERM_FISH_USER_CONFIG_ALREADY_LOADED"], "1")
        expectFalse(protectedKeys.contains("XDG_CONFIG_HOME"))
        expectTrue(protectedKeys.contains("COTERM_FISH_INTEGRATION_FILE"))
        expectTrue(protectedKeys.contains("COTERM_FISH_USER_CONFIG_ALREADY_LOADED"))
    }

    @Test
    func testApplyManagedFishStartupEnvironmentAvoidsRecursiveConfigHome() {
        var environment = ["XDG_CONFIG_HOME": "/Applications/coterm.app/Contents/Resources/shell-integration/"]
        var protectedKeys: Set<String> = []
        ...
        expectEqual(environment["XDG_CONFIG_HOME"], "/Applications/coterm.app/Contents/Resources/shell-integration/")
        expectEqual(environment["COTERM_FISH_INTEGRATION_FILE"], "/Applications/coterm.app/Contents/Resources/shell-integration/fish/config.fish")
        expectEqual(environment["COTERM_FISH_USER_CONFIG_ALREADY_LOADED"], "1")
        expectFalse(protectedKeys.contains("XDG_CONFIG_HOME"))
        expectTrue(protectedKeys.contains("COTERM_FISH_INTEGRATION_FILE"))
    ...
    func testManagedFishShellCommandLetsGhosttyApplyLoginExecWrapper() {
        let command = TerminalSurface.managedFishShellCommand(shell: "/Applications/coterm DEV fishsh.app/fish")

        expectEqual(command, "'/Applications/coterm DEV fishsh.app/fish' -il --init-command 'source \"$COTERM_FISH_INTEGRATION_FILE\"'")
    ...
    func testFishIntegrationSourcesUserConfigAndReportsShellState() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-fish-shell-integration-\(UUID().uuidString)")
        ...
        let result = try runInteractiveFish(
            command: """
            functions -e _coterm_socket_is_unix
            function _coterm_socket_is_unix
                return 0
            end
            functions -e _coterm_send_bg
            function _coterm_send_bg
                printf '%s\\n' $argv[1] >> "$COTERM_TEST_LOG"
            end
            set -g _COTERM_TTY_NAME ttys777
            set -g _COTERM_TTY_REPORTED 0
            _coterm_preexec
            _coterm_prompt
            printf 'USER_CONFIG=%s\\n' "$COTERM_USER_FISH_CONFIG_SOURCED"
            printf 'USER_CONFD=%s\\n' "$COTERM_USER_FISH_CONFD_SOURCED"
            printf 'XDG_CONFIG_HOME=%s\\n' "$XDG_CONFIG_HOME"
            printf 'FUNCTION_PATH=%s\\n' (string join : $fish_function_path)
            printf 'COMPLETION_PATH=%s\\n' (string join : $fish_complete_path)
            printf 'FUNCTION_RESULT=%s\\n' (coterm_test_function)
            printf 'CLAUDE_RESULT=%s\\n' (claude test)
            printf 'GROK_RESULT=%s\\n' (grok test)
            printf 'UNIVERSAL=%s\\n' "$COTERM_UNIVERSAL_TEST"
            cat "$COTERM_TEST_LOG"
            """,
            extraEnvironment: [
                "COTERM_TEST_USER_CONFIG_HOME": userConfigHome.path,
                "COTERM_TEST_LOG": logPath.path,
                "COTERM_SOCKET_PATH": root.appendingPathComponent("coterm-test.sock").path,
    ...
    func testFishIntegrationRelayPromptReportsPWD() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-fish-relay-pwd-\(UUID().uuidString)")
        ...
        let cotermPath = binDir.appendingPathComponent("coterm", isDirectory: false)
        ...
        let result = try runInteractiveFish(
            ...
            set -g _COTERM_PWD_LAST_PWD /tmp/local-launch
            _coterm_prompt
            for _coterm_i in (seq 1 20)
            ...
            extraEnvironment: [
                ...
                "COTERM_PANEL_ID": "22222222-2222-2222-2222-222222222222",
                "COTERM_BUNDLED_CLI_PATH": cotermPath.path,
            ]
        )
    ...
    func testGeneratedFishBootstrapStagesIntegrationAndPreservesUserConfigHome() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-fish-shell-bootstrap-\(UUID().uuidString)")
        ...
        try writeExecutableShellFile(
            ...
            #!/bin/sh
            {
              ...
              printf 'ARGS=%s\\n' "$*"
              printf 'CONFIG<<EOF\\n'
              cat "$COTERM_FISH_INTEGRATION_FILE"
              printf '\\nEOF\\n'
            } > "$COTERM_CAPTURE_FISH"
            """
        )
        ...
        let script = RemoteInteractiveShellBootstrapBuilder.script(
            remoteRelayPort: 0,
            shellFeatures: "",
            bundledFishIntegration: "set -gx COTERM_FISH_TEST_INTEGRATION 1"
        )
        let result = runProcess(
            ...
            arguments: [
                ...
                "COTERM_CAPTURE_FISH=\(capturePath.path)",
                "/bin/sh",
                "-c",
                script,
            ],
        ...
        expectTrue(output.contains("COTERM_FISH_INTEGRATION_FILE=\(home.path)/.coterm/relay/0.shell/fish/config.fish"), output)
        expectTrue(output.contains("COTERM_FISH_USER_CONFIG_ALREADY_LOADED=1"), output)
        expectTrue(output.contains("ARGS=-il --init-command source \"$COTERM_FISH_INTEGRATION_FILE\""), output)
        expectTrue(output.contains("set -gx COTERM_FISH_TEST_INTEGRATION 1"), output)
    }
    ...
    private func writeUserFishConfig(at userConfigHome: URL) throws {
        ...
        let userFishConfD = userConfigHome.appendingPathComponent("fish/conf.d/coterm-test.fish", isDirectory: false)
        let userFishFunction = userConfigHome.appendingPathComponent("fish/functions/coterm_test_function.fish", isDirectory: false)
        let userFishCompletion = userConfigHome.appendingPathComponent("fish/completions/coterm-test.fish", isDirectory: false)
        ...
        try """
        set -gx COTERM_USER_FISH_CONFIG_SOURCED 1
        function claude
    ...
    private func runInteractiveFish(
        ...
        try writeExecutableShellFile(at: testBinDir.appendingPathComponent("coterm-claude-wrapper"), body: "#!/bin/sh\necho coterm-claude-wrapper \"$@\"\n")
        try writeExecutableShellFile(at: testBinDir.appendingPathComponent("grok"), body: "#!/bin/sh\necho coterm-grok-wrapper \"$@\"\n")
        ...
        var environment: [String: String] = [
            ...
            "USER": NSUserName(),
            "XDG_CONFIG_HOME": extraEnvironment["COTERM_TEST_USER_CONFIG_HOME"] ?? root.appendingPathComponent("user-config").path,
            "COTERM_SHELL_INTEGRATION": "1",
            "COTERM_SHELL_INTEGRATION_DIR": testIntegrationDir.path,
            "COTERM_FISH_INTEGRATION_FILE": integrationFile.path,
            "COTERM_FISH_USER_CONFIG_ALREADY_LOADED": "1",
        ]
        extraEnvironment.forEach { environment[$0] = $1 }
        environment.removeValue(forKey: "COTERM_TEST_USER_CONFIG_HOME")

        let result = runProcess(
            executablePath: fishExecutable,
            arguments: ["-i", "--init-command", #"source "$COTERM_FISH_INTEGRATION_FILE""#, "-c", command],
