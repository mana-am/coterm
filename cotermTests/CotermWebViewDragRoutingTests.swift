...
private var cotermUnitTestWKWebViewDragLifecycleOverrideInstalled = false
private var cotermUnitTestWKWebViewDragLifecycleEvents: [String]?

extension WKWebView {
    @objc func cotermUnitTest_draggingEntered(_ sender: any NSDraggingInfo) -> NSDragOperation {
        if cotermUnitTestWKWebViewDragLifecycleEvents != nil {
            cotermUnitTestWKWebViewDragLifecycleEvents?.append("entered")
            return .copy
        }
        return cotermUnitTest_draggingEntered(sender)
    }

    @objc func cotermUnitTest_draggingUpdated(_ sender: any NSDraggingInfo) -> NSDragOperation {
        if cotermUnitTestWKWebViewDragLifecycleEvents != nil {
            cotermUnitTestWKWebViewDragLifecycleEvents?.append("updated")
            return .copy
        }
        return cotermUnitTest_draggingUpdated(sender)
    }

    @objc func cotermUnitTest_prepareForDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        if cotermUnitTestWKWebViewDragLifecycleEvents != nil {
            cotermUnitTestWKWebViewDragLifecycleEvents?.append("prepare")
            return true
        }
        return cotermUnitTest_prepareForDragOperation(sender)
    }

    @objc func cotermUnitTest_performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        if cotermUnitTestWKWebViewDragLifecycleEvents != nil {
            cotermUnitTestWKWebViewDragLifecycleEvents?.append("perform")
            return true
        }
        return cotermUnitTest_performDragOperation(sender)
    }

    @objc func cotermUnitTest_concludeDragOperation(_ sender: (any NSDraggingInfo)?) {
        if cotermUnitTestWKWebViewDragLifecycleEvents != nil {
            cotermUnitTestWKWebViewDragLifecycleEvents?.append("conclude")
            return
        }
        cotermUnitTest_concludeDragOperation(sender)
    }
}

private func installCotermUnitTestWKWebViewDragLifecycleOverride() {
    guard !cotermUnitTestWKWebViewDragLifecycleOverrideInstalled else { return }
    ...
    swizzle(
        ...
        #selector(WKWebView.cotermUnitTest_draggingEntered(_:))
    )
    swizzle(
        #selector(NSView.draggingUpdated(_:)),
        #selector(WKWebView.cotermUnitTest_draggingUpdated(_:))
    )
    swizzle(
        #selector(NSView.prepareForDragOperation(_:)),
        #selector(WKWebView.cotermUnitTest_prepareForDragOperation(_:))
    )
    swizzle(
        #selector(NSView.performDragOperation(_:)),
        #selector(WKWebView.cotermUnitTest_performDragOperation(_:))
    )
    swizzle(
        #selector(NSView.concludeDragOperation(_:)),
        #selector(WKWebView.cotermUnitTest_concludeDragOperation(_:))
    )

    cotermUnitTestWKWebViewDragLifecycleOverrideInstalled = true
}
...
@MainActor
final class CotermWebViewDragRoutingTests: XCTestCase {
    override func setUp() {
        super.setUp()
        installCotermUnitTestWKWebViewDragLifecycleOverride()
        cotermUnitTestWKWebViewDragLifecycleEvents = nil
    }
    ...
    func testRejectsInternalPaneDragEvenWhenFilePromiseTypesArePresent() {
        XCTAssertTrue(
            CotermWebView.shouldRejectInternalPaneDrag([
    ...
    func testAllowsRegularExternalFileImageAndURLDrops() {
        ...
        for pasteboardTypes in externalPayloads {
            XCTAssertFalse(
                CotermWebView.shouldRejectInternalPaneDrag(pasteboardTypes),
    ...
    func testWebsiteDragPayloadReachesWebKitDragLifecycle() {
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("coterm.web-drag.\(UUID().uuidString)"))
        ...
        cotermUnitTestWKWebViewDragLifecycleEvents = []
    ...
    func testInternalPaneDragDoesNotReachWebKitDragLifecycle() {
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("coterm.internal-drag.\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertEqual(cotermUnitTestWKWebViewDragLifecycleEvents, [])
    }
}
...
final class BrowserScreenshotPipelineTests: XCTestCase {
    ...
    func testFullPageSnapshotWritesPNGAndTIFFToPasteboard() async throws {
        let pasteboard = NSPasteboard(name: .init("coterm-browser-screenshot-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testScreenshotPasteboardWriterClearsExistingContentsBeforeWriting() async throws {
        let pasteboard = NSPasteboard(name: .init("coterm-browser-screenshot-existing-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testSectionSnapshotCropsSelectionBeforeWritingPasteboard() async throws {
        let pasteboard = NSPasteboard(name: .init("coterm-browser-screenshot-section-\(UUID().uuidString)"))
        pasteboard.clearContents()
