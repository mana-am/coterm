...
final class CotermSSHURLRequestTests: XCTestCase {
    deinit {}
    ...
    func testCommandPreviewIncludesSocketPathWhenProvided() throws {
        ...
        case .success(.some(let request)):
            XCTAssertEqual(
                request.cliPreview(socketPath: "/tmp/coterm-urlcmd.sock"),
                "coterm --socket /tmp/coterm-urlcmd.sock ssh --name \"Dev SSH\" dev.example.com"
            )
        case .success(nil):
            XCTFail("Expected SSH URL request")
        case .failure(let error):
    ...
    func testParsesStableNightlyAndDevSchemes() throws {
        for scheme in ["coterm", "coterm-nightly", "coterm-dev"] {
            var components = URLComponents()
            ...
            switch CotermSSHURLRequest.parse(url, supportedSchemes: CotermSSHURLRequest.supportedSchemes) {
            case .success(.some(let request)):
                XCTAssertEqual(request.destination, "dev.example.com")
            case .success(nil):
    ...
    func testDefaultParserIgnoresOtherProductSchemes() throws {
        let inactiveScheme = try XCTUnwrap(CotermSSHURLRequest.supportedSchemes.first {
    ...
    func testAcceptsTextURLAtMaxLength() throws {
        let text = String(repeating: "a", count: CotermTextURLRequest.maxTextLength)
        let url = try XCTUnwrap(textURL(host: "prompt", queryItems: [
        ...
        switch CotermTextURLRequest.parse(url) {
        case .success(.some(let request)):
            XCTAssertEqual(request.text.count, CotermTextURLRequest.maxTextLength)
        case .success(nil):
            XCTFail("Expected prompt URL request")
        case .failure(let error):
            XCTFail("Unexpected parse error: \(error)")
    ...
    func testRejectsTextURLExceedingMaxLength() throws {
        let text = String(repeating: "a", count: CotermTextURLRequest.maxTextLength + 1)
        let url = try XCTUnwrap(textURL(host: "prompt", queryItems: [
            URLQueryItem(name: "text", value: text)
        ]))

        switch CotermTextURLRequest.parse(url) {
        case .failure(.textTooLong(maxLength: CotermTextURLRequest.maxTextLength)):
            break
        default:
    ...
    private func parsedOptional(_ url: URL) throws -> CotermSSHURLRequest? {
        switch CotermSSHURLRequest.parse(url) {
...
final class CotermNavigationURLRequestTests: XCTestCase {
    private let supportedScheme = "coterm-test"
    ...
    func testGeneratedLinksRoundTrip() throws {
        let workspaceURL = try XCTUnwrap(URL(string: CotermNavigationURLRequest.workspaceLink(workspaceId: workspaceId, scheme: supportedScheme)))
        let paneURL = try XCTUnwrap(URL(string: CotermNavigationURLRequest.paneLink(workspaceId: workspaceId, paneId: paneId, scheme: supportedScheme)))
        let surfaceURL = try XCTUnwrap(URL(string: CotermNavigationURLRequest.surfaceLink(workspaceId: workspaceId, surfaceId: surfaceId, scheme: supportedScheme)))
    ...
    func testIgnoresOtherCotermRoutesAndInactiveSchemes() throws {
        ...
        let inactiveURL = try XCTUnwrap(URL(string: "coterm-other://workspace/\(workspaceId.uuidString)"))
    ...
    func testRejectsQueryFragmentAuthorityAndExtraPathComponents() throws {
        ...
        for rawURL in cases {
            let url = try XCTUnwrap(URL(string: rawURL))
            switch CotermNavigationURLRequest.parse(url, supportedSchemes: [supportedScheme]) {
    ...
    private func parsedOptional(_ url: URL) throws -> CotermNavigationURLRequest? {
        switch CotermNavigationURLRequest.parse(url, supportedSchemes: [supportedScheme]) {
        case .success(let request):
            return request
        case .failure(let error):
    ...
    private func parsedTarget(_ url: URL) throws -> CotermNavigationURLRequest.Target {
