...
final class TerminalNotificationPolicyEngineTests: XCTestCase {
    private func evaluate(
        ...
        hooks: [CotermResolvedNotificationHook]
    ...
    func testHookCanDisableDesktopAndTransformBody() async throws {
        ...
        let hook = CotermResolvedNotificationHook(
            id: "filter",
    ...
    func testPartialNotificationPatchPreservesOmittedPayloadFields() async throws {
        ...
        XCTAssertEqual(patched.context.configPath, "/tmp/coterm.json")
...
final class GhosttyCrashBreadcrumbTests: XCTestCase {
    ...
    func testPendingCrashIgnoresNewerCrashFromDifferentExecutable() throws {
        ...
        _ = try writeCrashEnvelope(
            ...
            executablePath: "/private/tmp/coterm-tbinput-unit/Build/Products/Debug/coterm DEV.app/Contents/MacOS/coterm DEV",
    ...
    func testDefaultCrashDirectoryUsesCotermStatePath() throws {
        XCTAssertTrue(
            GhosttyCrashBreadcrumb.defaultCrashDirectoryURL.path.hasSuffix("/.local/state/coterm/crash"),
...
final class NotificationDockBadgeTests: XCTestCase {
    ...
    func testNotificationClickActionRoundTripsAndIsStored() {
        ...
        let path = "/tmp/coterm-crash-\(UUID().uuidString).ghosttycrash"
    ...
    func testNotificationClickActionDoesNotMarkReadWhenRevealTargetIsMissing() throws {
        ...
        let notification = TerminalNotification(
            ...
            createdAt: Date(),
            isRead: false,
            clickAction: .revealInFinder(path: "/tmp/coterm-missing-\(UUID().uuidString)/missing.ghosttycrash")
        )
    ...
    func testJumpToLatestUnreadSkipsClickActionNotifications() {
        let clickActionNotification = TerminalNotification(
            ...
            createdAt: Date(),
            isRead: false,
            clickAction: .revealInFinder(path: "/tmp/coterm-crash.ghosttycrash")
        )
    ...
    func testNotificationSoundUsesSystemSoundForDefaultAndNamedSounds() {
        ...
        let stagingDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-notification-sound-\(UUID().uuidString)", isDirectory: true)
        defer {
    ...
    func testNotificationCustomStagingPreservesSourceFileWithCotermPrefix() {
        let suiteName = "NotificationDockBadgeTests.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated UserDefaults suite")
            return
        ...
        let sourceURL = soundsDirectory.appendingPathComponent(
            "coterm-custom-notification-sound.source-\(UUID().uuidString).wav",
            isDirectory: false
        )
        defer {
        ...
        XCTAssertTrue(stagedName.hasPrefix("coterm-custom-notification-sound-"))
    ...
    func testNotificationCustomUnsupportedExtensionsStageAsCaf() {
        ...
        XCTAssertTrue(stagedA.hasPrefix("coterm-custom-notification-sound-"))
    ...
    func testNotificationCustomPreparationKeepsActiveSourceMetadataSidecar() {
        ...
        let sourceURL = soundsDirectory.appendingPathComponent(
            "coterm-custom-notification-sound.metadata-\(UUID().uuidString).wav",
            isDirectory: false
        )
        do {
            try Data("test".utf8).write(to: sourceURL, options: .atomic)
    ...
    func testNotificationCustomSoundReturnsNilWhenPreparationFails() {
        ...
        let invalidSourceURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-invalid-sound-\(UUID().uuidString).mp3", isDirectory: false)
        defer {
            try? FileManager.default.removeItem(at: invalidSourceURL)
            let stagedURL = URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
                .appendingPathComponent("Library", isDirectory: true)
                .appendingPathComponent("Sounds", isDirectory: true)
                .appendingPathComponent("coterm-custom-notification-sound.caf", isDirectory: false)
    ...
    func testNotificationCustomPreparationReportsMissingFile() {
        let missingPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-missing-\(UUID().uuidString).wav", isDirectory: false)
            .path
    ...
    func testFocusedTerminalSuppressedNotificationRunsCustomCommand() throws {
        ...
        let commandOutputURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-notification-command-\(UUID().uuidString).txt", isDirectory: false)
        ...
        defaults.set("none", forKey: NotificationSoundSettings.key)
        defaults.set(
            "printf '%s\\n%s\\n%s' \"$COTERM_NOTIFICATION_TITLE\" \"$COTERM_NOTIFICATION_SUBTITLE\" \"$COTERM_NOTIFICATION_BODY\" > '\(commandOutputURL.path)'",
...
final class MenuBarBuildHintFormatterTests: XCTestCase {
    func testReleaseBuildShowsNoHint() {
        XCTAssertNil(MenuBarBuildHintFormatter.menuTitle(appName: "Coterm DEV menubar-extra", isDebugBuild: false))
    }

    func testDebugBuildWithTagShowsTag() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "Coterm DEV menubar-extra", isDebugBuild: true),
    ...
    func testDebugBuildWithoutTagShowsUntagged() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "Coterm DEV", isDebugBuild: true),
            "Build: DEV (untagged)"
        )
    }
}
