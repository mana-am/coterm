...
import struct CotermSettings.IntegrationsCatalogSection
import enum CotermSettings.KiroNotificationLevel
...
private typealias StoredShortcut = Coterm_DEV.StoredShortcut
private typealias ShortcutStroke = Coterm_DEV.ShortcutStroke
private typealias AppIconMode = Coterm_DEV.AppIconMode
#elseif canImport(Coterm)
@testable import Coterm
private typealias StoredShortcut = Coterm.StoredShortcut
private typealias ShortcutStroke = Coterm.ShortcutStroke
private typealias AppIconMode = Coterm.AppIconMode
#endif
...
final class KeyboardShortcutSettingsFileStoreTests: XCTestCase {
    ...
    private let settingsFileBackupsDefaultsKey = "coterm.settingsFile.backups.v1"
    ...
    func testSettingsFileStoreUsesLegacyFallbackWhenCanonicalConfigHasNoSetting() throws {
        ...
        try writeSettingsFile(
            """
            {
              "$schema": "https://raw.githubusercontent.com/emergent-inc/coterm/main/web/data/coterm-settings.schema.json",
              "shortcuts": {
    ...
    @MainActor
    func testReloadConfigurationMenuActionReloadsRegisteredCotermConfigStore() throws {
#if DEBUG
        let directoryURL = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directoryURL) }
        ...
        XCTAssertNotNil(cotermConfigStore.resolvedAction(id: "first"))
        XCTAssertNil(cotermConfigStore.resolvedAction(id: "second"))
        ...
        let appMenuItem = NSMenuItem(title: "coterm", action: nil, keyEquivalent: "")
        let appMenu = NSMenu(title: "coterm")
        ...
        XCTAssertEqual(
            ...
            NSUserInterfaceItemIdentifier("com.coterm.reloadConfiguration")
        )
        ...
        XCTAssertNil(cotermConfigStore.resolvedAction(id: "first"))
        XCTAssertNotNil(cotermConfigStore.resolvedAction(id: "second"))
#else
    ...
    func testBootstrapCreatesCommentedTemplateWhenPrimaryAndFallbackAreMissing() throws {
        ...
        let settingsFileURL = directoryURL
            .appendingPathComponent(".config/coterm", isDirectory: true)
        ...
        XCTAssertTrue(contents.contains(#""$schema": "https://raw.githubusercontent.com/emergent-inc/coterm/main/web/data/coterm.schema.json""#))
    ...
    func testSettingsFileURLForEditingPrefersInvalidPrimaryForRepair() throws {
        ...
        let primaryURL = directoryURL.appendingPathComponent("primary/coterm.json", isDirectory: false)
        let fallbackURL = directoryURL.appendingPathComponent("fallback/coterm.json", isDirectory: false)
...
final class StoredShortcutMatchingTests: XCTestCase {
    ...
    func testSystemWideHotkeyNormalizationReportsCotermActionConflictByRecordedPhysicalKey() {
        KeyboardShortcutSettings.resetAll()
        defer { KeyboardShortcutSettings.resetAll() }
...
final class WorkspaceCreationWorkingDirectoryInheritanceTests: XCTestCase {
    ...
    func testNewWorkspaceInheritsSourceWorkingDirectoryByDefault() throws {
        try withWorkspaceWorkingDirectoryInheritanceSetting(nil) {
            let sourceCwd = "/tmp/coterm-source-\(UUID().uuidString)"
            let manager = TabManager(
    ...
    func testExplicitWorkspaceWorkingDirectoryWinsWhenInheritanceIsDisabled() throws {
        try withWorkspaceWorkingDirectoryInheritanceSetting(false) {
            let sourceCwd = "/tmp/coterm-source-\(UUID().uuidString)"
            let explicitCwd = "/tmp/coterm-explicit-\(UUID().uuidString)"
            let manager = TabManager(
                initialWorkingDirectory: sourceCwd,
                autoWelcomeIfNeeded: false
            )
    ...
    func testDetachedWorkspaceTransferDirectoryWinsWhenInheritanceIsDisabled() throws {
        try withWorkspaceWorkingDirectoryInheritanceSetting(false) {
            let sourceCwd = "/tmp/coterm-source-\(UUID().uuidString)"
            let transferCwd = "/tmp/coterm-detached-\(UUID().uuidString)"
            let manager = TabManager(
                initialWorkingDirectory: sourceCwd,
                autoWelcomeIfNeeded: false
            )
    ...
    func testDetachedWorkspaceDoesNotPersistProcessDetectedResumeBinding() throws {
        let manager = TabManager(
            initialWorkingDirectory: "/tmp/coterm-source-\(UUID().uuidString)",
            autoWelcomeIfNeeded: false
        )
        let source = try XCTUnwrap(manager.selectedWorkspace)
...
final class WorkspaceCreationConfigSanitizationTests: XCTestCase {
    private final class UnsafeConfigSnapshotTabManager: TabManager {
        private var injectedConfig: CotermSurfaceConfigTemplate?
        var capturedConfigTemplate: CotermSurfaceConfigTemplate?

        func installInjectedConfig(fontSize: Float) {
            var config = CotermSurfaceConfigTemplate()
            config.fontSize = fontSize
            config.workingDirectory = "/tmp/coterm-workspace-snapshot"
            config.command = "echo snapshot"
            config.environmentVariables = ["COTERM_INHERITED_ENV": "1"]
...
final class WorkspaceReorderTests: XCTestCase {
    ...
    func testMoveTabToTopPublishesWorkspaceReorderedEvent() throws {
        CotermEventBus.shared.resetForTesting()
        defer { CotermEventBus.shared.resetForTesting() }
        ...
        let event = try XCTUnwrap(CotermEventBus.shared.retainedSnapshot().last)
...
final class WorkspaceSplitWorkingDirectoryTests: XCTestCase {
    ...
    func testNewTerminalSplitFallsBackToRequestedWorkingDirectoryWhenReportedDirectoryIsStale() {
        ...
        let requestedDirectory = "/tmp/coterm-requested-split-cwd-\(UUID().uuidString)"
...
final class WorkspacePanelGitBranchTests: XCTestCase {
    ...
    func testForkAgentConversationInRemoteWorkspaceUsesRemoteStartupCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/coterm-fork-remote.sock",
        ...
        XCTAssertEqual(forkPanel.surface.debugInitialCommand(), "ssh coterm-macmini")
        XCTAssertNil(forkPanel.requestedWorkingDirectory)
        XCTAssertEqual(workspace.panelDirectories[forkPanel.id], "/Users/coterm/project")
        XCTAssertEqual(forkPanel.surface.initialInput, snapshot.forkCommand.map { $0 + "\n" })
    ...
    func testForkAgentConversationInRemoteWorkspaceUsesFallbackDirectoryInForkCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/coterm-fork-fallback-remote.sock",
                terminalStartupCommand: "ssh coterm-macmini"
            ),
            autoConnect: false
        )
        workspace.currentDirectory = "/Users/coterm/fallback repo"
        let sourcePanelId = try XCTUnwrap(workspace.focusedPanelId)
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .codex,
            sessionId: "019dad34-d218-7943-b81a-eddac5c87951",
        ...
        XCTAssertEqual(forkPanel.surface.debugInitialCommand(), "ssh coterm-macmini")
        XCTAssertNil(forkPanel.requestedWorkingDirectory)
        XCTAssertEqual(workspace.panelDirectories[forkPanel.id], "/Users/coterm/fallback repo")
        XCTAssertEqual(
            forkPanel.surface.initialInput,
            "cd -- '/Users/coterm/fallback repo' 2>/dev/null || [ ! -d '/Users/coterm/fallback repo' ] && '/Users/example/.bun/bin/codex' 'fork' '019dad34-d218-7943-b81a-eddac5c87951'\n"
        )
    }
    ...
    func testSessionIndexRemoteSplitDoesNotInjectRemoteStartupCommand() throws {
        ...
        let hookStateRoot = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-session-drop-hook-state-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: hookStateRoot, withIntermediateDirectories: true)
        let previousHookStateDir = getenv("COTERM_AGENT_HOOK_STATE_DIR").map { String(cString: $0) }
        setenv("COTERM_AGENT_HOOK_STATE_DIR", hookStateRoot.path, 1)
        defer {
            if let previousHookStateDir {
                setenv("COTERM_AGENT_HOOK_STATE_DIR", previousHookStateDir, 1)
            } else {
                unsetenv("COTERM_AGENT_HOOK_STATE_DIR")
            }
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/coterm-session-drop-remote.sock",
                terminalStartupCommand: "ssh coterm-macmini"
            ),
            autoConnect: false
        )
    ...
    func testForkAgentWorkspaceLaunchInRemoteWorkspacePreservesRemoteContext() throws {
        let workspace = Workspace()
        let agentSocketPath = "/tmp/coterm-fork-agent.sock"
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                destination: "coterm-macmini",
                port: 2222,
                identityFile: "/Users/example/.ssh/coterm",
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/coterm-fork-remote.sock",
                terminalStartupCommand: "ssh -p 2222 -i /Users/example/.ssh/coterm -o ServerAliveInterval=30 -o ForwardAgent=yes -tt coterm-macmini",
        ...
        XCTAssertEqual(launch.workingDirectory, "/Users/coterm/project")
        XCTAssertNil(launch.terminalWorkingDirectory)
        XCTAssertEqual(
            launch.initialTerminalCommand,
            "ssh -p 2222 -i /Users/example/.ssh/coterm -o ServerAliveInterval=30 -o ForwardAgent=yes -tt coterm-macmini"
        )
        ...
        XCTAssertEqual(launch.remoteConfiguration?.destination, "coterm-macmini")
        XCTAssertEqual(launch.remoteConfiguration?.port, 2222)
        XCTAssertEqual(launch.remoteConfiguration?.identityFile, "/Users/example/.ssh/coterm")
    ...
    func testForkAgentWorkspaceLaunchFromPersistentSSHPTYDoesNotReuseParentRelayOrDaemonSlot() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/coterm-fork-persistent.sock",
        ...
        XCTAssertEqual(
            startupCommand,
            "ssh -p 2222 -i /Users/example/.ssh/coterm -tt coterm-macmini"
        )
    }
    ...
    func testForkAgentWorkspaceLaunchInRemoteWorkspaceUsesFallbackDirectoryInForkCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/coterm-workspace-fallback-remote.sock",
                terminalStartupCommand: "ssh coterm-macmini"
            ),
            autoConnect: false
        )
        ...
        XCTAssertEqual(launch.workingDirectory, "/Users/coterm/fallback repo")
        XCTAssertNil(launch.terminalWorkingDirectory)
        XCTAssertEqual(launch.initialTerminalCommand, "ssh -tt coterm-macmini")
        XCTAssertEqual(
    ...
    func testForkAgentConversationInRemoteConfiguredLocalWorkspaceAllowsLauncherScript() throws {
        ...
        XCTAssertEqual(forkPanel.requestedWorkingDirectory, "/Users/coterm/project")
        ...
        XCTAssertEqual(launch.terminalWorkingDirectory, "/Users/coterm/project")
        XCTAssertNil(launch.initialTerminalCommand)
        XCTAssertFalse(launch.autoConnectRemoteConfiguration)
        XCTAssertNil(launch.remoteConfiguration)
    ...
    func testForkAgentConversationFromLocalTerminalInRemoteWorkspaceStaysLocal() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/coterm-fork-local-remote.sock",
                terminalStartupCommand: "ssh coterm-macmini"
            ),
            autoConnect: false
        )
    ...
    func testUpdatingFocusedPanelPullRequestWithSameStateDoesNotRepublishWorkspace() {
        ...
        let pullRequestURL = URL(string: "https://github.com/emergent-inc/coterm/pull/2388")!
    ...
    func testSidebarPullRequestsTrackFocusedPanelOnly() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 1629,
            label: "PR",
            url: URL(string: "https://github.com/emergent-inc/coterm/pull/1629")!,
            status: .open
        )
    ...
    func testSidebarDerivedCollectionsMatchWhenUsingPrecomputedPanelOrder() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 101,
            label: "PR",
            url: URL(string: "https://github.com/emergent-inc/coterm/pull/101")!,
            status: .open
        )
        workspace.updatePanelPullRequest(
            ...
            number: 18,
            label: "MR",
            url: URL(string: "https://gitlab.com/emergentinc/coterm/-/merge_requests/18")!,
            status: .merged
        )
    ...
    func testForkConversationDefaultSettingFallsBackToRight() throws {
        let suiteName = "coterm.forkConversationDefault.\(UUID().uuidString)"
...
@MainActor
final class CotermVisibleBrandingTests: XCTestCase {
    func testHelpMenuResourcesUseCotermDestinations() {
        let docResources: [(CotermHelpResource, String)] = [
            (.gettingStarted, "https://coterm.com/docs/getting-started"),
            (.concepts, "https://coterm.com/docs/concepts"),
            (.configuration, "https://coterm.com/docs/configuration"),
            (.customCommands, "https://coterm.com/docs/custom-commands"),
            (.dock, "https://coterm.com/docs/dock"),
            (.keyboardShortcuts, "https://coterm.com/docs/keyboard-shortcuts"),
            (.apiReference, "https://coterm.com/docs/api"),
            (.browserAutomation, "https://coterm.com/docs/browser-automation"),
            (.notifications, "https://coterm.com/docs/notifications"),
            (.ssh, "https://coterm.com/docs/ssh"),
            (.skills, "https://coterm.com/docs/skills"),
            (.claudeCodeTeams, "https://coterm.com/docs/agent-integrations/claude-code-teams"),
            (.ohMyOpenCode, "https://coterm.com/docs/agent-integrations/oh-my-opencode"),
            (.ohMyCodex, "https://coterm.com/docs/agent-integrations/oh-my-codex"),
            (.ohMyClaudeCode, "https://coterm.com/docs/agent-integrations/oh-my-claudecode"),
            (.changelog, "https://coterm.com/docs/changelog"),
        ]

        for (resource, expectedURL) in docResources {
            XCTAssertEqual(resource.url.absoluteString, expectedURL)
        }
        XCTAssertEqual(CotermHelpResource.githubIssues.url, CotermBranding.githubIssuesURL)
        XCTAssertEqual(CotermHelpResource.discord.url.absoluteString, "https://discord.gg/zmWHDeZffZ")
    }

    func testSidebarAndHelpMenuShareCotermDestinations() {
        XCTAssertEqual(CotermBranding.docsBaseURL.absoluteString, "https://coterm.com/docs")
        XCTAssertEqual(CotermBranding.docsURL("changelog").absoluteString, "https://coterm.com/docs/changelog")
        XCTAssertEqual(CotermBranding.githubRepositoryURL.absoluteString, "https://github.com/emergent-inc/coterm")
        XCTAssertEqual(CotermBranding.githubIssuesURL.absoluteString, "https://github.com/emergent-inc/coterm/issues")
    }

    func testWindowTitleFallbackUsesCoterm() {
        let manager = TabManager()
        ...
        XCTAssertEqual(window.title, "coterm")
    }

    func testRebrandedRuntimeLabelsUseCoterm() {
        XCTAssertEqual(KeyboardShortcutSettings.Action.quit.label, "Quit Coterm")
        XCTAssertEqual(SocketControlMode.cotermOnly.displayName, "Coterm processes only")
        XCTAssertEqual(
            SocketControlMode.cotermOnly.description,
            "Only processes started inside coterm terminals can send commands."
        )
        XCTAssertEqual(
            BrowserImportAutomationError.destinationProfileNotFound("work").errorDescription,
            "No Coterm browser profile matches 'work'"
        )
        XCTAssertEqual(
            BrowserProfileAutomationError.ambiguousProfile("work").errorDescription,
            "Multiple Coterm browser profiles match 'work'. Use the profile ID instead."
        )
    }
}
...
final class ExtensionWorktreePrototypeTests: XCTestCase {
    func testPipeOutputCollectorDrainsBufferedOutputOnFinish() async throws {
        let pipe = Pipe()
        let collector = CotermExtensionPipeOutputCollector(fileHandle: pipe.fileHandleForReading)
    ...
    func testCreateWorktreeKeepsCotermDirectoryLocallyIgnored() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-worktree-prototype-\(UUID().uuidString)", isDirectory: true)
        ...
        _ = try runGit([
            "-c", "user.name=coterm Test",
            "-c", "user.email=coterm@example.invalid",
            "commit",
            "-m",
            "initial"
        ], in: projectRoot)

        let result = try await CotermExtensionWorktreePrototype.createWorktree(projectRootPath: projectRoot.path)

        XCTAssertTrue(FileManager.default.fileExists(atPath: result.worktreePath))
        XCTAssertTrue(result.workspaceTitle.hasPrefix("coterm-sidebar-"))
