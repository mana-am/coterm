...
struct PostHogAnalyticsPropertiesTests {
    ...
    func superPropertiesIncludePlatformVersionAndBuild() {
        ...
        #expect(properties["platform"] as? String == "coterm")
        #expect(properties["app_version"] as? String == "0.31.0")
        #expect(properties["app_build"] as? String == "230")
    }
    ...
    func propertiesOmitVersionFieldsWhenUnavailable() {
        ...
        #expect(superProperties["platform"] as? String == "coterm")
    ...
    func flushPolicyIncludesDailyAndHourlyActiveEvents() {
        #expect(PostHogAnalytics.shouldFlushAfterCapture(event: "coterm_daily_active"))
        #expect(PostHogAnalytics.shouldFlushAfterCapture(event: "coterm_hourly_active"))
        #expect(PostHogAnalytics.shouldFlushAfterCapture(event: "mac_error_captured"))
        #expect(!PostHogAnalytics.shouldFlushAfterCapture(event: "coterm_other_event"))
    }
    ...
    func nativeInteractionMetadataUsesSafeStableValues() {
        #expect(MacAnalyticsEvent.uiInteraction.rawValue == "coterm_ui_interaction")
    ...
    func macBugAlertClientSendsSafeSummaryPayloadWhenEnabled() async throws {
        ...
        let client = MacBugAlertClient(
            endpoint: try #require(URL(string: "https://coterm.test/api/bug-alerts")),
        ...
        #expect(request.request.url?.absoluteString == "https://coterm.test/api/bug-alerts")
        #expect(request.request.httpMethod == "POST")
        #expect(request.request.value(forHTTPHeaderField: "X-Coterm-Bug-Alerts-Secret") == "test-secret")
    ...
    func genericCaptureScrubsAndSendsMacEventsDirectlyToPostHog() async throws {
        let suiteName = "coterm.posthog.analytics.tests.\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer {
            defaults.removePersistentDomain(forName: suiteName)
        }

        let capturedQueue = DispatchQueue(label: "com.coterm.tests.posthog.generic.capture")
        ...
        let analytics = PostHogAnalytics.makeForTesting(
            workQueue: DispatchQueue(label: "com.coterm.tests.posthog.generic.analytics"),
            didStart: true,
        ...
        #expect(captured.properties["platform"] as? String == "coterm")
    }
    ...
    func activeEventCaptureFlushesBeforeShutdown() throws {
        ...
        let capturedQueue = DispatchQueue(label: "com.coterm.tests.posthog.capture")
        var capturedEvents: [(event: String, properties: [String: Any])] = []
        let eventsCaptured = DispatchSemaphore(value: 0)
        let flushCalled = DispatchSemaphore(value: 0)
        let analytics = PostHogAnalytics.makeForTesting(
            workQueue: DispatchQueue(label: "com.coterm.tests.posthog.analytics"),
            didStart: true,
            userDefaults: defaults,
            now: { fixedDate },
            capturePostHog: { event, properties in
        ...
        #expect(events.map(\.event) == ["coterm_daily_active", "coterm_hourly_active"])
...
private final class AnalyticsURLProtocolStubState: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.coterm.tests.analytics.urlprotocol")
...
private final class AnalyticsURLProtocolStub: URLProtocol {
    ...
    override func startLoading() {
        ...
        let response = HTTPURLResponse(
            url: request.url ?? URL(string: "https://coterm.test")!,
