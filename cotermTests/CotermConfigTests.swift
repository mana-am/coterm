...
final class CotermConfigDecodingTests: XCTestCase {

    private func decode(_ json: String) throws -> CotermConfigFile {
        let data = json.data(using: .utf8)!
        return try JSONDecoder().decode(CotermConfigFile.self, from: data)
    }

    private func resolvedActions(
        from config: CotermConfigFile,
        sourcePath: String? = nil
    ) -> [String: CotermResolvedConfigAction] {
        Dictionary(
            uniqueKeysWithValues: config.actions.compactMap { id, definition in
                CotermResolvedConfigAction.fromDefinition(
                    id: id,
    ...
    func testSurfaceTabBarActionReferenceUsesActionSourcePath() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-config-store-\(UUID().uuidString)", isDirectory: true)
        ...
        let globalConfigURL = globalDirectory.appendingPathComponent("coterm.json")
        let localConfigURL = localDirectory.appendingPathComponent("coterm.json")
        let globalJSON = """
        {
          "actions": {
    ...
    func testGlobalSVGIconAllowsNamespaceAndInternalGradient() throws {
        ...
        let configPath = root.appendingPathComponent("coterm.json").path
        ...
        let icon = CotermButtonIcon.imagePath("icons/codex.svg")
        XCTAssertEqual(
            icon.bonsplitIcon(
    ...
    func testProjectLocalSVGIconRejectsExternalReferences() throws {
        ...
        let globalConfigPath = globalDirectory.appendingPathComponent("coterm.json").path
        let projectConfigPath = projectDirectory.appendingPathComponent("coterm.json").path
        ...
        let icon = CotermButtonIcon.imagePath("icons/bad.svg")
        XCTAssertEqual(
            icon.bonsplitIcon(
    ...
    func testUntrustedProjectLocalIconUsesLockPlaceholder() throws {
        ...
        let icon = CotermButtonIcon.imagePath("icons/safe.svg")
        XCTAssertEqual(
            icon.bonsplitIcon(
                configSourcePath: projectConfigPath,
                globalConfigPath: globalConfigPath,
    ...
    func testConfigChangesRequireExplicitLoadByDefault() async throws {
        ...
        let store = CotermConfigStore(globalConfigPath: configURL.path)
        store.loadAll()
        XCTAssertNotNil(store.resolvedAction(id: "first"))
        XCTAssertNil(store.resolvedAction(id: "second"))

        let didAutoReload = expectation(description: "coterm.json should not hot reload")
    ...
    @MainActor
    func testConfigStoreParsesGlobalCotermJSONCSettingsAndActionSections() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-config-store-\(UUID().uuidString)",
            isDirectory: true
        )
        ...
        let data = """
        {
          // coterm-owned app settings share the global coterm.json file.
          "app": {
            "appearance": "dark",
          },
          "actions": {
    ...
    func testLocalWatcherDetectsFirstCanonicalConfigAfterDirectoryCreation() async throws {
        ...
        let configDirectory = projectDirectory.appendingPathComponent(".coterm", isDirectory: true)
        let configURL = configDirectory.appendingPathComponent("coterm.json", isDirectory: false)
        let store = CotermConfigStore(
            globalConfigPath: root.appendingPathComponent("missing-global.json").path,
            localConfigPath: configURL.path,
        ...
        let loaded = expectation(description: "created local coterm config is loaded")
        ...
        // watcher) to observe the .coterm directory creation, re-arm onto the new
        // coterm.json, reload, and republish loadedActions. vnode notification +
    ...
    @MainActor
    func testLocalWatcherDetectsFirstLegacyConfigWhenCotermDirectoryExists() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-config-store-\(UUID().uuidString)",
            isDirectory: true
        )
        ...
        let canonicalConfigURL = configDirectory.appendingPathComponent("coterm.json", isDirectory: false)
        let legacyConfigURL = projectDirectory.appendingPathComponent("coterm.json", isDirectory: false)
        let store = CotermConfigStore(
            globalConfigPath: root.appendingPathComponent("missing-global.json").path,
        ...
        let loaded = expectation(description: "created legacy coterm config is loaded")
        loaded.assertForOverFulfill = false
        var cancellable: AnyCancellable?
        cancellable = store.$loadedActions.dropFirst().sink { actions in
        ...
        // The store observes the legacy coterm.json write via a DispatchSource vnode
    ...
    func testGlobalNewWorkspaceActionUsesLocalActionOverride() throws {
        ...
        let globalConfigURL = root.appendingPathComponent("global-coterm.json")
        let localConfigURL = root.appendingPathComponent("local-coterm.json")
        try """
        {
          "actions": {
    ...
    func testNotificationHooksAppendThroughConfigHierarchy() throws {
        ...
        let globalDirectory = root.appendingPathComponent("global", isDirectory: true)
        let projectDirectory = root.appendingPathComponent("project", isDirectory: true)
        let parentConfigDirectory = projectDirectory.appendingPathComponent(".coterm", isDirectory: true)
        let childDirectory = projectDirectory.appendingPathComponent("child", isDirectory: true)
        let childConfigDirectory = childDirectory.appendingPathComponent(".coterm", isDirectory: true)
        try FileManager.default.createDirectory(at: globalDirectory, withIntermediateDirectories: true)
        ...
        let globalConfigURL = globalDirectory.appendingPathComponent("coterm.json")
        let parentConfigURL = parentConfigDirectory.appendingPathComponent("coterm.json")
        let childConfigURL = childConfigDirectory.appendingPathComponent("coterm.json")
        try """
        {
          "notifications": {
    ...
    func testNotificationHooksIncludeExplicitLocalConfigOutsideDiscoveredHierarchy() throws {
        ...
        let globalConfigURL = globalDirectory.appendingPathComponent("coterm.json")
        let explicitConfigURL = explicitDirectory.appendingPathComponent("custom-coterm.json")
        try """
        {
          "notifications": {
            "hooks": [{ "id": "global", "command": "cat" }]
    ...
    func testSurfaceTabBarWorkspaceCommandButtonRoundTrips() throws {
        let original = CotermSurfaceTabBarButton(
            id: "new-dev",
        ...
        let decoded = try JSONDecoder().decode(CotermSurfaceTabBarButton.self, from: data)
...
final class CotermCommandIdentityTests: XCTestCase {

    func testCommandIdIsDeterministic() {
        let cmd = CotermCommandDefinition(name: "Run tests", command: "test")
        XCTAssertEqual(cmd.id, "coterm.config.command.Run%20tests")
    }

    func testCommandIdEncodesSpecialCharacters() {
        let cmd = CotermCommandDefinition(name: "build & deploy", command: "make")
        XCTAssertTrue(cmd.id.hasPrefix("coterm.config.command."))
        XCTAssertFalse(cmd.id.contains("&"))
        XCTAssertFalse(cmd.id.contains(" "))
    }

    func testCommandIdIsUniqueForDifferentNames() {
        let cmd1 = CotermCommandDefinition(name: "build", command: "make build")
        let cmd2 = CotermCommandDefinition(name: "test", command: "make test")
        XCTAssertNotEqual(cmd1.id, cmd2.id)
    }

    func testCommandIdDoesNotCollideWithBuiltinPrefix() {
        let cmd = CotermCommandDefinition(name: "palette.newWorkspace", command: "echo")
        XCTAssertTrue(cmd.id.hasPrefix("coterm.config.command."))
...
@MainActor
final class CotermConfigWorkspaceCommandExecutionTests: XCTestCase {
    ...
    func testWorkspaceCommandCreatesNewWorkspaceByDefaultWhenNameAlreadyExists() {
        ...
        let command = CotermCommandDefinition(
            name: "Dev command",
            workspace: CotermWorkspaceDefinition(name: "Dev")
        )

        XCTAssertTrue(CotermConfigExecutor.execute(
            command: command,
            tabManager: manager,
            baseCwd: NSTemporaryDirectory(),
            configSourcePath: nil,
            globalConfigPath: "/tmp/coterm-test-global-config.json"
        ))
...
final class CotermSplitDefinitionTests: XCTestCase {
    ...
    func testSplitOrientationVertical() {
        let split = CotermSplitDefinition(direction: .vertical, split: nil, children: [])
...
@MainActor
final class CotermConfigCwdResolutionTests: XCTestCase {

    private let baseCwd = "/Users/test/project"

    func testNilCwdReturnsBase() {
        XCTAssertEqual(
            CotermConfigStore.resolveCwd(nil, relativeTo: baseCwd),
            baseCwd
        )
    }
    ...
    func testEmptyCwdReturnsBase() {
        XCTAssertEqual(
            CotermConfigStore.resolveCwd("", relativeTo: baseCwd),
            baseCwd
        )
    }
    ...
    func testAbsolutePathReturnedAsIs() {
        XCTAssertEqual(
            CotermConfigStore.resolveCwd("/tmp/other", relativeTo: baseCwd),
            "/tmp/other"
        )
    }
    ...
    func testRelativePathJoinedToBase() {
        XCTAssertEqual(
            CotermConfigStore.resolveCwd("backend/src", relativeTo: baseCwd),
    ...
    func testTildeSlashExpandsToHomePlusPath() {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        XCTAssertEqual(
            CotermConfigStore.resolveCwd("~/Documents/work", relativeTo: baseCwd),
    ...
    func testSingleSubdirectory() {
        XCTAssertEqual(
            CotermConfigStore.resolveCwd("src", relativeTo: baseCwd),
...
final class CotermLayoutEncodingTests: XCTestCase {

    func testPaneNodeRoundTrips() throws {
        let original = CotermLayoutNode.pane(CotermPaneDefinition(surfaces: [
            CotermSurfaceDefinition(type: .terminal, name: "shell")
        ]))
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(CotermLayoutNode.self, from: data)
    ...
    func testSplitNodeRoundTrips() throws {
        let original = CotermLayoutNode.split(CotermSplitDefinition(
            direction: .vertical,
            split: 0.7,
            children: [
                .pane(CotermPaneDefinition(surfaces: [CotermSurfaceDefinition(type: .terminal)])),
                .pane(CotermPaneDefinition(surfaces: [CotermSurfaceDefinition(type: .browser, url: "http://localhost")]))
            ]
        ))
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(CotermLayoutNode.self, from: data)
