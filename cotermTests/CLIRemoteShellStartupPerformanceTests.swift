...
struct CLIRemoteShellStartupPerformanceTests {
    ...
    func generatedSSHStartupDoesNotBlockOnRelayRPCWarmup() throws {
        ...
        environment["COTERM_BUNDLED_CLI_PATH"] = root.bin.appendingPathComponent("coterm").path
        environment["COTERM_SOCKET_PATH"] = "/tmp/coterm-test.sock"
        environment["COTERM_WORKSPACE_ID"] = "workspace-cli-perf"
        environment["COTERM_SURFACE_ID"] = "surface-cli-perf"
        environment["COTERM_FAKE_SHELL_MARKER"] = shellMarker.path
        environment["COTERM_FAKE_RELAY_RPC_GATE"] = relayRPCGate.path
    ...
    private func makeFakeRemoteShellRoot() throws -> FakeRemoteShellRoot {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("coterm-cli-shell-perf-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        let bin = root.appendingPathComponent("bin")
        try FileManager.default.createDirectory(at: home, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: bin, withIntermediateDirectories: true)
        try writeExecutable(at: bin.appendingPathComponent("coterm"), contents: """
        #!/bin/sh
        if [ "$1" = "rpc" ] && [ -n "${COTERM_FAKE_RELAY_RPC_GATE:-}" ]; then
          while [ ! -f "$COTERM_FAKE_RELAY_RPC_GATE" ]; do sleep 0.05; done
        fi
        exit 0
        """)
        ...
        #!/bin/sh
        if [ -n "${COTERM_FAKE_SHELL_MARKER:-}" ]; then printf started > "$COTERM_FAKE_SHELL_MARKER"; fi
        exit 0
        """)
    ...
    private func bundledCLIPath() throws -> String {
        ...
        let direct = appBundle.appendingPathComponent("Contents/Resources/bin/coterm")
        ...
        throw NSError(domain: "coterm.tests", code: 1, userInfo: [
            NSLocalizedDescriptionKey: "Bundled coterm CLI not found in \(appBundle.path)",
        ])
    }
