...
struct BrowserHiddenWebViewDiscardMediaPlaybackTests {
    /// Regression coverage for https://github.com/emergent-inc/coterm/issues/5409:
    ...
    /// so the memory bound from https://github.com/emergent-inc/coterm/issues/4539
    /// is preserved.
...
final class BrowserHiddenWebViewDiscardManagerTests: XCTestCase {
    ...
    // Regression coverage for https://github.com/emergent-inc/coterm/issues/5261:
...
final class BrowserPanelInitialNavigationTests: XCTestCase {
    ...
    func testDiffViewerURLIsNotPersistedForSessionRestore() throws {
        let schemeURL = try XCTUnwrap(URL(string: "\(CotermDiffViewerURLSchemeHandler.scheme)://token/index.html"))
        ...
        let loopbackURL = try XCTUnwrap(URL(string: "http://127.0.0.1:49152/token/diff.html#coterm-diff-viewer"))
        ...
        let aliasURL = try XCTUnwrap(URL(string: "http://coterm-loopback.localtest.me:49152/token/diff.html#coterm-diff-viewer"))
    ...
    func testDiffViewerURLIsNotRecordedInBrowserHistory() throws {
        let fileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-browser-history-\(UUID().uuidString).json")
...
final class BrowserPanelDiffViewerSchemeTests: XCTestCase {
    ...
    func testDiffViewerSchemeRegistrationIsIdempotentForCopiedConfiguration() {
        ...
        XCTAssertNotNil(config.urlSchemeHandler(forURLScheme: CotermDiffViewerURLSchemeHandler.scheme))
    }
    ...
    func testDiffViewerSchemeLoadsSameOriginModuleFromAllowlist() throws {
        ...
        try CotermDiffViewerURLSchemeHandler.shared.register(
            token: token,
            files: [
        ...
        let allowedURL = try XCTUnwrap(URL(string: "\(CotermDiffViewerURLSchemeHandler.scheme)://\(token)/index.html"))
        let allowedPatchURL = try XCTUnwrap(URL(string: "\(CotermDiffViewerURLSchemeHandler.scheme)://\(token)/index.patch"))
        let blockedURL = try XCTUnwrap(URL(string: "\(CotermDiffViewerURLSchemeHandler.scheme)://\(token)/not-allowed.html"))
        let queryURL = try XCTUnwrap(URL(string: "\(CotermDiffViewerURLSchemeHandler.scheme)://\(token)/index.html?copy=1"))
        XCTAssertNotNil(CotermDiffViewerURLSchemeHandler.shared.registeredFile(for: allowedURL))
        XCTAssertNotNil(CotermDiffViewerURLSchemeHandler.shared.registeredFile(for: allowedPatchURL))
        XCTAssertNil(CotermDiffViewerURLSchemeHandler.shared.registeredFile(for: blockedURL))
        XCTAssertNil(CotermDiffViewerURLSchemeHandler.shared.registeredFile(for: queryURL))
    ...
    func testDiffViewerSchemeRejectsSymlinkEscapeFromTrustedRoot() throws {
        ...
        let temporaryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-diff-viewer-security-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertThrowsError(try CotermDiffViewerURLSchemeHandler.shared.register(
            token: token,
            files: [
    ...
    func testEnsureReactGrabActiveRefreshesBridgeSessionTokenWhenAlreadyActive() async throws {
        ...
        _ = try await panel.evaluateJavaScript(
            ...
            window['\(panel.reactGrabBridgeSessionUpdaterName)'] = function(token) {
                window.__cotermTestRoundTripToken = token;
                return true;
            };
            true;
            """
        ...
        let refreshedToken = try await panel.evaluateJavaScript("window.__cotermTestRoundTripToken") as? String
...
final class BrowserWindowPortalLifecycleTests: XCTestCase {
    private final class TrackingPortalWebView: WKWebView {
        ...
        @objc(_enterInWindow)
        func cotermUnitTestEnterInWindow() {
            reattachRenderingStateCount += 1
        }

        @objc(_endDeferringViewInWindowChangesSync)
        func cotermUnitTestEndDeferringViewInWindowChangesSync() {
            reattachRenderingStateCount += 1
        }
    }
    ...
    // Regression guard for https://github.com/emergent-inc/coterm/issues/5733.
    ...
    // Crash regression for https://github.com/emergent-inc/coterm/issues/5733.
    //
    ...
    // both `coterm_performKeyEquivalent` and `coterm_makeFirstResponder` ->
    ...
    func testPortalSlotPinPreservesSideDockedInspectorManagedWebViewFrameOnRehost() {
        ...
        let webView = CotermWebView(frame: NSRect(x: 0, y: 0, width: 132, height: 160), configuration: WKWebViewConfiguration())
...
final class OmnibarNativeTextFieldCaretTests: XCTestCase {
    ...
    private func installOmnibarField(
        in window: NSWindow,
        stringValue: String = "https://github.com/emergent-inc/coterm"
    ...
    /// Regression for https://github.com/emergent-inc/coterm/issues/5268: a single,
