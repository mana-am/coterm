...
final class TabManagerSessionSnapshotTests: XCTestCase {
    ...
    private func reserveRemoteRestoreSocket() -> String {
        ...
        let requestedPath = "/tmp/coterm-restore-\(UUID().uuidString).sock"
    ...
    func testClosedItemHistoryPersistsRecordsWithoutSharedCapacityLimit() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-closed-history-\(UUID().uuidString)", isDirectory: true)
    ...
    func testClosedItemHistoryAsyncLoadMergesEarlyMutation() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-closed-history-merge-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testClosedItemHistoryFlushPendingSavesPersistsLatestRecords() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-closed-history-flush-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testClosedItemHistoryAsyncLoadReplaysQueuedPanelWorkspaceRemap() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-closed-history-remap-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testSessionSnapshotIncludesRemoteWorkspacesForRestore() throws {
        ...
        XCTAssertEqual(remoteSnapshot.remote?.destination, "coterm-macmini")
    }
    ...
    func testSessionSnapshotSkipsTemporaryDiffViewerBrowserPanels() throws {
        ...
        let url = try XCTUnwrap(URL(string: "\(CotermDiffViewerURLSchemeHandler.scheme)://token/index.html"))
        _ = try XCTUnwrap(
    ...
    func testSessionSnapshotRestoresSSHWorkspaceDescriptor() throws {
        ...
        let originalAgentSocketPath = "/tmp/coterm-original-restore-agent.sock"
        let restoredAgentSocketPath = "/tmp/coterm-current-restore-agent-\(UUID().uuidString).sock"
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/coterm-restore-test.sock",
        ...
        let snapshotURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ssh-session-restore-\(UUID().uuidString).json")
        ...
        let store = SessionSnapshotRepository<AppSessionSnapshot>(
            ...
            bundleIdentifier: "com.coterm.tests"
        )
    ...
    func testSessionSnapshotRestoreOmitsSSHAgentEnvironmentWhenSocketUnavailable() throws {
        ...
        let originalAgentSocketPath = "/tmp/coterm-original-missing-agent.sock"
        let previousAgentSocketPath = getenv("SSH_AUTH_SOCK").map { String(cString: $0) }
        defer {
            if let previousAgentSocketPath {
                setenv("SSH_AUTH_SOCK", previousAgentSocketPath, 1)
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/coterm-missing-agent-test.sock",
            terminalStartupCommand: "ssh dev@example.com",
            agentSocketPath: originalAgentSocketPath
        )
        remoteWorkspace.configureRemoteConnection(configuration, autoConnect: false)
    ...
    func testSessionSnapshotRestoresPersistentSSHPTYSessionAfterRelaunch() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/coterm-persist-test.sock",
        ...
        let snapshotURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ssh-pty-session-restore-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: snapshotURL) }
        let snapshot = AppSessionSnapshot(
            version: SessionSnapshotSchema.currentVersion,
            createdAt: Date().timeIntervalSince1970,
        ...
        XCTAssertTrue(expectedScrollback.contains("coterm perf synthetic scrollback"), expectedScrollback)
        ...
        XCTAssertTrue(
            restoredWorkspace.remoteConfiguration?.sshOptions.contains("ControlPath=/tmp/coterm-ssh-\(getuid())-64003-%C") == true
        )
        ...
        XCTAssertNil(restoredWorkspace.terminalPanel(for: restoredPanelId)?.requestedWorkingDirectory)
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("export COTERM_SOCKET_PATH=127.0.0.1:64003"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("export PATH=\"$HOME/.coterm/bin:$PATH\""),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(restoredDefaultRemoteCommand.contains("COTERM_SHELL_INTEGRATION_DIR"), restoredDefaultRemoteCommand)
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("coterm_workspace_id='__COTERM_WORKSPACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("'__COTERM_''WORKSPACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            restoredDefaultRemoteCommand.contains("[ -n '__COTERM_WORKSPACE_ID__' ]"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("coterm_surface_id='__COTERM_SURFACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("'__COTERM_''SURFACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            restoredDefaultRemoteCommand.contains("[ -n '__COTERM_SURFACE_ID__' ]"),
            restoredDefaultRemoteCommand
        )
        let substitutedRestoredDefaultRemoteCommand = restoredDefaultRemoteCommand
            .replacingOccurrences(of: "__COTERM_WORKSPACE_ID__", with: restoredWorkspace.id.uuidString)
            .replacingOccurrences(of: "__COTERM_SURFACE_ID__", with: restoredPanelId.uuidString)
        XCTAssertTrue(
            substitutedRestoredDefaultRemoteCommand.contains(
                "coterm_workspace_id='\(restoredWorkspace.id.uuidString)'"
            ),
            substitutedRestoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            substitutedRestoredDefaultRemoteCommand.contains("coterm_surface_id='\(restoredPanelId.uuidString)'"),
            substitutedRestoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            substitutedRestoredDefaultRemoteCommand.contains("COTERM_WORKSPACE_ID=__COTERM_WORKSPACE_ID__"),
            substitutedRestoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            substitutedRestoredDefaultRemoteCommand.contains("COTERM_SURFACE_ID=__COTERM_SURFACE_ID__"),
            substitutedRestoredDefaultRemoteCommand
        )
        ...
        XCTAssertTrue(restoredInitialCommand.contains("COTERM_SURFACE_ID"), restoredInitialCommand)
    ...
    func testSessionSnapshotRestoresSplitPersistentSSHPTYWithoutDefaultAttachScaffold() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "c", count: 64),
            localSocketPath: "/tmp/coterm-persist-split.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYRestoreRewritesStaleRemoteRelayContextIDs() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/coterm-relay-alias.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
        ...
        let refreshedRelayConfiguration = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "ControlMaster=auto",
                "ControlPersist=600",
                "ControlPath=/tmp/coterm-ssh-\(getuid())-64006-%C",
                "StrictHostKeyChecking=accept-new",
            ],
            localProxyPort: nil,
            relayPort: 64006,
            ...
            relayToken: String(repeating: "c", count: 64),
            localSocketPath: "/tmp/coterm-relay-alias-refreshed.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
    ...
    func testPersistentSSHPTYRestoreRewritesMovedSourceWorkspaceContextID() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "d", count: 64),
            localSocketPath: "/tmp/coterm-relay-moved-alias.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYReattachRewritesStaleRemoteRelayContextIDs() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "b", count: 64),
            localSocketPath: "/tmp/coterm-relay-reattach-alias.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYRestoreFallsBackToSnapshotPanelDefaultSessionIDWhenActiveMarkerExists() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "f", count: 64),
            localSocketPath: "/tmp/coterm-legacy-persist.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYRestoreDoesNotReattachEndedSnapshotPanel() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/coterm-ended-persist.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
    ...
    func testPersistentSSHPTYRestorePreservesLocalTerminalWorkingDirectory() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/coterm-local-terminal.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
        ...
        remoteWorkspace.configureRemoteConnection(configuration, autoConnect: false)
        let paneId = try XCTUnwrap(remoteWorkspace.bonsplitController.allPaneIds.first)
        let localDirectory = "/tmp/coterm-local-terminal"
    ...
    func testSessionSnapshotFallsBackWhenPersistentSSHPTYRestoreHasNoSocketPath() throws {
        ...
        remoteWorkspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "f", count: 64),
                localSocketPath: "/tmp/coterm-no-socket.sock",
                terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
                preserveAfterTerminalExit: true,
    ...
    func testSessionSnapshotFallsBackFromSkipBootstrapPersistentSSHPTYWithoutDaemonBridge() throws {
        ...
        let snapshotURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-ssh-pty-durable-restore-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: snapshotURL) }
        let snapshot = AppSessionSnapshot(
            version: SessionSnapshotSchema.currentVersion,
            createdAt: Date().timeIntervalSince1970,
    ...
    func testSessionRemoteWorkspaceSnapshotRequiresPersistentDaemonSlotForPTYRestore() throws {
        ...
        let configuration = try XCTUnwrap(snapshot.workspaceConfiguration(localSocketPath: "/tmp/coterm-restore.sock"))
    ...
    func testSessionRemoteWorkspaceSnapshotStripsTransientControlOptionsWhenPreservedRestoreFallsBack() throws {
        let snapshot = SessionRemoteWorkspaceSnapshot(
            ...
            sshOptions: [
                ...
                "ControlMaster=auto",
                "ControlPersist=600",
                "ControlPath=/tmp/coterm-ssh-501-64003-%C",
            ],
            preserveAfterTerminalExit: true,
            skipDaemonBootstrap: nil,
            relayPort: 64003,
    ...
    /// Regression for https://github.com/emergent-inc/coterm/issues/5931 — a restored
    ...
    func testRestoredTerminalPaneHeaderTitleSyncsToBonsplitTab() throws {
        ...
        let restoredTitle = "~/projects/coterm"
