...
@MainActor
@Suite(.serialized)
struct CotermWebViewContextMenuLinkCaptureTests {
    ...
    func openLinkInDefaultBrowserOpensTheLinkUnderTheRightClick() async throws {
        ...
        CotermWebView.contextMenuLinkCaptureAcceptsUntrustedEventsForTesting = true
        defer { CotermWebView.contextMenuLinkCaptureAcceptsUntrustedEventsForTesting = false }
    ...
    func syntheticContextMenuEventCannotPlantDecoyLink() async throws {
        ...
        // point is identical because CotermWebView (WKWebView) is flipped.
        let openedURL = try await openLinkInDefaultBrowser(
            webView,
            menuEventLocation: NSPoint(x: 60, y: 60),
            menuEventTimestamp: ProcessInfo.processInfo.systemUptime
    ...
    /// CSS (300,300)-(420,420).
    private func makeLoadedTwoLinkWebView() async throws -> CotermWebView {
        _ = NSApplication.shared
        let webView = CotermWebView(
            frame: NSRect(x: 0, y: 0, width: 800, height: 600),
            configuration: WKWebViewConfiguration()
