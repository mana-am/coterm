...
@Suite struct SessionPersistenceResumeBindingTests {
    ...
    @Test func decodingAgentHookBindingRewritesPersistedPATHManagedAgentExecutable() throws {
        let executablePath = Self.homeManagedExecutablePath(
            ...
            "versions",
            "node",
            "coterm-missing-\(UUID().uuidString)",
            "bin"
        )
        let json = """
        {
        ...
        #expect(startupInput.contains("COTERM_CLAUDE_WRAPPER_SHIM"), "\(startupInput)")
    ...
    @Test func agentHookBindingRewritesSupportedLocalManagedExecutablePaths() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-surface-resume-stale-managed-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let executablePaths = [
            Self.localManagedExecutablePath(root: root, executableName: "codex", ".fnm", "current", "bin"),
            "/tmp/coterm-cli-shims/\(UUID().uuidString)/codex",
    ...
    @Test func agentHookBindingWithDirectEnvironmentAssignmentRewritesMovedExecutable() throws {
        ...
        let binding = SurfaceResumeBindingSnapshot(
            kind: "codex",
            command: "COTERM_TRACE=1 '\(staleExecutablePath)' 'resume' 'session-env-cli'",
        ...
        #expect(startupInput.contains("COTERM_TRACE=1 codex 'resume' 'session-env-cli'"), "\(startupInput)")
    ...
    @Test func agentHookBindingWithQuotedEnvAssignmentRewritesMovedExecutable() throws {
        ...
        let binding = SurfaceResumeBindingSnapshot(
            kind: "codex",
            command: "env 'COTERM_TRACE=1' '\(staleExecutablePath)' 'resume' 'session-quoted-env-cli'",
        ...
        #expect(startupInput.contains("env 'COTERM_TRACE=1' codex 'resume' 'session-quoted-env-cli'"), "\(startupInput)")
        #expect(!startupInput.contains(staleExecutablePath), "\(startupInput)")
    }
    ...
    @Test func agentHookClaudeBindingWithDirectEnvironmentAssignmentPreservesAssignmentSyntax() throws {
        ...
        let binding = SurfaceResumeBindingSnapshot(
            kind: "claude",
            command: "COTERM_TRACE='bar baz' '\(staleExecutablePath)' '--resume' 'session-env-cli'",
            checkpointId: "session-env-cli",
            source: "agent-hook",
            autoResume: true
        )
        ...
        #expect(startupInput.contains("/bin/sh -c"), "\(startupInput)")
        #expect(startupInput.contains("COTERM_CLAUDE_WRAPPER_SHIM"), "\(startupInput)")
        #expect(startupInput.contains("COTERM_TRACE="), "\(startupInput)")
    ...
    @Test func agentHookClaudeBindingWithShellOperatorKeepsOriginalCommandShape() throws {
        ...
        let redirection = "1>/tmp/coterm-claude-resume.log"
        let binding = SurfaceResumeBindingSnapshot(
            kind: "claude",
    ...
    @Test @MainActor func remoteWorkspaceLocalTerminalResumeBindingUsesLocalRepair() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-local-resume-binding-\(UUID().uuidString)", isDirectory: true)
        ...
        let suiteName = "coterm-session-resume-binding-\(UUID().uuidString)"
    ...
    @Test func agentHookSurfaceResumeStartupInputPreservesExistingPATHManagedAgentExecutable() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-surface-resume-existing-agent-\(UUID().uuidString)", isDirectory: true)
        let executable = root
    ...
    @Test func agentHookSurfaceResumeStartupInputFallsBackWhenRecordedAgentExecutableMoved() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("coterm-surface-resume-moved-agent-\(UUID().uuidString)", isDirectory: true)
        ...
        process.environment = [
            ...
            "COTERM_FAKE_CODEX_OUTPUT": outputURL.path,
        ]
        let stderr = Pipe()
    ...
    private func makeOhMyZshLauncherEnvironment(
        ...
        return [
            ...
            "COTERM_ZSH_ZDOTDIR": userZdotdirURL.path,
            "COTERM_SHELL_INTEGRATION": "1",
            "COTERM_SHELL_INTEGRATION_DIR": integrationDir.path,
            "COTERM_ZSH_RESTORE_TERM": "xterm-256color",
            "COTERM_CODEX_WRAPPER_SHIM": codexShimURL.path,
            "COTERM_FAKE_CODEX_OUTPUT": codexOutputURL.path,
    ...
    private func writeOhMyZshFixture(at root: URL) throws {
        ...
        try """
        autoload -Uz add-zsh-hook
        _coterm_test_autosuggest_precmd() { :; }
        _coterm_test_autosuggest_preexec() { :; }
        add-zsh-hook precmd _coterm_test_autosuggest_precmd
        add-zsh-hook preexec _coterm_test_autosuggest_preexec
        _coterm_test_autosuggest_self_insert() { zle .self-insert }
        zle -N self-insert _coterm_test_autosuggest_self_insert
        """.write(
        ...
        try """
        _coterm_test_syntax_highlighting_line_init() { :; }
        _coterm_test_syntax_highlighting_line_finish() { :; }
        zle -N zle-line-init _coterm_test_syntax_highlighting_line_init
        zle -N zle-line-finish _coterm_test_syntax_highlighting_line_finish
        """.write(
