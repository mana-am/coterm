...
final class WorkspaceCustomSidebarPullRequestContextTests: XCTestCase {
    ...
    func testCustomSidebarSurfacePersistsAndRestoresAsPane() throws {
        let sidebarName = "__coterm_restore_sidebar_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        try withTemporaryCustomSidebarsDirectory { directory in
            ...
            let surfaceEvent = try XCTUnwrap(
                CotermEventBus.shared.retainedSnapshot().first { $0["name"] as? String == "surface.created" }
            )
    ...
    func testSplitCustomSidebarPublishesNewPaneLifecycleEvents() throws {
        let sidebarName = "__coterm_split_sidebar_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        try withTemporaryCustomSidebarsDirectory { directory in
            let fileURL = directory.appendingPathComponent("\(sidebarName).swift")
    ...
    func testV2CustomSidebarOpenReturnsErrorWhenValidationFails() throws {
        let missingName = "__coterm_missing_sidebar_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
    ...
    func testV2CustomSidebarOpenRejectsMalformedWorkspaceTarget() throws {
        let sidebarName = "__coterm_target_sidebar_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        try withTemporaryCustomSidebarsDirectory { directory in
            let fileURL = directory.appendingPathComponent("\(sidebarName).swift")
    ...
    func testV2CustomSidebarOpenFallsBackWhenFocusedPanelCannotSplit() throws {
        let sidebarName = "__coterm_fallback_sidebar_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        try withTemporaryCustomSidebarsDirectory { directory in
            let fileURL = directory.appendingPathComponent("\(sidebarName).swift")
    ...
    func testValuesIncludePanelPullRequestWhenFocusedPanelMirrorIsNil() throws {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 5314,
            label: "PR",
            url: URL(string: "https://github.com/emergent-inc/coterm/pull/5314")!,
            status: .open
        )
        ...
        XCTAssertEqual(fields["url"], .string("https://github.com/emergent-inc/coterm/pull/5314"))
    ...
    private func withTemporaryCustomSidebarsDirectory<T>(_ body: (URL) throws -> T) throws -> T {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebars-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        return try CotermExtensionSidebarSelection.withCustomSidebarsDirectoryForTesting(directory) {
            try body(directory)
        }
    }
}
