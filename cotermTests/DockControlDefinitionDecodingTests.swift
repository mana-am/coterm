...
struct DockControlDefinitionDecodingTests {
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-dock-config-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func projectConfigIdentityUsesResolvedDockFile() throws {
        ...
        let cotermDirectory = root.appendingPathComponent(".coterm", isDirectory: true)
        try FileManager.default.createDirectory(at: cotermDirectory, withIntermediateDirectories: true)
        let dockConfig = cotermDirectory.appendingPathComponent("dock.json", isDirectory: false)
