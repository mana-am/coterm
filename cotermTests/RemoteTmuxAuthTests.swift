...
/// Behavior tests for the remote-tmux SSH auth path that backs `coterm ssh-tmux`:
...
@Suite struct RemoteTmuxAuthTests {
    ...
    @Test func controlModeArgumentsUseRemoteTmuxResolverAfterDestinationGuard() throws {
        ...
        #expect(remoteCommand.hasSuffix("'coterm-remote-tmux' '-CC' 'attach-session' '-t' 'work session'"))
    }
    ...
    @Test @MainActor func layoutChangePrunesRemovedPaneDiagnosticState() {
        ...
        connection.handleMessageForTesting(.subscriptionChanged(name: "coterm_reflow_4", value: "0|zsh"))
        connection.handleMessageForTesting(.subscriptionChanged(name: "coterm_reflow_5", value: "1|vim"))
    ...
    @Test func pastePaneCommandsProtectOptionLookingText() throws {
        ...
        #expect(commands.setBuffer == "set-buffer -b coterm-paste-7 -- '-n not-an-option'")
        #expect(commands.pasteBuffer == "paste-buffer -p -d -b coterm-paste-7 -t %7")
    }
    ...
    // MARK: - Interactive auth invocation (what `coterm ssh-tmux` runs in the tty)
