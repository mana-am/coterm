...
private extension GhosttyNSView {
    @objc func cotermUnitTest_interpretKeyEvents(_ eventArray: [NSEvent]) {
        if let hook = cjkIMEInterpretKeyEventsHook, hook(self, eventArray) {
            return
        }
        cotermUnitTest_interpretKeyEvents(eventArray)
    }

    @objc func cotermUnitTest_paste(_ sender: Any?) {
        ghosttyPasteActionHook?(self, sender)
        cotermUnitTest_paste(sender)
    }

    @objc func cotermUnitTest_pasteAsPlainText(_ sender: Any?) {
        ghosttyPasteAsPlainTextActionHook?(self, sender)
        cotermUnitTest_pasteAsPlainText(sender)
    }
}
...
func installCJKIMEInterpretKeyEventsSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.cotermUnitTest_interpretKeyEvents(_:))
...
private func installGhosttyPasteActionSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.cotermUnitTest_paste(_:))
    ...
    let plainTextSwizzledSelector = #selector(GhosttyNSView.cotermUnitTest_pasteAsPlainText(_:))
...
// MARK: - Space release regression (Codex hold-to-talk in coterm)
...
final class GhosttyKeyEquivalentRegressionTests: XCTestCase {
    ...
    private func cotermZshTerminalKeyboardResetSequence() throws -> Data {
        let repoRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let integrationPath = repoRoot
            .appendingPathComponent("Resources/shell-integration/coterm-zsh-integration.zsh")
            .path
        ...
        process.arguments = [
            ...
            if (( $+functions[_coterm_reset_terminal_keyboard_protocols] )); then
              _coterm_reset_terminal_keyboard_protocols
            fi
            """
        ]
        process.environment = [
            "COTERM_TEST_FORCE_KEYBOARD_RESET": "1"
        ]
        let output = Pipe()
    ...
    func testStaleKittyKeyboardAfterClearHistoryDoesNotEncodePlainLetterAsCSIU() throws {
        let captureReadyMarker = "COTERM_KBD_READY_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let captureMarker = "COTERM_KBD_HEX_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-kbd-capture-\(UUID().uuidString).py")
        let script = """
        import os
        import select
        import sys
        ...
        let keyboardResetData = try cotermZshTerminalKeyboardResetSequence()
        XCTAssertEqual(
            keyboardResetData,
            Data("\u{1B}[>m\u{1B}[<8u\u{1B}[?9l\u{1B}[?1000l\u{1B}[?1002l\u{1B}[?1003l\u{1B}[?1005l\u{1B}[?1006l\u{1B}[?1004l\u{1B}[?2004l\u{1B}[?2026l".utf8),
            "cotermZshTerminalKeyboardResetSequence must reset keyboard, mouse, and focus reporting state"
        )
