...
@Suite struct BrowserDownloadFilenameResolverTests {
    ...
    @Test func uniqueDownloadDestinationDedupesExistingFiles() throws {
        ...
        let directory = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-download-resolver-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    @Test func webDownloadQuarantineMetadataMarksRemoteDownloads() throws {
        let fileManager = FileManager.default
        let directory = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-download-quarantine-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: directory, withIntermediateDirectories: true, attributes: nil)
        defer { try? fileManager.removeItem(at: directory) }
        ...
        try fileURL.cotermApplyWebDownloadQuarantine(sourceURL: sourceURL)
    ...
    @Test func webDownloadQuarantineMetadataSkipsLocalFileSources() throws {
        let fileManager = FileManager.default
        let directory = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-download-quarantine-local-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: directory, withIntermediateDirectories: true, attributes: nil)
        defer { try? fileManager.removeItem(at: directory) }
        ...
        try fileURL.cotermApplyWebDownloadQuarantine(sourceURL: URL(fileURLWithPath: "/tmp/source.txt"))
    ...
    @Test func scriptedDownloadInterceptionKeepsFullHookOutOfSubframes() throws {
        ...
        let mainFrameScript = try #require(
            scripts.first {
                $0.source.contains("__cotermScriptedDownloadInstalled")
            }
        )
    ...
    @Test func promptedDownloadCompletionCallbacksPostFinalEvents() throws {
        ...
        let savedURL = URL(fileURLWithPath: "/tmp/coterm-download-report.csv")
        ...
        delegate.onDownloadFailed?(
            NSError(domain: "coterm.download.test", code: 7, userInfo: [
    ...
    @Test func sessionDownloadBridgePostsAutomationEvents() throws {
        ...
        (try #require(panel.webView as? CotermWebView)).onSessionDownloadEvent?([
            "type": "saved",
    ...
    @Test func downloadCookiesMatchRequestDomainAndPath() throws {
        ...
        let names = Set(CotermWebView.cookiesForDownloadRequest(cookies, url: url).map(\.name))
