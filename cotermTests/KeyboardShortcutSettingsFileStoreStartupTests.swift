...
import enum CotermSettings.BrowserSearchEngine
import struct CotermSettings.BrowserSearchSettingsStore
...
final class KeyboardShortcutSettingsFileStoreStartupTests: XCTestCase {
    ...
    /// coterm pins a fixed dark appearance; a leftover `app.appearance` key in
    /// coterm.json must be silently ignored (no managed default written and no
    ...
    func testConfirmQuitImportsEnumFromCotermJSON() throws {
        let defaults = UserDefaults.standard
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-settings-startup-\(UUID().uuidString)",
            isDirectory: true
        )
