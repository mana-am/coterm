...
final class AgentHibernationTests: XCTestCase {
    ...
    func testSocketLifecycleAcceptsRegisteredCustomAgentKey() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-custom-lifecycle-\(UUID().uuidString)", isDirectory: true)
        let configDirectory = root.appendingPathComponent(".coterm", isDirectory: true)
        ...
        """.write(to: configDirectory.appendingPathComponent("coterm.json"), atomically: true, encoding: .utf8)
    ...
    func testSettingsDefaultToOptInAndNotifyOnChanges() throws {
        let suiteName = "coterm-agent-hibernation-\(UUID().uuidString)"
    ...
    func testSessionIndexLoadsAgentLifecycleFromHookStore() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-hibernation-index-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSessionIndexUsesLiveHookPIDAsProcessID() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-hibernation-live-hook-pid-\(UUID().uuidString)", isDirectory: true)
        let storeURL = RestorableAgentKind.codex.hookStoreFileURL(homeDirectory: home.path)
        try FileManager.default.createDirectory(at: storeURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
        ...
        let index = RestorableAgentSessionIndex.load(
            ...
            processArgumentsProvider: { requestedPID in
                requestedPID == pid
                    ? CotermTopProcessArguments(
                        arguments: ["/bin/sleep", "30"],
                        environment: [
                            "COTERM_WORKSPACE_ID": workspaceId.uuidString,
                            "COTERM_SURFACE_ID": panelId.uuidString,
                            "COTERM_AGENT_LAUNCH_KIND": RestorableAgentKind.codex.rawValue,
                        ]
                    )
                    : nil
            }
    ...
    func testSessionIndexAcceptsNodeBackedClaudeProcessAsLiveHookPID() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-hibernation-claude-node-pid-\(UUID().uuidString)", isDirectory: true)
                ...
                requestedPID == pid
                    ? CotermTopProcessArguments(
                        ...
                        environment: [
                            "COTERM_WORKSPACE_ID": workspaceId.uuidString,
                            "COTERM_SURFACE_ID": panelId.uuidString,
                            "COTERM_AGENT_LAUNCH_KIND": RestorableAgentKind.claude.rawValue,
                        ]
                    )
                    : nil
            }
    ...
    func testLiveProcessScopeMatchingAcceptsLegacyEnvironmentKeys() throws {
        let workspaceId = UUID()
        let panelId = UUID()
        let process = CotermTopProcessArguments(
            arguments: ["/usr/bin/codex"],
            environment: [
                "COTERM_TAB_ID": workspaceId.uuidString,
                "COTERM_PANEL_ID": panelId.uuidString,
            ]
        )

        XCTAssertTrue(process.matchesCotermScope(workspaceId: workspaceId, surfaceId: panelId))
        XCTAssertFalse(process.matchesCotermScope(workspaceId: UUID(), surfaceId: panelId))
        XCTAssertFalse(process.matchesCotermScope(workspaceId: workspaceId, surfaceId: UUID()))
    }
    ...
    func testProcessDetectedSnapshotPreservesMatchingHookLifecycleWithoutRefreshingActivity() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-hibernation-detected-\(UUID().uuidString)", isDirectory: true)
    ...
    func testProcessDetectedSnapshotPreservesMatchingHookLifecycleWhenHookPIDIsStale() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-hibernation-stale-pid-\(UUID().uuidString)", isDirectory: true)
        let storeURL = RestorableAgentKind.opencode.hookStoreFileURL(homeDirectory: home.path)
        try FileManager.default.createDirectory(at: storeURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
    ...
    func testProcessDetectedSnapshotPreservesHookLifecycleWhenRestoredPanelIDsChange() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-hibernation-remapped-panel-\(UUID().uuidString)", isDirectory: true)
        let storeURL = RestorableAgentKind.opencode.hookStoreFileURL(homeDirectory: home.path)
        try FileManager.default.createDirectory(at: storeURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
    ...
    func testProcessDetectedOnlySnapshotDoesNotUseScanTimeAsActivity() {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-hibernation-empty-home-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSupportedAgentSnapshotsHaveResumeCommandsForHibernation() {
        let cwd = "/tmp/coterm-agent-hibernation"
    ...
    func testCustomRegisteredAgentSnapshotCanHibernateWhenResumeCommandExists() {
        let registration = CotermVaultAgentRegistration(
            id: "local-agent",
            name: "Local Agent",
            detect: CotermVaultAgentDetectRule(processName: "local-agent"),
    ...
    func testInvalidatedIndexedAgentSnapshotIsNotEligibleForHibernation() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-hibernation-invalidated-index-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: home, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
        ...
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            workingDirectory: "/tmp/coterm-agent-hibernation",
            launchCommand: launch("codex", "/usr/local/bin/codex", cwd: "/tmp/coterm-agent-hibernation")
        )
