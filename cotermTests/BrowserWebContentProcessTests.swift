...
struct BrowserWebContentProcessTests {
    private let recoveryURL = URL(string: "data:text/html,coterm-recovery")!
    ...
    func browserPanelInstallsWebAuthnBridgeWithoutExposingNativeHandler() async throws {
        ...
        let result = try await webView.evaluateJavaScript(
            """
            ({
              bridge: window.__cotermWebAuthnBridgeInstalled === true,
              handler: !!(
                window.webkit &&
                window.webkit.messageHandlers &&
                window.webkit.messageHandlers.cotermWebAuthn &&
                typeof window.webkit.messageHandlers.cotermWebAuthn.postMessage === "function"
              )
            })
            """
        ) as? [String: Bool]
