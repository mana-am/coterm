...
final class CotermOpenCommandTests: XCTestCase {
    ...
    func testDiffCommandGeneratesCodeViewAndOpensBrowserSplit() throws {
        ...
        try FileManager.default.createDirectory(at: cotermConfigURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: cotermAppSupportConfigURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        ...
        try ghosttyConfigContents.write(to: cotermAppSupportConfigURL, atomically: true, encoding: .utf8)
        try """
        {
          "shortcuts": {
            "bindings": {
        ...
        """.write(to: cotermConfigURL, atomically: true, encoding: .utf8)
        try """
        ...
        let serverHandled = startMockServer(listenerFD: listenerFD, state: state) { line in
            ...
            guard method == "browser.open_split",
                  ...
                  viewerURL.fragment == "coterm-diff-viewer" else {
                return Self.v2Response(id: id, ok: false, error: ["code": "unexpected", "message": method])
            }
            return Self.v2Response(
                id: id,
        ...
        XCTAssertEqual(viewerURL.fragment, "coterm-diff-viewer")
        ...
        XCTAssertTrue(html.contains("<script id=\"coterm-diff-viewer-config\" type=\"application/json\">") && html.contains("background: transparent;"), html)
        XCTAssertTrue(html.contains("<div id=\"root\"></div>"), html)
        XCTAssertTrue(html.contains("<script type=\"module\" src=\"./assets/coterm-diff-viewer-app/main.mjs\"></script>"), html)
        ...
        let appAssetDirectory = viewerFileURL.deletingLastPathComponent()
            .appendingPathComponent("assets", isDirectory: true)
            .appendingPathComponent("coterm-diff-viewer-app", isDirectory: true)
        ...
        XCTAssertTrue(html.contains("\"dark\":\"coterm-ghostty-dark-"), html)
        XCTAssertTrue(html.contains("\"light\":\"coterm-ghostty-light-"), html)
        ...
        XCTAssertTrue(files.contains { file in
            file["request_path"] as? String == "/assets/coterm-diff-viewer-app/main.mjs" &&
        ...
        try darkOnlyConfigContents.write(to: cotermAppSupportConfigURL, atomically: true, encoding: .utf8)
    ...
    func testDiffCommandUsesTaggedSocketAppAssetsAndServer() throws {
        ...
        let socketPath = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("coterm-debug-\(tag).sock", isDirectory: false)
            .path
        unlink(socketPath)
        let listenerFD = try bindUnixSocket(at: socketPath)
        let rootURL = FileManager.default.temporaryDirectory
        ...
        try writeTestDiffViewerAssets(
            ...
            appMain: "export const cotermTaggedSocketAssetMarker = 'target-\(tag)';\n"
        )
        try """
        ...
        let appEntry = try XCTUnwrap(files.first { file in
            (file["request_path"] as? String)?.hasSuffix("/assets/coterm-diff-viewer-app/main.mjs") == true
        })
        ...
        XCTAssertTrue(appMain.contains("cotermTaggedSocketAssetMarker = 'target-\(tag)'"), appMain)
    ...
    func testDiffCommandLinksOriginalDiffshubPRURL() throws {
        ...
        let result = try runDiffCLIAndReadHTML(
            ...
            environmentOverrides: ["COTERM_DIFF_VIEWER_STREAM_REMOTE": "1"],
    ...
    func testDiffViewerServerBoundsDeferredWaitRequests() throws {
        ...
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-diff-viewer-wait-\(UUID().uuidString)", isDirectory: true)
        ...
        try """
        <!doctype html>
        <html data-coterm-diff-pending="true">
        ...
        environment["COTERM_DIFF_VIEWER_WAIT_TIMEOUT_SECONDS"] = "0.05"
        ...
        let url = try XCTUnwrap(URL(string: "http://127.0.0.1:\(port)/__coterm_diff_viewer_wait/\(token)/pending.html"))
        // The bounded deferred wait is the unit under test. With
        // COTERM_DIFF_VIEWER_WAIT_TIMEOUT_SECONDS=0.05 the server must give up on the
        ...
        XCTAssertFalse(body.contains("data-coterm-diff-pending=\"true\""), body)
    ...
    func testDiffCommandFallsBackToNonEmptyGitSourceForSelector() throws {
        ...
        try runGit(["config", "user.name", "coterm tests"], in: repoURL)
        try runGit(["config", "user.email", "coterm@example.invalid"], in: repoURL)
        try "one\n".write(to: fileURL, atomically: true, encoding: .utf8)
    ...
    func testDiffCommandShowsFriendlyEmptyStateForLastTurnWithoutBaseline() throws {
        ...
        let result = try runDiffCLIAndReadHTML(
            ...
            environmentOverrides: [
                "COTERM_AGENT_HOOK_STATE_DIR": rootURL.appendingPathComponent("hook-state", isDirectory: true).path,
                "COTERM_WORKSPACE_ID": UUID().uuidString.lowercased(),
                "COTERM_SURFACE_ID": UUID().uuidString.lowercased()
            ],
            currentDirectoryURL: repoURL,
            readPatchSidecar: false
        )
    ...
    func testDiffCommandShowsFriendlyEmptyStateForEmptyLastTurnDiff() throws {
        ...
        let result = try runDiffCLIAndReadHTML(
            ...
            arguments: ["diff", "--last-turn"],
            environmentOverrides: [
                "COTERM_AGENT_HOOK_STATE_DIR": stateURL.path,
    ...
    func testDiffCommandSupportsGitSourcesAndSurfaceScopedLastTurn() throws {
        ...
        try runGit(["config", "user.name", "coterm tests"], in: siblingRepoURL)
        try runGit(["config", "user.email", "coterm@example.invalid"], in: siblingRepoURL)
        ...
        let homeStateURL = homeURL.appendingPathComponent(".coterm", isDirectory: true)
    ...
    func testAgentTurnDiffBaselineStoresUntrackedSnapshotsOutsideGit() throws {
        ...
        let untrackedRefs = try runGitStdout(
            ["for-each-ref", "--format=%(refname)", "refs/coterm/last-turn/untracked"],
            in: repoURL
        )
    ...
    func testDiffCommandOpensPendingViewerBeforeGitDiffCompletes() throws {
        ...
        if [ "${1:-}" = "rev-parse" ] && [ "${2:-}" = "--show-toplevel" ]; then
          printf '%s\\n' "$COTERM_FAKE_GIT_REPO_ROOT"
          exit 0
        fi
        if [ "${1:-}" = "rev-parse" ] && [ "${2:-}" = "--verify" ]; then
          : > "$COTERM_FAKE_GIT_STARTED"
          while [ ! -f "$COTERM_FAKE_GIT_RELEASE" ]; do
            sleep 0.05
          done
          exit 1
        fi
        if [ "${1:-}" = "diff" ] && [ "${2:-}" = "--cached" ]; then
          : > "$COTERM_FAKE_GIT_ALTERNATE_STARTED"
          while [ ! -f "$COTERM_FAKE_GIT_RELEASE_ALTERNATE" ]; do
            sleep 0.05
          done
          exit 0
        fi
        ...
        environment["COTERM_FAKE_GIT_REPO_ROOT"] = repoURL.path
        environment["COTERM_FAKE_GIT_STARTED"] = diffStartedURL.path
        environment["COTERM_FAKE_GIT_RELEASE"] = releaseDiffURL.path
        environment["COTERM_FAKE_GIT_ALTERNATE_STARTED"] = alternateStartedURL.path
        environment["COTERM_FAKE_GIT_RELEASE_ALTERNATE"] = releaseAlternateURL.path
        process.executableURL = URL(fileURLWithPath: cliPath)
        ...
        XCTAssertTrue(pendingHTML.contains("data-coterm-diff-pending=\"true\""), pendingHTML)
        ...
        XCTAssertTrue(waitUntil(timeout: 5) {
            ...
            return html.contains("data-coterm-diff-redirect=")
        ...
        XCTAssertFalse(html.contains("data-coterm-diff-pending=\"true\""), html)
    ...
    func testTopCommandOutputsWindowLevelProcessRows() throws {
        ...
        XCTAssertEqual(outputLines(result.stdout), [
            ...
            "2.0\t2000\t1\tprocess\t4129\twindow:1\tcoterm",
        ])
    }
    ...
    func testTopCommandSortsFlatWindowProcessesAndWorkspacesTogetherByMemory() throws {
        ...
        XCTAssertEqual(outputLines(result.stdout), [
            "2.0\t2000\t2\twindow\twindow:1\ttotal\t",
            "4.0\t10000\t1\tprocess\t4129\twindow:1\tcoterm",
            "1.0\t1000\t1\tworkspace\tworkspace:low\twindow:1\t",
        ])
    }
    ...
    private static func diffViewerRedirectURL(from html: String) -> String? {
        let marker = "data-coterm-diff-redirect=\""
    ...
    private func diffViewerConfig(from html: String) throws -> [String: Any] {
        let marker = "<script id=\"coterm-diff-viewer-config\" type=\"application/json\">"
    ...
    private func runGit(_ arguments: [String], in directory: URL) throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        if result.status != 0 {
            throw NSError(domain: "CotermOpenCommandTests.git", code: Int(result.status), userInfo: [NSLocalizedDescriptionKey: result.stderr])
        }
    }
    ...
    private func writeTestDiffViewerAssets(resourcesURL: URL, appMain: String) throws {
        ...
        try "self.cotermWorkerFixture = true;\n".write(
    ...
    private func readLine(from handle: FileHandle, timeout: TimeInterval) throws -> String {
        ...
        guard finished.wait(timeout: .now() + timeout) == .success else {
            throw NSError(domain: "coterm.tests", code: 1, userInfo: [
    ...
    private func fetchData(from url: URL, timeout: TimeInterval) throws -> (data: Data, statusCode: Int) {
        ...
        guard finished.wait(timeout: .now() + timeout) == .success else {
            ...
            throw NSError(domain: "coterm.tests", code: 2, userInfo: [
