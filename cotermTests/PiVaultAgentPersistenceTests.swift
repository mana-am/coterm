...
final class PiVaultAgentPersistenceTests: XCTestCase {
    ...
    func testBuiltInPiRegistrationUsesBrandedIconAsset() {
        let agent = RegisteredSessionAgent(registration: CotermVaultAgentRegistration.builtInPi)
    ...
    func testBuiltInAntigravityRegistrationUsesBrandedIconAsset() {
        let agent = RegisteredSessionAgent(registration: CotermVaultAgentRegistration.builtInAntigravity)
        ...
        XCTAssertEqual(CotermVaultAgentRegistration.builtInAntigravity.detect.processNames, ["agy", "antigravity"])
    }

    func testBuiltInAntigravityRegistrationLoadsHistoryDisplayAndWorkspace() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-antigravity-vault-history-\(UUID().uuidString)", isDirectory: true)
        ...
        var registration = CotermVaultAgentRegistration.builtInAntigravity
    ...
    func testBuiltInAntigravityRegistrationIndexesEachHistoryConversation() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-antigravity-vault-conversations-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testRegisteredAgentJSONLWorkspaceKeyIsSharedCWDMetadata() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-registered-workspace-cwd-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
        ...
        let registration = CotermVaultAgentRegistration(
            id: "acme-agent",
            name: "Acme Agent",
            detect: CotermVaultAgentDetectRule(processName: "acme-agent"),
    ...
    func testRegisteredAgentJSONLDisplayFieldIsNotSharedTitleMetadata() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-registered-display-title-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testRegisteredAgentJSONLSessionIDDoesNotUseAntigravityConversationID() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-registered-session-id-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testBuiltInGrokRegistrationUsesNativeSessionDirectory() {
        let registration = CotermVaultAgentRegistration.builtInGrok
    ...
    func testRegisteredAgentJSONLNativeSessionIDOverridesPathFallback() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-registered-native-id-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testRegisteredAgentCWDFilterUsesJSONLMetadataNotFallback() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-registered-cwd-filter-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testRegisteredAgentMetadataKeepsScanningForBranchWhenFallbackCWDSet() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-pi-vault-branch-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testPiJSONLTypedContentBlocksUseFirstUserTextAsTitle() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-pi-vault-title-blocks-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testPiJSONLTopLevelAssistantTypedContentDoesNotBecomeTitle() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-pi-vault-top-level-role-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testPiJSONLMessagesArrayUsesNilRoleTextAsTitle() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-pi-vault-messages-nil-role-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testPiJSONLTypedContentBlocksRequireTextType() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-pi-vault-typed-content-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testGrokVaultLoadsNativeChatHistoryFromEncodedDirectory() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-vault-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
        ...
        var registration = CotermVaultAgentRegistration.builtInGrok
    ...
    func testGrokVaultTitlePrefersUserQueryOverInjectedMetadata() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-vault-metadata-title-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testGrokVaultFindsBranchAfterStableMetadata() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-vault-late-branch-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testGrokVaultLoadsHookObservedShellGrokHome() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-vault-observed-home-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testGrokVaultLoadsHookObservedShellGrokHomeFromCustomStateDir() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-vault-custom-state-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
        ...
        let entries = await SessionIndexStore.loadGrokEntries(
            ...
            offset: 0,
            limit: 10,
            environment: ["COTERM_AGENT_HOOK_STATE_DIR": hookStateDir.path],
            homeDirectory: homeDirectory.path
        )
    ...
    func testRegisteredGrokSessionDirectoryUsesNativeDirectoryLayout() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-registered-grok-vault-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
        ...
        let registration = CotermVaultAgentRegistration(
            id: "custom-grok",
            name: "Custom Grok",
            detect: CotermVaultAgentDetectRule(processName: "custom-grok"),
    ...
    func testGrokVaultCWDFilterUsesEncodedProjectDirectory() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-vault-filter-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testGrokAgentSearchScopeUsesCurrentDirectoryCWDFilter() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-grok-agent-scope-filter-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testPiVaultAgentSnapshotRoundTripBuildsTargetedSessionCommand() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-pi-vault-agent-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
            ...
            SessionPanelSnapshot(
                ...
                terminal: SessionTerminalPanelSnapshot(
                    ...
                    agent: SessionRestorableAgentSnapshot(
                        ...
                        registration: CotermVaultAgentRegistration.builtInPi
                    ),
