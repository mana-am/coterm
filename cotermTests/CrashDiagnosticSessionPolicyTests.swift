...
struct CrashDiagnosticSessionPolicyTests {
    @Test
    func terminalDefaultFileOpenIgnoresGhosttyCrashReportsInCotermCrashDirectory() {
        let crashReport = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".local/state/coterm/crash/coterm.ghosttycrash", isDirectory: false)
    ...
    @Test
    func terminalDefaultFileOpenIgnoresSymlinkedGhosttyCrashReportsInCotermCrashDirectory() throws {
        let crashReport = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".local/state/coterm/crash/coterm.ghosttycrash", isDirectory: false)
        let symlink = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-symlinked-crash-\(UUID().uuidString).ghosttycrash", isDirectory: false)
    ...
    func appDelegateSessionSnapshotDropsCrashDiagnosticWindow() throws {
        ...
        let projectDirectory = "/tmp/coterm-project"
        ...
        let crashDirectory = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".local/state/coterm/crash", isDirectory: true)
            .path
    ...
    func sessionSnapshotDropsEmptyCrashDiagnosticWorkspace() {
        ...
        let pruned = SessionPersistencePolicy.pruningCotermCrashDiagnosticWindows(from: snapshot)
    ...
    func sessionSnapshotPruningDoesNotResolveSymlinkedCrashDirectories() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-crash-storage-symlink-\(UUID().uuidString)", isDirectory: true)
        ...
        #expect(!SessionPersistencePolicy.isCotermCrashStoragePath(
            symlink.path,
        ...
        #expect(!SessionPersistencePolicy.isCotermCrashDiagnosticWindow(
            window,
            homeDirectory: homeDirectory,
            environment: [:]
        ))
