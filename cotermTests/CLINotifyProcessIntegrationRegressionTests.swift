...
final class CLINotifyProcessIntegrationRegressionTests: XCTestCase {
    ...
    // https://github.com/emergent-inc/coterm/issues/6606
    //
    ...
    func testCodexPromptSubmitDoesNotRefreshTerminalLastTurnDiffBaseline() throws {
        ...
        _ = try runGit(["config", "user.name", "coterm tests"])
        _ = try runGit(["config", "user.email", "coterm@example.invalid"])
    ...
    // MARK: - Forked conversation restore (https://github.com/emergent-inc/coterm/issues/5908)
    //
    ...
    private func runClaudeHookWithoutServer(
        ...
        var environment = [
            ...
            "COTERM_SOCKET_PATH": context.socketPath,
            "COTERM_WORKSPACE_ID": context.workspaceId,
            "COTERM_SURFACE_ID": context.surfaceId,
            "COTERM_CLAUDE_HOOK_STATE_PATH": context.root.appendingPathComponent("claude-hook-sessions.json").path,
            "COTERM_CLI_SENTRY_DISABLED": "1",
            "COTERM_CLAUDE_HOOK_SENTRY_DISABLED": "1",
        ]
    ...
    func testClaudeForkSessionStartWithoutSurfaceIdentityDoesNotRegisterPIDOnFallbackPane() throws {
        ...
        environment["COTERM_SURFACE_ID"] = ""
        environment["COTERM_CLAUDE_PID"] = "12345"
        let result = runClaudeHookListingSurfaces(
            context: context,
            surfaceIds: [parentSurfaceId, context.surfaceId],
            arguments: ["hooks", "claude", "session-start"],
    ...
    func testClaudeLegacyStoreBackfillsPaneBoundaryFromWorkspaceActiveSlot() throws {
        ...
        let paneBPrompt = runClaudeHookWithoutServer(
            ...
            extraEnvironment: ["COTERM_SURFACE_ID": paneB]
        )
        ...
        let lateStop = runClaudeHookWithoutServer(
            ...
            extraEnvironment: ["COTERM_SURFACE_ID": paneA]
        )
    ...
    func testClaudeForkedSessionPromptSubmitRecordsWhileParentTurnActive() throws {
        ...
        XCTAssertEqual(
            ...
            true,
            "The forked session must become restorable so a coterm restart resumes the fork, not the parent"
        )
    ...
    func testClaudeForkedSessionPromptSubmitRecordsWithSurfaceRefForm() throws {
        ...
        let result = runClaudeHookListingSurfaces(
            ...
            arguments: ["hooks", "claude", "prompt-submit"],
            standardInput: #"{"session_id":"\#(forkedSessionId)","turn_id":"fork-turn-1","cwd":"\#(context.root.path)","hook_event_name":"UserPromptSubmit","prompt":"diverge here"}"#,
            extraEnvironment: ["COTERM_SURFACE_ID": "surface:2"]
        )
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
    ...
    func testClaudeStaleStopFromClosedPaneStaysStaleWhenSurfaceResolutionFallsBack() throws {
        ...
        let result = runClaudeHookListingSurfaces(
            ...
            extraEnvironment: ["COTERM_SURFACE_ID": closedSurfaceId]
        )
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
    ...
    func testClaudeStaleStopStaysStaleAfterAnotherPaneBecomesWorkspaceActive() throws {
        ...
        func runHook(_ subcommand: String, stdin: String, surface: String) -> ProcessRunResult {
            runClaudeHookWithoutServer(
                ...
                standardInput: stdin,
                extraEnvironment: ["COTERM_SURFACE_ID": surface]
            )
        }
    ...
    func testClaudeParentPaneStopAppliesAfterForkedSessionPromoted() throws {
        ...
        let result = runClaudeHookListingSurfaces(
            ...
            extraEnvironment: ["COTERM_SURFACE_ID": parentSurfaceId]
        )
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
    ...
    func testClaudePromptSubmitResumeBindingPersistsSafeAuthSelectionValues() throws {
        ...
        let launchEnvironment = [
            "COTERM_AGENT_LAUNCH_KIND": "claude",
            "COTERM_AGENT_LAUNCH_EXECUTABLE": "/usr/local/bin/claude",
            "COTERM_AGENT_LAUNCH_CWD": context.root.path,
            "COTERM_AGENT_LAUNCH_ARGV_B64": base64NULSeparated([
        ...
        XCTAssertEqual(environment["COTERM_PRESERVE_CLAUDE_AUTH_SELECTION_ENV"] as? String, "1")
        XCTAssertEqual(
            environment["COTERM_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS"] as? String,
    ...
    func testManagedCodexSubagentStopDoesNotReplaceResumeBinding() throws {
        ...
        let result = runCodexHook(
            ...
            extraEnvironment: codexLaunchEnvironment(context: context, sessionId: sessionId).merging([
                "COTERM_AGENT_MANAGED_SUBAGENT": "1",
                "COTERM_CODEX_TEAMS_THREAD_ID": "child-thread",
                "COTERM_CODEX_TEAMS_PARENT_THREAD_ID": "root-thread",
                "COTERM_CODEX_TEAMS_DEPTH": "1",
    ...
    func testRightSidebarInvalidCommandValidatesBeforeTargetResolution() throws {
        let cliPath = try bundledCLIPath()
        let missingSocketPath = "/tmp/coterm-test-missing-\(UUID().uuidString).sock"
        var environment = ProcessInfo.processInfo.environment
        environment["COTERM_SOCKET_PATH"] = missingSocketPath
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
    ...
    func testSSHForwardAgentRepeatedOptionUsesLastValue() throws {
        let run = try runMockedSSH(
            ...
            environmentOverrides: [
                "SSH_AUTH_SOCK": "/tmp/coterm-test-agent-\(UUID().uuidString).sock",
            ]
        )
        let createParams = try XCTUnwrap(params(for: "workspace.create", in: run.requests))
        let configureParams = try XCTUnwrap(params(for: "workspace.remote.configure", in: run.requests))
    ...
    func testSSHForwardAgentTildeSocketPathExpandsSocketPath() throws {
        let homeURL = try makeTemporaryDirectory(prefix: "coterm-ssh-home")
        let tildeSocketPath = "~/.ssh/coterm-test-agent.sock"
        let expandedSocketURL = homeURL.appendingPathComponent(".ssh/coterm-test-agent.sock")
    ...
    private func assertSSHPersistentPTYUsesReusableForegroundAuthControlConnection(
        ...
        XCTAssertTrue(initialScript.contains("COTERM_WORKSPACE_ID"), initialScript)
        XCTAssertTrue(initialScript.contains("COTERM_SURFACE_ID"), initialScript)
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            initialScript.contains("ssh-$coterm_ssh_pty_workspace_id-$coterm_ssh_pty_surface_id"),
            initialScript
        )
        XCTAssertTrue(initialScript.contains("254|255"), initialScript)
        assertSSHPTYAttachOmitsSurfaceArgument(initialScript)
        XCTAssertTrue(
            initialScript.contains("--workspace \"$coterm_ssh_pty_workspace_id\""),
            initialScript
        )
        XCTAssertEqual(
        ...
        XCTAssertTrue(terminalStartupScript.contains("COTERM_WORKSPACE_ID"), terminalStartupScript)
        XCTAssertTrue(terminalStartupScript.contains("COTERM_SURFACE_ID"), terminalStartupScript)
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            terminalStartupScript.contains("ssh-$coterm_ssh_pty_workspace_id-$coterm_ssh_pty_surface_id"),
            terminalStartupScript
        )
        ...
        XCTAssertTrue(
            terminalStartupScript.contains("--workspace \"$coterm_ssh_pty_workspace_id\""),
            terminalStartupScript
        )
        XCTAssertEqual(
    ...
    func testSSHPersistentPTYFallsBackWhenForegroundAuthCannotBeReused() throws {
        let cases: [(name: String, arguments: [String])] = [
            ...
            ("local-command", ["--ssh-option", "LocalCommand=echo coterm-test"]),
    ...
    func testSSHPTYAttachWithoutSurfaceDoesNotSendLocalAttachEnd() throws {
        ...
        environment["COTERM_SOCKET_PATH"] = socketPath
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment.removeValue(forKey: "COTERM_SURFACE_ID")
    ...
    func testSSHPTYAttachWaitUsesCurrentTerminalSizeForBridgeHandshake() throws {
        ...
        guard openpty(&masterFD, &slaveFD, nil, nil, nil) == 0 else {
            throw NSError(domain: "coterm.tests", code: Int(errno), userInfo: [
    ...
    func testSSHSessionAttachCreatesSurfaceWithPersistedPTYSessionID() throws {
        ...
        let serverHandled = startMockServer(listenerFD: listenerFD, state: state) { line in
            ...
            XCTAssertTrue(initialCommand.contains("COTERM_WORKSPACE_ID"), initialCommand)
            XCTAssertTrue(initialCommand.contains("COTERM_SURFACE_ID"), initialCommand)
    ...
    func testNotifyWithUUIDSurfaceDoesNotRequireCallerWorkspaceOrWindow() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["COTERM_SOCKET_PATH"] = socketPath
        environment["COTERM_WORKSPACE_ID"] = callerWorkspace
        environment["COTERM_SURFACE_ID"] = callerSurface
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
    ...
    func testCodexPromptSubmitRebindsRestoredSessionToCurrentCallerSurface() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-rebind-\(UUID().uuidString)", isDirectory: true)
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["COTERM_SOCKET_PATH"] = socketPath
        environment["COTERM_WORKSPACE_ID"] = currentWorkspaceId
        environment["COTERM_SURFACE_ID"] = currentSurfaceId
    ...
    func testCodexPromptSubmitWithForeignCotermEnvDoesNotFallbackToSelectedWorkspace() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("codex-foreign-env")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-foreign-env-\(UUID().uuidString)", isDirectory: true)
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["COTERM_SOCKET_PATH"] = socketPath
        environment["COTERM_WORKSPACE_ID"] = foreignWorkspaceId
        environment["COTERM_SURFACE_ID"] = foreignSurfaceId
        environment["COTERM_AGENT_HOOK_STATE_DIR"] = root.path
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment["CODEX_HOME"] = root.appendingPathComponent("codex-home", isDirectory: true).path
        ...
        XCTAssertFalse(
            ...
            "Foreign coterm env must not mutate the selected workspace, saw \(state.commands)"
        )
    }

    func testCodexPromptSubmitWithForeignCotermEnvIgnoresStaleMappedSession() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("codex-foreign-mapped")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-foreign-mapped-\(UUID().uuidString)", isDirectory: true)
        let foreignWorkspaceId = "11111111-1111-1111-1111-111111111111"
        let foreignSurfaceId = "22222222-2222-2222-2222-222222222222"
        let selectedWorkspaceId = "33333333-3333-3333-3333-333333333333"
        let selectedSurfaceId = "44444444-4444-4444-4444-444444444444"
        ...
        XCTAssertFalse(
            state.commands.contains { $0.contains("set_status codex Running") || $0.contains("notify_target_async") },
            "Foreign coterm env must not reuse stale mapped sessions, saw \(state.commands)"
        )
    }
    ...
    func testCodexPromptSubmitWithInvalidSurfaceDoesNotFallbackToFocusedSurface() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-invalid-surface-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
    ...
    func testCodexPromptSubmitWithInvalidMappedWorkspaceDoesNotFallbackToSelectedWorkspace() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-invalid-mapped-\(UUID().uuidString)", isDirectory: true)
        let staleWorkspaceId = "11111111-1111-1111-1111-111111111111"
        let staleSurfaceId = "22222222-2222-2222-2222-222222222222"
        let selectedWorkspaceId = "33333333-3333-3333-3333-333333333333"
        ...
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment["CODEX_HOME"] = root.appendingPathComponent("codex-home", isDirectory: true).path
        environment.removeValue(forKey: "COTERM_WORKSPACE_ID")
        environment.removeValue(forKey: "COTERM_SURFACE_ID")
    ...
    func testCodexTeamsForkPromptPublishesResumeBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-codex-teams-resume-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
        ...
        environment["COTERM_AGENT_HOOK_STATE_DIR"] = root.path
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment["COTERM_AGENT_LAUNCH_KIND"] = "codexTeams"
        environment["COTERM_AGENT_LAUNCH_EXECUTABLE"] = "/usr/local/bin/coterm"
        ...
        XCTAssertEqual(
            ...
            "cd -- '\(root.path)' 2>/dev/null || [ ! -d '\(root.path)' ] && '/usr/local/bin/coterm' 'codex-teams' 'resume' '\(sessionId)' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testAgentPromptClearsSurfaceResumeBindingWhenResumeCommandUnavailable() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-resume-unavailable-\(UUID().uuidString)", isDirectory: true)
            ...
            "sessions": [
                sessionId: [
                    ...
                    "launchCommand": [
                        "launcher": "omx",
                        "executablePath": "/usr/local/bin/coterm",
                        "arguments": ["/usr/local/bin/coterm", "omx", "hud"],
                        "workingDirectory": root.path,
                        "capturedAt": now,
                        "source": "test",
                    ],
    ...
    func testGenericAgentSessionEndClearsMatchingSurfaceResumeBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-agent-resume-clear-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer {
            Darwin.close(listenerFD)
            unlink(socketPath)
    ...
    func testSurfaceResumeSetCLIDoesNotScopeExplicitSurfaceToEnvWorkspace() throws {
        ...
        environment["COTERM_SOCKET_PATH"] = socketPath
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment["COTERM_WORKSPACE_ID"] = staleWorkspaceId
        environment["COTERM_SURFACE_ID"] = "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"
    ...
    private func agentLaunchEnvironment(
        ...
        [
            "COTERM_AGENT_LAUNCH_KIND": kind,
            "COTERM_AGENT_LAUNCH_EXECUTABLE": executable,
            "COTERM_AGENT_LAUNCH_CWD": context.root.path,
            "COTERM_AGENT_LAUNCH_ARGV_B64": base64NULSeparated(arguments ?? [executable]),
        ]
    }
    ...
    private func runAgentHook(
        ...
        var environment = [
            ...
            "COTERM_WORKSPACE_ID": context.workspaceId,
            "COTERM_SURFACE_ID": context.surfaceId,
            "COTERM_AGENT_HOOK_STATE_DIR": context.root.path,
            "COTERM_CLI_SENTRY_DISABLED": "1",
        ]
    ...
    private func makeClaudeHookContext(name: String) throws -> ClaudeHookContext {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-\(name)-\(UUID().uuidString)", isDirectory: true)
    ...
    func testBrowserImportUsesInteractiveDialogOutsideCodingAgent() throws {
        ...
        environment["COTERM_SOCKET_PATH"] = socketPath
        environment["COTERM_CLI_SENTRY_DISABLED"] = "1"
        environment.removeValue(forKey: "COTERM_AGENT_LAUNCH_KIND")
    ...
    private func makeExistingAgentSocketPath() throws -> String {
        let directory = try makeTemporaryDirectory(prefix: "coterm-agent")
...
extension CLINotifyProcessIntegrationRegressionTests {
    ...
    // stamped the focused surface (A), desyncing COTERM_SURFACE_ID from COTERM_PANEL_ID and jumbling codex
    // into the wrong surface on reload.
    func testTmuxCompatEnvStampsLaunchSurfaceNotFocusedPane() throws {
        let cliPath = try bundledCLIPath()
        let tmpDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-spawn-id-e2e-\(UUID().uuidString)", isDirectory: true)
        ...
        let result = runProcess(
            ...
            environment: [
                "COTERM_SOCKET_PATH": socketPath,
                "COTERM_WORKSPACE_ID": launchWorkspace,
                "COTERM_SURFACE_ID": launchSurface,
                "COTERM_PANEL_ID": launchSurface,
                "COTERM_TAB_ID": launchTab,
                "HOME": tmpDir.path,
        ...
        XCTAssertTrue(
            result.stdout.contains("COTERM_SURFACE_ID=\(launchSurface)"),
            "launcher must stamp the LAUNCH surface; stdout:\n\(result.stdout)\nstderr:\n\(result.stderr)"
        )
        XCTAssertFalse(
            result.stdout.contains("COTERM_SURFACE_ID=\(focusedSurface)"),
            "launcher must NOT stamp the focused surface; stdout:\n\(result.stdout)"
        )
        XCTAssertTrue(result.stdout.contains("COTERM_WORKSPACE_ID=\(launchWorkspace)"), result.stdout)
        // Matched-pair invariant: SURFACE == PANEL (the desync is exactly the bug). The surface-scoped
        // tab id passes through untouched.
        XCTAssertTrue(result.stdout.contains("COTERM_PANEL_ID=\(launchSurface)"), result.stdout)
        XCTAssertTrue(result.stdout.contains("COTERM_TAB_ID=\(launchTab)"), result.stdout)
    }
}
