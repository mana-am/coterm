...
private final class CotermCLISentryTelemetryBundleToken {}

@Suite struct CotermCLISentryTelemetryRegressionTests {
    ...
    @Test func staleSocketConnectRefusalDoesNotCaptureSentryTelemetry() throws {
        ...
        let root = URL(
            fileURLWithPath: "/tmp/coterm-sr-\(UUID().uuidString.prefix(8))",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let socketPath = root.appendingPathComponent("coterm.sock", isDirectory: false).path
    ...
    @Test func missingSocketDoesNotCaptureSentryTelemetry() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-cli-sentry-missing-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    @Test func unexpectedSocketTelemetryStoresWithoutBlockingForSentryFlush() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-cli-sentry-flush-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        environment["COTERM_CLI_SENTRY_STORE_PROBE_PATH"] = storeProbePath
    ...
    private func bundledCLIPath() throws -> String {
        try BundledCLITestSupport.bundledCLIPath(for: CotermCLISentryTelemetryBundleToken.self)
    }
    ...
    private func sentryProbeEnvironment(socketPath: String, probePath: String) -> [String: String] {
        ...
        environment["COTERM_CLI_SENTRY_CAPTURE_PROBE_PATH"] = probePath
