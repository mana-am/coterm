...
final class SessionIndexViewTests: XCTestCase {
    ...
    func testClaudeResumeCommandPinsSnapshotConfigDirectory() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-session-index-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertEqual(
            entry.resumeCommand,
            posixShWrappedForTest("env CLAUDE_CONFIG_DIR='\(configDir.path)' COTERM_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1 COTERM_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR \"$([ -x \"${COTERM_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$COTERM_CLAUDE_WRAPPER_SHIM\" || printf claude)\" --resume claude-session-123")
        )
    }
    ...
    // Regression for https://github.com/emergent-inc/coterm/issues/5262.
    // coterm captures Codex's *internal* sandbox-policy `type`, which is a superset of
    ...
    func testCodexResumeCommandReproducesBypassFlagForDisabledSandbox() {
        ...
        // The codex resume now routes the codex executable through the coterm codex
    ...
    func testCodexSQLSearchMatchesRolloutTranscriptContent() async throws {
        ...
        {"timestamp":"2026-05-01T09:01:00.000Z","type":"response_item","payload":{"type":"function_call","name":"exec_command","arguments":"{\\"cmd\\":\\"tail -n 80 /tmp/coterm-debug-task-activation-performance-harness.log\\"}","call_id":"call_1"}}
        """
        ...
        let outcome = await SessionIndexStore.loadCodexEntriesForTesting(
            ...
            needle: "/tmp/coterm-debug-task-activation-performance-harness.log",
            offset: 0,
            limit: 10,
    ...
    // Regression for https://github.com/emergent-inc/coterm/issues/6302.
    ...
    /// (fish/csh/tcsh). https://github.com/emergent-inc/coterm/issues/5639
