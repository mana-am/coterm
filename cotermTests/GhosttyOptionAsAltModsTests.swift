...
/// Regression coverage for https://github.com/emergent-inc/coterm/issues/5993:
/// coterm ignored `macos-option-as-alt` left/right and captured Option before
...
/// coterm sends. If coterm maps both physical Option keys to the same generic
...
@Suite struct GhosttyOptionAsAltModsTests {
    ...
    @Test func rightOptionCarriesAltAndAltRightSideBit() {
        ...
        let mods = cotermGhosttyModsFromFlags(modifierFlagsRawValue: raw)
    ...
    @Test func mouseModsNeverCarrySideBits() {
        ...
        let mods = cotermGhosttyMouseModsFromFlags(modifierFlagsRawValue: raw)
        #expect(mods.rawValue & GHOSTTY_MODS_ALT.rawValue != 0)
        #expect(mods.rawValue & GHOSTTY_MODS_SHIFT.rawValue != 0)
        #expect(mods.rawValue & GHOSTTY_MODS_ALT_RIGHT.rawValue == 0)
        #expect(mods.rawValue & GHOSTTY_MODS_SHIFT_RIGHT.rawValue == 0)
    ...
    @Test func translationFlagsDropOptionWhenGhosttyStripsAlt() {
        ...
        let translated = cotermTranslationModifierFlags(
            original: [.option],
