...
private final class IndexLockObserver: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.coterm.tests.index-lock-observer", qos: .utility)
    private let lock = NSLock()
...
private final class LockTouchingGitRunner: CommandRunning, @unchecked Sendable {
    ...
    func run(directory: String, executable: String, arguments: [String], timeout: TimeInterval?) async -> CommandResult {
        ...
        if arguments == ["remote", "-v"] {
            return CommandResult(
                stdout: "origin\thttps://github.com/emergent-inc/coterm.git (fetch)\n",
                stderr: "",
                exitStatus: 0,
                timedOut: false,
                executionError: nil
...
final class WorkspacePullRequestSidebarTests: XCTestCase {
    func testSidebarPullRequestsIgnoreStaleWorkspaceLevelCacheWithoutPanelState() throws {
        ...
        let panelId = UUID()
        let staleURL = try XCTUnwrap(URL(string: "https://github.com/emergent-inc/coterm/pull/1640"))
    ...
    func testSidebarPullRequestsPreferBestStateAcrossPanels() throws {
        ...
        let url = try XCTUnwrap(URL(string: "https://github.com/emergent-inc/coterm/pull/1640"))
    ...
    func testPullRequestRefreshRepositoryDiscoveryDoesNotBlockMainRunLoop() throws {
        ...
        for (index, workspace) in workspaces.enumerated() {
            ...
            workspace.updatePanelDirectory(
                panelId: panelId,
                directory: "/tmp/coterm-pr-refresh-main-thread-\(index)"
            )
    ...
    func testNoIndexLockTouchDuringSidebarGitMetadataRefreshWindow() throws {
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-index-lock-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testDisablingGitWatchClearsCachedPullRequestBadgesWhenPullRequestsAreShownByDefault() throws {
        ...
        let workspace = try XCTUnwrap(manager.selectedWorkspace)
        let panelId = try XCTUnwrap(workspace.focusedPanelId)
        let url = try XCTUnwrap(URL(string: "https://github.com/emergent-inc/coterm/pull/2722"))
        ...
        let response = TerminalController.shared.handleSocketLine(
            "report_pr 2722 https://github.com/emergent-inc/coterm/pull/2722 --label=PR --state=open --branch=issue-2722-git-index-lock-poll --tab=\(workspace.id.uuidString) --panel=\(panelId.uuidString)"
        )
        XCTAssertEqual(response, "OK")
    ...
    func testDisablingPullRequestSidebarClearsCachedPullRequestsWithoutClearingBranches() throws {
        ...
        let workspace = try XCTUnwrap(manager.selectedWorkspace)
        let panelId = try XCTUnwrap(workspace.focusedPanelId)
        let url = try XCTUnwrap(URL(string: "https://github.com/emergent-inc/coterm/pull/2746"))
    ...
    func testReenablingGitWatchRestartsRefreshFromCurrentPanelDirectories() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-reenable-git-watch-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: repoURL, withIntermediateDirectories: true)
        try writeMinimalGitRepository(at: repoURL)
    ...
    func testDetachedHeadRepositoryKeepsGitMetadataWatcherForLaterCheckout() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-detached-head-watch-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: repoURL, withIntermediateDirectories: true)
        try writeMinimalGitRepository(at: repoURL)
    ...
    // metadata behavior is covered by CotermGit/GitMetadataServiceTests; restoring
    ...
    func testUnrelatedDefaultsChangeDoesNotRestartGitMetadataRefreshes() throws {
        let defaults = UserDefaults.standard
        let unrelatedDefaultsKey = "coterm.tests.unrelated-defaults-\(UUID().uuidString)"
        let previousWatchGitStatus = defaults.object(forKey: SidebarWorkspaceDetailDefaults.watchGitStatusKey)
        defer {
        ...
        let workingDirectoryURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-unrelated-defaults-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testGitIndexVersionFourRefreshTracksIndexSignatureChanges() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-index-v4-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: repoURL, withIntermediateDirectories: true)
        try "seed\n".write(
    ...
    func testCleanIndexSignatureRebaselinesWhenIndexRewriteKeepsTrackedContentClean() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-stash-clean-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: repoURL, withIntermediateDirectories: true)
        try writeMinimalGitRepository(at: repoURL)
    ...
    func testIndexContentChangeAfterWorktreeDirtyRemainsDirty() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-staged-after-dirty-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: repoURL, withIntermediateDirectories: true)
        try writeMinimalGitRepository(at: repoURL)
    ...
    func testAssumeUnchangedGitIndexEntriesDoNotMarkModifiedWorktreeDirty() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-assume-unchanged-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: repoURL, withIntermediateDirectories: true)
        try writeMinimalGitRepository(at: repoURL)
    ...
    func testGitIndexVersionFourRefreshDecodesMultiByteStripLengths() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-index-v4-varint-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: repoURL, withIntermediateDirectories: true)
        let longTrackedPath = [
    ...
    func testEmptyGitIndexRefreshTracksIndexSignatureChanges() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-empty-index-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: repoURL, withIntermediateDirectories: true)
        try writeMinimalGitRepository(at: repoURL)
    ...
    func testSkipWorktreeGitIndexEntriesDoNotMarkSparseCheckoutDirty() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-sparse-index-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: repoURL, withIntermediateDirectories: true)
        try writeMinimalGitRepository(at: repoURL)
    ...
    func testMissingGitlinkSubmoduleMarksSidebarDirty() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-gitlink-index-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testGitlinkIndexEntriesTrackSubmoduleCommitChanges() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-gitlink-commit-\(UUID().uuidString)",
            isDirectory: true
        )
        let submoduleURL = repoURL.appendingPathComponent("vendor/lib", isDirectory: true)
    ...
    // gitlinks), and remote-slug parsing now live in the CotermGit package and are
    // unit-tested there (CotermGitTests: GitMetadataServiceTests / GitConfigIncludeTests).
    // The watcher's leading-edge coalescing is verified in CotermFileWatch's package
    ...
    func testModeOnlyTrackedChangesMarkSidebarDirty() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-mode-only-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: repoURL, withIntermediateDirectories: true)
        try writeMinimalGitRepository(at: repoURL)
    ...
    func testLargeTrackedFileSizeMatchesGitIndexTruncation() throws {
        ...
        let repoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-sidebar-large-file-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: repoURL, withIntermediateDirectories: true)
        try writeMinimalGitRepository(at: repoURL)
