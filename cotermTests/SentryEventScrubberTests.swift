...
@Suite struct SentryEventScrubberTests {
    ...
    @Test func scrubsFormattedOnlyMessageExceptionAndTransaction() {
        ...
        event.message = SentryMessage(formatted: "boom for lawrence@coterm.cc")
    ...
    @Test func scrubsContextDictionaries() {
        let event = Event()
        // Mirrors scope.setContext(value:key:): coterm puts cwd / path data here.
        event.context = [
            "ui": ["cwd": "/Users/lawrence/dev/coterm", "action": "open"],
            "auth": ["token": "abcdef0123456789secretvalue"],
        ]

        let scrubbed = scrubber.scrub(event)
        #expect(scrubbed.context?["ui"]?["cwd"] as? String == "/Users/<redacted>/dev/coterm")
    ...
    @Test func scrubsStackFramePathsContextAndVarsButKeepsSymbols() {
        let event = Event()
        let frame = Frame()
        frame.fileName = "/Users/buildbot/work/coterm/Sources/AppDelegate.swift"
        frame.function = "applicationDidFinishLaunching(_:)"
        frame.package = "/Users/buildbot/Library/Developer/Xcode/DerivedData/coterm/Build/coterm.app"
        frame.lineNumber = 1325
        frame.contextLine = "let home = \"/Users/lawrence/secret\""
        frame.preContext = ["// open lawrence@coterm.cc"]
        ...
        #expect(outFrame?.fileName == "/Users/<redacted>/work/coterm/Sources/AppDelegate.swift")
    ...
    @Test func scrubsDebugMetaCodeFilePaths() {
        let event = Event()
        let image = DebugMeta()
        image.codeFile = "/Users/lawrence/Library/Developer/Xcode/DerivedData/coterm/coterm.app/coterm"
