    ...
    func testPaneCloseOnLastRemotePanelKeepsWorkspaceDisconnected() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: nil,
                localSocketPath: "/tmp/coterm-debug-test.sock",
                terminalStartupCommand: "coterm remote websocket"
            ),
            autoConnect: false
        )
    ...
    func testChildExitAfterPersistentAttachEndKeepsExitedSurfaceVisible() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayID: String(repeating: "a", count: 16),
                relayToken: String(repeating: "b", count: 64),
                localSocketPath: "/tmp/coterm-debug-attach-end-test.sock",
                terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
                preserveAfterTerminalExit: true,
    ...
    func testChildExitOnSplitPersistentRemotePanelKeepsExitedSurfaceVisibleAndClearsOnlyThatPTYState() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayID: String(repeating: "a", count: 16),
                relayToken: String(repeating: "b", count: 64),
                localSocketPath: "/tmp/coterm-debug-split-test.sock",
                terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
                preserveAfterTerminalExit: true,
...
final class TabManagerPullRequestProbeTests: XCTestCase {
    ...
    // Pure pull-request selection/policy tests moved to the CotermGit package
    // (CotermGitTests.PullRequestProbeServiceTests) with the extraction.
    ...
    func testTrackedWorkspaceGitMetadataPollCandidatesIncludeMainAndMasterPanels() throws {
        ...
        let staleURL = try XCTUnwrap(URL(string: "https://github.com/emergent-inc/coterm/pull/371"))
    ...
    func testSameDirectoryInitialGitMetadataProbesShareOneSnapshotRead() async throws {
        ...
        let directoryURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "coterm-git-coalesced-probes-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testTrackedWorkspaceGitMetadataPollCandidatesExcludeDirectoriesWithoutResolvedGitMetadata() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-git-nonrepo-candidate-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testInheritedBackgroundWorkspaceFetchesGitBranchWithoutSelection() throws {
        ...
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-git-inherited-background-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testPeriodicWorkspaceGitMetadataRefreshUpdatesMainWorkspaceAfterCheckoutToFeatureBranch() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent("coterm-git-main-refresh-\(UUID().uuidString)")
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
    ...
    func testPeriodicWorkspaceGitMetadataRefreshRestoresClearedBranchForStaleTerminal() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "coterm-git-stale-branch-refresh-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
    ...
    // CotermProcessTests.resolvesCommandViaFallbackDirectoryOutsidePath when the
    // command runner was extracted into the CotermProcess package.

    func testPeriodicWorkspaceGitMetadataRefreshClearsStalePullRequestAfterBranchReset() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent("coterm-git-refresh-\(UUID().uuidString)")
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 1052,
            label: "PR",
            url: try XCTUnwrap(URL(string: "https://github.com/emergent-inc/coterm/pull/1052")),
            status: .open,
...
final class TabManagerCloseCurrentPanelTests: XCTestCase {
    ...
    func testCloseCurrentPanelHonorsWarnBeforeClosingTabDisabledFromCotermJSON() throws {
    ...
    func testCloseCurrentPanelHonorsWarnBeforeClosingTabEnabledFromCotermJSON() throws {
        try assertCloseCurrentPanelConfirmation(
    ...
    func testTabCloseButtonWarningHonorsCotermJSON() throws {
    ...
    func testHideTabCloseButtonHonorsCotermJSON() throws {
...
final class TabManagerSurfaceCreationTests: XCTestCase {
    ...
    func testNewBrowserSurfaceCanSelectBackgroundPaneWithoutTakingFocus() {
        ...
        guard let workspace = manager.selectedWorkspace,
              ...
              let url = URL(string: "file:///tmp/coterm-diff.html") else {
            XCTFail("Expected split setup to succeed")
            return
        }
    ...
    func testDuplicateBrowserPreservesDiffViewerChromeAndProxyBypass() throws {
        ...
        let url = try XCTUnwrap(URL(string: "http://127.0.0.1:49152/token/diff.html#coterm-diff-viewer"))
...
final class TabManagerEqualizeSplitsTests: XCTestCase {
    func testEqualizeSplitsKeepsMultiTabPaneAndBrowserAtHalfWidth() {
        ...
        workspace.applyCustomLayout(
            .split(CotermSplitDefinition(
                direction: .horizontal,
                split: 0.2,
                children: [
                    .pane(CotermPaneDefinition(surfaces: [
                        CotermSurfaceDefinition(type: .terminal, name: "Terminal A"),
                        CotermSurfaceDefinition(type: .terminal, name: "Terminal B")
                    ])),
                    .pane(CotermPaneDefinition(surfaces: [
                        CotermSurfaceDefinition(type: .browser, name: "Browser", url: "https://example.com")
                    ]))
                ]
            )),
    ...
    func testEqualizeSplitsBalancesThreeSameAxisSiblingPanesIntoThirds() {
        ...
        workspace.applyCustomLayout(
            .split(CotermSplitDefinition(
                ...
                children: [
                    .pane(CotermPaneDefinition(surfaces: [
                        CotermSurfaceDefinition(type: .terminal, name: "Left")
                    ])),
                    .split(CotermSplitDefinition(
                        direction: .horizontal,
                        split: 0.8,
                        children: [
                            .pane(CotermPaneDefinition(surfaces: [
                                CotermSurfaceDefinition(type: .terminal, name: "Middle")
                            ])),
                            .pane(CotermPaneDefinition(surfaces: [
                                CotermSurfaceDefinition(type: .browser, name: "Right", url: "https://example.com")
                            ]))
                        ]
                    ))
                ]
            ...
            .split(CotermSplitDefinition(
                ...
                children: [
                    .split(CotermSplitDefinition(
                        ...
                        children: [
                            .pane(CotermPaneDefinition(surfaces: [
                                CotermSurfaceDefinition(type: .terminal, name: "Top Terminal")
                            ])),
                            .pane(CotermPaneDefinition(surfaces: [
                                CotermSurfaceDefinition(type: .terminal, name: "Bottom Terminal")
                            ]))
                        ]
                    )),
                    .pane(CotermPaneDefinition(surfaces: [
                    ...
                    .split(CotermSplitDefinition(
                        ...
                        children: [
                            .split(CotermSplitDefinition(
                                ...
                                children: [
                                    .pane(CotermPaneDefinition(surfaces: [
                                        CotermSurfaceDefinition(type: .terminal, name: "Top Left")
                                    ])),
                                    .pane(CotermPaneDefinition(surfaces: [
                                        CotermSurfaceDefinition(type: .terminal, name: "Top Right")
                                    ]))
                                ]
                            )),
                            .pane(CotermPaneDefinition(surfaces: [
                                CotermSurfaceDefinition(type: .terminal, name: "Bottom")
                            ]))
                        ]
                    )),
                    .pane(CotermPaneDefinition(surfaces: [
...
final class TabManagerReopenClosedBrowserFocusTests: XCTestCase {
    ...
    func testTemporaryDiffViewerTabCloseDoesNotStageRestoreSnapshot() throws {
        let workspace = Workspace()
        let diffViewerURL = try XCTUnwrap(URL(string: "http://127.0.0.1:49152/token/diff.html#coterm-diff-viewer"))
        guard let paneId = workspace.bonsplitController.focusedPaneId,
