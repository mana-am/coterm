...
final class CommandPaletteSearchEngineTests: XCTestCase {
    ...
    private func makeCommandEntries(count: Int) -> [FixtureEntry] {
        (0..<count).map { index in
            ...
            case 5:
                ...
                subtitle = "Global"
                keywords = ["restart", "cli", "listener", "socket", "coterm"]
            case 6:
        ...
        (0..<count).map { index in
            ...
            let keywords = CommandPaletteSwitcherSearchIndexer(
                ...
                metadata: CommandPaletteSwitcherSearchMetadata(
                    directories: ["/Users/example/dev/coterm-hq/worktrees/feature-\(index)-rename-tab"],
    ...
    private func makeLargeWorkspaceSwitcherEntries(count: Int) -> [FixtureEntry] {
        (0..<count).map { index in
            ...
            let keywords = CommandPaletteSwitcherSearchIndexer(
                ...
                metadata: CommandPaletteSwitcherSearchMetadata(
                    directories: [
                        "/Users/example/dev/coterm-hq/worktrees/\(worktreeSlug)",
                        "/Users/example/dev/coterm-hq/worktrees/\(worktreeSlug)/repo",
                    ],
                    branches: [
    ...
    func testForkableAgentFallbackSnapshotRequiresVerifiedProbeForVisibility() {
        ...
        let omoOpenCode = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                arguments: ["/usr/local/bin/coterm", "omo"],
                workingDirectory: "/tmp/opencode repo",
                environment: nil,
                capturedAt: 123,
                source: "environment"
    ...
    func testCustomSnapshotWithForkTemplateIsForkable() {
        ...
        let customRegistration = CotermVaultAgentRegistration(
            id: "my-agent",
            name: "My Agent",
            detect: CotermVaultAgentDetectRule(processNames: ["my-agent"]),
    ...
    func testForkableAgentSnapshotFingerprintChangesWithForkCommand() {
        ...
        var second = first
        second.registration = CotermVaultAgentRegistration(
            id: "fork-fingerprint",
            name: "Fork Fingerprint",
            detect: CotermVaultAgentDetectRule(processName: "fork-fingerprint"),
            sessionIdSource: .argvOption("--session"),
                ...
                CommandPaletteSwitcherFingerprintContext(
                    ...
                    workspaces: [
                        CommandPaletteSwitcherFingerprintWorkspace(
                            ...
                            metadata: CommandPaletteSwitcherSearchMetadata(
                                directories: ["/Users/example/dev/coterm"],
    ...
    func testCommandSearchBenchmarkBeatsLegacyPipeline() {
        ...
        let queries = repeatedQueries(
            ["rename", "rename tab", "open dir", "toggle side", "apply update", "notif", "split right", "coterm"],
            repetitions: 12
        )
