...
final class GhosttyPasteboardHelperTests: XCTestCase {
    ...
    func testHTMLOnlyPasteboardExtractsPlainText() {
        let pasteboard = NSPasteboard(name: .init("coterm-test-html-\(UUID().uuidString)"))
    ...
    func testCapturedStandardClipboardWriteDoesNotTouchGeneralPasteboard() {
        ...
        XCTAssertEqual(captured, "/tmp/coterm-screen.txt")
    ...
    func testAlternatePlainTextUTIExtractsPlainText() {
        let pasteboard = NSPasteboard(name: .init("coterm-test-plain-text-uti-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    /// Regression test for https://github.com/emergent-inc/coterm/issues/2818 —
    ...
    func testPrefersUTF8PlainTextOverLegacyMacRomanType() {
        let pasteboard = NSPasteboard(name: .init("coterm-test-utf8-priority-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    /// Regression test for https://github.com/emergent-inc/coterm/issues/3910.
    ...
    func testPrefersFaithfulRichTextWhenPlainTextReplacesChineseWithQuestionMarks() {
        let pasteboard = NSPasteboard(name: .init("coterm-test-lossy-chinese-plain-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testFallsBackWhenOnlyNonPreferredPlainTextTypePresent() {
        let pasteboard = NSPasteboard(name: .init("coterm-test-only-legacy-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testEmptyPlainTextFallsBackToRichTextPayload() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-empty-plain-rich-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString("", forType: .string)
    ...
    /// Regression test for https://github.com/emergent-inc/coterm/issues/2940.
    /// Some apps place the same large clipboard payload onto `.string`, `.html`,
    /// and `.rtf`. coterm should hand the plain text to the terminal quickly
    ...
    func testLargePlainTextPasteStaysFastWhenRichTextTypesAreAlsoPresent() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("coterm-test-large-fast-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testXHTMLTypeFallsBackToRenderedHTMLText() {
        let pasteboard = NSPasteboard(name: .init("coterm-test-xhtml-html-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString(
    ...
    func testPublicURLPastePreservesOriginalURLText() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-public-url-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImageClipboardWithPlainTextFallbackStillFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-image-plain-text-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString(
    ...
    func testImageHTMLClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-image-html-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImageHTMLClipboardWithGenericPlainTextStillFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-image-html-generic-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString("<meta charset='utf-8'><img src=\"https://example.com/keyboard.png\">", forType: .html)
        pasteboard.setString(
            "https://example.com/keyboard.png",
    ...
    func testImageHTMLClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-image-html-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testJPEGClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-jpeg-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-rtfd-attachment-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDClipboardWithPlainTextFallbackStillFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-rtfd-attachment-string-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString(
    ...
    func testAttachmentOnlyRTFDNonImageClipboardDoesNotFallBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-rtfd-non-image-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRTFDClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-rtfd-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImageOnlyPasteboardProducesTempFileURL() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-drop-image-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImageDropPlanUploadsMaterializedFile() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-remote-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLocalImageDropPlanInsertsEscapedLocalPath() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-local-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLocalImageFileURLPastePlanUsesSinglePastePayload() throws {
        let imageDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm local image paste \(UUID().uuidString)")
    ...
    func testLocalImageFileURLDropPlanUsesDelayedPasteSegments() throws {
        let imageDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm local image drop \(UUID().uuidString)")
        try FileManager.default.createDirectory(at: imageDirectory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: imageDirectory) }
    ...
    func testRemoteImagePastePlanUploadsMaterializedFile() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-remote-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteFileURLPastePlanUploadsReadableFile() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("coterm-test-remote-file-url-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteDirectoryPastePlanFallsBackToEscapedPathInsertion() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("coterm-test-remote-directory-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLazyPastePlanSkipsTargetResolutionForPlainText() {
        let pasteboard = NSPasteboard(name: .init("coterm-test-lazy-text-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testPastePlanFallsBackToAlternatePlainTextWhenImageTypeIsUnusable() {
        let pasteboard = NSPasteboard(name: .init("coterm-test-raycast-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString(
            "hello from Raycast",
            forType: NSPasteboard.PasteboardType(UTType.plainText.identifier)
    ...
    func testLazyPastePlanResolvesTargetForFileURLPaste() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("coterm-test-lazy-file-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
        XCTAssertTrue(pasteboard.writeObjects([fileURL as NSURL]))
    ...
    func testLocalImagePastePlanInsertsEscapedLocalPath() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-local-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImagePasteExecutionUploadsAndCompletesWithRemotePath() throws {
        ...
        TerminalImageTransferPlanner.executeForTesting(
            ...
            uploadWorkspaceRemote: { _, _, finish in finish(.success(["/tmp/coterm-drop-123.png"])) },
        ...
        XCTAssertEqual(completedText, "/tmp/coterm-drop-123.png")
    }
    ...
    func testCancelledRemoteImagePasteExecutionSuppressesCompletionHandlers() throws {
        ...
        completion?(.success(["/tmp/coterm-drop-cancelled.png"]))
    ...
    func testCancelledOperationSuppressesLateLocalInsert() {
        ...
        let returnedOperation = TerminalImageTransferPlanner.executeForTesting(
            plan: .insertText("/tmp/coterm-drop-local.png"),
            operation: operation,
    ...
    func testRemoteImageDropHandlerUploadsAndSendsRemotePath() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-remote-handler-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        let handled = GhosttyNSView.handleDropForTesting(
            ...
            uploadRemote: { urls, finish in
                uploadedURLs = urls
                finish(.success(["/tmp/coterm-drop-abc123.png"]))
            },
        ...
        XCTAssertEqual(sentText, ["/tmp/coterm-drop-abc123.png"])
        XCTAssertEqual(failureCount, 0)
    }

    func testRemoteImageDropHandlerCleansUpMaterializedTemporaryImageAfterSuccess() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-remote-handler-cleanup-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .orange), forType: .png)
    ...
    func testRemoteDropUploadFailureTriggersFailureHandler() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-remote-handler-fail-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImageDropHandlerCleansUpMaterializedTemporaryImageAfterFailure() throws {
        let pasteboard = NSPasteboard(name: .init("coterm-test-remote-handler-failure-cleanup-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .cyan), forType: .png)
...
final class TerminalOffscreenStartupTests: XCTestCase {
    ...
    func testColdSocketInputQueuesInsteadOfDroppingWhenRuntimeSurfaceIsMissing() {
        ...
        XCTAssertNil(panel.surface.surface)
        panel.surface.sendInput("touch /tmp/coterm-cold-send\n")
    ...
    func testSendNamedKeyRecognizesCtrlFForceStopChord() {
        ...
        // keybinding. coterm must be able to deliver that chord to the focused terminal
    ...
    func testDaemonSendWorkspaceQueuesColdControlInputInsteadOfReportingDroppedOK() throws {
        ...
        let response = TerminalController.shared.handleSocketLine(
            "send_workspace \(workspace.id.uuidString) touch /tmp/coterm-daemon-cold-send\\n"
        )
...
final class GhosttyResponderResolutionTests: XCTestCase {
    ...
    func testResolvesGhosttyViewFromDescendantResponder() {
        ...
        XCTAssertTrue(cotermOwningGhosttyView(for: descendant) === ghosttyView)
    }

    func testResolvesGhosttyViewFromGhosttyResponder() {
        let ghosttyView = GhosttyNSView(frame: NSRect(x: 0, y: 0, width: 200, height: 120))
        XCTAssertTrue(cotermOwningGhosttyView(for: ghosttyView) === ghosttyView)
    }

    func testReturnsNilForUnrelatedResponder() {
        let view = FocusProbeView(frame: NSRect(x: 0, y: 0, width: 40, height: 40))
        XCTAssertNil(cotermOwningGhosttyView(for: view))
    }
    ...
    func testDoesNotReadTextViewDelegateForGhosttyResponderResolution() {
        ...
        XCTAssertNil(cotermOwningGhosttyView(for: textView))
        XCTAssertEqual(
...
final class TerminalOpenURLTargetResolutionTests: XCTestCase {
    ...
    func testResolvesFileSchemeAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("file:///tmp/coterm.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/coterm.txt")
        default:
    ...
    func testResolvesAbsolutePathAsExternalFileURL() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("/tmp/coterm-path.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/coterm-path.txt")
        default:
    ...
    func testResolvesHostlessHTTPSAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("https:///tmp/coterm.txt"))
        switch target {
        case let .external(url):
            XCTAssertEqual(url.scheme, "https")
...
final class TerminalControllerSocketListenerHealthTests: XCTestCase {
    ...
    func testStartReclaimsTaggedRefusedSocketFileWithoutReusableLockMarker() throws {
        ...
        let path = "/tmp/coterm-debug-reclaim-\(UUID().uuidString.lowercased()).sock"
        let listenerFD = try bindUnixSocket(at: path)
        Darwin.close(listenerFD)
        defer {
            unlink(path)
    ...
    func testReservedStartupSocketPathFeedsActivePathBeforeListenerStarts() {
        ...
        let reservedPath = "/tmp/coterm-reserved-startup-\(UUID().uuidString).sock"
        defer {
            unlink(reservedPath)
        ...
        XCTAssertEqual(
            TerminalController.shared.activeSocketPath(preferredPath: "/tmp/coterm-preferred.sock"),
            reservedPath
        )
        ...
        TerminalController.shared.start(
            tabManager: TabManager(),
            socketPath: TerminalController.shared.activeSocketPath(preferredPath: "/tmp/coterm-preferred.sock"),
            accessMode: .allowAll
        )
    ...
    private func makeTempSocketPath() -> String {
        "/tmp/coterm-socket-health-\(UUID().uuidString).sock"
    }
