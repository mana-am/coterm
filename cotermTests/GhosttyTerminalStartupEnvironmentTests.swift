...
struct GhosttyTerminalStartupEnvironmentTests {
    @MainActor
    @Test
    func testTerminalSurfaceStartupEnvironmentIncludesCotermContextValues() throws {
        ...
        let expectedContextValues = [
            ...
            "COTERM_SURFACE_ID": surface.id.uuidString,
            "COTERM_TAB_ID": workspaceId.uuidString,
            "COTERM_PANEL_ID": surface.id.uuidString,
        ]
        ...
        let socketPath = try #require(
            surface.startupEnvironmentValue("COTERM_SOCKET_PATH"),
            "COTERM_SOCKET_PATH should be present"
        )
    ...
    func testApplyManagedGitWatchEnvironmentDisablesShellGitWatch() {
        ...
        expectEqual(environment["COTERM_NO_GIT_WATCH"], "1")
        expectTrue(protectedKeys.contains("COTERM_NO_GIT_WATCH"))
    }
    ...
    func testApplyManagedGitWatchEnvironmentClearsInheritedOptOutWhenEnabled() {
        ...
        expectEqual(merged["COTERM_NO_GIT_WATCH"], "")
    }
    ...
    func testApplyManagedGitWatchEnvironmentDisablesShellPullRequestWatchWhenHidden() {
        ...
        expectEqual(environment["COTERM_NO_GIT_WATCH"], "")
        expectEqual(environment["COTERM_NO_PR_WATCH"], "1")
        expectTrue(protectedKeys.contains("COTERM_NO_GIT_WATCH"))
        expectTrue(protectedKeys.contains("COTERM_NO_PR_WATCH"))
    }
    ...
    func testInstallClaudeCommandShimCreatesExecutableOutsideBundleBin() throws {
        ...
        let bundleBin =
            root
            .appendingPathComponent("coterm.app", isDirectory: true)
        ...
        let wrapperURL = bundleBin.appendingPathComponent("coterm-claude-wrapper", isDirectory: false)
        try """
        #!/usr/bin/env bash
        set -euo pipefail
        {
            printf 'shim=%s\\n' "${COTERM_CLAUDE_WRAPPER_SHIM:-}"
            printf 'root=%s\\n' "${COTERM_CLAUDE_WRAPPER_SHIM_ROOT:-}"
            printf 'args=%s\\n' "$*"
        } > "$COTERM_TEST_LOG"
    ...
    func testClaudeCommandShimFallsBackToCurrentBundledWrapperWhenEmbeddedWrapperWasReaped() throws {
        ...
        let staleWrapperURL = oldBundleBin.appendingPathComponent("coterm-claude-wrapper", isDirectory: false)
        try writeExecutable(staleWrapperURL, """
        #!/usr/bin/env bash
        printf 'stale %s\\n' "$*" > "$COTERM_TEST_LOG"
        """)

        let currentWrapperURL = currentBundleBin.appendingPathComponent("coterm-claude-wrapper", isDirectory: false)
        try writeExecutable(currentWrapperURL, """
        #!/usr/bin/env bash
        printf 'current %s\\n' "$*" > "$COTERM_TEST_LOG"
        """)
        let currentCLIURL = currentBundleBin.appendingPathComponent("coterm", isDirectory: false)
        ...
        process.environment = [
            "PATH": "/usr/bin:/bin",
            "COTERM_BUNDLED_CLI_PATH": currentCLIURL.path,
            "COTERM_TEST_LOG": logURL.path,
        ]
        try process.run()
        process.waitUntilExit()
    ...
    @Test
    func testClaudeCommandShimFallbackSkipsInheritedCotermShimRoots() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "GhosttyTerminalStartupEnvironmentTests-\(UUID().uuidString)", isDirectory: true)
        let bundleBin =
    ...
    @Test
    func testMergedStartupEnvironmentAllowsSessionReplayAndInitialEnvCotermKeys() {
        let replayPath = "/tmp/coterm-replay-\(UUID().uuidString)"
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: [
                "PATH": "/usr/bin",
                "COTERM_SURFACE_ID": "managed-surface",
            ],
            protectedKeys: ["PATH", "COTERM_SURFACE_ID"],
            additionalEnvironment: [
                SessionScrollbackReplayStore.environmentKey: replayPath
            ],
            initialEnvironmentOverrides: [
                "COTERM_INITIAL_ENV_TOKEN": "token-123"
            ]
        )

        expectEqual(merged[SessionScrollbackReplayStore.environmentKey], replayPath)
        expectEqual(merged["COTERM_INITIAL_ENV_TOKEN"], "token-123")
    }
    ...
    func testMergedStartupEnvironmentProtectsManagedKeysOnly() {
        let merged = TerminalSurface.mergedStartupEnvironment(
            ...
            protectedKeys: ["PATH", "COTERM_SURFACE_ID"],
            additionalEnvironment: [
                "COTERM_SURFACE_ID": "user-surface",
                "CUSTOM_FLAG": "1",
            ],
            initialEnvironmentOverrides: [
                "PATH": "/tmp/bin",
                "COTERM_SURFACE_ID": "override-surface",
            ]
        )

        expectEqual(merged["PATH"], "/usr/bin")
        expectEqual(merged["COTERM_SURFACE_ID"], "managed-surface")
    ...
    func testMergedStartupEnvironmentDoesNotDeriveHermesCodexBaseURLForGenericTerminals() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-hermes-codex-startup-\(UUID().uuidString)", isDirectory: true)
