...
describe("cloud DB config", () => {
  test("uses a direct URL when DATABASE_URL is present", () => {
    expect(
      cloudDbConfig({
        DATABASE_URL: "postgres://coterm:coterm@localhost:15432/coterm",
      }).driver,
    ).toBe("url");
  });
  ...
  test("uses Vercel Marketplace Aurora OIDC env when requested", () => {
    const config = cloudDbConfig({
      ...
      AWS_ROLE_ARN: "arn:aws:iam::123456789012:role/vercel-coterm-staging",
      PGHOST: "coterm-staging.cluster-example.us-west-2.rds.amazonaws.com",
      PGPORT: "5432",
      PGUSER: "coterm_app",
      PGDATABASE: "coterm",
      COTERM_DB_POOL_MAX: "3",
      COTERM_DB_SSL_REJECT_UNAUTHORIZED: "true",
      COTERM_DB_SSL_CA_PEM_BASE64: Buffer.from("-----BEGIN CERTIFICATE-----\ntest\n-----END CERTIFICATE-----").toString("base64"),
    });

    expect(config).toEqual({
      driver: "aws-rds-iam",
      awsRegion: "us-west-2",
      awsRoleArn: "arn:aws:iam::123456789012:role/vercel-coterm-staging",
      host: "coterm-staging.cluster-example.us-west-2.rds.amazonaws.com",
      port: 5432,
      user: "coterm_app",
      database: "coterm",
      poolMax: 3,
  ...
  test("allows explicitly disabling RDS certificate verification only when requested", () => {
    const config = cloudDbConfig({
      COTERM_DB_SSL_REJECT_UNAUTHORIZED: "false",
      AWS_REGION: "us-west-2",
      AWS_ROLE_ARN: "arn:aws:iam::123456789012:role/vercel-coterm-staging",
      PGHOST: "coterm-staging.cluster-example.us-west-2.rds.amazonaws.com",
      PGPORT: "5432",
  ...
  test("rejects malformed base64 CA bundles", () => {
    expect(() =>
      cloudDbConfig({
        ...
        PGUSER: "coterm_app",
        PGDATABASE: "coterm",
        COTERM_DB_SSL_CA_PEM_BASE64: "not base64!!!",
      }),
    ).toThrow("COTERM_DB_SSL_CA_PEM_BASE64 must be valid base64");
  });
