...
describe("analytics route", () => {
  ...
  test("uses client anonymous id when no authenticated user is present", async () => {
    authenticatedUser = null;

    const response = await route.POST(analyticsRequest({
      batch: [
        {
          event: "mac_action_performed",
          distinct_id: "client-anon",
          properties: { action_id: "canvas.zoom_in" },
        },
      ],
    }));
    ...
    const body = JSON.parse(String(calls[0]?.[1].body));
    expect(body.batch[0].distinct_id).toBe("client-anon");
  });
