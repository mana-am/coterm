...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: "blog.cotermClaudeTeams" });
  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    keywords: [
      "coterm", "Claude Code", "agent teams", "teammate mode", "tmux",
    ...
    alternates: buildAlternates(locale, "/blog/coterm-claude-teams"),
  };
}

export default function CotermClaudeTeamsPage() {
  const t = useTranslations("blog.posts.cotermClaudeTeams");
  const tc = useTranslations("common");

  return (
    <>
      <BlogSchema postKey="cotermClaudeTeams" path="/blog/coterm-claude-teams" datePublished="2026-03-30T00:00:00Z" />
      <div className="mb-8">
        <Link
          href="/blog"
