...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: "blog.introducingCoterm" });
  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    keywords: [
      "coterm", "terminal", "macOS", "Ghostty", "libghostty",
    ...
    alternates: buildAlternates(locale, "/blog/introducing-coterm"),
  };
}

export default function IntroducingCotermPage() {
  const t = useTranslations("blog.posts.introducingCoterm");
  const tc = useTranslations("common");

  return (
    <>
      <BlogSchema postKey="introducingCoterm" path="/blog/introducing-coterm" datePublished="2026-02-12T00:00:00Z" />
      <div className="mb-8">
        <Link
          href="/blog"
