...
export async function POST(request: Request): Promise<Response> {
  ...
  // PostHog ingest directly), so an anonymous proxy is no weaker than today.
  //
  // Rate limiting is deferred for Phase A (tracked in
  // https://github.com/emergent-inc/coterm/issues/5569). The only reusable limiter in
  ...
  // anonymous IP/edge rate limit on coterm's own compute is the follow-up. The
