...
@Suite(.serialized) struct CMUXCLIErrorOutputRegressionTests {
    ...
    @Test func testThemesSetIsDisabledAndDoesNotWriteThemeOverride() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("cmux-themes-disabled-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertFalse(result.timedOut, result.stdout)
        XCTAssertNotEqual(result.status, 0, result.stdout)
        XCTAssertTrue(result.stdout.contains("Terminal theme is managed by cmux and cannot be changed."), result.stdout)
    ...
    @Test func testThemesClearIsDisabledAndDoesNotContactRunningApp() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("cmux-themes-clear-disabled-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }

        let socketPath = "/tmp/cmux-theme-clear-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(path: socketPath, response: "OK")
        defer { responder.stop() }
    ...
    @Test func testThemesListReportsCursorDarkAsManagedTheme() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("cmux-themes-list-managed-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    @Test func testBareInteractiveThemesDoesNotLaunchPicker() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("cmux-themes-picker-disabled-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    @Test func testBareInteractiveThemesListsManagedThemeWithoutSocket() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("cmux-themes-picker-list-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        XCTAssertFalse(result.timedOut, result.stdout)
        XCTAssertEqual(result.status, 0, result.stdout)
        XCTAssertTrue(result.stdout.contains("Terminal theme is managed by cmux"), result.stdout)
    }
