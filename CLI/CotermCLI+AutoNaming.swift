...
// the bundled CLI target (which drives it from agent hooks) and the coterm-unit
...
/// debounce shape proven by community prior art (emergent-inc/coterm#2043)
...
/// that would recurse into coterm hooks or the parent agent session while
...
struct AutoNamingEnvironmentPolicy: Sendable {
    /// Exact variables that mark a live agent session or coterm terminal and
    ...
    func summarizerEnvironment(from env: [String: String]) -> [String: String] {
        env.filter { key, _ in
            if key.hasPrefix("COTERM_") { return false }
