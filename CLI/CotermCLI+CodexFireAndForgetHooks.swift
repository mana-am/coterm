...
extension CotermCLI {
    ...
    /// coterm subcommand they call and the codex hook timeout (ms). Lifecycle
    ...
    /// truth for `coterm-codex-wrapper`'s injection, mirrored from the historic
    /// hand-rolled `coterm_codex_add_hook` calls in the wrapper.
    static let codexWrapperInjectionEvents: [(agentEvent: String, cotermSubcommand: String, timeoutMs: Int)] = [
    ...
    /// splice ahead of the user's args to enable + inject coterm's fire-and-forget
    ...
    /// hook returns `{}` to codex instantly and backgrounds the real coterm call.
    ...
    func emitCodexWrapperInjectArgs() throws {
        ...
        // invocations, so they are written once into a coterm-owned dir (~/.coterm/
        ...
        for event in Self.codexWrapperInjectionEvents {
            let ff = Self.codexFireAndForgetAgentHookShellCommand(
                "coterm hooks codex \(event.cotermSubcommand)", for: codexDef
            )
            let command: String
            if let scriptPath = hooksDir.flatMap({
                Self.writeCodexHookScript(subcommand: event.cotermSubcommand, body: ff, in: $0)
            }), !scriptPath.contains("'''") {
                command = scriptPath
            } else {
                command = ff
    ...
    /// The coterm-owned directory holding the generated codex hook scripts.
    /// `~/.coterm/hooks` (NOT the user's `~/.codex`), created on demand. Returns
    ...
    static func writeCodexHookScript(subcommand: String, body: String, in dir: URL) -> String? {
        ...
        let url = dir.appendingPathComponent("coterm-codex-hook-\(safeName).sh", isDirectory: false)
        let contents = "#!/bin/sh\n\(body)\n"
    ...
    static func codexFireAndForgetAgentHookShellCommand(_ command: String, for def: AgentHookDef) -> String {
        ...
        return [
            "coterm_cli=\"${COTERM_BUNDLED_CLI_PATH:-}\"",
            "if [ -z \"$coterm_cli\" ] || [ ! -x \"$coterm_cli\" ]; then coterm_cli=\"$(command -v coterm 2>/dev/null || true)\"; fi",
            "agent_pid=\"${COTERM_CODEX_PID:-${PPID:-}}\"",
            "if [ -n \"$COTERM_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && [ -n \"$coterm_cli\" ]; then payload=\"$(mktemp \"${TMPDIR:-/tmp}/coterm-codex-hook.XXXXXX\" 2>/dev/null || mktemp -t coterm-codex-hook 2>/dev/null)\" || { echo '{}'; exit 0; }; cat >\"$payload\" || true; if [ -n \"${COTERM_SOCKET_PATH:-}\" ]; then COTERM_CODEX_PID=\"$agent_pid\" nohup sh -c '\(runner)' coterm-codex-hook \"$payload\" \"$coterm_cli\" --socket \"$COTERM_SOCKET_PATH\" \(routedArguments) >/dev/null 2>&1 & else COTERM_CODEX_PID=\"$agent_pid\" nohup sh -c '\(runner)' coterm-codex-hook \"$payload\" \"$coterm_cli\" \(routedArguments) >/dev/null 2>&1 & fi; echo '{}'; else echo '{}'; fi",
        ].joined(separator: "; ")
    }
}
