...
extension CotermCLI {
    ...
    func tmuxOMXHudConfigDisablesHud(cwd: String?) -> Bool {
        ...
        if tmuxBooleanValue(environment["OMX_HUD_ENABLED"]) == false
            || tmuxBooleanValue(environment["COTERM_OMX_HUD_ENABLED"]) == false
            || tmuxBooleanValue(environment["OMX_HUD_DISABLED"]) == true
            || tmuxBooleanValue(environment["COTERM_OMX_HUD_DISABLED"]) == true {
            return true
        }
    ...
    func tmuxCommandLooksLikeOMXHud(_ commandTokens: [String]) -> Bool {
        ...
        let environment = ProcessInfo.processInfo.environment
        let launchedThroughOMXShim = environment["COTERM_OMX_COTERM_BIN"] != nil
    ...
    func tmuxDebugDiagnosticsEnabled() -> Bool {
        let environment = ProcessInfo.processInfo.environment
        return tmuxBooleanValue(environment["COTERM_DEBUG"]) == true
            || tmuxBooleanValue(environment["COTERM_TMUX_DEBUG"]) == true
    }

    func tmuxWriteDebugDiagnostic(_ message: String) {
        guard tmuxDebugDiagnosticsEnabled(),
              let data = "[coterm] \(message)\n".data(using: .utf8) else {
            return
        }
        cliWriteStderr(data)
    }
