...
extension CotermCLI {
    private static let piExtensionMarker = "coterm-pi-session-extension-marker"
    private static let piExtensionFilename = "coterm-session.ts"
    ...
    func installPiExtensionHooks(_ def: AgentHookDef) throws {
        ...
        if !existing.isEmpty, !existing.contains(Self.piExtensionMarker) {
            throw CLIError(message: String.localizedStringWithFormat(
                String(
                    localized: "cli.hooks.pi.error.notCotermExtension",
    ...
    func uninstallPiExtensionHooks(_ def: AgentHookDef) throws {
        ...
        guard fm.fileExists(atPath: extensionURL.path) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "No Pi coterm extension found at %@"
                ),
                extensionURL.path
            ))
            return
        ...
        print(String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Removed Pi coterm extension from %@"
            ),
            extensionURL.path
        ))
    }
