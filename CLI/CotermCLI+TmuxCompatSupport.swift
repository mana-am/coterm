...
extension CotermCLI {
    ...
    ///
    /// coterm hands a respawn/start command to the surface as the pane's process
    ...
    ///
    /// Teammate panes are respawned by coterm's surface layer, not by `coterm
    ...
    /// can appear in a cwd, quoted value, or other non-flag position): the `coterm
    /// claude-teams` launcher makes that decision once from its own argv and records
    /// it in `COTERM_CLAUDE_TEAMS_SANDBOXED` (see `claudeTeamsExtraEnvVars`). That
    ...
    func tmuxClaudeTeamsRespawnEnvironment() -> [(key: String, value: String)] {
        guard ProcessInfo.processInfo.environment["COTERM_CLAUDE_TEAMS_SANDBOXED"] == "1" else {
            return []
        }
    ...
    func tmuxStartupScript(commandTokens: [String], cwd: String?) -> String? {
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-tmux-command-\(UUID().uuidString.lowercased()).sh")
        var lines = [
            "#!/bin/sh",
