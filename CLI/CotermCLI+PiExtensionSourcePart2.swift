...
function sendHook(subcommand: string, ctx: ExtensionContext, extra: HookExtra = {}): boolean {
  if (process.env.COTERM_PI_HOOKS_DISABLED === "1") return true;
  if (!process.env.COTERM_SURFACE_ID) return true;
  ...
  const result = runCoterm(["hooks", "pi", subcommand], cwd, JSON.stringify(payload));
  if (!result.ok) {
    warn(ctx, "coterm hook command failed", {
      subcommand,
...
function surfaceTargetArgs(): string[] | null {
  const surfaceId = firstString(process.env.COTERM_SURFACE_ID);
  if (!surfaceId) return null;
  const args: string[] = [];
  const workspaceId = firstString(process.env.COTERM_WORKSPACE_ID);
...
function ensureResumeBinding(ctx: ExtensionContext, sessionId: string, cwd: string): void {
  if (process.env.COTERM_PI_HOOKS_DISABLED === "1") return;
  ...
  const verified = parseJSONOutput(runCoterm(["--json", "surface", "resume", "get", ...target], cwd));
...
function sendFeed(eventName: "PreToolUse" | "PostToolUse", ctx: ExtensionContext, event: unknown, extra: HookExtra = {}): void {
  ...
  try {
    const child = spawn(cotermExecutable(), ["hooks", "feed", "--source", "pi", "--event", eventName], {
...
export default function cotermPiSessionExtension(pi: ExtensionAPI) {
  ...
  pi.on("agent_end", async (event, ctx) => {
    ...
    if (notificationRouted) stopPayload.coterm_notification_routed = true;
