...
extension CotermCLI {
    private static let ompExtensionMarker = "coterm-omp-session-extension-marker"
    private static let ompExtensionFilename = "coterm-omp-session.ts"
    private static let ompExtensionSource = #"""
// coterm-omp-session-extension-marker v1
// Bridges OMP session lifecycle events into coterm's restorable session store.
// Installed by `coterm hooks omp install` or `coterm hooks setup`.
...
function hookEnvironment(cwd: string): NodeJS.ProcessEnv {
  ...
  if (!env.COTERM_AGENT_LAUNCH_ARGV_B64) {
    ...
    env.COTERM_AGENT_LAUNCH_KIND = "omp";
    env.COTERM_AGENT_LAUNCH_EXECUTABLE = argv[0] || resolveExecutable("omp");
...
function hookInvocation(subcommand: string, ctx: ExtensionContext, extra: Record<string, unknown> = {}): HookInvocation | null {
  if (process.env.COTERM_OMP_HOOKS_DISABLED === "1") return null;
  if (!process.env.COTERM_SURFACE_ID) return null;
  ...
  const coterm = process.env.COTERM_OMP_COTERM_BIN || "coterm";
  return {
    coterm,
    cwd,
...
async function sendHook(subcommand: string, ctx: ExtensionContext, extra: Record<string, unknown> = {}): Promise<void> {
  ...
  await new Promise<void>((resolve) => {
    ...
    try {
      const child = spawn(invocation.coterm, ["hooks", "omp", subcommand], {
        env: invocation.env,
...
export default function cotermOmpSessionExtension(api: ExtensionAPI) {
    ...
    func installOmpExtensionHooks(_ _: AgentHookDef) throws {
        ...
        if !existing.isEmpty, !existing.contains(Self.ompExtensionMarker) {
            throw CLIError(message: String.localizedStringWithFormat(
                String(
                    localized: "cli.hooks.omp.error.notCotermExtension",
                    defaultValue: "%@ exists and is not a coterm extension; leaving it alone"
                ),
                extensionURL.path
            ))
        }
    ...
    func uninstallOmpExtensionHooks(_ _: AgentHookDef) throws {
        ...
        guard fm.fileExists(atPath: extensionURL.path) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "No OMP coterm extension found at %@"
                ),
                extensionURL.path
            ))
            return
        ...
        guard existing.contains(Self.ompExtensionMarker) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "Refusing to remove %@: missing coterm marker"
                ),
                extensionURL.path
            ))
            return
        ...
        print(String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Removed OMP coterm extension from %@"
            ),
            extensionURL.path
        ))
    }
