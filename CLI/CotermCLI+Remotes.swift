...
// `coterm remotes` (alias `remote`): manage the team's device-registry routes so
...
extension CotermCLI {
    static let remotesUsage = """
        Usage: coterm remotes <list|add|remove> [options]

        Manage the remote Macs in your team's coterm device registry. Added remotes
        appear in your iOS app's device list and the phone can attach to them.

          coterm remotes list [--json]
              List your team's registered remotes (name, deviceId, routes, tag, last seen).

          coterm remotes add <name> --route <host:port> [--route <host:port> ...] [--tag <tag>] [--json]
          ...
          coterm remotes remove <name-or-deviceId> [--json]
              Remove a remote you registered from the device registry.

        Examples:
          coterm remotes add my-studio --route 100.64.1.2:51001
          coterm remotes add my-studio --route my-studio.tailnet.ts.net:51001 --tag stable
          coterm remotes list --json
          coterm remotes remove my-studio
        """
    ...
    func runRemotesCommand(commandArgs: [String], client: SocketClient, jsonOutput: Bool) throws {
        ...
        case "list", "ls":
            ...
            if remotes.isEmpty {
                print("No remotes. Add one: coterm remotes add <name> --route <host:port>")
                return
            }
            ...
            guard !routeValues.isEmpty else {
                throw CLIError(message: """
                    remotes add requires at least one --route host:port.
                      ...
                      coterm remotes add \(name) --route 100.64.1.2:51001
                    """)
            }
            ...
            guard let target = positionals.first, !target.isEmpty else {
                throw CLIError(message: """
                    remotes remove requires a name or deviceId.
                      ...
                      coterm remotes remove <name-or-deviceId>

                    List remotes: coterm remotes list
                    """)
            }
    ...
    /// link CotermMobileCore); the app and server remain the authoritative check.
