...
extension CotermCLI {
    ...
    private func sigpipeProbeExecutablePath() throws -> String {
        let candidate: String? = {
            if let explicit = ProcessInfo.processInfo.environment["COTERM_CLI_PATH"],
               !explicit.isEmpty {
                return explicit
            }
        ...
        guard let path = candidate,
            ...
            throw CLIError(message: "SIGPIPE probe could not resolve coterm executable path")
        }
        return path
    }
    ...
    func runSIGPIPEProbe(commandArgs: [String]) throws {
        ...
        let inspectionURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("coterm-sigpipe-\(UUID().uuidString).json")
