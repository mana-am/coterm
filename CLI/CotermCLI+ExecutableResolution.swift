...
extension CotermCLI {
    ...
    func isBundledProviderExecutable(at path: String) -> Bool {
        let candidate = URL(fileURLWithPath: path, isDirectory: false)
            .standardizedFileURL
            .path
        if isCotermAppBundleResourceBinChild(candidate) {
            return true
        }
    ...
    func isCotermClaudeWrapper(at path: String) -> Bool {
    ...
    func isCotermClaudeCommandShim(at path: String) -> Bool {
        let candidate = URL(fileURLWithPath: path, isDirectory: false)
            .standardizedFileURL
            .path
        let environment = ProcessInfo.processInfo.environment
        let shimPaths = [
            environment["COTERM_CLAUDE_WRAPPER_SHIM"],
        ]
    ...
    func resolveClaudeExecutable(configuredCandidates: [String?], searchPath: String?) -> String? {
        for raw in configuredCandidates {
            ...
            guard FileManager.default.fileExists(atPath: trimmed, isDirectory: &isDirectory),
                  ...
                  !isCotermClaudeCommandShim(at: trimmed),
                  !isCotermClaudeWrapper(at: trimmed) else { continue }
    ...
    func resolveClaudeExecutable(searchPath: String?) -> String? {
        resolveExecutableInSearchPath(
            ...
            skip: { self.isCotermClaudeCommandShim(at: $0) || self.isCotermClaudeWrapper(at: $0) }
        )
    }
    ...
    /// in `COTERM_CLAUDE_TEAMS_SANDBOXED` so teammate respawns (which run as a separate
    /// `coterm __tmux-compat` process and cannot see this argv) re-apply the same
    ...
    func claudeTeamsExtraEnvVars(commandArgs: [String]) -> [(key: String, value: String)] {
        ...
        if claudeTeamsHasDangerousSkipPermissions(commandArgs: commandArgs) {
            ...
            vars.append((key: "COTERM_CLAUDE_TEAMS_SANDBOXED", value: "1"))
        }
        return vars
    }
    ...
    /// The whole point of `coterm claude-teams` is "just start a team." Claude Code's
    ...
    /// *what*?" — so a plain `coterm claude-teams "make a demo team with 5 subagents"`
    ...
    /// restore command) so that stays canonical; restore re-invokes `coterm
    ...
    var claudeTeamsTeamSpawnGuidance: String {
        """
        You are Claude Code running inside coterm, started with `coterm claude-teams`. \
    ...
    private func providerExecutableSearchDirectories(searchPath: String?) -> [String] {
        ...
        return directories.compactMap { rawDirectory in
            ...
            let standardized = URL(fileURLWithPath: trimmed, isDirectory: true)
                .standardizedFileURL
                .path
            guard !isCotermAppBundleResourceBinDirectory(standardized) else { return nil }
    ...
    private func isCotermAppBundleResourceBinDirectory(_ path: String) -> Bool {
    ...
    private func isCotermAppBundleResourceBinChild(_ path: String) -> Bool {
        cotermAppBundleResourceBinComponentIndex(path).map { index in
            URL(fileURLWithPath: path, isDirectory: false).standardizedFileURL.pathComponents.count > index + 4
        } ?? false
    }

    private func cotermAppBundleResourceBinComponentIndex(_ path: String) -> Int? {
        let components = URL(fileURLWithPath: path).standardizedFileURL.pathComponents
        guard components.count >= 4 else { return nil }
        for index in components.indices {
            guard components[index].hasSuffix(".app"),
                  components[index].lowercased().contains("coterm"),
