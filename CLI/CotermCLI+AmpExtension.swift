...
extension CotermCLI {
    private static let ampExtensionMarker = "coterm-amp-session-extension-marker"
    private static let ampExtensionFilename = "coterm-session.ts"
    private static let ampExtensionSource = #"""
// coterm-amp-session-extension-marker v2
// Bridges Amp session lifecycle events into coterm's restorable session store
// AND reports live agent status (idle/thinking/tool calls/done/error) into
// the coterm tab status bar.
// Installed by `coterm hooks amp install` or `coterm hooks setup`.
...
function hookEnvironment(cwd: string): NodeJS.ProcessEnv {
  ...
  if (!env.COTERM_AGENT_LAUNCH_ARGV_B64) {
    const argv = normalizedLaunchArgv();
    env.COTERM_AGENT_LAUNCH_KIND = "amp";
    env.COTERM_AGENT_LAUNCH_EXECUTABLE = argv[0] || resolveExecutable("amp");
    env.COTERM_AGENT_LAUNCH_ARGV_B64 = base64NulSeparated(argv);
    env.COTERM_AGENT_LAUNCH_CWD = cwd || process.cwd();
  }
  return env;
}
...
function sendHook(
  ...
  if (process.env.COTERM_AMP_HOOKS_DISABLED === "1") return;
  if (!process.env.COTERM_SURFACE_ID) return;
  if (!sessionId) return;
  ...
  const coterm = process.env.COTERM_AMP_COTERM_BIN || "coterm";
  try {
    const child = spawn(coterm, ["hooks", "amp", subcommand], {
...
// Fires `coterm set-status` / `coterm clear-status` / `coterm log` so the tab
...
// Short verbs shown in the coterm status bar for each Amp tool.
...
// SF Symbol names rendered inside the coterm status badge.
...
// Pin every coterm call to the workspace this plugin process was launched in.
// coterm sets COTERM_WORKSPACE_ID in every pane env, so this is stable across
// async callbacks. Without --workspace, coterm defaults to whichever pane is
...
function workspaceArgs(): string[] {
  const ws = process.env.COTERM_WORKSPACE_ID;
  return ws ? ["--workspace", ws] : [];
}

// Sanitized environment for fire-and-forget coterm status subprocesses.
// Strips Amp-provided secrets (`AMP_API_KEY`) so we never propagate them to
// every spawned `coterm set-status` / `coterm log` / `coterm clear-status` child.
...
function runCoterm(args: string[]): void {
  if (process.env.COTERM_AMP_HOOKS_DISABLED === "1") return;
  if (!process.env.COTERM_SURFACE_ID) return;
  const coterm = process.env.COTERM_AMP_COTERM_BIN || "coterm";
  try {
    const child = spawn(coterm, args, {
...
function clearStatus(): void {
  runCoterm(["clear-status", STATUS_KEY, ...workspaceArgs()]);
}
...
export default function (amp: PluginAPI) {
  ...
  // final status badge with "thinking". coterm runs one Amp session per terminal
...
"""#
    ...
    func installAmpExtensionHooks(_ def: AgentHookDef) throws {
        ...
        if !existing.isEmpty, !existing.contains(Self.ampExtensionMarker) {
            throw CLIError(message: "\(extensionURL.path) exists and is not a coterm plugin; leaving it alone")
        }
        if !skipConfirm {
    ...
    func uninstallAmpExtensionHooks(_ def: AgentHookDef) throws {
        ...
        guard fm.fileExists(atPath: extensionURL.path) else {
            print("No Amp coterm plugin found at \(extensionURL.path)")
            return
        }
        let existing = (try? String(contentsOf: extensionURL, encoding: .utf8)) ?? ""
        guard existing.contains(Self.ampExtensionMarker) else {
            print("Refusing to remove \(extensionURL.path): missing coterm marker")
            return
        }
        try fm.removeItem(at: extensionURL)
        print("Removed Amp coterm plugin from \(extensionURL.path)")
    }
}
