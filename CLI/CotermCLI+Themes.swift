...
extension CotermCLI {
    static let cotermThemeOverrideBundleIdentifier = CotermGhosttyConfigPathResolver.releaseBundleIdentifier
    static let cotermThemesBlockStart = "# coterm themes start"
    static let cotermThemesBlockEnd = "# coterm themes end"
    static let cotermThemesReloadNotificationName = "com.coterm.themes.reload-config"
    static let cotermEnforcedTerminalThemeName = "Anysphere Dark"
    static let cotermThemeChangesDisabledMessage = String(
        localized: "cli.themes.disabled",
        defaultValue: "Terminal theme is managed by coterm and cannot be changed."
    )
    ...
    private func runInteractiveThemes(
        ...
        // including `origin/main`) reads `CMUX_THEME_PICKER_*` env vars. The cmux->coterm
        // rename never landed on the ghostty side and no `coterm`-flavored artifact has been
        // published, so these names must stay `CMUX_` to match the helper actually shipped.
        environment["CMUX_THEME_PICKER_CONFIG"] = try cotermThemeOverrideConfigURL(
    ...
    func runThemes(
        ...
        case "set":
            throw CLIError(message: Self.cotermThemeChangesDisabledMessage)
        case "clear":
            if commandArgs.count > 1 {
        ...
        default:
            if subcommand.hasPrefix("-") {
                throw CLIError(message: "Unknown themes subcommand '\(subcommand)'. Run 'coterm themes --help'.")
            }
    ...
    private func printThemesList(
        ...
        if jsonOutput {
            let currentPayload: [String: Any] = [
                "raw_value": Self.cotermEnforcedTerminalThemeName,
                "light": Self.cotermEnforcedTerminalThemeName,
                "dark": Self.cotermEnforcedTerminalThemeName,
                "source_path": NSNull()
            ]
            let payload: [String: Any] = [
                "themes": themes.map { theme in
                    [
                        "name": theme,
                        "current_light": Self.cotermEnforcedTerminalThemeName.caseInsensitiveCompare(theme) == .orderedSame,
                        "current_dark": Self.cotermEnforcedTerminalThemeName.caseInsensitiveCompare(theme) == .orderedSame
                    ]
                },
                "current": currentPayload,
                "fixed_theme": Self.cotermEnforcedTerminalThemeName,
                "managed": true
            ]
        ...
        print(
            String(
                ...
                defaultValue: "Current theme: Anysphere Dark (managed by coterm)"
            )
        )
        print(Self.cotermThemeChangesDisabledMessage)
        print("")
        ...
        for theme in themes {
            var badges: [String] = []
            if Self.cotermEnforcedTerminalThemeName.caseInsensitiveCompare(theme) == .orderedSame {
                badges.append("light")
            }
            if Self.cotermEnforcedTerminalThemeName.caseInsensitiveCompare(theme) == .orderedSame {
                badges.append("dark")
