...
extension CotermCLI {
    func runConfigCommand(
        ...
        case "get":
            guard args.count == 2, let key = canonicalFontSizeKey(args[1]) else {
                throw CLIError(message: "Usage: coterm config get <sidebar-font-size|surface-tab-bar-font-size>")
            }
            try runConfigGetFontSize(forKey: key, jsonOutput: wantsJSON)
        case "set":
            guard args.count == 3, let key = canonicalFontSizeKey(args[1]) else {
                throw CLIError(message: "Usage: coterm config set <sidebar-font-size|surface-tab-bar-font-size> <points>")
            }
        ...
        case CotermGhosttyConfigSettingEditor.sidebarFontSizeKey, CotermGhosttyConfigSettingEditor.surfaceTabBarFontSizeKey:
            if args.count == 1 {
                ...
                throw CLIError(message: "Usage: coterm config \(subcommand) [points]")
            }
        case "path", "paths":
            guard args.count == 1 else {
                throw CLIError(message: "Usage: coterm config path")
            }
            printSettingsPaths(jsonOutput: wantsJSON)
        case "docs", "documentation":
            guard args.count == 1 else {
                throw CLIError(message: "Usage: coterm config docs")
            }
        ...
        case "doctor", "check", "validate":
            ...
            if report.errorCount > 0 {
                throw CLIError(message: "coterm config doctor found \(report.errorCount) error(s)")
            }
        case "reload":
            guard args.count == 1 else {
                throw CLIError(message: "Usage: coterm config reload")
            }
            guard let socketPath else {
                throw CLIError(message: "coterm config reload requires a socket-backed coterm command path")
            }
            ...
            print(response)
        default:
            throw CLIError(message: "Unknown config subcommand '\(subcommand)'. Run 'coterm config --help'.")
        }
    }
    ...
    func configCommandDoesNotNeedSocket(_ commandArgs: [String]) -> Bool {
        ...
        if subcommand == CotermGhosttyConfigSettingEditor.sidebarFontSizeKey
            || subcommand == CotermGhosttyConfigSettingEditor.surfaceTabBarFontSizeKey {
    ...
    func configUsage() -> String {
        return """
        Usage: coterm config <doctor|check|validate|path|paths|docs|documentation|reload|get|set|sidebar-font-size|surface-tab-bar-font-size>

        Inspect coterm.json, print configuration references, update selected Ghostty config keys, or reload the running app.

        Subcommands:
          doctor|check|validate [--path <path>]   Validate JSONC syntax for coterm config files.
          path|paths                              Print coterm.json paths, docs URL, and schema URL.
          docs|documentation                      Print the same output as `coterm docs settings`.
          reload                                  Reload Ghostty config + coterm.json and refresh terminals (alias for `coterm reload-config`).
          get <key>                               Print sidebar-font-size or surface-tab-bar-font-size.
          set <key> <points>                      Set sidebar-font-size (10-20 pt) or surface-tab-bar-font-size (8-24 pt), then reload if coterm is running.
        ...
        Examples:
          ...
          coterm config doctor --path .coterm/coterm.json
          coterm config set sidebar-font-size 14
          coterm config sidebar-font-size 12.5
          coterm config set surface-tab-bar-font-size 13
          coterm config surface-tab-bar-font-size 11
          coterm config reload
        """
    }
    ...
    func printSettingsPaths(jsonOutput: Bool) {
        let payload: [String: Any] = [
            ...
            "reload_command": "coterm reload-config",
            "reload_scope": "Reloads Ghostty config + coterm.json and refreshes terminals in place. No app restart needed.",
            "backup": "Back up any existing coterm.json file to a timestamped .bak copy before editing so the user can revert.",
        ]
        ...
        print()
        print("Reload after editing (covers BOTH coterm.json and Ghostty config; no app restart needed):")
        print("  coterm reload-config")
    }
    ...
    private func canonicalFontSizeKey(_ raw: String) -> String? {
        ...
        case CotermGhosttyConfigSettingEditor.sidebarFontSizeKey:
            return CotermGhosttyConfigSettingEditor.sidebarFontSizeKey
        case CotermGhosttyConfigSettingEditor.surfaceTabBarFontSizeKey:
            return CotermGhosttyConfigSettingEditor.surfaceTabBarFontSizeKey
        default:
            return nil
        }
    }
    ...
    private func fontSizeConfig(
        ...
        case CotermGhosttyConfigSettingEditor.sidebarFontSizeKey:
            return (
                CotermGhosttyConfigSettingEditor.defaultSidebarFontSize,
                CotermGhosttyConfigSettingEditor().clampedSidebarFontSize,
                CotermGhosttyConfigSettingEditor().formattedSidebarFontSize,
                { CotermGhosttyConfigSettingEditor().parsedSidebarFontSize(in: $0) }
            )
        case CotermGhosttyConfigSettingEditor.surfaceTabBarFontSizeKey:
            return (
                CotermGhosttyConfigSettingEditor.defaultSurfaceTabBarFontSize,
                CotermGhosttyConfigSettingEditor().clampedSurfaceTabBarFontSize,
                CotermGhosttyConfigSettingEditor().formattedSurfaceTabBarFontSize,
                { CotermGhosttyConfigSettingEditor().parsedSurfaceTabBarFontSize(in: $0) }
            )
        default:
            return nil
        }
    ...
    private func runConfigGetFontSize(forKey key: String, jsonOutput: Bool) throws {
        ...
        let url = try cotermGhosttyConfigURLForCLI()
    ...
    private func runConfigSetFontSize(
        ...
        case "failed":
            ...
            print("Run `coterm config reload` after coterm is running to apply it.")
        default:
            print("OK \(key) = \(formattedValue) (saved)")
            print("Run `coterm config reload` to apply it.")
        }
        print("path: \(Self.tildePath(url.path))")
    }

    private func cotermGhosttyConfigURLForCLI() throws -> URL {
        let environment = ProcessInfo.processInfo.environment
        let fileManager = FileManager.default
        let appSupportDirectories = CotermApplicationSupportDirectories(environment: environment, fileManager: fileManager)
            .userDirectories
        ...
        let bundleIdentifier = normalizedConfigValue(environment["COTERM_BUNDLE_ID"])
        ...
        for appSupportDirectory in appSupportDirectories {
            if let existing = CotermGhosttyConfigPathResolver().loadConfigURLs(
        ...
        return CotermGhosttyConfigPathResolver().activeOrEditableConfigURL(
            currentBundleIdentifier: bundleIdentifier,
    ...
    private struct ConfigDoctorReport {
        ...
        var payload: [String: Any] {
            [
                ...
                "reload_command": "coterm reload-config",
                "docs_url": CotermCLI.settingsDocsURL,
                "schema_url": CotermCLI.settingsSchemaURL,
            ]
        }
    }
        ...
        while index < arguments.count {
            ...
            if argument == "--path" {
                ...
                guard nextIndex < arguments.count else {
                    throw CLIError(message: "coterm config doctor --path requires a path")
                }
    ...
    private func configDoctorFinding(for target: ConfigDoctorTarget) -> ConfigDoctorFinding {
        ...
        guard fileManager.fileExists(atPath: target.path, isDirectory: &isDirectory) else {
            let message = target.missingIsError
                ? "file not found"
                : "not found; coterm will use defaults until this file exists"
    ...
    private func printConfigDoctorReport(_ report: ConfigDoctorReport) {
        ...
        print("Reload: coterm reload-config")
    }
