...
// coterm-pi-session-extension-marker v2
// Bridges Pi session lifecycle, tool telemetry, notifications, and resume bindings into coterm.
// Installed by `coterm hooks pi install` or `coterm hooks setup`.
...
function safeCotermEnvKey(key: string): boolean {
  if (key.startsWith("COTERM_TEST_PI_")) return !secretLikeEnvKey(key);
  if (key.startsWith("COTERM_AGENT_LAUNCH_")) return !secretLikeEnvKey(key);
  if (key === "COTERM_AGENT_HOOK_STATE_DIR") return true;
  if (key === "COTERM_PI_COTERM_BIN" || key === "COTERM_PI_HOOKS_DISABLED") return true;
  if (key === "COTERM_SURFACE_ID" || key === "COTERM_WORKSPACE_ID" || key === "COTERM_WINDOW_ID") return true;
  if (key === "COTERM_PANE_ID" || key === "COTERM_TAB_ID" || key === "COTERM_PANEL_ID") return true;
  if (key === "COTERM_SOCKET" || key === "COTERM_SOCKET_PATH") return true;
  if (key === "COTERM_BUNDLE_ID" || key === "COTERM_BUNDLED_CLI_PATH") return true;
  if (key === "COTERM_CLI_SENTRY_DISABLED" || key === "COTERM_DEBUG_LOG") return true;
  return false;
}

function shouldPreserveEnvKey(key: string): boolean {
  if (safeCotermEnvKey(key)) return true;
...
function hookEnvironment(cwd: string, includeSocketPassword = false): NodeJS.ProcessEnv {
  ...
  // Only coterm CLI children need the socket credential; keep it out of the generic allowlist.
  if (includeSocketPassword) {
    const socketPassword = process.env.COTERM_SOCKET_PASSWORD;
    if (socketPassword) env.COTERM_SOCKET_PASSWORD = socketPassword;
  }
  if (!env.COTERM_AGENT_LAUNCH_ARGV_B64) {
    const argv = normalizedLaunchArgv();
    env.COTERM_AGENT_LAUNCH_KIND = "pi";
    env.COTERM_AGENT_LAUNCH_EXECUTABLE = argv[0] || resolveExecutable("pi");
...
function warn(ctx: ExtensionContext | null, message: string, details: Record<string, unknown> = {}): void {
  const payload = { source: "coterm-pi-extension", level: "warning", message, ...details };
  try {
    console.warn(JSON.stringify(payload));
  } catch (_) {
    console.warn(`[coterm-pi-extension] ${message}`);
  }
  const ui = (ctx as unknown as { ui?: { notify?: (message: string, type?: string) => void } } | null)?.ui;
  try {
    ui?.notify?.("coterm Pi integration warning - check the terminal for details", "warning");
  } catch (_) {}
}

function cotermExecutable(): string {
  return process.env.COTERM_PI_COTERM_BIN || "coterm";
}

function runCoterm(args: string[], cwd: string, input?: string): CommandResult {
  try {
    const result = spawnSync(cotermExecutable(), args, {
      input,
      encoding: "utf8",
