...
extension CotermCLI {
    ...
    func themeDirectoryURLs() -> [URL] {
        ...
        for appSupportDirectory in CotermApplicationSupportDirectories(environment: processEnv).userDirectories {
            appendIfExisting(
                appSupportDirectory
                    .appendingPathComponent(Self.cotermThemeOverrideBundleIdentifier, isDirectory: true)
                    .appendingPathComponent("themes", isDirectory: true)
            )
        }
        appendIfExisting(
    ...
    func validatedThemeName(_ rawValue: String, availableThemes: [String]) throws -> String {
        ...
        throw CLIError(message: "Unknown theme '\(trimmed)'. Run 'coterm themes' to list available themes.")
    }
    ...
    func themeConfigSearchURLs(targetBundleIdentifier: String) -> [URL] {
        ...
        for appSupportDirectory in CotermApplicationSupportDirectories(
    ...
    func cotermThemeOverrideConfigURL(targetBundleIdentifier: String) throws -> URL {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            throw CLIError(message: "Unable to resolve Application Support directory")
        }
        return CotermGhosttyConfigPathResolver().editableConfigURL(
            currentBundleIdentifier: targetBundleIdentifier,
    ...
    func writeManagedThemeOverride(
        ...
        let fileManager = FileManager.default
        let configURL = try cotermThemeOverrideConfigURL(targetBundleIdentifier: targetBundleIdentifier)
        ...
        let strippedContents = removingManagedThemeOverride(from: existingContents)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let block = """
        \(Self.cotermThemesBlockStart)
        theme = \(rawThemeValue)
        \(Self.cotermThemesBlockEnd)
        """
    ...
    func reloadThemesIfPossible(
        ...
        DistributedNotificationCenter.default().post(
            name: Notification.Name(Self.cotermThemesReloadNotificationName),
            object: nil,
            userInfo: [
    ...
    func themeTargetBundleIdentifier(socketPath: String) -> String {
        bundleIdentifierForThemeReloadSocketPath(socketPath)
            ?? currentCotermAppBundleIdentifier()
            ?? Self.cotermThemeOverrideBundleIdentifier
    }
    ...
    private func bundleIdentifierForThemeReloadSocketPath(_ socketPath: String) -> String? {
        ...
        switch name {
        case "coterm.sock":
            return Self.cotermThemeOverrideBundleIdentifier
        case "coterm-debug.sock":
            return "coterm.com.emergent.app.debug"
        case "coterm-nightly.sock":
            return "coterm.com.emergent.app.nightly"
        case "coterm-staging.sock":
            return "coterm.com.emergent.app.staging"
        default:
            break
        }

        if name.range(of: #"^coterm-\d+\.sock$"#, options: .regularExpression) != nil {
            return Self.cotermThemeOverrideBundleIdentifier
        }

        if let slug = themeReloadSocketSlug(name, prefix: "coterm-debug-", suffix: ".sock") {
            return "coterm.com.emergent.app.debug.\(slug)"
        }
        if let slug = themeReloadSocketSlug(name, prefix: "coterm-nightly-", suffix: ".sock") {
            return "coterm.com.emergent.app.nightly.\(slug)"
        }
        if let slug = themeReloadSocketSlug(name, prefix: "coterm-staging-", suffix: ".sock") {
            return "coterm.com.emergent.app.staging.\(slug)"
        }
        return nil
    }
    ...
    func currentCotermAppBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["COTERM_BUNDLE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
